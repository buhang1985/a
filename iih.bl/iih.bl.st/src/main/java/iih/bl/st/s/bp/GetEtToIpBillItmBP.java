 ...
 *
 */
public class GetEtToIpBillItmBP {
	 ...
	 * @throws BizException
	 */
	public EtToIpBillDTO[] exec(String[] stIdArr) throws BizException {
		...
		sql.append(
				" and not exists(select * from bl_cg_itm_oep_tfrcd tfrcd where tfrcd.id_cgitmoep=cgitem.id_cgitmoep and tfrcd.FG_ACTIVE='Y') ");
		sql.append(" ORDER BY ");
		sql.append(" CGITEM.ID_OR, ");
		sql.append(" CGITEM.ID_ORSRV, ");
		sql.append(" CGITEM.ONLYCODE, ");
		...
		List<EtToIpBillDTO> list = (List<EtToIpBillDTO>) new DAFacade().execQuery(sql.toString(), 
				new BeanListHandler(EtToIpBillDTO.class));

		return list.toArray(new EtToIpBillDTO[0]);
	}
}
