...
public class BlStIpRefundLinstrener implements IBusinessListener {
	...
	private BlIihIeDTO assembly(BlEventInfoDTO blEventInfoDTO) throws BizException {
		...
		BlpaypatipAggDO blpaypatipAggDO = blEventInfoDTO.getBlpaypatipRedAggDO();//结算收付款信息
		...
		bl_iih_ie_DTO.setTranstacttype(BookRtnDirectEnum.RETURNS.toString());//交易类型
		...
		for(BlStIpInccaInfoDTO blStIpInccaInfoDTO : blStIpInccaInfoDTOArr){
			...
			if(blincipAggDO != null && blincipAggDO.getParentDO() != null){
				...
				blIihIe_bill_DTO.setRed_rcptno(blincipAggDO.getParentDO().getIncno());//红冲账单流水号
				if(blincipAggDO.getParentDO().getDt_inc() != null){
			...
			FArrayList2 cgList = new FArrayList2();
			for(BlCgIpDO blCgIpDO : blCgIpDOArr){
				...
				blIihIe_item_DTO.setInc_code1(blCgIpDO.getCode_srv());//费别编码
				blIihIe_item_DTO.setInc_name1(blCgIpDO.getName_srv());//费别名称
				blIihIe_item_DTO.setBill_item_no("08");//细目号
//				if(blCgIpDO.getFg_bb().booleanValue()){
//					blIihIe_item_DTO.setFg_bb("1");//婴儿标志(是)
//					blIihIe_item_DTO.setFg_bb_no(blCgIpDO.getNo_bb());//是否婴儿标记编码
//				}else{
//					blIihIe_item_DTO.setFg_bb("0");//婴儿标志(否)
//				}
				blIihIe_item_DTO.setSettlement_times(1);//结算次数
				blIihIe_item_DTO.setCharge_status_code(IBlConst.FEE_BL_TYPE_CODE_CZ);//费用状态编码
				blIihIe_item_DTO.setCharge_status_name(IBlConst.FEE_BL_TYPE_NAME_CZ);//费用状态名称（录入, 上帐, 收费, 冲账, 结算）
...
//					throw new BizException("处方信息不存在！");
				...
				if(deptApDO != null){
//					throw new BizException("申请科室信息不存在！");
				...
				if(deptMpDO != null){
//					throw new BizException("费用执行科室信息不存在！");
