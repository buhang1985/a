 ...
 *
 */
public class WsParamPrepayValidator extends WsParamBaseValidator{
	...
	public void validate(WsParamPrepay param) throws BizException{
		...
		if(param.getPrepay() == null){
			throw new BizException("预交金充值入参数据为空");
		}
		...
		if(IBlWsConst.ENTP_OP.equals(prepayInfo.getEntpCode())){
			if(StringUtil.isEmpty(prepayInfo.getPatCode()) && 
				...
				throw new BizException("门诊预交金充值入参关键信息全为空:患者编码,就诊卡号,身份证号");
			}
		}else{
			if(StringUtil.isEmpty(prepayInfo.getPatCode()) 
					&& StringUtil.isEmpty(prepayInfo.getEnTimes())
					&& StringUtil.isEmpty(prepayInfo.getIPCode())){
				throw new BizException("住院预交金充值入参关键信息全为空:患者编码,就诊次数,住院登记流水号");
			}
		}
		...
		if(!StringUtil.isEmpty(prepayInfo.getPaymentMethod()) && IBdPripmCodeConst.CODE_PRI_PM_CASH.equals(prepayInfo.getPaymentMethod())){
			
		}else{
			if(StringUtil.isEmpty(prepayInfo.getDocumentNumber()) && StringUtil.isEmpty(prepayInfo.getOrderNumber())){
