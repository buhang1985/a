 ...
 * @date 2018-09-19
 */
public class SamplerChargeOpBp {
	...
	public void exec(Billsupply[] supplyBillArr) throws BizException
	{
		...
		BlOrderAppendBillParamDTO[] chargedBillItemDtoArr=orderAppendBillService.SetOrderAppendBill_ByItms(orderBillItmDto.toArray(new BlOrderAppendBillParamDTO[orderBillItmDto.size()]), IBdFcDictCodeConst.SD_CODE_ENTP_OP);
	}	
	...
	private List<BlOrderAppendBillParamDTO> assemblyBlorderAppendParamDtoLst(IpCompensateBillDTO[] compensateBillDtoArr) throws BizException
	{
		...
		for(IpCompensateBillDTO billDto:compensateBillDtoArr)
		{
			...
			billItmDto.setFg_additm(FBoolean.TRUE);
	...
	{	
		...
		for(Billsupply supplyDto:supplyBillArr)
		{
			...
			ipCompensateBillDto.setSrcfunc_des(IBlCgCodeConst.EexcConfirm);
