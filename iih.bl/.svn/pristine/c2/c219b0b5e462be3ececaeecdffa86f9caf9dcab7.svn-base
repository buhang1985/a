...
/**
 * 门诊退记账逻辑类 部分退记账时，需要红冲原记账，重新记账
 * 
 * @author ly 2018/08/25 -->待重构
 *
 */
public class BlOpRefundAccountBP {
	 ...
	 * @throws BizException
	 */
	public BlCgAccountRltDTO exec(BlCgRefundAccountDTO[] refundDtos, BlCgRefundAccountSetDTO accSetDto)
			throws BizException {
		...
		if (StringUtil.isEmpty(this.accSet.getEu_rfdmode())) {
			throw new BizException("退记账模式为空");
				...
				&& !RefundModeEnum.SUSP_KEY.equals(this.accSet.getEu_rfdmode())) {
		...
		//发生退款事件
		BlOepCgEvent.invokeCancAccount(rlt, accSetDto);
		return rlt;
	}
	...
	private BlCgAccountRltDTO refundAccount(BlCgRefundAccountDTO[] refundDtos) throws BizException {
		...
		// ---> 以下开始做保存操作
		this.saveOper = true;
		BlcgoepAggDO[] newAggDos = null;
		// 退挂起记账数据
		if (this.accSet.getEu_rfdmode().equals(RefundModeEnum.SUSP_KEY)) {
			// 删除记账数据
			this.deleteFuspCgData(refundDtos, cgAggDos);
		} else {
			// 组装,保存退记账数据
			newAggDos = this.assembleAndSaveData(refundDtos, cgAggDos);
		}
		// 回写账户
	 ...
	 * @throws BizException
	 */
	private void deleteFuspCgData(BlCgRefundAccountDTO[] refundDtos, BlcgoepAggDO[] cgAggDos) throws BizException {

		List<String> liststid = new ArrayList<String>();
			...
			for (BlcgoepAggDO aggDO : cgAggDos) {
				...
				for (BlCgItmOepDO itmDO : aggDO.getBlCgItmOepDO()) {
					...
					if (refundDto.getAttrVal(accDtoKeyColumn).equals(itmDO.getAttrVal(cgKeyColumn))) {
						if (!StringUtil.isEmpty(itmDO.getId_stoep())) {
							liststid.add(itmDO.getId_stoep());
						}
						// 设置隐藏字段 剩余数量
		...
		BlStOepDO[] blStOepDOs = stRService.findByIds(liststid.toArray(new String[] {}), FBoolean.FALSE);
		// 查询发票数据
		IBlincoepRService incRSerice = ServiceFinder.find(IBlincoepRService.class);
		BlincoepAggDO[] blincoepAggDOs = incRSerice.findByAttrValStrings("Id_stoep", liststid.toArray(new String[] {}));
		// 删除记账数据
		IBlcgoepCudService cgCudService = ServiceFinder.find(IBlcgoepCudService.class);
		cgCudService.delete(cgAggDos);
		// 删除 结算数据 合发票数据
		if (!ArrayUtil.isEmpty(blStOepDOs)) {
			IBlstoepCudService stCudService = ServiceFinder.find(IBlstoepCudService.class);
			stCudService.delete(blStOepDOs);
		}
		if (!ArrayUtil.isEmpty(blincoepAggDOs)) {
			IBlincoepCudService incCudService = ServiceFinder.find(IBlincoepCudService.class);
			incCudService.delete(blincoepAggDOs);
		}
	}
	...
	private Map<String, List<BlCgRefundAccountDTO>> mapAccData(BlCgRefundAccountDTO[] refundDtos) {
		...
		Map<String, List<BlCgRefundAccountDTO>> patAccDtoMap = new HashMap<String, List<BlCgRefundAccountDTO>>();
	 ...
	 * @throws BizException
	 */
	private boolean checkCgData(Map<String, List<BlCgRefundAccountDTO>> patAccDtoMap, BlCgItmOepDO[] cgDos)
			throws BizException {
		...
		for (String patId : patAccDtoMap.keySet()) {
			...
			try {
				validator.validate(patAccDtoMap.get(patId), cgDos, this.accDtoKeyColumn, this.cgKeyColumn, this.accSet,
						this.dataWh);
			} catch (BizException e) {
	...
	/**
	 * 组装保存数据 半退时全部红冲，重收剩余部分
	 * 
	 * @param refundDtos
	 * @param cgAggDos
	 * @return
	 */
	private BlcgoepAggDO[] assembleAndSaveData(BlCgRefundAccountDTO[] refundDtos, BlcgoepAggDO[] cgAggDos)
			throws BizException {
			...
			for (BlcgoepAggDO aggDO : cgAggDos) {
				...
				for (BlCgItmOepDO itmDO : aggDO.getBlCgItmOepDO()) {
					...
					if (refundDto.getAttrVal(accDtoKeyColumn).equals(itmDO.getAttrVal(cgKeyColumn))) {
						...
						// 统计退医嘱服务id
						if (!StringUtil.isEmpty(itmDO.getId_orsrv())
								&& !this.updateOrsrvList.contains(itmDO.getId_orsrv())
								&& FDouble.ZERO_DBL.equals(quanRemain)) {
		...
		for (BlcgoepAggDO aggDO : updateAggList) {
			...
			for (BlCgItmOepDO itmDO : aggDO.getBlCgItmOepDO()) {
				...
				if (!FDouble.ZERO_DBL.equals(itmDO.getAttrVal(QUAN_REMAIN))) {
					...
					BlCgItmOepDO reItmDO = BlCgDataUtil.buildReCgData(itmDO, (FDouble) itmDO.getAttrVal(QUAN_REMAIN),
							this.accSet);
	...
	protected void writebackAccount() throws BizException {
		...
		List<PiPatAccDO> accList = new ArrayList<PiPatAccDO>();
		for (String key : this.dataWh.getPersonalConsumeMap().keySet()) {
			...
			acc.setAcc_lock(acc.getAcc_lock().add(amt));
	...
	private void writebackOrBlState() throws BizException {
		...
		orService.UpdateOrdChargeRelInfo2(this.updateOrsrvList.toArray(new String[0]),
				ICiDictCodeConst.SD_SU_BL_REFOUND_ID, ICiDictCodeConst.SD_SU_BL_REFOUND, null,
