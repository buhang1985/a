...
public class EtBackChargeBP {
	 ...
	 */
	public EtBackChargeBP(List<ICharge> chargeBPList) {
	 ...
	 * @throws BizException
	 */
	public void exec(String entId, FDateTime cgEndTime,boolean outDept) throws BizException {
		...
		BLTaskLogger.info("本次计价截止时间:" + cgEndTime.toString());
		...
		// 1、获取待记账患者信息
		List<IpCgPatDTO> patList = new GetNeedCgPatListBP().execEt(cgEndTime, entId);
		...
		if(!StringUtil.isEmpty(entId)){
			this.chargeNoTrans(patList.get(0), cgEndTime, outDept);
		}else{
	...
	private List<ICharge> getDefaultChargeList(){
		...
		//chargeBPList.add(new ChargeBedBP());// 床位费
		chargeBPList.add(new ChargeBedNewBP(false)); //床位费新版
