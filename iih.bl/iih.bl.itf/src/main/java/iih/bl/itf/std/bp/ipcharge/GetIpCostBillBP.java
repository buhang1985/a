 ...
 *
 */
public class GetIpCostBillBP extends BlItfBaseBP<WsParamGetCostBill,WsResultIpGetCostBill>{
	...
	protected void checkParam(WsParamGetCostBill param) throws BizException {
		...
		patId = this.findPat(patInfo);
		
		if (patInfo==null) {
			throw new BizException();
		}
		if (!StringUtil.isEmpty(patInfo.getPatCode())&&!StringUtil.isEmpty(patInfo.getEnTimes())) {
			getPatiVisitDO("1", patInfo.getPatCode(),patInfo.getEnTimes());
			return;
		}else if(!StringUtil.isEmpty(patInfo.getIPCode())){
		    getPatiVisitDO("2",patInfo.getIPCode(),patInfo.getEnTimes());
			return;
		}else if(!StringUtil.isEmpty(patInfo.getCardCode())){
			getPatiVisitDO("3",patInfo.getCardCode(),null);
			return;
		}else if(!StringUtil.isEmpty(patInfo.getSdCode())&&!StringUtil.isEmpty(patInfo.getEnTimes())){
			getPatiVisitDO("4",patInfo.getSdCode(),patInfo.getEnTimes());
			return;
		}else if(!StringUtil.isEmpty(patInfo.getEnCode())){
			getPatiVisitDO("5",patInfo.getEnCode(),null);
			return;
		}else {
	...
	@Override
	protected WsResultIpGetCostBill doWork(WsParamGetCostBill param) throws BizException {		
		...
		//住院汇总
		WsResultBillIpTotalAmtInfo wsResultBillIpTotalAmtInfo = setBlIpSummary(patInfo);
		wsResultIpBill.setIPTotalcostInfo(wsResultBillIpTotalAmtInfo);
		//分类汇总
		List<WsResultBillTypeInfo> wsResultBillTypeInfos = setBlIpTaxonomy(patInfo);
		wsResultIpBill.setIPTypecostInfo(wsResultBillTypeInfos);
		//住院账单项目分类明细
		List<WsResultBillOrderInfo> wsResultBillOrderInfos = setBillclassification(patInfo);
		wsResultIpBill.setIPDetailcostInfo(wsResultBillOrderInfos);
		
		WsResultIpGetCostBill wsRult=new WsResultIpGetCostBill();
		WsResultMsg resultMsg = setResultMsg("0");
		wsRult.setRltMsg(resultMsg);
		WsResultInfoIpGetCostBill wGetCostBill=new WsResultInfoIpGetCostBill();
		wGetCostBill.setIpBill(wsResultIpBill);
		wsRult.setRltInfo(wGetCostBill);
		return wsRult;
	}
	...
	/**
	 * 
	 * @Title: getPatiVisitDO   
	 * @Description: 根据不同入参获取就诊信息  
	...
	private void getPatiVisitDO(String attributeName,String value,String value2)throws BizException{
		...
		switch (attributeName){
			case "1":
				...
				InpatientDO[] inpatientDOs = inpatientRService.findByAttrValStrings(InpatientDO.ID_ENT,idEnts.toArray(new String[]{}));
			...
			case "3":
				...
				PiCardDO[] piCardDOs = iCardRService.findByAttrValString(" code "
						,value);
				if (!ArrayUtil.isEmpty(piCardDOs)) {
					...
					if (!ArrayUtil.isEmpty(patiVisitDOs2)) {
						InpatientDO[] inpatientDOs3=inpatientRService.findByAttrValString("id_ent",patiVisitDOs2[0].getId_ent());
			...
			case "5":
				...
				if (!ArrayUtil.isEmpty(patiVisitDOs3)) {
					InpatientDO[] inpatientDOs5=inpatientRService.findByAttrValString(InpatientDO.ID_ENT,patiVisitDOs3[0].getId_ent());
	...
	/***
	 * 
	 * @Title: setBlIpSummary   
	 * @Description: 获取住院费用汇总并赋值  
	 * @param: @return      
	 * @return: WsResultInfoIpGetCostBill      
	 * @throws
	 */
	private WsResultBillIpTotalAmtInfo  setBlIpSummary(WsParamBillPatInfo patInfo)throws BizException{
		//获取就诊账户信息
		...
		sBuffer.append(" select sum(amt_ratio*eu_direct) totalAmt ,0 settled from bl_cg_ip "
				+ "where id_ent='").append(this.InpatientDO.getId_ent()).append("'");
		if(!StringUtil.isEmpty(patInfo.getStartDate())){
			sBuffer.append(" and dt_cg >= '"+patInfo.getStartDate()+"' ");
		}
		if(!StringUtil.isEmpty(patInfo.getEndDate())){
			sBuffer.append(" and dt_cg <= '"+patInfo.getEndDate()+"' ");
		}
		...
		//获取住院总费用与已结算金额
        @SuppressWarnings("unchecked")
		List<WsResultBillIpTotalAmtInfo> result =(List<WsResultBillIpTotalAmtInfo>)new DAFacade().execQuery(sBuffer.toString(), 
        		new BeanListHandler(WsResultBillIpTotalAmtInfo.class));
        ...
        SqlParam param = new SqlParam();
        param.addParam(this.InpatientDO.getId_ent());
        
        Number stAmt = (Number)new DAFacade().execQuery(sql.toString(),param, new ColumnHandler());//结算金额
        
        wsRltInfo.setPrepayment(BlAmtConverUtil.toXmlString(balanceDTO.getAmt_prepay()));//预交金总额
        wsRltInfo.setBalance(BlAmtConverUtil.toXmlString(balanceDTO.getAvailable()));//预交金余额
        
        wsRltInfo.setUnsettled(BlAmtConverUtil.toXmlString(balanceDTO.getAmt_uncg()));//未结算金额
        wsRltInfo.setSettled(BlAmtConverUtil.toXmlString(new FDouble(stAmt.doubleValue())));//已结算金额
        wsRltInfo.setTotalAmt(this.addUtil(wsRltInfo.getUnsettled(), wsRltInfo.getSettled()));//住院总费用
        
        if(!StringUtil.isEmpty(patInfo.getStartDate()) && !StringUtil.isEmpty(patInfo.getEndDate())){
            wsRltInfo.setDayAmt(BlAmtConverUtil.toXmlString(new FDouble(wsRltInfo.getDayAmt())));//日总金额
        }else{
        	wsRltInfo.setDayAmt(null);
        }
        
        if (balanceDTO.getAvailable().compareTo(new FDouble(0))<=0) {
			wsRltInfo.setNote("欠费敬告：您的预交金已用完，请速到收费处补交预交金");
		}
		return wsRltInfo;
	}
	...
	/**
	 * 
	 * @Title: setBlIpTaxonomy   
	 * @Description: 获取住院汇总分类费用情况 
	 * @param: @return
	 * @param: @throws BizException      
	 * @return: WsResultBillIPTypecostInfo      
	 * @throws
	 */
	private List<WsResultBillTypeInfo> setBlIpTaxonomy(WsParamBillPatInfo patInfo )throws BizException{
		StringBuffer sql = new StringBuffer();
		sql.append("   SELECT     Z.CODE_INCCAITM TYPECODE, ");
		sql.append("         NVL(Z.NAME_INCCAITM, '未知')  TYPENAME, ");
		sql.append("         trim(to_char(SUM(X.EU_DIRECT * X.AMT_RATIO),'9999990.00')) TYPEAMOUNT ");
        ...
        if(!StringUtil.isEmpty(patInfo.getStartDate())){
        	sqlp.addParam(patInfo.getStartDate());
		}
        if(!StringUtil.isEmpty(patInfo.getEndDate())){
        	sqlp.addParam(patInfo.getEndDate());
		}
		List<WsResultBillTypeInfo> list = (List<WsResultBillTypeInfo>) new DAFacade()
				.execQuery(sql.toString(), sqlp, new BeanListHandler(
						WsResultBillTypeInfo.class));
		
		for (WsResultBillTypeInfo itm : list) {
			itm.setTypeAmount(BlAmtConverUtil.toXmlString(new FDouble(itm.getTypeAmount())));
		}
	...
	/**
	 * 
	 * @Title: setBillclassification   
	 * @Description: 获取到住院账单项目分类明细  
	 * @param: @return
	 * @param: @throws BizException      
	 * @return: WsResultBillOrderInfo      
	 * @throws
	 */
	private List<WsResultBillOrderInfo> setBillclassification(WsParamBillPatInfo patInfo)throws BizException{
		StringBuffer sql = new StringBuffer();
		sql.append(" select  cg.id_cgip,nvl(ord.code_or,-1) code_or,ord.sd_srvtp,cg.fg_st,cg.dt_cg,dep.name name_dep, ");
		...
		List<BlBillClassificationDTO> list = (List<BlBillClassificationDTO>) new DAFacade()
				.execQuery(sql.toString(), sqlp, new BeanListHandler(
						BlBillClassificationDTO.class));
		...
		//最后返回的总集合
		List<WsResultBillOrderInfo> billOrderInfos= new ArrayList<>();
 		for (String s:IPDetailcostInfoMap.keySet()) {
			...
			WsResultBillOrderInfo wsorderInfo = setWsResultBillOrderInfo(list3.get(0));
			List<WsResultBillSrvInfo> wsResultBillSrvInfos=new ArrayList<>();
...
//					List<BlBillClassificationDTO> srvByPriceList = new ArrayList<BlBillClassificationDTO>();
...
//				}
				...
				WsResultBillSrvInfo resultBillSrvInfo = setWsResultBillSrvInfo(list5.get(0),amt_all,quan_all);
				wsResultBillSrvInfos.add(resultBillSrvInfo);
			}
			wsorderInfo.setSrvList(wsResultBillSrvInfos);
	...
	/**
	 * 
	 * @Title: setWsResultBillSrvInfo   
	 * @Description: 为医嘱项目明细赋值  
	 * @param: @param bDto
	 * @param: @param amt_all
	 * @param: @param quan_all
	 * @param: @return      
	 * @return: WsResultBillSrvInfo      
	 * @throws
	 */
	private WsResultBillSrvInfo setWsResultBillSrvInfo(BlBillClassificationDTO bDto,FDouble amt_all,FDouble quan_all){
		WsResultBillSrvInfo wSrvInfo=new WsResultBillSrvInfo();
		wSrvInfo.setSrvCode(bDto.getCode_srv());
		wSrvInfo.setSrvName(bDto.getName_srv());
		wSrvInfo.setUnit(bDto.getSrvu());
		if (bDto.getFg_mm().booleanValue()) {
			wSrvInfo.setSepcification(bDto.getSpec());
		}
		wSrvInfo.setPrice(BlAmtConverUtil.toXmlString(bDto.getPrice()));
		wSrvInfo.setInccacode(bDto.getCode_inccaitm());
		wSrvInfo.setInccaItm(bDto.getName_inccaitm());
	...
	/**
	 * 
	 * @Title: setWsResultBillOrderInfo   
	 * @Description: 为医嘱列表赋值  
	 * @param: @param bDto
	 * @param: @return      
	 * @return: WsResultBillOrderInfo      
	 * @throws
	 */
	private WsResultBillOrderInfo setWsResultBillOrderInfo(BlBillClassificationDTO bDto){
		WsResultBillOrderInfo wsorderInfo=new WsResultBillOrderInfo();
		if (!bDto.getCode_or().equals("-1")) {
			wsorderInfo.setApplyCode(bDto.getCode_or());
			wsorderInfo.setApplyTpCode(bDto.getSd_srvtp());
		}
		wsorderInfo.setStflag(bDto.getFg_st().toString());
		wsorderInfo.setCgTime(bDto.getDt_cg().toString());
		wsorderInfo.setOrDepName(bDto.getName_dep());
		wsorderInfo.setOrEmpName(bDto.getName_emp());
		return wsorderInfo;
	}
