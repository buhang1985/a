...
import iih.bl.itf.std.bean.input.charge.sub.ExaminationtfgCgItmDto;
 ...
 * 
 * @author zhangxin
 * @date 2019年6月19日 暂时挪移 细节还需要优化
 */
public class SetOpCostBillBP extends BlItfBaseBP<WsParanExamunationCg, WsResultMsgInfo> {
	...
	@Override
	protected void checkParam(WsParanExamunationCg param) throws BizException {
		if (param == null || param.getOrderItem() == null || param.getOrderItem().size() <= 0)
			throw new BizException("门诊补退费:参数空异常！");
		List<String> orderList = new ArrayList<String>();
		Integer index = 0;
		for (ExaminationtfgCgDto order : param.getOrderItem()) {
			if (StringUtil.isEmptyWithTrim(order.getMpDepCode())) {
				throw new BizException("门诊补退费:第【" + (index + 1) + "】行执行科室空异常！");
			}
			orderList.add(order.getCodeApply());
			index++;
		}

		for (int i = 0; i < param.getOrderItem().size(); i++) {
			if (!param.getOrderItem().get(i).getMpDepCode().equals(param.getOrderItem().get(0).getMpDepCode())) {
				throw new BizException("门诊补退费：执行科室不一致！");
			}
		}
	}

	@Override
	protected WsResultMsgInfo doWork(WsParanExamunationCg param) throws BizException {

		WsResultMsgInfo rltMsg = new WsResultMsgInfo();
		try {
			List<ExaminationtfgCgDto> orderItem = param.getOrderItem();
			ExaminationtfgCgDto[] params = orderItem.toArray(new ExaminationtfgCgDto[] {});
			// 3 .退补费
			...
			rltMsg.setMessage(e.getMessage());
			// String output = rltMsg.serialize();
			return rltMsg;
		}
	...
	private OrdSrvDO initOr(ExaminationtfgCgDto params) throws BizException {
	 ...
	 * @throws BizException
	 */
	private String findPat(ExaminationtfgCgDto param) throws BizException {
	 ...
	 * @throws BizException
	 */
	private PatiVisitDO initPatiVisitInfo(ExaminationtfgCgDto params,String patId) throws BizException {		
		StringBuilder sql = new StringBuilder();
	 ...
	 * @throws BizException
	 */
	private void setOrderFee(ExaminationtfgCgDto[] params) throws BizException {
		// 获取科室信息列表 code,id
		Map<String, String> mapDep = getMapDep(params);
		// 获取人员信息列表 code,id
		Map<String, PsnDocDO> mapPsn = getMapPsn(params);
		// 获取服务信息
		MedSrvDO[] medSrvDOs = this.initMedSvrInfo(params);
		// 循环最外层 以每一个患者为维度
		for (ExaminationtfgCgDto cgDto : params) {
			// 记账数据集合
			...
			if (medSrvDOs != null) {
				...
				for (MedSrvDO medSrvDO : medSrvDOs) {
					// 查询费用明细内容
					ExaminationtfgCgItmDto itemParamDTO = this.getBlCostAddItemParamDTO(params, medSrvDO);
					...
					// 筛选、组装记账信息
					if (directflag.equals(BookRtnDirectEnum.CHARGE)) {
						listAccount = this.getBlcgAccount(patId,entDO, ciorderdo, medSrvDO, params, itemParamDTO, mapDep,
								mapPsn,meterialDO);
					} else if (directflag.equals(BookRtnDirectEnum.RETURNS)) {
						listRefend = this.getBlCgRefundAccountDTO(patId,entDO, ciorderdo, medSrvDO, params, itemParamDTO,meterialDO);
					}
				}
				// 调用记账新接口
				...
				if (directflag.equals(BookRtnDirectEnum.CHARGE)) {
					...
					if (!keepAccountForOp.getFg_success().booleanValue()) {
						BLThirdItfLogger.error(keepAccountForOp.getExcptionstack());
					...
					if (!refundAccountForOp.getFg_success().booleanValue()) {
						BLThirdItfLogger.error(refundAccountForOp.getExcptionstack());
	 ...
	 * @return
	 */
	private BlCgRefundAccountSetDTO setCgRefundAccoutnSetDto(ExaminationtfgCgDto cgDto, Map<String, String> mapDep,
	...
	private List<BlCgRefundAccountDTO> getBlCgRefundAccountDTO(String patId,PatiVisitDO entDO, OrdSrvDO ciorderdo, MedSrvDO medSrvDO,
			ExaminationtfgCgDto[] params, ExaminationtfgCgItmDto itemParamDTO,MeterialDO meterialDO) throws BizException {
		...
		if (ciorderdo != null) {
			...
			whereStr.append(" and itmoep.id_orsrv ='" + ciorderdo.getId_orsrv() + "'");
		}
		...
		for (BlCgItmOepDO cgItmOepDo : listItmOep) {
			// 真正 要退的
			FDouble relQuan = itfQuan.sub(cgItmOepDo.getQuan());
			BlCgRefundAccountDTO cgRefundDTO = new BlCgRefundAccountDTO();
			if (relQuan.compareTo(FDouble.ZERO_DBL) > 0) {
				cgRefundDTO = this.setBlCgRefundAccountDTO(cgItmOepDo, relQuan);
			} else {
				cgRefundDTO = this.setBlCgRefundAccountDTO(cgItmOepDo, itfQuan);
			}
	 ...
	 * @return
	 */
	private BlCgOpAccountSetDTO setCgAccoutnSetDto(ExaminationtfgCgDto params, Map<String, String> mapDep,
			Map<String, PsnDocDO> mapPsn) {
	...
	private List<BlCgAccountDTO> getBlcgAccount(String patId,PatiVisitDO entDO, OrdSrvDO ciorderdo, MedSrvDO medSrvDO,
			ExaminationtfgCgDto[] params, ExaminationtfgCgItmDto itemParamDTO, Map<String, String> mapDep,
			Map<String, PsnDocDO> mapPsn,MeterialDO meterialDO ) {
		List<BlCgAccountDTO> listcgAccount = new ArrayList<BlCgAccountDTO>();
		for (ExaminationtfgCgDto cgDto : params) {
			...
			if (ciorderdo != null) {
				...
				cgAccount.setId_orsrv(ciorderdo.getId_orsrv());
			}
			// 服务id
			...
			if(!StringUtil.isEmpty(cgDto.getPeCode())){
				cgAccount.setAttrVal(IBlImplicitParamKeyConst.BL_PECODE, cgDto.getPeCode());
			}
			listcgAccount.add(cgAccount);
		}

		return listcgAccount;
	}

	private Integer getDirectFlag(ExaminationtfgCgDto params) throws BizException {
	 ...
	 * @throws BizException
	 */
	private Map<String, String> getMapDep(ExaminationtfgCgDto[] params) throws BizException {
		List<String> listdep = new ArrayList<String>();
		Map<String, String> depMap = new HashMap<String, String>();
		// 获取计量单位id集合
		for (ExaminationtfgCgDto peItfDto : params) {
	 ...
	 * @throws BizException
	 */
	private Map<String, PsnDocDO> getMapPsn(ExaminationtfgCgDto[] params) throws BizException {
	...
	private ExaminationtfgCgItmDto getBlCostAddItemParamDTO(ExaminationtfgCgDto[] params, MedSrvDO medSrvDO) {
		String chargeCode = medSrvDO.getCode();
		if (chargeCode != null) {
			for (ExaminationtfgCgDto param : params) {
				List<ExaminationtfgCgItmDto> chargeItem = param.getChargeItem();
				for (int i = 0; i < chargeItem.size(); i++) {
					ExaminationtfgCgItmDto subItem = (ExaminationtfgCgItmDto) chargeItem.get(i);
	 ...
	 */
	@SuppressWarnings("unchecked")
	private MedSrvDO[] initMedSvrInfo(ExaminationtfgCgDto[] params) throws BizException {
		FArrayList listMedSrvform = new FArrayList();
		for (ExaminationtfgCgDto param : params) {
			List<ExaminationtfgCgItmDto> chargeItem = param.getChargeItem();
			for (int i = 0; i < chargeItem.size(); i++) {
				ExaminationtfgCgItmDto subItem = (ExaminationtfgCgItmDto) chargeItem.get(i);
				if(StringUtil.isEmpty(subItem.getChargeCode())){
					if(StringUtil.isEmpty(subItem.getMmCode())){
		...
		if (listMedSrvform.size() > 0) {
			...
			if (medsrvs == null || medsrvs.length < 0) {
				throw new BizException("补费：服务编码为" + listMedSrvform + "的服务不存在！");
			}
			...
			// throw new BizException("补费：服务编码为" + medSrvDO.getCode() +
			// "的服务无效！");
			// }
