 ...
 *
 */
public class BlSaveBillApplyAuditBP {
	...
	public void exec(BlRefundSrvItemDTO [] bItemDTOs) throws BizException{
		...
		//回写审核申请单审核状态
		this.updateRefundApply(bItemDTOs[0].getId_rfdap());
	 ...
	 * @throws BizException
	 */
	private void updateRefundApply(String id_rfdap)throws BizException{
		
		if(StringUtil.isEmpty(id_rfdap)){
			throw new BizException("退费申请单id为空！");
		}
		SqlParam param=new SqlParam();
		param.addParam(id_rfdap);
		new DAFacade().execUpdate(" update bl_rfd_ap set fg_audit='Y' where id_rfdap=? ",param);
	}
	/**
	 * 新增退记账
	 * @param adoptItemList
