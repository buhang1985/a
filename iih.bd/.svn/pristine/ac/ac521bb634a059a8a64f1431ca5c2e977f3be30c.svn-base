...
public class ConsumablesUpdateEventListener implements IBusinessListener {
	...
	public void doAction(IBusinessEvent event) throws BizException {
		...
		for (int i = 0; i < objs.length; i++) {
			if (((MeterialAggDO) objs[i]).getParentDO().getSd_mmtp().equals("301")) {
		...
		ArrayList<MeterialDTO> lists = new ArrayList<MeterialDTO>();
