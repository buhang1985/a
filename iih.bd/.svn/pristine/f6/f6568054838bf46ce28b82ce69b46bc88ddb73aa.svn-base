...
public class MmExpiAlertPlugin implements IBackgroundWorkPlugin {
	...
	public PreAlertObject executeTask(BgWorkingContext bgwc) throws BizException {
		...
		// 获取数据集
		ValidPeriodView[] valis = FindExpiDataBp.queryExpiData(bgwc);
		...
		for (Mmexpialterdto dto : result.values()) {
			...
			igservice.GenerateMessageAndMap("MmExpiAlert", map, null);
	...
	private void setMmExpiDesc(List<List<String>> jsonList, ValidPeriodView validPeriodView) {
		...
		table_string.add(String.format("%s%s", validPeriodView.getQuan_stockact().setScale(4, FDouble.ROUND_UP), validPeriodView.getPkgact_name()));
