...
public class AutoCompTreatByNameSql implements ITransQry {
	...
	public String getQrySQLStr() {
		...
		sqlBuilder.append("LEFT JOIN (SELECT NAME, MAX(CODE) AS CODE, MAX(ID_HP) AS ID_HP, MAX(ID_GRP) AS ID_GRP, MAX(ID_ORG) AS ID_ORG FROM BD_HP_ZL_ORGINAL GROUP BY NAME) TREAT_ORGINAL							   ");
		sqlBuilder.append("ON (SRV.NAME = TREAT_ORGINAL.NAME AND　TREAT_ORGINAL.ID_HP = ? AND TREAT_ORGINAL.ID_GRP = ? AND TREAT_ORGINAL.ID_ORG = ?)	   ");
		sqlBuilder.append(
