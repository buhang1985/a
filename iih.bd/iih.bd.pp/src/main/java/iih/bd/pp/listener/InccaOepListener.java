...
/**
 * MS021门诊费用分类代码(发票分类（门诊）)------通过
 * */
public class InccaOepListener implements IBusinessListener {

	@Override
	public void doAction(IBusinessEvent event) throws BizException {
		if(!(event.getSourceID().equals(IncCaItmDODesc.CLASS_FULLNAME)))return;
		String action = "";
		if(event.getEventType().equals(IBdSrvIEEventConst.TYPE_INSERT_INCCA_MZ)){
			action = "insert";
		}else if(event.getEventType().equals(IBdSrvIEEventConst.TYPE_UPDATE_INCCA_MZ)){
			action = "update";
		}else if(event.getEventType().equals(IBdSrvIEEventConst.TYPE_DELETE_INCCA_MZ)){
			action = "delete";
		}else{
			return;
		}
