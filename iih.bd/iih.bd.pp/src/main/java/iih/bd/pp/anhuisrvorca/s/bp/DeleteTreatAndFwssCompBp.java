...
import iih.bd.pp.anhuisrvorca.s.bp.sql.DeleteTreatAndFwssCompSql;
...
public class DeleteTreatAndFwssCompBp {
	 ...
	 * @throws BizException
	 */
	private TreatCompDTO[] DeleteTreatAndFwssComp(TreatCompDTO[] treatCompDTOs) throws BizException {
		String wherePart = GetWherePart(treatCompDTOs);
		DeleteTreatAndFwssCompSql sql = new DeleteTreatAndFwssCompSql(wherePart);
	...
	private void UpdateTreatCompDTO(TreatCompDTO[] treatCompDTOs) {
		for (TreatCompDTO treatCompDTO : treatCompDTOs) {
			...
			treatCompDTO.setEu_hpsrvtp(HpSrvTpEnum.ZL.toString());
