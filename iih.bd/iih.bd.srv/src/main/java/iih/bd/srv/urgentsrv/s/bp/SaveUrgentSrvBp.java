...
public class SaveUrgentSrvBp {
	...
	public void exec(EnTypeDO enTypeDo, UrgentSrvDTO[] urgentSrvDtos) throws BizException {
		...
		for (MedSrvDO medSrvDO : medSrvDos) {
			...
			if (this.equalsFBoolean(urgentFlag, FBoolean.TRUE)
					&& this.equalsFBoolean(defUrgentFlag, urgentSrvDTO.getFg_urgent_def())) {
				continue;
			}

			medSrvDO.setAttrVal(urgentAttrName, FBoolean.TRUE);
			medSrvDO.setAttrVal(defUrgentAttrName, urgentSrvDTO.getFg_urgent_def());
