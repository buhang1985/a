...
public class CiOrSrvSetSumPriMdCalBP {
	...
	/**
	 * 供后台价格同步使用 by yzh 2017-05-23 10:04:12
	 * 折扣价
	 * @param param
	 * @param id_pripat
	 * @return
	 * @throws BizException
	 */
	public FDouble[] execSync(Map<String,Map<String,SrvPricalRateAndPriceDTO>> param,String id_pripat,String id_srv)  throws BizException{
		//有效性判断处理
		if(param == null)return null;
		
		//获得价格计算服务对象数组信息
		Object[] calsrvgrps=getPriCalSrvObjsSync(param,id_srv);
		IPriCalService ipricalsrv=CiOrdAppUtils.getPriCalService();
	...
	/**
	 * 供后台价格同步使用 by yzh 2017-05-23 10:04:38
	 * 获得价格计算服务对象数组信息
	 * @param list
	 * @return
	 */
	private Object[] getPriCalSrvObjsSync(Map<String,Map<String,SrvPricalRateAndPriceDTO>> Map,String id_srv){
		Object[] rtn=new Object[2];
		BdSrvPriCalParamDTO param=null;
		String id_primd=null;
		ArrayList<PriStdSrvDTO> pristdsrvdtos=new ArrayList<PriStdSrvDTO>();
		ArrayList<String> compoundsrvs=new ArrayList<String>();
		Map<String,SrvPricalRateAndPriceDTO> listMap = Map.get(id_srv);
		//遍历
		Set<Entry<String, SrvPricalRateAndPriceDTO>> set= listMap.entrySet();  
		Iterator<Entry<String, SrvPricalRateAndPriceDTO>>  it=  set.iterator();
		while(it.hasNext()){  
            Entry<String, SrvPricalRateAndPriceDTO> item=it.next();  
            String key=item.getKey();  
            SrvPricalRateAndPriceDTO value=item.getValue();  
            try {
				id_primd=CiOrdAppUtils.getMedsrvMDORService().findById(param.getId_srv()).getId_primd();
			} catch (BizException e) {
				e.printStackTrace();
