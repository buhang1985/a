...
public class SyncBdSrvPriceTask implements IBackgroundWorkPlugin {
...
//				obj.setReturnType(PreAlertReturnType.RETURNNOTHING);
			...
			// 是否同步物品价格
			String isSyncMm = "Y";
			if (arg0.getKeyMap().containsKey("isSyncMm"))
				isSyncMm = CiOrdUtils.isEmpty(arg0.getKeyMap().get("isSyncMm")) ? ""
						: arg0.getKeyMap().get("isSyncMm").toString();
			...
			for (int i = 0; i < page; i++) {
				...
				if (i == 0) {
					syncSrvOrSetPrice4(medsrvCrudServiceImpl,
							(MedsrvAggDO[]) resultList.toArray(new MedsrvAggDO[resultList.size()]), true);
				} else {
					syncSrvOrSetPrice4(medsrvCrudServiceImpl,
							(MedsrvAggDO[]) resultList.toArray(new MedsrvAggDO[resultList.size()]), false);
				}
			}
...
//				dafacade.execUpdate(upSql);
			...
			// 根据前台传入的参数判断是否同步物品价格 前台传入空 或Y 则同步物品价格 by yzh 2017-10-26 11:34:06
			if (CiOrdUtils.isEmpty(isSyncMm) || !isSyncMm.equals("N"))
...
//		obj.setReturnType(PreAlertReturnType.RETURNNOTHING);
	...
	private MedsrvAggDO[] syncSrvOrSetPrice4(MedsrvCrudServiceImpl medsrvCrudServiceImpl, MedsrvAggDO[] szMedsrvAggDO,
			boolean isDelete) throws BizException {
		BdSrvLogUtils.info("=>开始执行后台服务价格同步<=");
		// 待插入list
