...
public class CiOrBdSrvPriceCalBP {
	...
	public MedSrvPriceDO exec(BdSrvPriCalParamDTO param,String id_pripat)  throws BizException{
		...
		//增加集团和组织ID by yzh 2017-07-26 14点04分
		medSrvPriceDO.setId_org(Context.get().getOrgId());
		medSrvPriceDO.setId_grp(Context.get().getGroupId());
		...
		if(CiOrSrvPriHelper.isSrvSelfPrimd(id_primd)){//本服务定价
			...
			medSrvPriceDO.setPrice_ratio(o.getPrice().multiply(o.getRate()));
