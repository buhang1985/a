...
public class GetPregDocBP {
	...
	public PregDocDTO exec(String patId) throws BizException {
		...
		// 设置当日预约数
		setCountToday(pregDocDTO);

		// 设置当月数
		setCountMonth(pregDocDTO);
	 ...
	 * @throws BizException
	 */
	public void setCountToday(PregDocDTO pregDocDTO) throws BizException {
		StringBuilder sqlSb = new StringBuilder();
		sqlSb.append(" select count(*) count");
		sqlSb.append(" from EN_DOC doc");
		sqlSb.append(" inner join EN_DOC_PREG preg on doc.id_entdoc = preg.id_entdoc");
		sqlSb.append(" where preg.eu_status in (?,?)");
		sqlSb.append(" and preg.d_expect = ? ");
		SqlParam params = new SqlParam();
		params.addParam(DocStatusEnum.EU_STATUS_APPT);
		params.addParam(DocStatusEnum.EU_STATUS_REG);
		params.addParam(pregDocDTO.getD_expect());
		@SuppressWarnings("unchecked")
		Map<String, Integer> dates = (Map<String, Integer>) new DAFacade().execQuery(sqlSb.toString(), params,
				new MapHandler());
		pregDocDTO.setNum_today(dates.get("count").toString());
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void setCountMonth(PregDocDTO pregDocDTO) throws BizException {
		StringBuilder sqlSb = new StringBuilder();
		sqlSb.append(" select count(*) count");
		sqlSb.append(" from EN_DOC doc");
		sqlSb.append(" inner join EN_DOC_PREG preg on doc.id_entdoc = preg.id_entdoc");
		sqlSb.append(" where preg.eu_status in (?,?)");
		sqlSb.append(" and preg.d_expect like '?%'");
		SqlParam params = new SqlParam();
		params.addParam(DocStatusEnum.EU_STATUS_APPT);
		params.addParam(DocStatusEnum.EU_STATUS_REG);
		params.addParam(pregDocDTO.getD_expect().toString().substring(0, 8));
		Map<String, Integer> dates = (Map<String, Integer>) new DAFacade().execQuery(sqlSb.toString(), params,
				new MapHandler());
		pregDocDTO.setNum_month(dates.get("count").toString());
	}
}
