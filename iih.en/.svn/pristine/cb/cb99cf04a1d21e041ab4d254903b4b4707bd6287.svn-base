...
public class GetEntListByPatcodeBP {
	...
	public PatEntInfoDTO[] exec(String patCode, String dtBegin, String dtEnd, String entpCode) throws BizException {
		...
		if (EnValidator.isEmpty(dtBegin) || EnValidator.isEmpty(dtEnd)) {
			return null;
		}
		//从chis同步患者
		...
		sqlSb.append(" select");
		sqlSb.append(" pat.code as patCode,");
		...
		if(!EnValidator.isEmpty(patCode)){
			sqlSb.append(" and ent.id_pat in (select id_pat from pi_pat where code = ?)");
