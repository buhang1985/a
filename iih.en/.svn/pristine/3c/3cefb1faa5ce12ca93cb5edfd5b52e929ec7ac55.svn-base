...
/**
 * 修改用户 - 组织关系后,同步到科室员工关系表(删除)  监听类
 * @author zhang.zl
 *
 */
public class SynchroEmpDepDelListener implements IBusinessListener{
	...
	public void doAction(IBusinessEvent arg0) throws BizException {
		...
		FArrayList empIds = new FArrayList();
		for(SubjectOrgDO o:dos){
			empIds.add(o.getSubjectid());	
		}
		IEmpdepRService serv2 = ServiceFinder.find(IEmpdepRService.class);
		EmpDepDO[] emps = serv2.findByAttrValList(EmpDepDO.ID_EMP,empIds);
		if(emps == null || emps.length<=0){
			return;
		}
		for (EmpDepDO empDepDO : emps) {
			empDepDO.setStatus(DOStatus.DELETED);
			list.add(empDepDO);
		}
	...
	private boolean validation(IBusinessEvent arg0) {
		if (IEnEventConst.EVENT_SOURCE_EN_IP_SYNCHROEMPDEP.equals(arg0.getSourceID()) && IEventType.TYPE_DELETE_AFTER.equals(arg0.getEventType()))
			return true;
		return false;
	}
