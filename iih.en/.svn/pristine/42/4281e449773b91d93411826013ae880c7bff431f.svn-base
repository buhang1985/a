 ...
 *
 */
public class GetUrgNurErInfoBP {
	/***
	 * 获取急诊留观护士站-急诊信息 DTO数据
	 * 
	 * @param id_ent
	...
	private UrgNurErInfoDTO getDataBySql(String id_ent) throws BizException{
		...
		sql.append("left join en_ent_ip_er iper on iper.id_ent = ent.id_ent " );
		sql.append("left join bd_udidoc scale on scale.id_udidoc = iper.id_scale_triage " );
		sql.append("left join en_erpre er on er.id_erpre = iper.id_erpre " );
		sql.append("left join (select di.id_ent,di.name_didef_dis from en_ent_di di inner join en_ent ent on ent.id_ent = di.id_ent where di.fg_maj = 'Y' group by di.id_ent,di.name_didef_dis) endi " );
		sql.append("on endi.id_ent = ent.id_ent " );
		sql.append("left join en_erpre_score score on score.id_erpre = iper.id_erpre " );
	...
	private UrgNurErInfoDTO getScore(UrgNurErInfoDTO urgNurErInfoDTO,String id_ent) throws BizException{
		StringBuffer sql = new StringBuffer();
		sql.append("select iper.sd_scale_triage as Sd_level,");
		sql.append("scale_triage.name as Name_level,");
		...
		sql.append("left join en_erpre_score score on score.id_erpre = iper.id_erpre " );
		sql.append("left join bd_udidoc sco on sco.code = score.sd_scotp and sco.id_udidoclist in (select id_udidoclist from bd_udidoclist where code = ? ) " );
		sql.append("left join bd_udidoc scale_triage on iper.id_scale_triage = scale_triage.id_udidoc ");
		sql.append("where ent.code_entp = ? ");
		sql.append("and ent.id_ent = ? ");
		SqlParam param = new SqlParam();
		param.addParam(IEnDictCodeTypeConst.SD_ERPRE_SCOTP);
	...
	private void getGreenChannel(UrgNurErInfoDTO urgNurErInfoDTO,String id_ent) throws BizException{
		StringBuffer sql = new StringBuffer();
		sql.append("select er.id_erpre,er.ids_green_channel_sym ");
		sql.append("from en_erpre er " );
		sql.append("inner join en_ent_ip_er iper on  iper.id_erpre = er.id_erpre " );
		sql.append("where iper.id_ent = ? ");
		SqlParam param = new SqlParam();
		param.addParam(id_ent);
		@SuppressWarnings("unchecked")
		List<EnErPreDO> list = (List<EnErPreDO>) new DAFacade().execQuery(sql.toString(), param, new BeanListHandler(EnErPreDO.class));
		if(list != null && list.size() > 0){
