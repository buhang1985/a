...
public class LvHosCheckBP {
	 ...
	 * @throws BizException
	 */
	public void exec(LvHosAppDTO[] appDTOs) throws BizException {
	 ...
	 * @throws BizException
	 */
	private void checkOr(LvHosAppDTO[] appDTOs, OutDepChkInfo chkInfo) throws BizException {
		// 返回状态
		if (EnValidator.isEmpty(appDTOs))
			return;
	 ...
	 * @throws BizException
	 */
	private void checkMm(LvHosAppDTO[] appDTOs, OutDepChkInfo chkInfo) throws BizException {
		if (EnValidator.isEmpty(appDTOs))
			return;
	 ...
	 * @throws BizException
	 */
	private void checkMt(LvHosAppDTO[] appDTOs, OutDepChkInfo chkInfo) throws BizException {
		if (EnValidator.isEmpty(appDTOs))
			return;
		//获取医技应该核查的类型
	 ...
	 * @throws BizException
	 */
	private void checkMp(LvHosAppDTO[] appDTOs, OutDepChkInfo chkInfo) throws BizException {
		if (EnValidator.isEmpty(appDTOs))
			return;
		//获取执行应该核查的类型
	 ...
	 * @throws BizException
	 */
	private void checkBl(LvHosAppDTO[] appDTOs, OutDepChkInfo chkInfo) throws BizException {
		if (EnValidator.isEmpty(appDTOs))
			return;
	 ...
	 * @return
	 */
	public static String[] assemlyOrParams(LvHosAppDTO[] appDTOs){
	 ...
	 * @return
	 */
	public static String[] assemlyLvMmParams(LvHosAppDTO[] appDTOs){
		return EnAppUtils.getKeyPropArrayFromDO("Id_ent", appDTOs);
	}
	/**
	 * 组装检查费用参数
	 ...
	 * @return
	 */
	public static BlIpOutPatPriQryDTO[] assemlyBlCheckParams(LvHosAppDTO[] appDTOs){
		Map<String, LvHosAppDTO> map = EnFMapUtils.getMap("Id_ent", appDTOs);
		...
		for (LvHosAppDTO appDTO : appDTOs) {
			...
			if(FBoolean.TRUE.equals(appDTO.getFg_blrecall())){
				...
				if(FBoolean.TRUE.equals(appDTO.getFg_nb()) && appDTO.getDt_outor() == null){
					if(!EnValidator.isEmpty(appDTO.getId_ent_mom()) && map.containsKey(appDTO.getId_ent_mom())){
						LvHosAppDTO mom = map.get(appDTO.getId_ent_mom());
						qryDTO.setId_ent(appDTO.getId_ent());
						qryDTO.setDt_end(mom.getDt_outor());
						flag = false;
					}
				}
				if (flag) {
	 ...
	 * @throws BizException
	 */
	public static EnServiceParamDTO[] assemlyMpParams(LvHosAppDTO[] appDTOs, String fg_tech) {
	 ...
	 * @throws BizException
	 */
	private void checkOther(LvHosAppDTO[] appDTOs) throws BizException {
		if(EnValidator.isEmpty(appDTOs))
			return;
