 ...
 *
 */
public class GetOperateLogListBP {
	/***
	 * 查询登录日志
	 * 
	 * @param pg
	 ...
	 * @throws BizException
	 */
	public PagingRtnData<EnOperateLogDTO> exec(PaginationInfo pg, EnOperateLogDTO nodeDTO) throws BizException{
		return getsql(pg,nodeDTO);
	}
	/***
	 * 获取登录日志sql
	 * @param pg
	 ...
	 * @throws BizException
	 */
	private PagingRtnData<EnOperateLogDTO> getsql(PaginationInfo pg,EnOperateLogDTO nodeDTO) throws BizException{
		...
		EnOperateLogDTO regInfo = new EnOperateLogDTO();
		PagingServiceImpl<EnOperateLogDTO> pageQryService = new PagingServiceImpl<EnOperateLogDTO>();	
		PagingRtnData<EnOperateLogDTO> pgRtData = pageQryService.findByPageInfo(regInfo, pg, sql.toString(), null, param);
		return pgRtData;
	}
}
