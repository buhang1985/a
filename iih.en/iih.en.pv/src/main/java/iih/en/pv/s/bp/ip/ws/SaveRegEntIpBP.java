...
import iih.en.comm.ep.AdminAreaEP;
...
import iih.en.pv.s.bp.ip.ws.bean.SaveRegEntIpBean;
import iih.en.pv.s.bp.ip.ws.bean.SaveRegEntIpResult;
import iih.en.pv.s.bp.ip.ws.bean.SaveRegEntIpResultBean;
 ...
 *
 */
public class SaveRegEntIpBP {

	/**
	 * 操作类型-修改
	 */
	private String OPERTP_UPDATE = "1";
	/**
	 * 操作类型-修改
	 */
	private String OPERTP_NEW = "0";
	...
	public String exec(String param)  {
		SaveRegEntIpResultBean resultBean = new SaveRegEntIpResultBean();
        try {
            final SaveRegEntIpBean params = WsXmlUtils.deSerializeBaseDO(param, SaveRegEntIpBean.class);
            //校验入参是否为空
    		this.validateParam(params, resultBean);
    		...
    		//设置患者DTO数据
    		EnHosPatDTO enHosPat = this.getEnHosPatInfo(params, udilistMap);
    		//设置就诊DTO数据
    		EnHosRegDTO enHosReg = this.getEnHosRegInfo(params, enHosPat, udilistMap);
    		//数据校验
    		...
    		//设置返回结果
    		this.getResultInfo(resultBean, enHosPat, enHosReg);
	 ...
	 * @throws BizException
	 */
	private void validateParam(SaveRegEntIpBean params, SaveRegEntIpResultBean resultBean) throws BizException {
		String message = null;
		if (EnValidator.isEmpty(params.getOperTp())){
			message = "参数操作类型为空!";
		}
		if (EnValidator.isEmpty(params.getCodePat())){
			message = "参数患者编号为空！";
		}
		if (EnValidator.isEmpty(params.getName())){
			message = "参数姓名为空！";
		}
		if (OPERTP_UPDATE.equals(params.getOperTp())){
			if (EnValidator.isEmpty(params.getCode_amr_ip())){
				message = "参数住院档案号为空！";
			}
			if (EnValidator.isEmpty(params.getTimes_ip())){
				message = "参数住院次数为空！";
			}
		}
		if (OPERTP_NEW.equals(params.getOperTp())){
			if (EnValidator.isEmpty(params.getId_scaptip())){
				message = "参数入院许可证为空！";
			}
		}
		if (!EnValidator.isEmpty(message)) {
	...
	private Map<String, Map<String, UdidocDO>> getUdidocListMap() throws BizException{
		...
		Map<String, Map<String, UdidocDO>> udiListMap = 
				new UdiDocEP().getUdiDocsMapByCodes(udiCodeList.toArray(new String[0]));
		return udiListMap;
	}
	 ...
	 * @throws BizException
	 */
	private Map<String, AdminAreaDO> getAdminAreaMap(SaveRegEntIpBean params) throws BizException{
		ArrayList<String> list = new ArrayList<String>();
		if (!EnValidator.isEmpty(params.getSd_admdiv_nati())) {//籍贯
			list.add(params.getSd_admdiv_nati());
		}
		if (!EnValidator.isEmpty(params.getBirth_sd_admdiv())) {//出生地
			list.add(params.getBirth_sd_admdiv());
		}
		if (!EnValidator.isEmpty(params.getCuraddr_sd_admdiv())) {//现住址
			list.add(params.getCuraddr_sd_admdiv());
		}
		if (!EnValidator.isEmpty(params.getWorkunit_sd_addmdiv())) {//单位地址
			list.add(params.getWorkunit_sd_addmdiv());
		}
		if (!EnValidator.isEmpty(params.getIdent_sd_admdiv())) {//户口地址
		...
		Map<String, AdminAreaDO> adminAreaMap = 
				new AdminAreaEP().getAdminAreasByCodeList(list.toArray(new String[0]));
		return adminAreaMap;
	}
	 ...
	 * @throws BizException 
	 */
	private EnHosPatDTO getEnHosPatInfo(SaveRegEntIpBean params, Map<String, Map<String, UdidocDO>> udilistMap) throws BizException{
		...
		PatDO[] patis = new PiPatEP().getMRServ().findByAttrValString("Code", params.getCodePat());
		if (patis == null) {
		...
		enHosPat.setCode_pat(params.getCodePat());
		//性别
		if (!EnValidator.isEmpty(params.getSd_sex())) {
		...
		//出生日期
		if(!EnValidator.isEmpty(params.getDt_birth())){
		...
		//移动电话
		if (!EnValidator.isEmpty(params.getTelno())) {
			enHosPat.setMob(params.getTelno());
		}else{
			throw new BizException("未获取到患者的移动电话");
		}
		//证件类型
		if (!EnValidator.isEmpty(params.getSd_idtp())) {
		...
		//证件号码
		if (!EnValidator.isEmpty(params.getId_code())) {
			enHosPat.setId(params.getId_code());
		}else{
			throw new BizException("未获取到患者的证件号码");
		}
		//婚否
		if (!EnValidator.isEmpty(params.getSd_marry())) {
		...
		//国家地区
		if (!EnValidator.isEmpty(params.getSd_country())) {
		...
		//籍贯-行政区划
		if (!EnValidator.isEmpty(params.getSd_admdiv_nati())) {
		...
		//民族
		if (!EnValidator.isEmpty(params.getSd_nation())) {
		...
		//现住址-行政区划
		if (!EnValidator.isEmpty(params.getCuraddr_sd_admdiv())) {
		...
		//单位地址-行政区划
		if (!EnValidator.isEmpty(params.getWorkunit_sd_addmdiv())) {
			if (adminAreaMap != null && adminAreaMap.containsKey(params.getWorkunit_sd_addmdiv())) {
				AdminAreaDO admdivWork = adminAreaMap.get(params.getWorkunit_sd_addmdiv());
		...
		//户口地址-行政区划
		if (!EnValidator.isEmpty(params.getIdent_sd_admdiv())) {
		...
		//出生地-行政区划
		if (!EnValidator.isEmpty(params.getBirth_sd_admdiv())) {
		...
		//职业
		if (!EnValidator.isEmpty(params.getSd_occu())) {
		...
		//联系人类型
		if (!EnValidator.isEmpty(params.getCont_sd_conttp())) {
	 ...
	 * @throws BizException
	 */
	private EnHosRegDTO getEnHosRegInfo(SaveRegEntIpBean params, EnHosPatDTO enHosPat, Map<String, Map<String, UdidocDO>> udilistMap) throws BizException {
		...
		if (OPERTP_NEW.equals(params.getOperTp())) {
			...
			//生成就诊编码
			enHosReg.setCode_ent(EnCodeUtils.createEntCode());
			//患者分类、价格分类、信用分类
			PiPatCaDO piPatCa = new PiPatEP().getPatDefaultCate();
		...
		//入院时间
		if (!EnValidator.isEmpty(params.getDt_entry())) {			
			enHosReg.setDt_entry(new FDateTime(params.getDt_entry()));
		}else{
			throw new BizException("未获取到患者的入院时间");
		}
		//入院方式
		if (!EnValidator.isEmpty(params.getSd_referalsrc())) {
		...
		if (!EnValidator.isEmpty(params.getId_scaptip())) {
			...
			//入院医生
			if(!EnValidator.isEmpty(aptIp.getId_emp_phy_op())){
			...
			//入院科室
			if (!EnValidator.isEmpty(aptIp.getId_dep_phy_ip())) {
			...
			//入院病区
			if (!EnValidator.isEmpty(aptIp.getId_dep_nur_ip())) {
			...
			//入院病情
			if (!EnValidator.isEmpty(aptIp.getSd_level_dise())) {
			...
			//门诊主要诊断
			//优先取预诊，没有取门诊主诊断
			if (!EnValidator.isEmpty(aptIp.getId_didef_op())) {
	 ...
	 * @param enHosReg
	 */
	private void getResultInfo(SaveRegEntIpResultBean resultBean, EnHosPatDTO enHosPat, EnHosRegDTO enHosReg){
		SaveRegEntIpResult result = new SaveRegEntIpResult();
		result.setCodePat(enHosPat.getCode_pat());
		result.setCode_amr_ip(enHosPat.getCode_amr_ip());
		result.setTimes_ip(Integer.toString(enHosReg.getTimes()));
        resultBean.setData(new SaveRegEntIpResult[]{result});
	}
