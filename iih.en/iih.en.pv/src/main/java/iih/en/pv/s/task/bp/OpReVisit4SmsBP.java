...
public class OpReVisit4SmsBP {
	...
	public void exec(String orgId) throws BizException{
		...
		for(OpApt4SmsDTO opApt4SmsDTO : opApt4SmsDTOs){
			...
			String result = "";
			try{
				result = icservice.sendOpSmsMsg(opApt4SmsDTO.getMobile(), IScSmsConst.SC_DIAG_END_MSG_REVISIT,opApt4SmsDTO);
	...
	public OpApt4SmsDTO[] getOpApt4SmsDTO(String orgId) throws BizException{
		
		String str = EnParamUtils.getNoteRtnDepIds(orgId);
		if(EnValidator.isEmpty(str))
			return null;
		String[] depIds = str.split(",");		
		StringBuilder sqlBuilder = new StringBuilder();
		SqlParam param = new SqlParam();
		sqlBuilder.append("select ent.id_ent as Id_ent,ent.name_pat as Name_pat,pat.code as code_pat,ent.dt_entry as Dt_entry,ent.telno_pat as Mobile,dep.name as Name_dep, ");
		...
		sqlBuilder.append("on op.id_ent=ent.id_ent ");
		...
		sqlBuilder.append("and ent.dt_acpt < ? ");
		sqlBuilder.append("and ").append(EnSqlUtils.getInSqlByIds("ent.id_dep_phy", depIds));		
		sqlBuilder.append("and ent.id_pat not in ");	
		sqlBuilder.append("(select b.id_pat from en_ent b ");
		sqlBuilder.append("where b.dt_acpt > ? ");
		sqlBuilder.append("and ").append(EnSqlUtils.getInSqlByIds("b. id_dep_phy", depIds));
		sqlBuilder.append(" ) ");
		
		param.addParam(IBdFcDictCodeConst.SD_CODE_ENTP_OP);
		param.addParam(FBoolean.FALSE);
		FDate today = EnAppUtils.getServerDate();
		param.addParam(today.getDateAfter(-14));
		param.addParam(today.getDateAfter(-13));
		param.addParam(today.getDateAfter(-13));
