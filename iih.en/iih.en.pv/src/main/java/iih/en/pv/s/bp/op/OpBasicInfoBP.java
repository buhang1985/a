...
public class OpBasicInfoBP {
	 ...
	 * @throws BizException
	 */
	private void savePatContInfo(OpBasic4EditDTO dto, int type) throws BizException {
	...
	private int geiPatContMaxSortNoById(String id_pat) throws BizException {
		...
		if (!EnValidator.isEmpty(id_pat)) {
			String sql = String.format("SELECT MAX(SORTNO) AS SORTNO FROM PI_PAT_CONT WHERE ID_PAT = '%s' ", id_pat);
			sortNo = this.queryMaxBySql(sql);
		}
		return sortNo;
	}
	...
	private int getEntContMaxSortNoById(String id_ent) throws BizException {
		...
		if (!EnValidator.isEmpty(id_ent)) {
			String sql = String.format("SELECT MAX(SORTNO) AS SORTNO FROM EN_ENT_CONT WHERE ID_ENT = '%s' ", id_ent);
			sortNo = this.queryMaxBySql(sql);
		}
		return sortNo;
	}
	...
	private int queryMaxBySql(String sql) throws BizException {
		int sortNo = 0;
		FArrayList list = AppFwUtil.getMapListWithDao(sql);
		if (!EnValidator.isEmpty(list)) {
			Map<String, Object> map = (Map<String, Object>) list.get(0);
			if (null != map && null != map.get("sortno")) {
				sortNo = Integer.parseInt(map.get("sortno").toString());
			}
		}
		return sortNo;
	}
