 ...
 *
 */
public class EnOrCancelCheck4IllState extends AbstractOrCancelCheckListener {
	...
	protected void doYourActionAfterOrCancelCheck(CiOrderDO[] ors) throws BizException {
		...
		EntStateDO[] entStateDOs = ep.getEntstates(ors[0].getId_en(), ors[0].getId_pat(), ors[0].getCode_entp(), new String[]{IBdFcDictCodeConst.SD_ENSTSTATETP_DISE});
		if (EnValidator.isEmpty(entStateDOs))
			return;
		GetMatchLevelValBP valBp = new GetMatchLevelValBP();
		String val = valBp.exec(IBdFcDictCodeConst.SD_ENSTSTATETP_DISE, ors[0].getId_srv());
		if (entStateDOs[0].getVal() != null 
			...
			entStateDTO.setVal(new EntStateEP().getEnStateDefVal(IBdFcDictCodeConst.SD_ENSTSTATETP_DISE));//
