 ...
 *
 */
public class OpCallingNumBP {
	
    private EnQueEP eqBP;
	
	public OpCallingNumBP(){
	 ...
	 * @throws BizException
	 */
	public EnQueueDO callingNum(QueDO que, QueSiteDO queSite, EnTrDocSiteInfoDTO siteInfo, String entId, Integer callNum) throws BizException{
		...
		this.checkMaxCallNum(que, queSite, siteInfo, entId, callNum);
		
		EnQueueDO[] enQueueDOs = this.callingNums(que, queSite, siteInfo);
		if(!EnValidator.isEmpty(enQueueDOs)){
			if(IEnDictCodeConst.SD_STATUS_ACPT_CALL.equals(enQueueDOs[0].getSd_status_acpt())){
				//触发叫号事件
				this.invoke(que.getId_que(), queSite.getId_quesite(), enQueueDOs[0].getCode_entp(), enQueueDOs[0].getId_ent());
				return enQueueDOs[0];
			}
		}
		return null;
	 ...
	 * @throws BizException
	 */
	public EnQueueDO[] callingNums(QueDO que, QueSiteDO queSite, EnTrDocSiteInfoDTO siteInfo) throws BizException{
		EnValidator.validateParam("医生出诊队列", que);
		EnValidator.validateParam("医生工作站点", queSite);
		EnValidator.validateParam("当前日期", siteInfo.getDate());
		EnValidator.validateParam("当前时间分组", siteInfo.getId_dayslot());
		EnQueueDO[] nques = this.getCallEntQues(que, queSite, siteInfo);
		if(EnValidator.isEmpty(nques))
			return null;
		int sortnoCalled = this.getMaxSortnoCalled(que.getId_que(), queSite.getId_quesite(), siteInfo);
		...
		for(EnQueueDO enque : nques){
			...
			if(enque.getSortno_called() != null && enque.getSortno_called() > 0)
				continue;
			enque.setId_que(queSite.getId_que());
			enque.setId_que_site(queSite.getId_quesite());
			enque.setId_emp_acpt(queSite.getId_emp());
			enque.setName_emp_acpt(queSite.getEmp_name());
			if(isTriageToEmp)
	 ...
	 * @throws BizException 
	 */
	public EnQueueDO[] getCallEnt(QueDO que, QueSiteDO queSite, EnTrDocSiteInfoDTO siteInfo) throws BizException{
		String whereStr = String.format("%s.ID_QUE = '%s' AND %s.ID_QUE_SITE='%s' AND %s.SD_STATUS_ACPT = '%s' AND %s.DT_REGIST = '%s' AND %s.ID_DATESLOT = '%s' AND %s.FG_ACTIVE = '%s' ",
	 ...
	 * @throws BizException
	 */
	public EnQueueDO[] getCallEntQues(QueDO que, QueSiteDO queSite, EnTrDocSiteInfoDTO siteInfo) throws BizException{
		List<EnQueueDO> nques = new LinkedList<>();
		//1.获取叫号的记录
		EnQueueDO[] callQues = this.getCallEnt(que, queSite, siteInfo);
		...
		//获取候诊队列
		List<EnQueueDO> waitList = this.getWaitingEntQues(que, queSite, siteInfo);
	 ...
	 * @throws BizException
	 */
	private List<EnQueueDO> getWaitingEntQues(QueDO que, QueSiteDO queSiteDO, EnTrDocSiteInfoDTO siteInfo) throws BizException{
		...
		if (IBdFcDictCodeConst.SD_MODEL_OP_EMP.equals(que.getSd_mode())) {
			...
			builder.append("AND ( EQ.ID_QUE_SITE = ?  OR ISNULL(EQ.ID_QUE_SITE,'~')='~') ");
	 ...
	 * @throws BizException
	 */
	public int getMaxSortnoCalled(String queId, String siteId, EnTrDocSiteInfoDTO siteInfo) throws BizException {
		StringBuilder builder = new StringBuilder();
	 ...
	 * @throws BizException 
	 */
	private void checkMaxCallNum(QueDO que, QueSiteDO queSite, EnTrDocSiteInfoDTO siteInfo, String entId, Integer callNum) throws BizException{
		if (EnValidator.isEmpty(entId)|| callNum == null)
			return;
		int maxCallNum = EnParamUtils.getMaxCallNum(que.getId_dep());
		if(maxCallNum > 0 && callNum >= maxCallNum){
			//取出[候诊队列]的第一个就诊记录,如果其为叫号状态
			EnQueueDO[] queues = getCallEnt(que, queSite, siteInfo);
