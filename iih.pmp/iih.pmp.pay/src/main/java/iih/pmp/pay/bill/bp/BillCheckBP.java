...
public class BillCheckBP {
	...
	private void checkTrade(ChannelBillDO[] chls) throws BizException{
		for(int i=0;i<chls.length;i++){
			...
			String sql=" select a.Id_channel_bill_item,b.id_trade, a.mch_trade_no,a.total_fee,b.amount,b.trade_status,a.check_rst,b.check_rst as tcheck_rst from pmp_chlbillitem a left join pmp_trade b on a.mch_trade_no=b.trade_no where "+"   a.trade_status='SUCCESS' and "+ChlBillItemDO.ID_CHANNEL_BILL+"='"+bill.getId_channel_bill()+"'";
			...
			for(int j=0;j<b.size();j++){
				...
				double total_fee=this.getdouble(o.get("total_fee"));
...
//						sb2.put(getString(o.get("id_trade")), BillChekRst.CHECKFAIL);
						...
						if(ts==TradeStatusEnum.PAY_SUCCESS||ts==TradeStatusEnum.TURN_REFUND){
						if(sb3.length()==0){
			...
			this.updateUncheck(bill.getDt_bill(), bill.getDt_bill().getDateAfter(1));
...
//		 _tradeservice.save(items);
	...
	private void updateUncheck(FDate begine,FDate end) throws BizException{
if(begine!=null&&end!=null){
			 ITradeRService _traderservice=ServiceFinder.find(ITradeRService.class);
			 TradeDO[] items=_traderservice.find(BillCheckAppUtils.getTimeSpanSql(begine, end, TradeDO.DT_REQ), "", FBoolean.FALSE);
			 for (TradeDO chlBillItemDO : items) {
