...
public class AlipayQuery2BP {
	 ...
	 * @throws BizException
	 */
	public OrdQryRstDTO[] exec(OrdReqDTO dto) throws BizException {
		...
		// 如果订单状态为已支付则回写对应支付成功交易记录的交易号和交易完成时间
		OrdQryRstDTO rst = pmputils.ord2qryrst(o);
		if (o.getOrd_status() == OrdStatusEnum.PAID) {
			TradeDO trade = findPayTrade(o);
			if (trade != null) {
				rst.setTrade_no(trade.getTrade_no());
				rst.setTradetime(trade.getDt_finish());
			}
		}

		if (result != null && result.getTradeStatus() == TradeStatus.UNKNOWN) {
			rst.setBiz_status(BizStatusEnum.BIZ_FAIL);
			rst.setErr_code(Constants.ERROR);
			rst.setErr_code_des("支付宝查询服务暂不可用");
		} else {
	...
	private TradeDO findPayTrade(OrdDO o) throws BizException {

		ITradeRService service = ServiceFinder.find(ITradeRService.class);
		// 根据订单获取已成功支付交易记录，此记录每个订单只有一条
		String whereSql = TradeDO.ID_ORD + " = '" + o.getPkVal() + "' and " + TradeDO.EU_DIRECT + " = "
				+ PmpPayConst.TRADE_DIRECT_PAY + " and " + TradeDO.TRADE_STATUS + " = " + TradeStatusEnum.PAY_SUCCESS;
		TradeDO[] rst = service.find(whereSql, "", FBoolean.FALSE);
		return ArrayUtil.isEmpty(rst) ? null : rst[0];
	}
