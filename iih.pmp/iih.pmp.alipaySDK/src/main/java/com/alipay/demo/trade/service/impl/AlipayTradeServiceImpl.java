...
public class AlipayTradeServiceImpl extends AbsAlipayTradeService {
	...
	public static class ClientBuilder {
		...
		public AlipayTradeServiceImpl build() throws NumberFormatException, BizException {
	...
	public AlipayTradeServiceImpl(ClientBuilder builder) throws NumberFormatException, BizException {
	...
	public AlipayF2FPayResult tradePay(AlipayTradePayRequestBuilder builder) throws AlipayApiException {
		...
		PmpAliPayLogUtils.info("支付入参：%s.", JSON.toJSONString(request));

		// 首先调用支付api
		AlipayTradePayResponse response = (AlipayTradePayResponse) getResponse(client, request);

		PmpAliPayLogUtils.info("支付出参：%s.", JSON.toJSONString(response));
