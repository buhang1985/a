...
import iih.ei.itf.md2.markdown.builder.CodeBuilder;
import iih.ei.itf.md2.markdown.builder.CommonTextBuilder;
import iih.ei.itf.md2.markdown.builder.HeaderBuilder;
import iih.ei.itf.md2.markdown.builder.OrderedListBuilder;
import iih.ei.itf.md2.markdown.builder.QuoteBuilder;
import iih.ei.itf.md2.markdown.builder.UnorderedListBuilder;
		...
		try {
			...
			for (int idx = 0, si = lines.size(); idx < si; idx++) {
				...
				if (str.trim().startsWith(MDToken.CODE) && hasCode) {
					String codeType = str.trim().substring(3, str.trim().length());
					...
					if (isCodeEnd) {
						block = new CodeBuilder(sb.toString(), codeType).bulid();
			...
			for (int i = 0, l = lines.size(); i < l; i++) {
				...
				if (hasTable) {
					if ((i + 1) < l) { // 检查到符合规范的table头之后，检测下一行是否为 ---|---的类似字符串
						...
						String[] nextParts = nextLine.substring(1, nextLine.length()).split("\\|");

						tableColumnWidths = new float[nextParts.length];
						float sum = 0F;
						for (int j = 0; j < nextParts.length; j++) {
							String part = nextParts[j];
							if (part.trim().replaceAll("-", "").length() > 0) {
								hasTable = false;
							}
							if (!hasTable) {
								break;
							...
							tableColumnWidths[j] = count;
							sum += count;
						}

						for (int j = 0; j < tableColumnWidths.length; j++) {
							tableColumnWidths[j] = tableColumnWidths[j] / sum;
						}
					} else {
						hasTable = false;
					}
				...
				if (hasTable) { // 检查到真的有table存在
					...
					for (int j = i; j < l; j++) {
						...
						if (tableLine.startsWith("|")) {
							tableLine = tableLine.substring(1, tableLine.length());
						}
...
//		}
	 ...
	 * @return
	 */
	public static List<ValuePart> analyzeTextLine(String text, List<String> notCheckMDTokens,
			...
			if (j > -1) { // 该语法完整
				...
				if (linkOrImageBeanTmp != null) {
					...
					if (linkOrImageBeanTmp.isLink()) {
						...
						valuePart.setValue(valuePart.getTitle() + "(" + valuePart.getUrl() + ")");
					} else {
