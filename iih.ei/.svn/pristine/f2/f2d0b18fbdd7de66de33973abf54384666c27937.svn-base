package iih.ei.std.s.v1.bp.pis.userreg;
 ...
 *
 */
public class UserRegisterBp extends IIHServiceBaseBP<UserRegDTO, PisReturnDTO>{
	...
	protected PisReturnDTO process(UserRegDTO param) throws BizException {
		...
		try {
			pisMbwMaintance.userRegister(userDTO);
		} catch (Exception e) {
			returnDTO.setCode("0");
			message += e.getMessage();
			System.out.println(e.getMessage());
