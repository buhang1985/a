 ...
 *
 */
public class DispDrugFinishBp extends BaseCallWsBp {

	public DispDrugFinishResultDTO[] exec(DispDrugFinishParamDTO paramDto) {
		DispDrugFinishResultDTO[] resultDtos = null;

		try {
			String wsParamStr = getCallWsParam(paramDto);
			String wsResultStr = callWebService(wsParamStr);
			resultDtos = getResultDtos(paramDto, wsResultStr);
		} catch (Exception e) {
			writeLog(e);
		}
		return resultDtos;
	 ...
	 * @throws BizException
	 */
	protected String getCallWsParam(DispDrugFinishParamDTO paramDto) throws BizException {
	 ...
	 * @return
	 */
	protected DispDrugFinishResultDTO[] getResultDtos(DispDrugFinishParamDTO paramDto, String wsResultStr) {
