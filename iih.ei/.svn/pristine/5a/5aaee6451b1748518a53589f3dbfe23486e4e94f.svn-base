package iih.ei.std.s.v1.bp.ci.thirdws.op.diag;
...
import iih.ei.std.d.v1.ci.diag.d.EmsDiagDTO;
 ...
 * @author LiYue
 */
public class OpDiagSaveBP  extends IIHServiceBaseBP<EmsWsDTO, ResultDTO> {
	...
	// 入参校验
	protected void checkParam(EmsWsDTO param) throws BizException {
		param.getCode_org();
		checkEmpty(param.getCode_org(),"就诊信息中的所属组织不能为空" );
		checkEmpty(param.getCode_pat(),"就诊信息中的患者编码不能为空" );
		checkEmpty(param.getCode_entp(),"就诊信息中的就诊类型编码不能为空" );
		checkEmpty(param.getCode_org(),"就诊信息中的所属组织不能为空" );
		checkEmpty(param.getCode_org(),"就诊信息中的所属组织不能为空" );
		checkEmpty(param.getCode_org(),"就诊信息中的所属组织不能为空" );
		checkEmpty(param.getCode_org(),"就诊信息中的所属组织不能为空" );
		checkEmpty(param.getCode_org(),"就诊信息中的所属组织不能为空" );
		checkEmpty(param.getCode_org(),"就诊信息中的所属组织不能为空" );
		List<EmsDiagDTO> diagList = param.getEmsdiagdto();
		for(int i=0;i<diagList.size();i++){
			EmsDiagDTO emsDiagDto = diagList.get(i);
			checkEmpty(emsDiagDto.getSortno(),"第"+(i+1)+"个诊断信息中的序号不能为空");
			checkEmpty(emsDiagDto.getDt_create(),"第"+(i+1)+"个诊断信息中的开立时间不能为空");
			checkEmpty(emsDiagDto.getName_didef_syn(),"第"+(i+1)+"个诊断信息中的证候诊断名称不能为空");
			checkEmpty(emsDiagDto.getName_didef(),"第"+(i+1)+"个诊断信息中的诊断编码不能为空");
		}
	}

	//数据组合及保存
	protected ResultDTO process(EmsWsDTO entWsDiagDTO) throws BizException {
		// 数据准备
		ThirdInfoUtils urils = new ThirdInfoUtils();
		entdo = urils.getEntDO(entWsDiagDTO.getCode_en());
		...
		CiDiagDO ciDiagDO = assembleDiag(entWsDiagDTO, entdo);
	 ...
	 * @throws BizException
	 */
	private CiEnContextDTO assembleEnt(EmsWsDTO entWsDiagDTO) throws BizException {
		...
		EmsDiagDTO diagDto = (EmsDiagDTO)entWsDiagDTO.getEmsdiagdto().get(0);
		ctx.setAttrVal("Id_emp_or", diagDto.getCode_emp_create());
		ctx.setAttrVal("Id_denp_en", diagDto.getCode_dep_create());
		return ctx;
	 ...
	 * @throws BizException
	 */
	public CiDiagDO assembleDiag(EmsWsDTO entWsDiagDTO, PatiVisitDO entdto) throws BizException {
		List<EmsDiagDTO> diagList = entWsDiagDTO.getEmsdiagdto();
	...
	private DIDTO[] assembleDidto(EmsWsDTO entWsDiagDTO) {
		List<EmsDiagDTO> diagList = entWsDiagDTO.getEmsdiagdto();
		DIDTO[] diDTOs = new DIDTO[diagList.size()];
		for (int i = 0; i < diagList.size(); i++) {
			DIDTO didto = new DIDTO();
