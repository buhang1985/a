 ...
 *
 */
public class EIncWriteOffInvoke {

	private String oriData = "";
	
	public BlEIncWriteOffResult exec(BlEIncWriteOffParam param) throws BizException{
		if(param == null){
			throw new BizException("入参为空");
		}
		
		//String url = "http://172.18.150.87:8081/medical-web/api/medical/writeOffEBill";//http://<ip>:<port>/<service>/api/medical/接口服务标识,这里ip随便写一个
		// 应用账号
		...
		EIThirdLogger.info("电子发票冲红接口请求数据:"+data);
		oriData = data;
		...
		EIThirdLogger.info("电子发票冲红接口最终请求串:"+map.toString());
		BlEIncWriteOffResult finalResult = httpUrlInvoke(map, url, appID, appKey, version,60);//http调用
	 ...
	 * @throws BizException 
	 */
	public BlEIncWriteOffResult httpUrlInvoke(Map<String, String> map, String url, String appID, String appKey, String version,Integer timeout) throws BizException {
		try {
			...
			if (resp.getStatusLine().getStatusCode() == 200) {
				...
				EIThirdLogger.info("电子发票冲红接口返回数据"+"返回串--》" + rev);
				...
				EIThirdLogger.info("电子发票冲红接口返回数据" + "验签-》" + (StringUtils.equals(rsign, rmd5)));
				// 4.获取返回结果
				String busData = StringCodingUtils.Utf8_Base64_Decode(result.get("data").toString());
				EIThirdLogger.info("电子发票冲红接口返回数据" +"返回业务数据--》" + busData);
				...
				if(finalCode != null && finalCode.equals("S0000")){
					//json转javabean
					EIThirdLogger.info("电子发票冲红接口返回数据" +"业务信息解密--》" + finalCode + " " + finalMessage);
					//BlEIncWriteOffResult finalResult = JSON.parseObject(busData,BlEIncWriteOffResult.class);
					
					BlEIncWriteOffMsgResult messageRlt = JSON.parseObject(finalMessage,BlEIncWriteOffMsgResult.class);
					BlEIncWriteOffResult finalResult = new BlEIncWriteOffResult();
					...
					EIThirdLogger.info(String.format("%s返回失败,失败编码：%s,失败内容：%s", "电子票据冲红接口",finalCode,finalMessage));
					BlEIncWriteOffResult finalResult = new BlEIncWriteOffResult();
					finalResult.setResult(finalCode);
					...
					finalResult.setOutputStr(finalMessage);
					return finalResult;
					//throw new BizException(String.format("%s返回失败,失败编码：%s,失败内容：%s", "电子票据冲红接口",finalCode,finalMessage));
				}

			} else {
				throw new BizException(String.format("冲红接口：%s调用异常，错误代码:%s,错误内容:%s", url,resp.getStatusLine().getStatusCode(),resp.getStatusLine().getReasonPhrase()));
			}
		} catch (Exception e) {
