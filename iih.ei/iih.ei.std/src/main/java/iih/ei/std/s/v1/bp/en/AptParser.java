...
public class AptParser {
	...
	public static RegInfoDTO aptToRegDTO(OpAptDTO aptDTO) throws BizException{
		...
		PatDO patDO = patRService.findById(aptDTO.getId_pat());
		if(patDO!=null){
			if(StringUtils.isNullOrEmpty(regInfo.getId_patca()))
				regInfo.setId_patca(patDO.getId_paticate());
			if(StringUtils.isNullOrEmpty(regInfo.getId_pripat()))
				regInfo.setId_pripat(patDO.getId_patpritp());
			if(StringUtils.isNullOrEmpty(regInfo.getId_patcredtp()))
		...
		regInfo.setId_emp_entry(EiContextUtils.getPsnId()); //当前操作员ID，对应之后取号时的“门诊票据人员信息”
