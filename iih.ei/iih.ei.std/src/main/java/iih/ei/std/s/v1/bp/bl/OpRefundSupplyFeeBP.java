...
import iih.ei.std.d.v1.bl.oprefundsupplydto.d.CgItm;
import iih.ei.std.d.v1.bl.oprefundsupplydto.d.OpRefundSupplyParamDTO;
import iih.ei.std.d.v1.bl.oprefundsupplydto.d.OrderItm;
 ...
 * @date: 2019.10.21
 */
public class OpRefundSupplyFeeBP extends IIHServiceBaseBP<OpRefundSupplyParamDTO, NoParamDTO> {

	@Override
	protected void checkParam(OpRefundSupplyParamDTO param) throws BizException {

		if (param == null) {
			throw new BizException("入参为空");
		}

		FArrayList arr = param.getOrderitm();
		...
		for (Object itm : arr) {
			OrderItm orderItm = (OrderItm) itm;

			if (StringUtil.isEmpty(orderItm.getCode_pat())) {
				throw new BizException("患者编码为空");
			}

			if (StringUtil.isEmpty(orderItm.getCode_dep_mp())) {
				throw new BizException("执行科室为空");
			}

			if (StringUtil.isEmpty(orderItm.getEu_direct())) {
				throw new BizException("补退费方向为空");
			}

			if (!"1".equals(orderItm.getEu_direct()) && !"-1".equals(orderItm.getEu_direct())) {
				throw new BizException("补退费方向值不正确");
			}

			FArrayList arr1 = orderItm.getCgitm();
			...
			for (Object itm1 : arr1) {
				CgItm cgItm = (CgItm) itm1;

				if (!StringUtil.isEmpty(cgItm.getCode_mm())) {
					throw new BizException("门诊不允许补药品");// 因为门诊不药品时，需要传入id_orsrv，而这个字段通过接口没法获取
				}
			...
			if (!StringUtil.isEmpty(orderItm.getCode_emp_or())) {
				// 设置人员上下文
				PsnDocDO psndocDO = BlAmtConverUtil.getPsnInfo(orderItm.getCode_emp_or());
				if (psndocDO == null) {
			...
			if (!StringUtil.isEmpty(orderItm.getCode_emp_cg())) {
				// 设置人员上下文
				PsnDocDO psndocDO = BlAmtConverUtil.getPsnInfo(orderItm.getCode_emp_cg());
				if (psndocDO == null) {
	...
	@Override
	protected NoParamDTO process(OpRefundSupplyParamDTO param) throws BizException {

		List<OrderItm> orderItem = param.getOrderitm();
		OrderItm[] params = orderItem.toArray(new OrderItm[] {});
		// 3 .退补费
	...
	private OrdSrvDO initOr(OrderItm params) throws BizException {
		DAFacade daFacade = new DAFacade();
		if (!StringUtil.isEmpty(params.getCode_or())) {
			...
			sqlp.addParam(params.getCode_or());
	 ...
	 * @throws BizException
	 */
	private String findPat(OrderItm param) throws BizException {
	 ...
	 * @throws BizException
	 */
	private PatiVisitDO initPatiVisitInfo(OrderItm params,String patId) throws BizException {
		StringBuilder sql = new StringBuilder();
		...
		if(!StringUtil.isEmpty(params.getTimes_op())){
			param.addParam(params.getTimes_op());
		}else{
			// 查询这个患者当前的就诊次数
	 ...
	 * @throws BizException
	 */
	private void setOrderFee(OrderItm[] params) throws BizException {
		// 获取科室信息列表 code,id
		Map<String, String> mapDep = getMapDep(params);
		// 获取人员信息列表 code,id
		Map<String, PsnDocDO> mapPsn = getMapPsn(params);
		// 循环最外层 以每一个患者为维度
		for (OrderItm cgDto : params) {
			// 获取服务信息
			...
			if (medSrvDOs != null) {
				...
				for (MedSrvDO medSrvDO : medSrvDOs) {
					// 查询费用明细内容
					CgItm itemParamDTO = this.getBlCostAddItemParamDTO(cgDto, medSrvDO);

					MeterialDO meterialDO = null;
					if (!StringUtil.isEmpty(itemParamDTO.getCode_mm())) {
						IMeterialMDORService mSerivce = ServiceFinder.find(IMeterialMDORService.class);//
						MeterialDO[] tmpDOs = mSerivce.findByAttrValString(MeterialDO.CODE, itemParamDTO.getCode_mm());
						if (!ArrayUtil.isEmpty(tmpDOs)) {
							...
							if (mmPriceDTO != null && mmPriceDTO.getPrice_act() != null) {
								if (mmPriceDTO.getPrice_act().compareTo(new FDouble(itemParamDTO.getPrice())) != 0) {
									...
									sqlStrBuf.append(itemParamDTO.getCode_mm());
					...
					// 筛选、组装记账信息
					if ("1".equals(cgDto.getEu_direct())) {
						...
						//体检流水号赋值
						if(!StringUtil.isEmpty(cgDto.getCode_pe())){
							cgAccountDto.setAttrVal(IBlImplicitParamKeyConst.BL_PECODE, cgDto.getCode_pe());
						}
						
						listAccount.add(cgAccountDto);
					} else if ("-1".equals(cgDto.getEu_direct())) {
	 ...
	 * @return
	 */
	private BlCgRefundAccountSetDTO setCgRefundAccoutnSetDto(OrderItm cgDto, Map<String, String> mapDep,
		...
		// 记账人员
		if (!StringUtil.isEmpty(cgDto.getCode_emp_cg())) {
			accSetDto.setId_emp_cg(mapPsn.get(cgDto.getCode_emp_cg()).getId_psndoc());
		}
		if (!StringUtil.isEmpty(cgDto.getCode_dep_mp())) {
			// 记账部门 执行部门
			accSetDto.setId_dep_cg(mapDep.get(cgDto.getCode_dep_mp()));
		}
		if (!StringUtil.isEmpty(cgDto.getDt_cg())) {
			// 记账时间
			accSetDto.setDt_cg(new FDateTime(cgDto.getDt_cg()));
		}
	...
	private List<BlCgRefundAccountDTO> getBlCgRefundAccountDTO(String patId,PatiVisitDO entDO, OrdSrvDO ciorderdo, MedSrvDO medSrvDO,
			OrderItm[] params, CgItm itemParamDTO, MeterialDO meterialDO, List<BlCgRefundAccountDTO> listRefend)
			throws BizException {
		...
		if (ListUtil.isEmpty(listItmOep)) {
			throw new BizException("服务编码 " + itemParamDTO.getCode_srv() + "无可退费的数据");
		}
		...
		if (canrnt.sub(itfQuan).compareTo(FDouble.ZERO_DBL) < 0) {
			throw new BizException("服务编码 " + itemParamDTO.getCode_srv() + "可退数量小于退费数量");
		}
		for (BlCgItmOepDO cgItmOepDo : listItmOep) {
	 ...
	 * @return
	 */
	private BlCgOpAccountSetDTO setCgAccoutnSetDto(OrderItm params, Map<String, String> mapDep,
			Map<String, PsnDocDO> mapPsn) {
		BlCgOpAccountSetDTO accSetDto = new BlCgOpAccountSetDTO();

		// 判断记账模式
		if (StringUtil.isEmpty(params.getSupplyway()) || params.getSupplyway().equals(FBoolean.TRUE.toString())) {
			// 记账模式 预交金记账
		...
		// 记账人员
		if (!StringUtil.isEmpty(params.getCode_emp_cg())) {
			accSetDto.setId_emp_cg(mapPsn.get(params.getCode_emp_cg()).getId_psndoc());
		}
		if (!StringUtil.isEmpty(params.getCode_dep_mp())) {
			// 记账部门 执行部门
			accSetDto.setId_dep_cg(mapDep.get(params.getCode_dep_mp()));
		}
		if (!StringUtil.isEmpty(params.getDt_cg())) {
			// 记账时间
			accSetDto.setDt_cg(new FDateTime(params.getDt_cg()));
		}
	...
	private BlCgAccountDTO getBlcgAccount(String patId,PatiVisitDO entDO, OrdSrvDO ciorderdo, MedSrvDO medSrvDO, OrderItm cgDto,
			CgItm itemParamDTO, Map<String, String> mapDep, Map<String, PsnDocDO> mapPsn, MeterialDO meterialDO) {
		...
		// 比例系数
		FDouble Price_ratio = new FDouble(itemParamDTO.getRatio_pripat());
		if (Price_ratio != null) {
		...
		// 实际价格
		FDouble Amt_ratio = new FDouble(itemParamDTO.getAmt());
		cgAccount.setPrice_ratio(Price.multiply(Price_ratio));
		// 数量
		FDouble dQuan = new FDouble(itemParamDTO.getQuan());
		cgAccount.setQuan(dQuan);
		// 执行科室id
		cgAccount.setId_dep_mp(mapDep.get(cgDto.getCode_dep_mp()));
		// 开立人员
		if(!StringUtil.isEmpty(cgDto.getCode_emp_or())){
			cgAccount.setId_emp_or(mapPsn.get(cgDto.getCode_emp_or()).getId_psndoc());
		}
		// 开立科室(如果入参传入开立科室，则取入参的，如果没传，则取开立人员所在科室作为开立科室)
		if(!StringUtil.isEmpty(cgDto.getCode_dep_or())){
			cgAccount.setId_dep_or(mapDep.get(cgDto.getCode_dep_or()));
		}else{
			if(!StringUtil.isEmpty(cgDto.getCode_emp_or())){
				cgAccount.setId_dep_or(mapPsn.get(cgDto.getCode_emp_or()).getId_dep());
			}
		}
	...
	private Integer getDirectFlag(OrderItm params) throws BizException {
		String sDirectFlag = params.getEu_direct();
	 ...
	 * @throws BizException
	 */
	private Map<String, String> getMapDep(OrderItm[] params) throws BizException {
		List<String> listdep = new ArrayList<String>();
		Map<String, String> depMap = new HashMap<String, String>();
		// 获取计量单位id集合
		for (OrderItm peItfDto : params) {
			if(!StringUtil.isEmpty(peItfDto.getCode_dep_mp())){
				listdep.add(peItfDto.getCode_dep_mp());
			}
			
			if(!StringUtil.isEmpty(peItfDto.getCode_dep_or())){
				listdep.add(peItfDto.getCode_dep_or());
			}
		}
		...
		if (!ArrayUtils.isEmpty(deptDOs)) {
			...
			throw new BizException("门诊补退费：编码为" + params[0].getCode_dep_mp() + "的执行科室不存在！");
		}
		return depMap;
	}
	 ...
	 * @throws BizException
	 */
	private Map<String, PsnDocDO> getMapPsn(OrderItm[] params) throws BizException {
		List<String> listpsn = new ArrayList<String>();
		Map<String, PsnDocDO> psnMap = new HashMap<String, PsnDocDO>();
		// 获取人员编码集合
		for (OrderItm peItfDto : params) {
			// 记账人员编码
			if(!StringUtil.isEmpty(peItfDto.getCode_emp_cg())){
				listpsn.add(peItfDto.getCode_emp_cg());
			}
			
			// 开立人员编码
			if(!StringUtil.isEmpty(peItfDto.getCode_emp_or())){
				listpsn.add(peItfDto.getCode_emp_or());
			}
			
			// 执行人员编码
			if(!StringUtil.isEmpty(peItfDto.getCode_emp_mp())){
				listpsn.add(peItfDto.getCode_emp_mp());
			}
		}
		// 人员信息服务
	...
	private CgItm getBlCostAddItemParamDTO(OrderItm params, MedSrvDO medSrvDO) {
		String chargeCode = medSrvDO.getCode();
		if (chargeCode != null) {
			for (Object itm : params.getCgitm()) {
				CgItm subItem = (CgItm) itm;
				if (chargeCode.equals(subItem.getCode_srv())) {
					return subItem;
				}
			}
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	private MedSrvDO[] initMedSvrInfo(OrderItm params) throws BizException {
		FArrayList listMedSrvform = new FArrayList();
		for (Object itm : params.getCgitm()) {
			CgItm subItem = (CgItm)itm;
			if (StringUtil.isEmpty(subItem.getCode_srv())) {
				if (StringUtil.isEmpty(subItem.getCode_mm())) {
					throw new BizException("门诊补退：物品编码为空");
				} else {
					MedSrvDO tmpMedSrvDo = this.GetMedSrvInfo(subItem.getCode_mm());
					subItem.setCode_srv(tmpMedSrvDo.getCode());
					listMedSrvform.add(subItem.getCode_srv());
				}
			} else {
				listMedSrvform.add(subItem.getCode_srv());
			}
		...
		if (listMedSrvform.size() > 0) {
			...
			if (medsrvs.length != listMedSrvform.size()) {
				...
				throw new BizException("门诊补费：患者编码为" + params.getCode_pat() + "的服务编码" + err + "所对应的服务不存在");
			}
			return medsrvs;
		}
		return null;
