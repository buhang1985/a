...
//		bf.append("0.0 amt_hp,");//医保报销金额
		bf.append("ENT.DT_ENTRY Dt_entry ,");//就诊时间
		bf.append("nvl(hpdi.code,entdi.code_didef_dis) Diagnosis_in , ");//入院诊断编码
		bf.append("nvl(hpdi.code,entdi.code_didef_dis) Diagnosis_out, ");//出院诊断编码
		...
		bf.append("nvl(hpdi.name,entdi.name_didef_dis) Hs_diagnosis_in_name , ");//入院诊断名称
		bf.append("nvl(hpdi.name,entdi.name_didef_dis) Hs_diagnosis_out_name, ");//出院诊断名称		
		bf.append("ent.code Hs_in_number,");//病案号
		bf.append("ent.code Hs_number,");//住院号\门诊号
		...
		bf.append("'01' Medical_type , ");//就医方式 01门诊，03住院
		...
		bf.append("sto.dt_st Settle_date,");//结算日期
		...
		bf.append(" INNER JOIN en_ent_op eop on eop.id_ent = ent.id_ent ");
		bf.append(" INNER JOIN bd_dep dep on dep.id_dep = eop.id_dep_reg ");
		bf.append(" LEFT JOIN BL_ST_OEP sto on sto.id_ent = ent.id_ent and sto.fg_canc = 'N' ");
		bf.append(" LEFT JOIN pi_pat_ca patca on patca.id_patca = ent.id_patca ");
		bf.append(" LEFT JOIN EN_ENT_DI entdi on entdi.id_ent = ent.id_ent ");
		bf.append(" LEFT JOIN bd_hp_di_vs_his hpdi on hpdi.id_didef = entdi.id_didef_dis ");
	...
	private void SetEntDi(VerifyEntInputDTO entInputDto,String id_ent) throws DAException{
		...
		if(diInfos != null && diInfos.size() >0){
			...
			for(DiInfoRegInParamDTO di : diInfos){
				...
				}else if(!entInputDto.getDiagnosis_in().equals(di.getDi_code())){
					if(isDS)
						other.append("|");
			...
			if(!StringUtils.isBlank(other.toString())){
				entInputDto.setDiagnosis_tother(other.toString());
			}else{
				entInputDto.setDiagnosis_tother(entInputDto.getDiagnosis_out());
			}
		}
	}
}
