 ...
 *
 */
public class SyncInsurDrugDataSql implements ITransQry {

	private String _hpId;
	private String[] _pks;

	public SyncInsurDrugDataSql(String hpId, String[] pks) {
		this._hpId = hpId;
		this._pks = pks;
	}
	...
	public SqlParam getQryParameter(StringBuffer arg0) {
		...
		sqlParam.addParam(MiSrvTp.DRUG);
		sqlParam.addParam(this._hpId);
		sqlParam.addParam(Context.get().getGroupId());
		sqlParam.addParam(Context.get().getOrgId());
		return sqlParam;
	...
	public String getQrySQLStr() {
		...
		sqlBuilder.append("    RATE_IMPORT_DIFF,				      ");
		sqlBuilder.append("    NOTE						      ");
		sqlBuilder.append("  FROM BD_HP_DRUG_ORGINAL				      ");
		sqlBuilder.append("  WHERE BD_HP_DRUG_ORGINAL.CODE = BD_HP_SRVORCA.CODE	AND BD_HP_DRUG_ORGINAL.ID_GRP =? AND BD_HP_DRUG_ORGINAL.ID_ORG =? AND BD_HP_DRUG_ORGINAL.ID_HP = ? ");
		sqlBuilder.append("  )							      ");
