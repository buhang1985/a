 ...
 * @date  :2020年7月8日
 */
public class MiCheckOpDosageRule  extends MiCheckBaseRule{
...
//		FMap patDrugMap =this.getPatDrugMap(inParamDto.getId_pat());
		...
		for (Object item : inParamDto.getCiorderlist()) {
			...
			for (Object orsrv : orsrvLst) {
				...
				//开单量为1不校验 (ci_or_srv_mm.quan_cur)
				boolean isOne = new FDouble(1).compareTo(ordSrvDO.getQuan()) == 0;
				if(isOne){
					continue;
				}
				// 判断是草药还是西药 草药按付数提示 西药按医嘱的开单天数进行校验 -> 改成 当前物品持有天数
				int days = ordSrvDO.getDays_available();				
...
//				if (FBoolean.FALSE.equals(bSpecJudge)) {
				...
				if (ciOrderDO.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_HERBDRUG)) {
					if (days > HerbhaveDays) {
						String errorMsg = MiCheckWholeMsgUtilBP.getChronicOrDosageExcessMsg(ciOrderDO.getName_or(), ordSrvDO.getName_mm(), HerbhaveDays, days, unit);
						outParamDto.setFg_checkflag(FBoolean.FALSE);
						itmChkRstDTO = setErrorInfoBP.exec(ciOrderDO, ordSrvDO, MiMcCheckFailResultEnum.GREATER_THAN_GET_OEP, errorMsg);
						outParamDto.getErroritm_list().add(itmChkRstDTO);						
					}
					...
					if (FBoolean.TRUE.equals(fg_ExistChronicDi)) {
						if (days > haveDays_mxb) {
							String errorMsg = MiCheckWholeMsgUtilBP.getChronicOrDosageExcessMsg(ciOrderDO.getName_or(), ordSrvDO.getName_mm(), haveDays_mxb, days, unit);
							outParamDto.setFg_checkflag(FBoolean.FALSE);
							itmChkRstDTO = setErrorInfoBP.exec(ciOrderDO, ordSrvDO, MiMcCheckFailResultEnum.GREATER_THAN_GET_OEP, errorMsg);
							outParamDto.getErroritm_list().add(itmChkRstDTO);
						}
						...
						if (days > haveDays_xdbb) {
							outParamDto.setFg_checkflag(FBoolean.FALSE);
							String errorMsg = MiCheckWholeMsgUtilBP.getDiffToMoveDosageMsgExcessMsg(ciOrderDO.getName_or(), ordSrvDO.getName_mm(), haveDays_xdbb, days, unit);
							itmChkRstDTO = setErrorInfoBP.exec(ciOrderDO, ordSrvDO, MiMcCheckFailResultEnum.GREATER_THAN_GET_OEP, errorMsg);
							outParamDto.getErroritm_list().add(itmChkRstDTO);
						}
						...
						// 普通
						outParamDto.setFg_checkflag(FBoolean.FALSE);
						String errorMsg = MiCheckWholeMsgUtilBP.getBillDosageExcessMsg(ciOrderDO.getName_or(), ordSrvDO.getName_mm(), haveDays, days, unit);
						itmChkRstDTO = setErrorInfoBP.exec(ciOrderDO, ordSrvDO, MiMcCheckFailResultEnum.GREATER_THAN_GET_OEP, errorMsg);
						outParamDto.getErroritm_list().add(itmChkRstDTO);
					}
				}
