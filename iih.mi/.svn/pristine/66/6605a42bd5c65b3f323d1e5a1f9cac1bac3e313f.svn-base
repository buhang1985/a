...
public class SyncInsurFwssDataSql implements ITransQry {
	...
	public SqlParam getQryParameter(StringBuffer arg0) {
		...
		sqlParam.addParam(MiSrvTp.SRVFAC);
		sqlParam.addParam(this._hpId);
		sqlParam.addParam(Context.get().getGroupId());
		sqlParam.addParam(Context.get().getOrgId());
		return sqlParam;
	...
	public String getQrySQLStr() {
		...
		sqlBuilder.append("  WHERE BD_HP_FWSS_ORGINAL.code = bd_hp_srvorca.code	and BD_HP_FWSS_ORGINAL.id_grp =? AND BD_HP_FWSS_ORGINAL.id_org =? AND BD_HP_FWSS_ORGINAL.ID_HP = ? ");
		sqlBuilder.append("  )							       ");
