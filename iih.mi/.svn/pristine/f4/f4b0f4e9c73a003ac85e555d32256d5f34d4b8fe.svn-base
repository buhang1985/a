...
/**通过编码和名称查找sql
 * @author hexx
 * @version 创建时间：2018年2月2日 上午9:26:18 自动药品对照目录sql
 */
public class AutoCompDrugByNameAndCodeSql implements ITransQry {

	private String _misId;

	public AutoCompDrugByNameAndCodeSql(String misId) {
		this._misId = misId;
	}
	...
	public String getQrySQLStr() {
		...
		baseSQL.append("   (SELECT MI_BD_DRUG.CODE,	       			    ");
		...
		baseSQL.append(" GROUP BY MI_BD_DRUG.CODE,				    ");
		baseSQL.append("  MI_BD_DRUG.NAME		       			    ");
		baseSQL.append("  ) MIDRUG			       			    ");
		baseSQL.append("  ON MIDRUG.CODE  = BDMM.CODE	       			    ");
		baseSQL.append(" AND MIDRUG.NAME = BDMM.NAME 	       			    ");
