...
public class GetApplyPrinterBP {
	/**
	 * 获取医技预约条码打印机
	 *
	 * @author fanlq
	...
	public ParamSetDO exec() throws BizException {
		ParamSetDO result = GetApplyPrinter();
		return result;
	}
	 ...
	 * @throws BizException
	 */
	private ParamSetDO GetApplyPrinter() throws BizException {
	...
	private ParamTmplSetDO GetParamTmpl() throws BizException {
		...
		if (result == null || result.length <= 0) {
			String msg = String.format("获取参数模板失败，参数编码:\"%s\"。", ScParamUtils.SCPC_PRINTER_MTAPPLY);
			throw new BizException(msg);
		}
		return result[0];
	}
