 ...
 * 
 * @author yank
 * @modifier zhengcm 补全缺少的字段数据 2017-06-12
 *
 */
public class MtApt4IpListener implements IBusinessListener {
	/**
	 * 医技预约发消息
	 */
	@Override
	public void doAction(IBusinessEvent event) throws BizException {
		...
		buo.setMsgObj(msg);
		//调用客开处理
		ScAptCustomServiceUtils.handleWrapScApt4IeObj(buo);
	...
	private void validate(IBusinessEvent event) throws BizException {
		if (!IScEventConst.EVENT_SOURCE_SC_MT_APT.equalsIgnoreCase(event.getSourceID())
				|| !IScEventConst.EVENT_SC_MT_APT.equals(event.getEventType())) {
	...
	private MtApt4IpDTO wrap4Ip(MtApt4EsDTO apt4EsDTO) throws BizException {
		...
		sqlBuilder.append("apt.dt_b as dt_apt,");// 预约开始时间
		sqlBuilder.append("apt.dt_e as dt_apt_e,");// 预约结束时间
		...
		sqlBuilder.append("ent.code as code_ent,");// 就诊流水号
		sqlBuilder.append("ent.code_entp as code_entp,");// 就诊类型编码
		sqlBuilder.append("decode(ent.code_entp,'00',pat.barcode_chis,'10',pat.code_amr_ip,pat.barcode_chis) as id_ent,");// 就诊号
		sqlBuilder.append("decode(ent.code_entp,'00','门诊','10','住院','01','急诊') as name_entp,");// 就诊类型
		sqlBuilder.append("card.code as num_healthcard,");// 健康卡号
		...
		sqlBuilder.append(" left join bd_psndoc psn on apt.id_emp_appt = psn.id_psndoc");
		...
		sqlBuilder.append(" left join pi_pat_card card on card.id_pat = pat.id_pat and card.sd_patcardtp = '05' ");
		...
		dto.setArea("01");
		//转成指定格式的字符串
		if(dto.getDt_apt()!=null){
			dto.setDt_apt_begin(dto.getDt_apt().toString(TimeZone.getDefault(),new SimpleDateFormat("yyyyMMddHHmmss")));
		}
		if(dto.getDt_apt_e()!=null){
			dto.setDt_apt_end(dto.getDt_apt_e().toString(TimeZone.getDefault(),new SimpleDateFormat("yyyyMMddHHmmss")));
		}
		return dto;
	}
