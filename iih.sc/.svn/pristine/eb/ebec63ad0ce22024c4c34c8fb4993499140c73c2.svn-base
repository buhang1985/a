...
public class SrvMutexCheck4ManualMtAptBP {
	 ...
	 * @author zhengcm
	 */
	public void exec(String id_pat, String id_srv,String id_ent, FDateTime aptTime, MtResDTO resDTO,FDate d_sch,FBoolean fg_hp) throws BizException {
		...
		if(!isHp.booleanValue()){
			...
			// 2、互斥检查
			this.mutexCheck(id_srv,id_ent, aptApplDOList, aptTime, resDTO,d_sch,isHp);
		}else{//医保时处理
			// 1.查询日期区间内患者已预约的所有预约记录
			List<ScAptApplDO> aptApplDONotHpList = this.getPatAptScSrvList(id_pat, begin_date,FBoolean.FALSE);
			// 2、互斥检查 自费规则
			this.mutexCheck(id_srv,id_ent, aptApplDONotHpList, aptTime, resDTO,d_sch,FBoolean.FALSE);
			// 3、查询日期区间内患者已预约的医保支付的预约记录
			List<ScAptApplDO> aptApplDOHpList = this.getPatAptScSrvList(id_pat, begin_date,isHp);
			// 4、 互斥检查 医保规则
			this.mutexCheck(id_srv,id_ent, aptApplDOHpList, aptTime, resDTO,d_sch,isHp);
		}
	}
	 ...
	 */
	// FIXME 根据排斥类型要有返回值
	private void mutexCheck(String id_srv,String id_ent, List<ScAptApplDO> aptApplDOList, FDateTime nowTime, MtResDTO resDTO,FDate d_sch,FBoolean isHp)
			throws BizException {
		...
		this.check(id_srv, forward_datas, aptApplDOList, ScAppUtils.getServerDateTime(), 0, resDTO,d_sch);
		this.check(id_srv, reverse_datas, aptApplDOList, ScAppUtils.getServerDateTime(), 1, resDTO,d_sch);
	}
	...
	private void check(String id_srv, List<ScSrvExclDO> datas, List<ScAptApplDO> aptApplDOList, FDateTime nowTime,
			int type,MtResDTO resDTO,FDate d_sch) throws BizException {
		...
		for (ScSrvExclDO scSrvExclDO : datas) {
			measIdList.add(scSrvExclDO.getId_measdoc_period());//计量单位
		}
		...
		for (ScSrvExclDO scSrvExclDO : datas) {
			...
			if(aptIdSrvs.contains(idSrv)){
				...
				if(IScDictCodeConst.ID_SCSRVEXCLTP_TIMES_LIMIT.equals(scSrvExclDO.getId_scsrvexcltp())){//FIXME 考虑 统计一个互斥规则对应的排班服务内的idsrv的次数
					// 校验次数互斥限制
	...
	private void handleScAptExclTimes(int type, MtResDTO resDTO, List<ScAptApplDO> aptApplDOList, MeasDocDO measDocDO,
		...
		for (ScAptApplDO scAptApplDO : aptApplDOList) {
			...
			FDate dt_e = scAptApplDO.getDt_b().getDate().getDateAfter(scSrvExclDO.getPeriod()-1).asEnd();
			...
			if(countMax == scSrvExclDO.getTimes()){
				...
				dtMap.put("is_show", FBoolean.FALSE.toString());
				dtMap.put("pk", pk);
	...
	private void handleScAptExclDate(int type, MtResDTO resDTO, List<ScAptApplDO> aptApplDOList, MeasDocDO measDocDO,
		...
		for (ScAptApplDO scAptApplDO : aptApplDOList) {
			...
			int period = scSrvExclDO.getPeriod() -1; //period 期限
	...
	private long getExclSeconds(ScSrvExclDO exclDO, MeasDocDO docDO, ScAptDO aptDO) throws BizException {
		...
		case IBdFcDictCodeConst.CODE_TIME_SEC:// 秒
			multiple = 1;
			break;
		case IBdFcDictCodeConst.CODE_TIME_MIN:// 分
			multiple = 60;
			break;
		case IBdFcDictCodeConst.CODE_TIME_HOUR:// 时
			multiple = 60 * 60;
			break;
		case IBdFcDictCodeConst.CODE_TIME_DAY:// 天
			multiple = 60 * 60 * 24;
			break;
		default:
			multiple = 1;
