...
public class TicksEP {
	...
	public PeriodResDTO[] getTicksBySch(String schId) throws BizException {
		...
		List<PeriodResDTO> prdResList = (List<PeriodResDTO>) new DAFacade().execQuery(sqlBuilder.toString(),param,
	...
	public PeriodResDTO getRecentAvlTicks(String schId, String chlTp) throws BizException {
		...
		ScSchTickDO schTickDO = tickEP.getCanUseMinTickBySchId(schId, chls[0].getId_scchl());
		if(schTickDO == null)
	 ...
	 * @throws BizException
	 */
	public ScSchTicksDO addTicket(String schId, String addEmpId, int addNum, Integer addEuTp, MtSchDetailDTO schDetailDTO) throws BizException {
		IScticksRService tksRService = ServiceFinder.find(IScticksRService.class);
		String whereStr = String.format("%s.ID_SCH='%s' AND %s.FG_ACTIVE = '%s'", ScSchTicksDODesc.TABLE_ALIAS_NAME,
				schId, ScSchTicksDODesc.TABLE_ALIAS_NAME, "Y");
		String orderStr = String.format("%s.QUENO_B DESC", ScSchTicksDODesc.TABLE_ALIAS_NAME);
		...
		// 修改号段
		ScSchTicksDO ticksDO = scSchTicksDOs[0];
		ScSchTicksDO newTicksDO = (ScSchTicksDO) ticksDO.clone();
		newTicksDO.setStatus(DOStatus.NEW);
		newTicksDO.setId_ticks(null);
		newTicksDO.setQuecn(addNum);
		newTicksDO.setQuecn_appt(addNum);
		newTicksDO.setQuecn_used(0);
		newTicksDO.setQueno_b(ticksDO.getQueno_b()+ticksDO.getQuecn());
		newTicksDO.setT_b(ticksDO.getT_e());
		newTicksDO.setT_e(schDetailDTO.getT_e_acpt());
		newTicksDO.setIds_kinditm(null);
		newTicksDO.setFg_used(new FBoolean(newTicksDO.getQuecn_used() > (ticksDO.getQuecn_appt() - 1)));
		newTicksDO.setFg_add(FBoolean.TRUE);
		IScticksCudService tksCudService = ServiceFinder.find(IScticksCudService.class);
		scSchTicksDOs = tksCudService.save(new ScSchTicksDO[] { newTicksDO });
		return scSchTicksDOs[0];
	}
	/**
	 * 号段加号
	 ...
	 * @throws BizException
	 */
	public ScSchTicksDO occupyBySchId(String schId, String scchlId) throws BizException {
		TickEP tickEP = new TickEP();
	...
	public ScSchTicksDO drawback(String ticksectId, Integer tickAmt) throws BizException {
		ScLogUtil.getInstance().logError("1.【TicksEP_号段退号,开始】号段id"+ticksectId);
		...
		ticksDO.setStatus(DOStatus.UPDATED);
		ScLogUtil.getInstance().logError("2.【TicksEP_号段退号,结束】号段id"+ticksectId);
