 ...
 */
public class OptCancApt4IEListener implements IBusinessListener {
	/**
	 * 取消手术预约发消息
	 */
	@Override
	...
	private void validate(IBusinessEvent event) throws BizException {
		if (!IScEventConst.EVENT_SOURCE_SC_OPT_APT.equalsIgnoreCase(event.getSourceID())
				|| !IScEventConst.EVENT_SC_OPT_CANC_APT.equals(event.getEventType())) {
	...
	private OptApt4IEDTO wrap4Ie(OptApt4OrmDTO apt4IeDTO) throws BizException {
		...
		.append(" pat.code_amr_ip as visitOrdNo,")//就诊流水号 改为 住院号 0172808: BS009手术预约信息服务中所传信息有误
		...
		.append(" left join bd_dep depphy on ip.id_dep_phyadm = depphy.id_dep")
		...
		.append(" where  appl.sd_sctp = '04' and apt.id_apt = ?");
