 ...
 *
 */
public class CheckAptTickChannel4AutoAptBP {
	/**
	 ...
	 * @throws BizException
	 */
	public Map<String,Map<Integer,Integer>> exec(String sd_tickmd, String id, String id_scchl, String id_srv,String idEnttp) throws BizException {
		...
		if (IScDictCodeConst.SD_TICKMD_TICK.equals(sd_tickmd)) {
			//号位模式
			return checkAptTick4TickMdAndPolcn(id, id_scchl, id_srv,idEnttp);
		} else if (IScDictCodeConst.SD_TICKMD_TICKS.equals(sd_tickmd)) {
			//号段模式
			return checkAptTick4TicksMdAndPolcn(id, id_scchl, id_srv, idEnttp);
		}
		return null;
	}
	/**
	 ...
	 * @throws BizException
	 */
	private Map<String, Map<Integer,Integer>> checkAptTick4TickMdAndPolcn(String id_tick, String id_scchl, String id_srv,String idEnttp) throws BizException {
		ScValidator.validateParam("id_tick", id_tick);
		OralAptEP aptEP = new OralAptEP();
		ScSchTickDO tickDO = aptEP.getSchTickById(id_tick);
		// 检查当前渠道的剩余可预约数
		List<String> schIdList = new ArrayList<String>();
		schIdList.add(tickDO.getId_sch());
		Map<String, Map<Integer,Integer>> map = ServiceFinder.find(IScSchOutQryService.class).batchGetChlSchRemainFilterEntpAmount4AutoApt(
	 ...
	 * @throws BizException
	 */
	private Map<String, Map<Integer,Integer>> checkAptTick4TicksMdAndPolcn(String id_ticks, String id_scchl, String id_srv,String idEnttp) throws BizException {
		ScValidator.validateParam("id_ticks", id_ticks);
		OralAptEP aptEP = new OralAptEP();
		ScSchTicksDO ticksDO = aptEP.getSchTicksByWhere(String.format("id_ticks = '%s'", id_ticks), null)[0];
		// 检查当前渠道的剩余可预约数
