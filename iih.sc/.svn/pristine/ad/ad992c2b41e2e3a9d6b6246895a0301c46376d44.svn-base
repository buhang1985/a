 ...
 *
 */
public class GetAptSchTicks4WeChatBP {
	...
	public FArrayList exec(String id_sch, String id_scchl, String id_srv, String id_enttp,String sd_sctp,String id_pat,String id_ent,FDate d_sch,FBoolean fg_hp) throws BizException {
		...
		if (schDO.getSd_tickmd().equals(IScDictCodeConst.SD_TICKMD_TICK)) {// 号位模式
			...
			// 根据渠道设置号段是否可用
			ScSchTickDO[] tickDOs = this.setTicksActiveByScchl(ticksDOs, id_scchl, remain,polcnMap);
			if (!ArrayUtil.isEmpty(ticksDOs)) {
				if(!ArrayUtil.isEmpty(tickDOs)){
					for (ScSchTickDO scSchTickDO : tickDOs) {
						flist.add(scSchTickDO);
					}
				}
	...
	private ScSchTickDO[] getTick(String id_sch) throws BizException {
		...
		return new OralAptEP().getSchTickByWhere(cond,null);
	}
	 ...
	 * @throws BizException
	 */
	private ScSchTickDO[] setTicksActiveByScchl(ScSchTicksDO[] ticksDOs, String id_scchl, int remain,Map<Integer,Integer> polcnMap) throws BizException {
		...
		String cond = String.format(
				"fg_active = 'Y' and eu_usescope = %d " + " and "
						+ ScSqlUtils.getInSqlByIds("id_ticks", ids), EuUseScope.APT.intValue());
		ScSchTickDO[] tickDOs = new OralAptEP().getSchTickByWhere(cond,null);
		if (!ArrayUtil.isEmpty(tickDOs)) {
			// 指定了当前渠道的可预约的号位数量
			int canuse = 0;
			for (ScSchTickDO tickDO : tickDOs) {
			...
			if (remain > canuse) {
				for (ScSchTickDO tickDO : tickDOs) {
					for (Integer polcn : polcnMap.keySet()) {
						if (tickDO.getFg_active().booleanValue() && (StringUtil.isEmptyWithTrim(tickDO.getIds_scchl()) || tickDO.getIds_scchl().contains(id_scchl))
		...
		return tickList.toArray(new ScSchTickDO[0]);
	}
