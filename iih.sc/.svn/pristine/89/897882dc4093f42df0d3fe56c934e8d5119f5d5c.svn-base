...
/**
 * 根据预约释放规则释放预约未缴费定时任务BP
 * @author yzh
 * @date 2018年8月21日 15:32:11
 */
public class ReleaseUnFeeAptBP {
	...
	private String getSql(String id_dep,boolean isDepFilter){
		...
		.append(" where appl.fg_bl='N' and appl.sd_sctp = '03' and appl.fg_canc='N'")
		.append(" and mt.fg_aptcfm='N' and apt.dt_b >= ?");
		if (isDepFilter) {
	...
	private ScaptrlsruleDO[] getScaptrlsrulesByCond(String id_dep,String id_org,boolean isDepFilter) throws BizException{
		...
		cond.append(ScaptrlsruleDO.ID_APT_RLS_TP).append("='").append(unFeeId).append("' and fg_active = 'Y' and sd_sctp='@@@@AA1000000001D8EG' ");
