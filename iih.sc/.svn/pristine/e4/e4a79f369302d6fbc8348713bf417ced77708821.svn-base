 ...
 * 
 * @author f
 * yaohl 2019/3/15 修改分页
 */
public class GetScBdSrvListBp {
	...
	public PagingRtnData<ScBdSrvDTO> exec(ScBdSrvDTO scBdSrvDTO, PaginationInfo pg) throws BizException {
        ...
        sqlSb.append(" kind.fg_autoapt_op,");
        sqlSb.append(" kind.fg_autoapt_ip");
		...
		if (!StringUtil.isEmptyWithTrim(scBdSrvDTO.getId_scsrvkinditem())) {
			sqlSb.append(" and itm.id_scsrvkinditm in (");
			String[] items = scBdSrvDTO.getId_scsrvkinditem().split(",");
