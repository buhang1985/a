 ...
 *
 */
public class AptCheckChlPeriodAutoBP {
	 ...
	 * @throws BizException
	 */
	public MtResDTO[] exec(String id_scchl, String sd_sctp,MtResDTO[] scSchs) throws BizException {
		// 参数检查
		...
		// 根据渠道获取该渠道下所有排班
		//ScSchDO[] schDOs = new OralAptEP().getSchByWhere(getSchQry(id_scchl).toString());
		List<MtResDTO> result = new ArrayList<MtResDTO>();
		// 检查
		if (!ArrayUtil.isEmpty(scSchs)) {
			for (MtResDTO scSchDO : scSchs) {
				if(this.check(periodDO, scSchDO)){
					result.add(scSchDO);
				}
			}
		}
	 ...
	 * @return
	 */
	private StringBuffer getSchQry(String id_scchl){
		StringBuffer qry = new StringBuffer();
		qry.append("id_sch in (select id_sch from sc_sch_chl where id_scchl = '").append(id_scchl).append("'");
		return qry;
	}
	/**
	 * 检查
	 ...
	 * @throws BizException
	 */
	private boolean check(ScChlPeriodDO periodDO, MtResDTO schDO) throws BizException {
