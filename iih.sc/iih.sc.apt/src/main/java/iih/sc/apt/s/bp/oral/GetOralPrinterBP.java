 ...
 * 
 * @author zhengcm
 * @date 2017-10-09 16:44:28
 *
 */
public class GetOralPrinterBP {
	...
	public ParamSetDO exec() throws BizException {
		ParamSetDO result = GetOralPrinter();
		return result;
	}
	 ...
	 * @throws BizException
	 */
	private ParamSetDO GetOralPrinter() throws BizException {
	...
	private ParamSetDO GetParamSetByPcId(String pcId) throws BizException {
		...
		String wherePart = String.format("paramcode = '%s' and id_instance = '%s'", ScParamUtils.ENPC_APTNOTE_PRINTER,
				pcId);
	...
	private ParamTmplSetDO GetParamTmpl() throws BizException {
		...
		ParamTmplSetDO[] result = paramtmplsetRService.findByAttrValString(ParamTmplSetDO.PARAMCODE,
				ScParamUtils.ENPC_APTNOTE_PRINTER);
		if (result == null || result.length <= 0) {
