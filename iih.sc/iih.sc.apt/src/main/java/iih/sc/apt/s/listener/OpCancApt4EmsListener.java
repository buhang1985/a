...
import iih.sc.pub.listener.IOpCancApt4EmsListener;
 ...
 *
 */
public class OpCancApt4EmsListener extends IOpCancApt4EmsListener {
	/**
	 * 执行
	 */
	@Override
	...
	private void validate(IBusinessEvent event) throws BizException {
		if (!IScEventConst.EVENT_SOURCE_SOURCE_SC_OP_APT.equalsIgnoreCase(event.getSourceID())
				|| !IScEventConst.EVENT_SC_OP_CANC_APT.equals(event.getEventType())) {
	...
	private void sendMsg(OpApt4SmsDTO aptDTO) throws BizException {
		...
		// 根据短信类型判断调用短信模板
		String msgTpl = IScSmsConst.SC_CANCEL_APT_MSG_TEMPLATE;// 取消预约模板
		if (null != aptDTO.getType_msg() && aptDTO.getType_msg() == 2) {
			msgTpl = IScSmsConst.SC_STOPCANCEL_OP_APT_MSG_TEMPLATE;// 停诊取消预约模板
		}
		smsMsg.exec(aptDTO.getMobile(), msgTpl, aptDTO);
	}
}
