 ...
 *
 */
public class GetResBySch4CancReg {
	/**
	 * 根据查询模板查询资源
	 * 
	 * @param qryScheme
	 ...
	 * @throws BizException
	 */
	public RegResDTO[] exec(String patId,ResQrySchemeDTO qryScheme) throws BizException {
		...
		// 2、添加查询条件
		this.addFilter(patId,sqlBuilder, param, qryScheme);
	...
	private void initSql(StringBuilder sqlBuilder, ResQrySchemeDTO qryScheme) throws BizException {
		...
		sqlBuilder.append(" inner join en_ent ent on ent.id_ent = op.id_ent ");
		sqlBuilder.append(" INNER JOIN sc_sch sch ON sch.ID_SCH = op.ID_SCH ");
		...
		sqlBuilder.append(" INNER JOIN SC_SRV SRV ON OP.ID_SCSRV=SRV.ID_SCSRV ");
	 ...
	 * @throws BizException
	 */
	private void addFilter(String patId,StringBuilder sqlBuilder, SqlParam param, ResQrySchemeDTO qryScheme) throws BizException {
