package iih.sc.sch.scticks.s.bp;
 ...
 * 
 */
public class OccupyTicks4TimeMdBP {
	/**
	 * 预约选择的时间点是否未使用
	 * 
	 * @author liubin
	 ...
	 * @throws BizException
	 */
	public ScSchTicksDO exec(String id_sch, SchTickMdTimeDTO schTickMdTime) throws BizException{
		//1.验证
		...
		//4.拆分号段
		return splitTicks(schTicksDo, sch, schTickMdTime);
	}
	/**
	 * 获取需要拆分的号段信息
	 * 
	 ...
	 * @throws BizException
	 */
	private ScSchTicksDO splitTicks(ScSchTicksDO schTicksDo, ScSchDO schDo, SchTickMdTimeDTO schTickMdTime) throws BizException{
		...
		if(t_b.before(schTickMdTime.getT_b()))
			ticksDOList.add(wrapInsertTicksDo(schDo.getId_sch(), t_b, schTickMdTime.getT_b(), FBoolean.FALSE));
		ticksDOList.add(wrapInsertTicksDo(schDo.getId_sch(), schTickMdTime.getT_b(), schTickMdTime.getT_e(), FBoolean.TRUE));
		if(t_e.after(schTickMdTime.getT_e()))
			ticksDOList.add(wrapInsertTicksDo(schDo.getId_sch(), schTickMdTime.getT_e(), t_e, FBoolean.FALSE));
		TicksEP ticksEP = new TicksEP();
	 ...
	 * @throws BizException
	 */
	private ScSchTicksDO wrapInsertTicksDo(String id_sch, FTime t_b, FTime t_e, FBoolean fg_used)throws BizException{
