 ...
 *
 */
public class OccupyChlTickByAchAndTime4ChangeAptBP {
	/**
	 * 根据排班和时间片时段占用号位
	 * 
	 * 
	 ...
	 * @throws BizException
	 */
	public ScSchTickDO exec(String id_sch, SchTickMdTimeDTO schTickMdTime, String id_scchl,String id_enttp) throws BizException {
		//1.校验参数
		this.validateParam(id_sch, schTickMdTime, id_scchl);
		
		//2.查询排班渠道信息
		ScSchChlDO schChlDo = getScSchChlDO(id_sch, id_scchl,id_enttp);
		//3.排班加锁
		lockSch(id_sch);
		//4.验证排班剩余时间是否足够使用
	...
	private ScSchChlDO getScSchChlDO(String id_sch, String id_scchl,String id_enttp) throws BizException{
		...
		if (schDo.getFg_setentp().booleanValue()) {
			...
			return schChlList.toArray(new ScSchChlDO[0])[0];
		}
		return schChlDOs[0];
	}
	/**
