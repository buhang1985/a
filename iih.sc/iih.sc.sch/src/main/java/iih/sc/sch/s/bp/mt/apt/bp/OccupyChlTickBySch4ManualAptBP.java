 ...
 *
 */
public class OccupyChlTickBySch4ManualAptBP {
	/**
	...
	public ScSchTickDO exec(String schId,Map<Integer,Integer> polcnMap, String ticksId, String chlId,String idEnttp,MtResDTO resDTO,String idSrv) throws BizException {
		// 校验参数
		this.validateParam(schId, polcnMap, chlId);
		...
		if (IScDictCodeConst.SD_TICKMD_TICKS.equals(tickMd)) {
			...
			if (!StringUtil.isEmpty(ticksId)) {
				...
				ScSchTicksDO ticksDO = ticksBP.occupy(ticksId);
				// 过滤类别明细
				...
				ScSchTicksDO ticksDO = ticksBP.occupyBySchId(schId, chlId,idEnttp,polcnMap,resDTO,idSrv);// 根据排班占用号段
		...
		// 2、占用号位
		if (IScDictCodeConst.SD_TICKMD_TICKS.equals(tickMd)) {
			tickDO = tickEP.occupyByTicksId(ticksId, polcnMap, chlId, idEnttp,resDTO,idSrv,schId);// 号段
		} else {
			tickDO = tickEP.occupyBySchId(schId, polcnMap, chlId,idEnttp,resDTO,idSrv);// 号位
		}
		if(tickDO != null){
			// 3、占用排班号源
