...
public class SchEP {
	 ...
	 * @throws BizException
	 */
	public void occupyTick4Auto(String schId, Integer polcn, Integer tickAmt) throws BizException {
		ScSchDO schDO = this.getByPK(schId);
		if (schDO == null) {
			return;
		}
		this.occupyTick4Auto(schDO, polcn, tickAmt);
	}
	 ...
	 * @throws BizException
	 */
	public void occupyTick4AutoLocate(String schId, Integer polcn, Integer tickAmt) throws BizException {
		ScSchDO schDO = this.getByPK(schId);
		if (schDO == null) {
			return;
		}
		this.occupyTick4AutoLocate(schDO, polcn, tickAmt);
	}
	/**
	 * 占用号源
	 * 
	...
	public ScSchDO occupyTick(ScSchDO schDO, Integer polcn, Integer tickAmt) throws BizException {
		...
		ScSchDO[] schDOArray = this.update(new ScSchDO[] { schDO });
		return schDOArray[0];
	}
	/**
	 * 占用号源
	...
	public ScSchDO occupyTick4Auto(ScSchDO schDO, Integer polcn, Integer tickAmt) throws BizException {
		...
		if(!this.validatePolCn4Auto(polcn)){
			return null;
		}
		if (!FBoolean.TRUE.equals(schDO.getFg_active())) {
			return null;
	...
	public ScSchDO drawbackTick(ScSchDO schDO, Integer polcn, Integer tickAmt) throws BizException {
		ScLogUtil.getInstance().logError("1.【SchEP_排班退还票号,开始】排班id:"+schDO.getId_sch()+"号源池号:"+polcn+"消减数:"+tickAmt);
		...
		ScLogUtil.getInstance().logError("2.【SchEP_排班退还票号,开始】排班id:"+schDO.getId_sch()+"号源池号:"+polcn+",排班该号源池已用:"+schDO.getAttrVal(quan_used_attr));
		...
		ScLogUtil.getInstance().logError("3.【SchEP_排班退还票号,结束】排班id:"+schDO.getId_sch());
		return schDOArray[0];
	}
	/**
	 * 退还票号(把a号源池的号退到b号源池)
	 ...
	 * @throws BizException
	 */
	public void addTicket(ScSchDO schDO, Integer addNum, Integer addEnum, Integer polCn,FTime t_e_acpt)
			throws BizException {
		if (addNum == null || addNum <= 0) {
			return;
		}
		this.validatePolCn(polCn);
		schDO.setT_e_acpt(t_e_acpt);
		schDO.setQuan_total((schDO.getQuan_total() != null ? schDO.getQuan_total() : 0) + addNum); // 号源池加号
		schDO.setQuan_total_appt((schDO.getQuan_total_appt() != null ? schDO.getQuan_total_appt()
				: 0) + addNum); // 可预约总数加号
		String quan_appt_attr = String.format("Quan%s_appt", polCn);
