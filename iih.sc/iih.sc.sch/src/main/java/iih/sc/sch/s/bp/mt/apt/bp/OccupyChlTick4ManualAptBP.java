 ...
 *
 */
public class OccupyChlTick4ManualAptBP {
	/**
	 * 按渠道占用号位
	 * 
	 * @author zhengcm
	...
	public ScSchTickDO exec(String tickId, Map<Integer,Integer> polcnMap, String chlId, MtResDTO resDTO,String id_srv) throws BizException {
		...
		// 1、占用号位
		ScSchTickDO tickDO = new TickChl4MtAptEP().occupyByTickId(tickId, polcn, chlId, resDTO,polcnMap);
		if (tickDO == null) {
			return null;
		}
		// 当前排班加锁 add by zhengcm 2018-03-14
		new TickChl4MtAptEP().lockSch(tickDO.getId_sch());
	 ...
	 * @throws BizException
	 */
	private ScSchTickDO filterTickByScSrvKinditm(ScSchTickDO tick, String idSrv, String idSch, MtResDTO resDTO) throws BizException {
		...
		if(ArrayUtil.isEmpty(resDTO.getDays_excl_must().toArray()) &&
				...
				((!ArrayUtil.isEmpty(resDTO.getDays_together_must().toArray())) || (!ArrayUtil.isEmpty(resDTO.getDays_together_hint().toArray())))
						)
		{
