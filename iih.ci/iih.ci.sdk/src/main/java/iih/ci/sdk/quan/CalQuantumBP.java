...
public class CalQuantumBP {
	...
	public FDouble getQuantum(QuantumParamDTO param) throws BizException {
		if (ObjectUtils.isEmpty(param.getFg_mm()) || !param.getFg_mm().booleanValue() 
				|| (!ObjectUtils.isEmpty(param.getSd_srvtp()) && param.getSd_srvtp().equals(IBdSrvDictCodeConst.SD_SRVTP_BLOODPROD_USE))) {
...
//		FDouble factor = this.getFactorForUnitBaseToUnitSale(mmpackageUnits, param.getId_unit_sale());
		...
		for(MMPackageUnitDO mmpackageUnit:mmagg.getMMPackageUnitDO()){
			if (mmpackageUnit.getEn_ippkgtp_ref() == 1) {
	...
	private String getSdMupakgu(String code_entp, FBoolean fg_pres_outp, String sd_ipmutp, String sd_opmutp)
			throws BizException {
		if (CiDwsUtils.isOpUrgentWf(code_entp) || !ObjectUtils.isTrue(fg_pres_outp)) {
			return sd_opmutp;
		} else if (CiDwsUtils.isIpWf(code_entp)) {
			return sd_ipmutp;
		} else {
	...
	private FDouble getFactorForUnitBaseToUnitSale(MMPackageUnitDO[] mmpackageUnits, String id_unit_sale)
			throws BizException {
		if (ObjectUtils.isEmpty(id_unit_sale)) {
