...
/**
 * 临床医嘱选择服务，服务套套内项目合计价格计算操作BP  (有待优化）
 * @author wangqingzhu
 *
 */
public class CiOrSrvSetSumPriMdCalBP {
	 ...
	 * @throws BizException
	 */
	public FDouble exec(BdSrvPriCalParamDTO param, Map<String,FDouble> setquanmap) 
			throws BizException{
		//有效性判断处理
		...
		//本服务定价价格
		if (!ObjectUtils.isEmpty(calsrvgrps[0])) {
			stdpris = ipricalsrv.CalManySrvsStdPrices((PriStdSrvDTO[]) calsrvgrps[0]);
			rtn = CiOrSrvPriHelper.getPriceSum(stdpris, setquanmap);
		}

		// 组合定价价格
		if (!ObjectUtils.isEmpty(calsrvgrps[1])) {
			stdpris = ipricalsrv.CalMultiSrvCompPrice((String[]) calsrvgrps[1]);
			FDouble rtn1 = CiOrSrvPriHelper.getPriceSum(stdpris, setquanmap);
			if (!ObjectUtils.isEmpty(rtn1) && !ObjectUtils.isEmpty(rtn)) {
				rtn = rtn1.add(rtn);
			} else {
				rtn = rtn1;
			}
	 ...
	 * @throws BizException
	 */
	public FDouble[] exec(BdSrvPriCalParamDTO param, String id_pripat, Map<String,FDouble> setquanmap) 
			throws BizException{
		//有效性判断处理
		if (!validateCheck(param))
			return null;
		...
		if (!ObjectUtils.isEmpty(calsrvgrps[0])) {
			...
			FDouble[] szPrice = CiOrSrvPriHelper.getPriceSum(szSrvPricalRateAndPriceDTO, setquanmap);
		...
		if(!ObjectUtils.isEmpty(calsrvgrps[1])){
			...
			for (String id_srv_comp : szCalSrvGrps){
				if (rstMap.containsKey(id_srv_comp)){
					...
					if(setquanmap!=null && setquanmap.size()>0 && setquanmap.containsKey(id_srv_comp)){
						for(String id_srv:srvPricalRateAndPriceMap.keySet()){
							setquanmap.put(id_srv, setquanmap.get(id_srv_comp));
						}
					}
	...
	private boolean validateCheck(BdSrvPriCalParamDTO param){
		...
		if(ObjectUtils.isEmpty(param.getSrvsetitms()))return false;
		return true;
	}
	...
	private Object[] getPriCalSrvObjs(FArrayList list){
		...
		for(int i=0;i<list.size();i++){
			...
			if(ObjectUtils.isEmpty(id_primd))
			{
				try {
			...
			if(CiOrSrvPriHelper.isSrvSelfPrimd(id_primd)){
				...
				for(PriStdSrvDTO pristdDto:pristdsrvdtos){
					if(pristdDto.getId_srv().equals(param.getId_srv()))isContains=true;
				}
				if(!isContains){
