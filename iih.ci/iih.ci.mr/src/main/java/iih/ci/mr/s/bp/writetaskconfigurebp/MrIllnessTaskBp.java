...
public class MrIllnessTaskBp {

	public void writeTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr,Boolean flag) throws BizException{
		if(flag){
			createIllnessTask(ors,rkRuleDoArr);
		}else{
			cancelIllnessTask(ors,rkRuleDoArr); 
		}
	}
	
	
	public void createIllnessTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr)  throws BizException{
	...
	public void cancelIllnessTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr)  throws BizException{
		for (int j = 0; j < ors.length; j++) {
			CiOrderDO ciorderdo=ors[j];
			if(ciorderdo==null||!IBdSrvDictCodeConst.SD_SRVTP_ENTRUST_ILLSTATE.equals(ciorderdo.getSd_srvtp()))
				return;
