...
import iih.ci.ord.pub.listener.AllOpOrSignListener;
...
public class MrAllOpOrSignListener extends AllOpOrSignListener{
//手术医嘱下达后MR监听处理
	@Override
	protected void doYourActionAfterOrSign(CiOrderDO[] ors) throws BizException {
		...
		for (int j = 0; j < ors.length; j++) {
			...
			MrTaskRuleDO[] rkRuleDoArr=ruleService.find(" code_udidoc='"+ICiMrDictCodeConst.SD_MR_TASK_EVENT_SHOUSHU+"' and fg_active='Y' and id_mrcactm in (select id_mrcactm from bd_mrca_ctm where code_entp = '"+ciorderdo.getCode_entp()+"')  " , "", FBoolean.FALSE);
			if (rkRuleDoArr.length<=0) {
				return;
			}
			ICiorappsurgeryMDORService opService=ServiceFinder.find(ICiorappsurgeryMDORService.class);
			...
			for (int i = 0; i < rkRuleDoArr.length; i++) {
				...
				if (opService!=null) {
					OrdApOpDO[] opDoArr=opService.findByAttrValString(OrdApOpDO.ID_OR, ciorderdo.getId_or());
					if (opDoArr!=null&&opDoArr.length>0) {
						mrTaskDO.setDt_begin(opDoArr[0].getDt_plan());
						mrTaskDO.setDt_end(new FDateTime(opDoArr[0].getDt_plan().getMillis() + ICalendar.MILLIS_PER_HOUR * rkRuleDoArr[i].getLimittime()));
					}else {
