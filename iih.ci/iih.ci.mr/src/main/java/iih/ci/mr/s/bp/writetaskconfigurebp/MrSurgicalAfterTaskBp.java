...
public class MrSurgicalAfterTaskBp {

	public void writeTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr,Boolean flag) throws BizException{
		if(flag){
			createSurgicalAfterTask(ors,rkRuleDoArr);
		}else{
			cancelSurgicalAfterTask(ors,rkRuleDoArr); 
		}
	}
	
	public void createSurgicalAfterTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr)  throws BizException{
		for (int j = 0; j < ors.length; j++) {
			...
			CiOrderDO ciorderdo=ors[j];
				...
				for (int i = 0; i < rkRuleDoArr.length; i++) {
					...
					mrTaskDO.setMrtask_name(rkRuleDoArr[i].getTiptext()+"_术后医嘱");
	...
	public void cancelSurgicalAfterTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr)  throws BizException{
		for (int j = 0; j < ors.length; j++) {
			CiOrderDO ciorderdo=ors[j];
