...
public class MrSurgicalTaskBp {
	
	public void writeTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr,Boolean flag) throws BizException{
		if(flag){
			createSurgicalTask(ors,rkRuleDoArr);
		}else{
			cancelSurgicalTask(ors,rkRuleDoArr); 
		}
	}
	
	public void createSurgicalTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr) throws BizException {
			...
			for (int i = 0; i < rkRuleDoArr.length; i++) {
				...
				if (opService!=null) {
					...
					if (opDoArr!=null&&opDoArr.length>0) {
						...
						mrTaskDO.setDt_end(new FDateTime(opDoArr[0].getDt_plan().getMillis() + ICalendar.MILLIS_PER_HOUR * Long.valueOf(rkRuleDoArr[i].getLimittime())));
					}else {
	...
	public  void cancelSurgicalTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr)  throws BizException {
		for (int j = 0; j < ors.length; j++) {
			CiOrderDO ciorderdo=ors[j];
