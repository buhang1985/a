...
import iih.ci.ord.pub.listener.DeathOrSignListener;
...
public class MrDeathOrSignListener extends DeathOrSignListener{
//死亡医嘱下达后MR监听处理
	@Override
	protected void doYourActionAfterOrSign(CiOrderDO[] ors) throws BizException {
		...
		for (int j = 0; j < ors.length; j++) {
			...
			MrTaskRuleDO[] rkRuleDoArr=ruleService.find(" code_udidoc='"+ICiMrDictCodeConst.SD_MR_TASK_EVENT_SIWANG+"' and fg_active='Y' and id_mrcactm in (select id_mrcactm from bd_mrca_ctm where code_entp = '"+ciorderdo.getCode_entp()+"')  ", "", FBoolean.FALSE);
