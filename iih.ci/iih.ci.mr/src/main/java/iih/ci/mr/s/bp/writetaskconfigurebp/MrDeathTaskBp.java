...
public class MrDeathTaskBp {

	public void writeTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr,Boolean flag) throws BizException{
		if(flag){
			createDeathTask(ors,rkRuleDoArr);
		}else{
			cancelDeathTask(ors,rkRuleDoArr); 
		}
	}
	
	public void createDeathTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr)  throws BizException{
	...
	public void cancelDeathTask(CiOrderDO[] ors,MrTaskRuleDO[] rkRuleDoArr)  throws BizException{
		for (int j = 0; j < ors.length; j++) {
			CiOrderDO ciorderdo=ors[j];
			if(ciorderdo==null||!IBdSrvDictCodeConst.SD_SRVTP_PATIMAN_CLIDEATH.equals(ciorderdo.getSd_srvtp()))
				return;
