package iih.ci.event.ord.bps.diag.ip.logicbp;
 ...
 * @author F
 *
 * @date 2019年9月18日下午3:44:56
 *
 * @classpath iih.ci.event.ord.bps.diag.ip.logicbp.IpDiagLogicDealWithBP
 */
public class IpDiagLogicDealWithBP  extends CommonParamBP{
	...
	private void MappingPvDiagEnDTO(IEOpPvDiagEnDTO pvDaigEnt, String id_ent, CiEnContextDTO ctx) throws BizException {
		...
		if (qryService != null) {
			...
			if (OpBasicDTO != null) {
				...
				pvDaigEnt.setP_bar_code(piPatDO.getParentDO().getCode_amr_ip());// 就诊号(取住院号)pi_pat.code_amr_ip
				...
				pvDaigEnt.setSerialno(OpBasicDTO.getVisitordno());// 就诊流水号en_ent.code
			}
		}
	}
