 ...
 * @author F
 *
 * @date 2019年8月19日下午3:58:49
 *
 * @classpath iih.ci.event.ord.bps.sign.op.sql.OpOpSignSql
 */
public class OpOpSignSql {

	/**
	 * BS007
	 * 门诊手术 签署 手术申请医嘱信息(手术就诊信息)【IEOpOpOrEnDTO】
	 * @param id_or4sql
	 * @return
	 */
	public String getOpOpSignEntInfoSql(String id_or4sql){
		...
		stringBuilder.append(" (case when q.fg_er_sug = 'Y' then 0 when q.fg_xq_sug = 'Y' then 1 when q.fg_zq_sug = 'Y' then 2 end) as operationKind, ");//--手术性质编码
		stringBuilder.append(" (case when q.fg_er_sug = 'Y' then '急症' when q.fg_xq_sug = 'Y' then  '限期' when q.fg_zq_sug = 'Y' then  '择期' end) as operationKindName, ");//--手术性质名称
		...
		//一些莫名其妙的的字段
		stringBuilder.append(" E.code_ie as op_scale, ");//--手术等级编码  ?为何在这一层
		...
		stringBuilder.append(" left outer join bd_psndoc S ON Q.id_emp_helper = S.id_psndoc ");
		...
		stringBuilder.append(" where M1.Fg_Maj = 'Y' and a.id_or ");
		stringBuilder.append(id_or4sql);
		return stringBuilder.toString();
	}
	/**
	 * BS007
	 * 门诊 签署 手术信息【IEOperationInfoDTO】
	 * @param id_or4sql
	 * @return
	 */
	public String getOpOpSignOperationInfoSql(String id_or4sql){
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(" select ");
		...
		stringBuilder.append(" ");//--是否计划
		stringBuilder.append(" ");//--非计划原因
		stringBuilder.append(" ");//--是否感染手术
	 ...
	 * @return
	 */
	public String getOpOpSignAssistInfoSql(String id_or4sql){
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(" select ");
		...
		stringBuilder.append(" and c.name <> '主刀医师' ");
		stringBuilder.append(" and c.name like '%第一助手%' ");
