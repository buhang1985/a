 ...
 * @author F
 *
 * @date 2019年9月11日上午10:54:04
 *
 * @classpath iih.ci.event.ord.listeners.repres.op.OpRePresEnterListener
 */
public abstract class OpRePresEnterListener extends OpEnterCommonListener{
	...
	public boolean isMatchSourceIdAndEventType(IBusinessEvent event)throws BizException{
		OrdEventLogger.info(OpRePresEnterListener.class.getSimpleName(), "门诊重新分方入口监听");
		if(!(IOrdEventSources.OP_REPRES_ENTER.equals(event.getSourceID())&& IEventType.TYPE_UPDATE_AFTER.equals(event.getEventType()))){
			return false;
		}
		return true;
	}
