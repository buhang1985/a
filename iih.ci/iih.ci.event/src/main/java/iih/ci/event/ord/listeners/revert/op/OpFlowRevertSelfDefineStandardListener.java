...
/**
 * 急诊流水撤回标准版 定义医嘱类型以及根据定义的医嘱类型发送事件
 * @author F
 *
 * @date 2019年11月25日下午6:17:49
 *
 * @classpath iih.ci.event.ord.listeners.revert.op.OpFlowRevertSelfDefineStandardListener
 */
public class OpFlowRevertSelfDefineStandardListener extends OpRevertEnterListener{
	...
	protected boolean isOpData(CiOrderDO or){
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版,筛选急诊流水数据");
	...
	public String defineOrdTypeBySdSrvtp(String sd_srvtp) {
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版,进入定义医嘱类型");
	...
	public void fireMessageByOrdType(Hashtable<String, String> htors8ieevent) throws BizException {
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版根据定义的医嘱类型,调用发送消息逻辑");
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版 htors8ieevent:"+htors8ieevent.toString());
		//急诊流水撤回处方BS302 发送除了撤回的所有的(收费、未收费) 且new
		...
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版-草药id_ors:"+herbIdors+",西药id_ors:"+wcIdors+",id_ent:"+id_ent);
		...
		//急诊流水撤回检查BS002
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版-检查id_ors:"+htors8ieevent.get(IOrdEventSources.OP_RIS));
		...
		//急诊流水撤回病理BS452
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版-病理id_ors:"+htors8ieevent.get(IOrdEventSources.OP_PATH));
		...
		//急诊流水撤回检验BS006
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版-检验id_ors:"+htors8ieevent.get(IOrdEventSources.OP_LIS));
		...
		//急诊流水撤回手术BS005
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版-手术id_ors:"+htors8ieevent.get(IOrdEventSources.OP_OPERATION));
		...
		//急诊流水撤回治疗BS303
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版-治疗id_ors:"+htors8ieevent.get(IOrdEventSources.OP_TREAT));
		...
		//急诊流水撤回备血BS008
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版-备血id_ors:"+htors8ieevent.get(IOrdEventSources.OP_BT));
		...
		//急诊流水撤回用血BS309
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版-用血id_ors:"+htors8ieevent.get(IOrdEventSources.OP_BU));
		...
		//急诊流水撤回会诊BS318
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊流水撤回标准版-会诊id_ors:"+htors8ieevent.get(IOrdEventSources.OP_CON));
