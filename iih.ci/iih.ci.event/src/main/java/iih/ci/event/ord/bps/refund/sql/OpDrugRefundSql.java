 ...
 * @author F
 *
 * @date 2019年9月16日下午6:03:57
 *
 * @classpath iih.ci.event.ord.bps.refund.sql.OpDrugRefundSql
 */
public class OpDrugRefundSql {

	/**
	 * BS302
	 * 门诊 处方/药品退费 就诊信息【IEOpPharmOrEnDTO】
	 * @return
	 */
	public String getOpDrugRefundEntInfoSql() {
		...
		stringBuilder.append(" left outer join (select name,code ");
	...
	/**
	 * BS302 
	 * 门诊 处方退费 处方退费信息【IEOpPharmPresDTO】退费+未退费/B区/剩余=本次退费单下所有处方信息(已收费C区)
	 * 这里查询所有  通过收费标识id_su_bl区分哪些是退费的  哪些不是退费的 ，至于dto不是一个的问题 在BP那转一下 查出来的不多 转一下比再查一遍更快
	 * @return
	 */
	public String getOpDrugRefundPressAllInfoSql(String press) {
		StringBuilder stringBuilder=new StringBuilder();
		stringBuilder.append(" select distinct ");
		stringBuilder.append(" A.ID_PRES as id_iepharmpres, ");//处方id
	    stringBuilder.append(" A.Id_En as id_iepharmoren, ");//就诊id
	    stringBuilder.append(" A.code || '_' || A.id_pres as orderno, ");//处方号
	    stringBuilder.append(" h.id_su_bl as id_su_bl, ");//收费标识
		...
		stringBuilder.append(" left join ci_or_srv h on h.id_pres=a.id_pres ");
		stringBuilder.append(" where A.Id_Pres ");
		stringBuilder.append(press);
		return stringBuilder.toString();
	}
	/**
	 * BS302 B区未退费处方金额 哪个没退 计算哪个金额
	 * @param press
	 * @return
	 */
	public String getOpDrugRefundPressPriceInfoSql(String press) {
		StringBuilder stringBuilder=new StringBuilder();
		stringBuilder.append(" select ");
		stringBuilder.append(" sum(round(c.quan_cur * c.price_pgku_cur,2)) as prescriptionMoney, ");//处方金额
		stringBuilder.append(" '元' as prescriptionmoneyunit, ");//处方金额单位
		stringBuilder.append(" b.id_pres as id_iepharmpres ");//处方id
	    stringBuilder.append(" from ci_pres a ");
	    stringBuilder.append(" left join ci_or_srv b on b.id_pres=a.id_pres ");
	    stringBuilder.append(" left join ci_or_srv_mm c on b.id_orsrv =c.id_orsrv ");
	    stringBuilder.append(" left join ci_order d on d.id_or=b.id_or ");
	    stringBuilder.append(" where a.id_pres ");
	    stringBuilder.append(press);
	    stringBuilder.append(" and b.id_su_bl='@@@@AA10000000026639' ");
	    stringBuilder.append(" and d.fg_sign='Y' ");
	    stringBuilder.append(" and d.fg_canc = 'N' ");
	    stringBuilder.append(" and d.sd_srvtp like '01%' ");
	    stringBuilder.append(" group by b.id_pres ");
	    return stringBuilder.toString();
	}
	/**
	 * BS302
	 * 门诊 处方/药品签署 西药未退费信息【IEOpPharmWcOrDTO】
	 * @return
	 */
	public String getOpDrugRefundWcInfoSql() {
		StringBuilder stringBuilder=new StringBuilder();
		stringBuilder.append(" select ");
	...
	/**
	 * BS302
	 * 门诊 处方/药品签署 草药未退费信息【IEOpPharmHerbOrDTO】
	 * @return
	 */
	public String getOpDrugRefundHerbInfoSql() {
		StringBuilder stringBuilder=new StringBuilder();
		stringBuilder.append(" select ");
	...
	/**
	 * BS302
	 * 门诊 处方/药品签署 草药成分未退费信息【IEOpPharmOrMmDTO】
	 * @return
	 */
	public String getOpDrugRefundHerbElementsInfoSql() {
		StringBuilder stringBuilder=new StringBuilder();
		stringBuilder.append(" select ");
