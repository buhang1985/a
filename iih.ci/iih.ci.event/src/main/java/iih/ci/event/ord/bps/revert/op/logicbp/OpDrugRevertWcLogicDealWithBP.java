...
import iih.ci.event.ord.bps.revert.op.query.OpDrugRevertWcInfoQuery;
import iih.ci.event.ord.bps.revert.op.query.OpDrugRevertWcRefundInfoQuery;
 ...
 * @author F
 *
 * @date 2019年9月11日下午1:13:04
 *
 * @classpath iih.ci.event.ord.bps.revert.op.logicbp.OpDrugRevertWcLogicDealWithBP
 */
public class OpDrugRevertWcLogicDealWithBP {
	...
	public Hashtable<String, FArrayList2> exec(String id_preses) throws BizException {
		// 有效性校验
		if (OrdEventUtil.isEmpty(id_preses))
			return null;

		// 获得西成药处方明细信息
		OpDrugRevertWcInfoQuery qry = new OpDrugRevertWcInfoQuery(id_preses);
	...
	public Hashtable<String, FArrayList2> exec(String id_preses, String id_orsrvs) throws BizException {
		...
		// 获得西成药处方明细信息
		OpDrugRevertWcRefundInfoQuery qry = new OpDrugRevertWcRefundInfoQuery(id_preses, id_orsrvs);
		IEOpPharmWcOrDTO[] predtos = (IEOpPharmWcOrDTO[]) AppFwUtil.getDORstWithDao(qry, IEOpPharmWcOrDTO.class);
		if (OrdEventUtil.isEmpty(predtos))
			return null;
