...
import iih.ci.event.ord.bps.revert.op.query.OpRisRevertContacterQuery;
import iih.ci.event.ord.bps.revert.op.query.OpRisRevertInfoQuery;
 ...
 * @author F
 *
 * @date 2019年9月11日下午1:12:19
 *
 * @classpath iih.ci.event.ord.bps.revert.op.logicbp.OpRisRevertLogicDealWithBP
 */
public class OpRisRevertLogicDealWithBP extends CommonParamBP{
	...
	public BaseDTO[] exec(OrdEventFireDTO firedto) throws BizException{
		...
		// 获得sql串及其对应的结果集map
		OpRisRevertInfoQuery qry = new OpRisRevertInfoQuery(firedto.getIdors());
		
		//String sql = getSQlStr(id_ors);
		String sql = qry.getQrySQLStr();
		
		//Map<String, Object> map = OrdEventUtil.getRsMap(sql);
		SqlParam param = qry.getQryParameter(new StringBuffer());
		//日志记录查询语句 便于查错
		if(StringUtils.isNotEmpty(sql) && param != null) {
			OrdEventLogger.info(this.getClass().getSimpleName(), "BS002 门诊检查撤回所有信息sql:"+sql+" --param:"+param.toString());
		}
		//查询联系人集合
		OpRisRevertContacterQuery contacterQry = new OpRisRevertContacterQuery(firedto.getIdors());
