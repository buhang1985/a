		 ...
		 String wherestr = EntDiDODesc.TABLE_ALIAS_NAME+"."+EntDiDO.ID_ENT +"='"+id_ent+"'";
		  ...
		  if(entdidos == null)  return null;
		   ...
		   for(EntDiDO entdi:entdidos){
			   ...
			   if(FBoolean.TRUE.equals(entdi.getFg_maj())){
				   fg_maj +="★";
			   } 
			   if(FBoolean.TRUE.equals(entdi.getFg_suspdi())){
				   fg_maj +="?"; 
			   }
			   if(entdi.getName_didef_syn() != null){
				   diag.setName(entdi.getName_didef_dis()+" "+entdi.getName_didef_syn()+fg_maj);
			   }else{
