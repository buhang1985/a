 ...
 *
 */
public class DeletionOutDeptRecord  implements CommonInterface{
...
//	private static final int limitTime = 48;
	...
	public ArrayList isOk() throws Exception {
		/*
		 * 1 判断是否有转出，没有转入直接返回true
		 * 2 判断是否超过规定时间，没有则返回true
		 * 3 判断是否存在患者转出入记录信息以及  这种记录的文书
		 * 4 有转入记录且超过固定时间，有返回true，没有返回false
		 */

		// else if(!){//判断是否超过24小时
		// 无法获取到转出时间，不知道转出时间在哪个表，哪个字段
		// }
		if(mrtasklist.length==0){
			return null;
		}
