...
//								.getName_or());
	 ...
	 * @throws BizException
	 */
	public List<ReactExtOrsAndStopOrsDO> exec4Ip(CiorderAggDO[] aggors, FDateTime dt_effe) throws BizException{
		//有效性校验
		if(!validateCheck(aggors))return null;
	...
	private ReactExtOrsAndStopOrsDO getRtn(CiorderAggExtDO[] oraggextinfos,CiOrderDO[] willstoporders){
		...
		rtn.setStopords(CiOrdUtils.array2FArrayList(willstoporders));
