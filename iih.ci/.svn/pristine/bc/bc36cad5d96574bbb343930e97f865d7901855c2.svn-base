 ...
 * @author F
 *
 * @date 2019年9月11日上午10:54:16
 *
 * @classpath iih.ci.event.ord.listeners.repres.op.OpRePresSelfDefineStandardListener
 */
public class OpRePresSelfDefineStandardListener extends OpRePresEnterListener{
	...
	protected boolean isOpData(CiOrderDO ordo) {
		OrdEventLogger.info(this.getClass().getSimpleName(), "普通门诊重新分方标准版,筛选急诊流水数据");
	...
	public String defineOrdTypeBySdSrvtp(String sd_srvtp) {
		OrdEventLogger.info(this.getClass().getSimpleName(), "普通门诊重新分方,进入定义医嘱类型");
	...
	public void fireMessageByOrdType(Hashtable<String, String> htors8ieevent) throws BizException {
		OrdEventLogger.info(this.getClass().getSimpleName(), "普通门诊重新分方根据定义的医嘱类型,调用发送消息逻辑");
		OrdEventLogger.info(this.getClass().getSimpleName(), "普通门诊重新分方 htors8ieevent:"+htors8ieevent.toString());
		//普通门诊处方重新分方BS302
		...
		OrdEventLogger.info(this.getClass().getSimpleName(), "普通门诊重新分方标准版-草药id_ors:"+herbIdors+",西药id_ors:"+wcIdors+",id_ent:"+id_ent);
