...
import iih.ci.ord.s.ems.itf.bp.IOrderCancelCopyBP;
 ...
 *
 */
public class OrderCancelCopyBP implements IOrderCancelCopyBP {
	...
	public OrderRstDTO cancelcopy(CiEnContextDTO ctx,OrderOperateDTO arg) throws BizException {
		...
		if (paramList != null && paramList.size() > 0 && extension != null) {
			...
			boolean rtn = false;
			if (lstIdors.size() > 0) {
				rstDTO = cancelBP.exec(lstIdors.toArray(new String[] {}), ctx, arg.getExtension());
				rtn = rstDTO.getIsSuccess().booleanValue();
				rstDTO = copyBP.exec(lstIdors.toArray(new String[] {}), ctx, arg.getExtension());
