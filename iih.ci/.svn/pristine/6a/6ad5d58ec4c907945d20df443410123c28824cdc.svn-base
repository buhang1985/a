...
public class ConvertXml2DTOsUtils {
	 ...
	 * @throws BizException
	 */
	public BaseDTO exec(String param, Class<? extends BaseDTO> classInfo, Class<? extends BaseDTO> classInfoSub) throws BizException {

		// 1、基础校验
		validation(param);

		// 2、解析Xml
		BaseDTO rtns = parseXml(param, classInfo, classInfoSub);
	 ...
	 * @throws BizException
	 */
	public Map<String,Object>  exec(String param) 
			throws BizException {
		// 1、基础校验
		validation(param);

		// 2、解析Xml
		return parseXml(param);
	}
	 ...
	 * @throws BizException
	 */
	private Map<String,Object> parseXml(String xmlStr) 
			throws BizException {
		...
		try {
			...
			if (eleRoot != null) {
				...
				for (Element element : listEle) {
					if (element == null)
						continue;
					rtn.put(element.getName().toLowerCase(), element.getData());
				}
			}
			return rtn;
		} catch (Exception e1) {
	...
	private BaseDTO setChildclassVal(Element el,Class<?> classInfo) throws BizException {
		List<Element> childlistEle2=el.elements();
		if(childlistEle2!=null&&childlistEle2.size()>0) {
			List<String> listAttr = new ArrayList<String>();
			BaseDTO dto = initRtn(classInfo, listAttr);
			BaseDTO rtn = (BaseDTO) dto.clone();
			for (Element element : childlistEle2) {
				if (element == null)
					continue;
		    if (listAttr.contains(element.getName().toLowerCase())) {
				setVal(rtn, element);
	 ...
	 * @throws BizException
	 */
	private BaseDTO parseXml(String xmlStr, Class<?> classInfo, Class<?> childclassInfo) 
			throws BizException {
		List<String> listAttr = new ArrayList<String>();
		BaseDTO dto = initRtn(classInfo, listAttr);
		BaseDTO rtn = (BaseDTO) dto.clone();
			...
			if (eleRoot != null) {
				...
				for (Element element : listEle) {
					if (element == null)
						continue;
					// 遍历Xml节点，赋值给DTO
					List<Element> childlistEle=element.elements();
					if(childlistEle!=null&&childlistEle.size()>0) {
						if(listAttr.contains(element.getName().toLowerCase())) {
						String arrtName = element.getName();
						FArrayList fa=new FArrayList();
						for (Element EL : childlistEle) {
							BaseDTO oo= setChildclassVal(EL,childclassInfo);
							if(oo!=null)
							fa.add(oo);
						}
						rtn.setAttrVal(arrtName.substring(0, 1).toUpperCase() + arrtName.substring(1).toLowerCase(), fa);
						}
					}else  if (listAttr.contains(element.getName().toLowerCase())) {
						setVal(rtn, element);
					}
				}
			}
