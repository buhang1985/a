...
/**
 * 加急事件修改后和新增后，进行IC集成平台事件触发监听器 （仅集成平台事件发送用）
 *
 */

public class OrderUrgentClear extends AbstractEvent4CIListener{

	@Override
	protected void sendIEMsg(Object[] newObjs, String eventType) throws BizException {
		
		XLogger.info("加急事件修改和新增 ，触发缓存清理-->触发点"+this.getClass().getName());
		//查找缓存清除类
	...
	protected String getEventSourceid() throws BizException {
		...
		return IBdSrvEventConst.USEREQ_ADDDELMOD_EVENT_SOURCEID;
	}
