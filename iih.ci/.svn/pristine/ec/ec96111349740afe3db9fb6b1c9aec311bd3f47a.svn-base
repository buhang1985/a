...
import iih.ci.ord.hp.gj.HpMedicalInsurance;
...
public class CiOrAggAndRelInfoSaveBP {
	...
	public CiorderAggDO exec(CiOrAggAndRelDatum oraggandrelinfo, boolean iseventsave, Integer iemstp)
		...
		if (CiOrdUtils.IsMedInsuranceCheckUse().booleanValue()) {
			if (CiOrdUtils.isHpUsing(ctx)) {
				...
				if (!OrSourceFromEnum.IIHSRVCAHELPER.equals(eu_orsrcmdtp)
					...
					CiHpCheckResultDTO result = HpDetailedRuleBP.getInstance().getHpDetailedRule(oraggandrelinfo,
							ctx);
					...
					if (result != null && !result.getCheckflag().booleanValue()) {
						...
						// 住院 TODO
						if (ctx != null && !IBdFcDictCodeConst.SD_CODE_ENTP_IP.equals(ctx.getCode_entp())) {
							CiOrdUtils.setorderAggMap(ctx, oraggandrelinfo);
			...
			// 判断是否临床路径患者
			// String id_ent =
			// oraggandrelinfo.getOraggdo().getParentDO().getId_en();
			// if(CiOrdAppUtils.getIHpcpAppItfService().getEntIncpFlagExcludeWaitcp(id_ent)){
			// try{
			// CiOrdAppUtils.getIHpcpAppItfService().checkCiorderSrv(id_ent,
			// ciorList);
			...
			if (ctx != null
					&& (ctx.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_OP)
					...
					&& ctx.getSd_hptp() != null && ctx.getSd_hptp().startsWith("2")) {
				// 高端商保标志·

				String id_hp = "id_hp ='" + ctx.getId_hp() + "'";
			...
			} else if (ctx != null && ctx.getSd_hptp() != null && ctx.getSd_hptp().startsWith("1")) {
				// 医保信息 改成 在签署时调用 2016-11-23
				// JudgeCiHpSave(oraggandrelinfo, diagList);
				// 适应症 信息 是否启用医保标志
				if (CiOrdUtils.isHpUsing(ctx)) {
					FArrayList2 diagList = CiOrdUtils
							.getDiagItemList(oraggandrelinfo.getOraggdo().getParentDO().getId_en());
					// setMedicalInsurance(oraggandrelinfo,diagList);
					HpMedicalInsurance.setMedicalInsurance(oraggandrelinfo, diagList);
				} else {
					if (ctx != null && (ctx.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_OP)
							|| ctx.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_ET))) {
						if (oraggandrelinfo.getOraggdo().getParentDO().getSd_srvtp() != null
								&& oraggandrelinfo.getOraggdo().getParentDO().getSd_srvtp()
										.startsWith(IBdSrvDictCodeConst.SD_SRVTP_DRUG)) {
			...
			NeusoftHpCheckedBP neusoftHpCheckBP = new NeusoftHpCheckedBP();
			List<CiOrAggAndRelDatum> aggAndRelDatumList = new ArrayList<CiOrAggAndRelDatum>();
			aggAndRelDatumList.add(oraggandrelinfo);
			neusoftHpCheckBP.execChecked(ctx, aggAndRelDatumList);
		}
		// 设置医嘱是否保内标识
		this.setOrdFg_orhp(oraggandrelinfo.getOraggdo(), ctx);
		// 验证医保和诊断关系
		if (ctx != null && ctx.getSd_hptp() != null && ctx.getSd_hptp().startsWith("1")) {
...
//		}
	...
	private String ValidateOrderAndDig(CiorderAggDO aggdo) throws BizException {
		...
		if (map != null && !map.isEmpty()) {
			return aggdo.getParentDO().getName_or() + " 为医保医嘱 缺少诊断：" + CiOrdUtils.getFMapValuesConveretstrNoQuotes(map);
		}
		return null;
	}
	...
	private FMap getHPsrvItem(CiorderAggDO aggdo, Map IDHpMap) {
		...
		for (OrdSrvDO srvdo : aggdo.getOrdSrvDO()) {
			if (FBoolean.FALSE.equals(srvdo.getFg_selfpay())) {
	 ...
	 * @param ciorderAgg
	 */
	private void setOrdFg_orhp(CiorderAggDO ciorderAgg, CiEnContextDTO ctx) {
		...
		for (OrdSrvDO ordSrv : ordSrvs) {
			...
			if (ciOrder.getEu_hpindicjudge() == HpIndicJudgeEnum.SELFPAY) {
				...
				if (ordSrv.getStatus() == DOStatus.UNCHANGED)
	...
	private CiorderAggDO orAggDOSave(CiorderAggDO aggdo) throws BizException {
		...
		if (ciOrder.getStatus() == DOStatus.NEW) {
			ciOrder.setSortno(CiOrdUtils.GetNewCiOrderSortno(ciOrder.getId_en()));
		}
		// 用法为皮试用法时，皮试标识为Y
	...
	private void orAggAttachInfoSave(CiorderAggDO aggdo, Hashtable ht, Integer iemstp)
		...
		if (orsrvdos != null) {
			...
			for (int i = 0; i < orsrvdos.length; i++) {
				...
				// 为什么放在 循环里面？
				// if(srvsetsmap!=null &&
				// srvsetsmap.containsKey(ordo.getId_srv())){
		...
		//判断ci_or_srv_mm中是否存在重复数据
		valiteOrdSrvMmSameData(aggdo);
		// 医嘱对应的申请单保存
		ciOrAppSaveHandle(orappmap, id_or, iemstp);
	}
	/**
	 * 校验ci_or_srv_mm表中存在重复数据
	 * @param aggdo
	 * @throws DAException 
	 */
	void valiteOrdSrvMmSameData(CiorderAggDO aggdo) throws BizException{
		if(aggdo == null){
			return;
		}
		...
		for (OrdSrvDO ordSrvDO : ordsrvs) {
			if(ordSrvDO.getSd_srvtp() != null && ordSrvDO.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_DRUG))
				idorsrvs.add(ordSrvDO.getId_orsrv());
		}
		if(idorsrvs.size() > 0){
			String sql = " select id_orsrv from ci_or_srv_mm where ";
			sql += SqlUtils.getInSqlByIds("id_orsrv", idorsrvs);
			sql += " group by id_orsrv having count(id_orsrv) > 1";		
			DAFacade fa = new DAFacade();
			ArrayList list = (ArrayList)fa.execQuery(sql,new ColumnListHandler());
			if(list!=null && list.size()>0){
				...
				throw new BizException(String.format("医嘱【%s】中的ci_or_srv_mm中存在重复数据，重复主键:%s",aggdo.getParentDO().getName_or(),sb.toString()));
			}
		}
	}
	/**
