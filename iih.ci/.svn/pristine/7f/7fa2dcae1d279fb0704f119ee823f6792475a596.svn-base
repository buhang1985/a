...
import iih.ci.ord.s.bp.ems.CiOrRelUsgFeeOrSrvBuildBP;
...
public class CiOrEms2ModOrAggBP {
	 ...
	 * @throws BizException 
	 */
	private void ciOrSrvInfoHandle(CiorderAggDO aggdo,ArrayList<OrdSrvDO> orsrvs,Hashtable ht,CiEmsDTO ems,List<OrdSrvDO> orsetsrv) throws BizException{
			...
			if( FBoolean.TRUE.equals(srvdo.getIsmuldose()) || FBoolean.TRUE.equals(srvdo.getIsmulexec())){
				...
				if(CiOrdUtils.isSrvsetMemCntAdditionalPrimd(mainemssrv.getId_primd())){
					...
					for (Object ciemssrv : ems.getEmssrvs()) {
						...
						if (FBoolean.FALSE.equals(ciemssrvdto.getFg_or()) &&  FBoolean.TRUE.equals(ciemssrvdto.getFg_bl())) {
							if(FBoolean.TRUE.equals(srvdo.getIsmuldose())){
								if(ciord.getQuan_medu() != null && ciemssrvdto.getQuan_med()!=null){
							...
							if(FBoolean.TRUE.equals(srvdo.getIsmulexec())){
	 ...
	 * @throws BizException
	 */
	private void ciOrSrvInfoHandleForOrMode(CiorderAggDO aggdo,ArrayList<OrdSrvDO> rtn,Hashtable ht,CiEmsDTO ems,Integer[] srvsetitemindexs,MedSrvDO srvdo,List<OrdSrvDO> orsetsrv) throws BizException{
		//套医嘱开立模式时套内服务项目的医嘱服务处理逻辑
		ArrayList<OrdSrvDO> setrelsrvs=srvSetRelSrvHandle4OrMode(aggdo,ems,srvsetitemindexs,rtn,srvdo,ht,orsetsrv);
	 ...
	 * @throws BizException 
	 */
	private ArrayList<OrdSrvDO> srvSetRelSrvHandle4OrMode(CiorderAggDO aggdo,CiEmsDTO ems,Integer[] srvsetitemindexs,ArrayList<OrdSrvDO> rtn,MedSrvDO setsrvdo,Hashtable ht,List<OrdSrvDO> orsetsrv) throws BizException{
	...
	/**
	 * 创建组合定价模式医嘱项目对应的医嘱项目,增加组合定价关联物品逻辑
	 * @param aggdo
	 * @param ems
	 * @param orsrvdos
	 * @param ht
	 ...
	 * @throws BizException
	 */
	private ArrayList<OrdSrvDO> getCiOrCompPriOrSrvs(CiorderAggDO aggdo,CiEmsDTO ems,OrdSrvDO[] orsrvdos,Hashtable ht) throws BizException{
		//有效性校验
		if(ems==null || ems.getEmssrvs()==null || ems.getEmssrvs().size()==0)return null;
		if(CiOrdUtils.isHasPriSrvHandled(ems).booleanValue()) return null;//前台是否已经算过费用项目
	...
	/**
	 * 获得组合定价模式医嘱项目对应的费用医嘱项目集合数据信息，增加组合定价关联物品逻辑
	 * @param aggdo
	 * @param ems
	 * @param ipos
	 * @param ht
	 ...
	 * @throws BizException
	 */
	private ArrayList<OrdSrvDO> boilRelationFee(CiorderAggDO aggorder,CiEmsDTO ems,ArrayList<OrdSrvDO> orsrvs,Hashtable ht) throws BizException{
