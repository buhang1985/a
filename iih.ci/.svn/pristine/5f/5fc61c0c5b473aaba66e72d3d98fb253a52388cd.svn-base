...
//		CiOrdAppUtils.getOrSrvService().save(ordSrvDOs);
	...
	public void delPres(String[] idors) throws BizException{
		List<String> preslist = modOrdSrv(idors);
		if (preslist == null || preslist.isEmpty())
			return;
		IPresCudService service = ServiceFinder.find(IPresCudService.class);
		IPresRService rService = ServiceFinder.find(IPresRService.class);
		CiPresDO[] ciPresDOs = rService.findByIds(preslist.toArray(new String[0]), FBoolean.FALSE);
		service.delete(ciPresDOs);
	}
}
