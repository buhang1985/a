...
public class OrderDeleteBP implements IOrderDeleteBP {
	...
	public OrderRstDTO delete(CiEnContextDTO ctx, OrderOperateDTO arg) throws BizException {
		...
		//判断关联的数据表是否存在，存在不允许删除
		//diriveDataDelValide(lstIdors);
		// 获得医嘱集合
		CiorderAggDO[] aggdos = CiOrdAppUtils.getOrAggQryService()
				.findByIds(lstIdors.toArray(new String[lstIdors.size()]), FBoolean.FALSE);
		...
		for (CiorderAggDO aggdo : aggdos) {
			list.add(aggdo.getParentDO());
		}
		CiOrderDO[] ciors = (CiOrderDO[]) list.toArray(new CiOrderDO[list.size()]);
		CiOrSufInfo4ScAptApplBp sufInfo4ScAptApplBp = new CiOrSufInfo4ScAptApplBp();
		sufInfo4ScAptApplBp.sendSugInfo4ScAptAppl(ctx, ciors);
		rstDTO.setIsSuccess(FBoolean.TRUE);
		return rstDTO;
	}

	private void diriveDataDelValide(List<String> lstIdors) throws BizException {
		CiOrdUtils.DiriveDataDelValide(lstIdors);
	}
