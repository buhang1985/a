...
import iih.ci.ord.dto.ems.uiemsdto.d.EmsDrugsItemViewDTO;
...
public class OpBaseEmsValidate implements IEmsValidate{
	...
	protected void checkDrugQuanInfo(FArrayList srvList,List<String> errors){
		for(Object obj : srvList){
			EmsDrugsItemViewDTO srvItem = (EmsDrugsItemViewDTO)obj;
	...
	protected void checkMmStockInfo(FArrayList srvList,List<String> errors) throws BizException{
		...
		for(Object obj : srvList){
			EmsDrugsItemViewDTO emsDTO = (EmsDrugsItemViewDTO)obj;
			if(FBoolean.TRUE.equals(emsDTO.getFg_mm())&& emsDTO.getStatus()!=DOStatus.DELETED && !FBoolean.TRUE.equals(emsDTO.getFg_self())){
		...
		if(reqDtos.size()>0){
			...
			for(Object obj : srvList){
				...
				for(MaterialStockDTO materialDo : stockdto){
					...
					if (materialDo.getMmstatus() == (int)MaterialStatus.NORELATION && !errors.contains(emsDTO.getName_srv() ))
					{
						errors.add(String.format("服务：%s,药房无此药！",emsDTO.getName_srv() ));
					}
					else if (materialDo.getMmstatus() == (int)MaterialStatus.STOP  && !errors.contains(emsDTO.getName_srv() ))
					{
						errors.add(String.format("服务：%s,已停发！",emsDTO.getName_srv() ));
					}
					else if (emsDTO.getQuan_cur()!=null && materialDo.getQuan_usable().toDouble() < emsDTO.getQuan_cur().toDouble()  && !errors.contains(emsDTO.getName_srv() ))
					{
	 ...
	 * @param errors
	 */
	protected void checkEmptyDrugInfo(EmsDrugsItemViewDTO srvItem,List<String> errors){
