...
import iih.ci.event.ord.bps.sign.ip.query.IpLisSignAllInfoQuery;
 ...
 * @author F
 *
 * @date 2019年8月27日下午3:47:01
 *
 * @classpath iih.ci.event.ord.bps.sign.ip.logicbp.IpLisSignLogicDealWithBP
 */
public class IpLisSignLogicDealWithBP extends CommonParamBP{
	...
	public IEOpLisOrEnDTO[] exec(OrdEventFireDTO firedto) throws BizException{
		...
		IpLisSignAllInfoQuery qry = new IpLisSignAllInfoQuery(firedto.getIdors());
		//获得sql串及其对应的结果集map
	...
	private void setLisEnDto8Map(IEOpLisOrEnDTO ieoplisdto, Map<String, Object> map) throws BizException {
		...
		ieoplisdto.setBirthday(new FDateTime(map.get("birthday").toString()));
		ieoplisdto.setAge(AgeCalcUtil.getAge(new FDate(new FDateTime(map.get("birthday").toString()).toString())));    
		}
