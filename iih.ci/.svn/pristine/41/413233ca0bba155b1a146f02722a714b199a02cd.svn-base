...
public class CiOrderCopyBP {
	...
	public MoreEmsParamDTO exec() throws BizException {
		...
		CiOrTmplAggDTOMappingCiEmsChildDTO ciOrTmpl = new CiOrTmplAggDTOMappingCiEmsChildDTO();
		return ciOrTmpl.Mapping(ciEnContext, ortmpls, null);
	...
	private CiOrTmplDTO getCiOrTmplCopy(CiorderAggDO ciorderAgg) throws BizException {
		...
		if (FBoolean.TRUE.equals(ciorderDO.getFg_set())) {
			...
			for (OrdSrvDO ordSrv : orderSrvs) {
				...
				if (FBoolean.TRUE.equals(ordSrv.getFg_or())) {
	...
	private void copyCiOrTmpl(CiOrderDO source, CiOrTmplDTO target) {
		...
		// 设置长期临时状态
		if (FBoolean.TRUE.equals(source.getFg_long())) {
			target.setEu_long(LongTempOrEnum.LONGOR);// 长期
		} else if (FBoolean.FALSE.equals(source.getFg_long())) {
		...
		target.setId_excessive_reason(source.getId_excessive_reason());
		target.setSd_excessive_reason(source.getSd_excessive_reason());
	...
	public void copyCiOrTmplSrv(OrdSrvDO source, CiOrTmplSrvDTO target) throws BizException {
		...
		if (FBoolean.TRUE.equals(source.getFg_selfsrv())) {
		...
		// 如果是物品
		if (FBoolean.TRUE.equals(source.getFg_mm())) {
