...
public class OrderLoadBP implements IOrderLoadBP {
	...
	public OrderRstDTO load(CiEnContextDTO ctx, OrderOperateDTO arg) throws BizException {
		...
		String whereStr = String.format(
				" a0.id_en='%s' and a0.code_entp='%s' and a0.fg_canc='N' and a0.fg_pres_outp='N' and a0.eu_orsrcmdtp!='%s'",
		...
		CiOrderDO[] szOrder = iCiorderMDORService.find(whereStr, " a0.dt_effe " + orderbyMode, FBoolean.FALSE);
		DAFacade dafacade = new DAFacade();

		FArrayList orderList = new FArrayList();
		DecimalFormat df = new DecimalFormat("#.00");
		for (CiOrderDO ord : szOrder) {
			FDouble or_total_pri = new FDouble("0");
			String id_or = ord.getId_or();
			Map<String,FDouble> ordPriceMap = CiprnUtils.getOrdPriceMap(id_or);
			if(ordPriceMap==null){
				continue;
			}
			 
			or_total_pri = ordPriceMap.get(id_or);
			or_total_pri= CiOrdUtils.CalculationPriRounding(2, ordPriceMap.get(id_or));
			if (or_total_pri.compareTo(new FDouble("0")) <= 0) {
				ord.setOr_total_pri("");
			} else {
				ord.setOr_total_pri(CiOrdUtils.getPrettyNumber(or_total_pri.toString()));
			}
		...
		QueryOrderListBP bp  =new QueryOrderListBP();
		bp.handlerOperationStatus(szOrder);
			...
			if (IBdFcDictCodeConst.SD_CODE_ENTP_OP.equals(ci.getCode_entp())) {
				if (ICiDictCodeConst.SD_SU_MP_PERFORMED.equals(ci.getSd_su_mp())) {
					if (ci.getFg_stop() != null) {
						if (ci.getFg_stop().equals(FBoolean.TRUE) && !isOpOrder) {
