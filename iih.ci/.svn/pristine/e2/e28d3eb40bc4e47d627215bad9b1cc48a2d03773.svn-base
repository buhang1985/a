...
import iih.ci.ord.s.bp.ems.CiOrGetMmPriceBP;
...
public class CiOrCreateEms4SkinTestBP {
	...
	public CiEmsDTO exec(MedsrvAggDO srvaggdo,CiEmsDTO ems) throws BizException{
		...
		SrvMatchEmsUtils srvmatch = new SrvMatchEmsUtils();
		MedSrvDO srvdo=srvaggdo.getParentDO();
		CiEmsDTO skinems=new CiEmsDTO();
		 
		// 诊断id
		DiagOutlineInfo diagOutlineInfo = DiagInfoUtils.GetDiagOutLineInfo(ems.getId_en());
		if (diagOutlineInfo != null) {
			skinems.setId_didef(diagOutlineInfo.getId_didef());// 主诊断的主项目id
			skinems.setName_didef(diagOutlineInfo.getName_diag());// 主诊断名称
			skinems.setId_diitm(diagOutlineInfo.getId_diitm());
		}
		...
		skinems.setId_srvca(srvdo.getId_srvca());
		skinems.setInnercode_srvca(srvdo.getSrvca_innercode());
		...
		skinems.setDays_or(1);
		//skinems.setName_route();
		skinems.setId_routedes(srvdo.getId_routedes());
		skinems.setId_unit_med(srvdo.getId_unit_med());
		skinems.setName_unit_med(srvdo.getMed_name());
		skinems.setQuan_medu(srvdo.getQuan_med());
		...
		SrvMatchEmsRstDTO retdto = srvmatch.matchEms(ems.getCode_entp(),ems.getId_emp_phy(),ems.getId_dep_phy(),skinems.getId_srv(), skinems.getSd_srvtp());
		if(retdto != null)
		{
		...
		if (ems.getFg_pres_outp()!=null&&FBoolean.TRUE.equals(ems.getFg_pres_outp())) {
			skinems.setFg_pres_outp(FBoolean.TRUE);
			skinems.setId_drugdeliverymethod("@@@@Z8100000000UF5XZ");
			skinems.setSd_drugdeliverymethod("2");
			skinems.setName_drugdeliverymethod("出院带药");
		}
		...
		//emssrvdto.setId_freq(bdsrvdo.getId_freq());
		 ...
		 if(OrSrvSplitUtil.isTrue(bdsrvdo.getFg_mm())){
			 ...
			 if(mmdo!=null){
				 ...
				 MaterialUnitDTO[] materUnit = BDMmDoseUnitInfoCache.getMaterialUnitInfos(ems.getCode_entp(), new String[] {mmdo.getId_mm()});
				 ...
				 //设置取整模式
				 if(IBdFcDictCodeConst.SD_CODE_ENTP_OP.equals(ems.getCode_entp())){//门诊
					 emssrvdto.setSd_roundmd(mmdo.getSd_opmutp());
				 }else if(IBdFcDictCodeConst.SD_CODE_ENTP_IP.equals(ems.getCode_entp())){//住院
					 emssrvdto.setSd_roundmd(mmdo.getSd_mupkgutp());
				 }else if(IBdFcDictCodeConst.SD_CODE_ENTP_ET_FLOW.equals(ems.getCode_entp())){//急诊流水
					 emssrvdto.setSd_roundmd(mmdo.getSd_ermutp());
				 }else if(IBdFcDictCodeConst.SD_CODE_ENTP_ET_FSTAID.equals(ems.getCode_entp())){//急诊抢救
					 emssrvdto.setSd_roundmd(mmdo.getSd_er1mutp());
				 }else if(IBdFcDictCodeConst.SD_CODE_ENTP_ET_OBS.equals(ems.getCode_entp())){//急诊留观
					 emssrvdto.setSd_roundmd(mmdo.getSd_er2mutp());
				 }
