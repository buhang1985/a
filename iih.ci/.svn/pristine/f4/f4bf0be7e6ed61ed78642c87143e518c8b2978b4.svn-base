 ...
 * @author F
 *
 * @date 2019年8月22日上午10:52:30
 *
 * @classpath iih.ci.event.ord.bps.sign.op.sql.OpRisSignSql
 */
public class OpRisSignSql {
	
	/**
	 * BS002
	 * 门诊 签署 检查病理 1.【IEOpRisOrEnDTO】+ 【IEOpRisIllHisDTO】 2.【IEOpRisOrDTO】  3.【IEOpRisOrItmDTO】
	 * @param str_idor_path
	 * @return
	 */
	public String getOpRisSignPathInfoSql(String str_idor_path ){
		...
		stringBuilder.append(" T.code as nation_code, ");//民族编码
	...
	/**
	 * BS002
	 * 门诊 签署 普通检查 1.【IEOpRisOrEnDTO】+ 【IEOpRisIllHisDTO】 2.【IEOpRisOrDTO】  3.【IEOpRisOrItmDTO】
	 * @param str_idor_ris
	 * @return
	 */
	public String getOpRisSignRisInfoSql(String str_idor_ris) {
		StringBuilder stringBuilder=new StringBuilder();
		stringBuilder.append(" select ");
		stringBuilder.append(" do.code as ser_type_code, ");//3.检查类型编码
		stringBuilder.append(" do.name as ser_type_name, ");//3.检查类型名称
		...
		stringBuilder.append(" al.name_alcla as allergy, ");//过敏史
		stringBuilder.append(" (select code from pi_pat_card card where A.Id_Pat = card.Id_Pat and card.fg_act = 'Y' and card.sd_patcardtp = '05' and rownum<=1) health_card_no, ");//健康卡号
		stringBuilder.append(" (case when phy2.code = '04' then '是' else '否' end) as is_breastfeeding, ");//3.是否哺乳
		stringBuilder.append(" (case when phy.code = '02' then '是' else '否' end) as is_pregnancy, ");//3.是否妊娠/怀孕
		stringBuilder.append(" '' as pregnancy_date, ");//3.妊娠/怀孕时间
		stringBuilder.append(" C.Id_Ent as id_ent, ");//就诊id
		stringBuilder.append(" A.id_en as id_ierisoren, ");//就诊id
		stringBuilder.append(" B.code as patient_id, ");//患者id
		...
		stringBuilder.append(" '' as id_ierisoritms, ");
		stringBuilder.append(" Q.NO_APPLYFORM as apply_serial, ");//2.检查申请单编号
		stringBuilder.append(" (case when substr(A.sd_srvtp, 0, 2) = '02' then ? when substr(A.sd_srvtp, 0, 2) = '03' then ? else '' end) as exam_type, ");//2.医嘱类型
		stringBuilder.append(" (case when substr(A.sd_srvtp, 0, 2) = '02' then '检查' when substr(A.sd_srvtp, 0, 2) = '03' then '化验' else '' end) as exam_type_name, ");//2.医嘱类型名称
		...
		stringBuilder.append(" left outer join ci_order A on Q.Id_Or = A.Id_Or ");
		stringBuilder.append(" left outer join pi_pat B ON A.Id_Pat = B.Id_Pat ");
		stringBuilder.append(" left join (select z0.id_pat, d0.code, z0.sv ");
		stringBuilder.append("            from pi_pat_phy z0 ");
		stringBuilder.append("            left join pi_pat_phyind d0 on d0.id_patphyind = z0.id_patphyind ");
		stringBuilder.append("            where d0.code = '02') phy ON phy.Id_Pat = B.Id_Pat ");
	...
	/**
	 * BS002
	 * 门诊签署 获取检查联系人集合【IEOpRisOrContacterDTO】
	 * @param idOrs
	 * @return
	 */
	public String getOpRisSignContacterInfoSql(String idOrs) {
		StringBuilder stringBuilder = new StringBuilder();
