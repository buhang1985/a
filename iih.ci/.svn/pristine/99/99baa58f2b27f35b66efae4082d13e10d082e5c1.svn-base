	...
	public  FDateTime[] exec(CiEmsDTO ems)  throws BizException{
		...
		if(CiOrdUtils.isOpUrgentWf(ems.getCode_entp())){//结束日期=开始日期
			...
			if(ems.getDt_end()== null){
				if(FBoolean.TRUE.equals(ems.getFg_long()) && ems.getSd_srvtp() != null && !ems.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_HERBDRUG)){
					rtn[1]=fd.getDateTimeAfter(ems.getDays_or()).subSeconds(1);//zwq 2016-09-28修改 结束日期在前台已经计算赋值	//liyue 2020-01-08修改减1秒  mantis： 0182799
				}else{
					rtn[1]=fd.getDateTimeAfter(ems.getDays_or());
				}
			}else{
				if(FBoolean.TRUE.equals(ems.getFg_long()) && ems.getSd_srvtp() != null && !ems.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_HERBDRUG)){
					rtn[1]=ems.getDt_end().subSeconds(1);//zwq 2016-09-28修改 结束日期在前台已经计算赋值	//liyue 2020-01-08修改减1秒  mantis： 0182799
				}else{
					rtn[1]=ems.getDt_end();
				}
			}
