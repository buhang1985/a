	...
	protected void doYourActionAfterOrSign(CiOrderDO[] ors) throws BizException {
		...
		for (int j = 0; j < ors.length; j++) {
			...
			CiOrderDO ciorderdo=ors[j];
				...
				MrTaskRuleDO[] rkRuleDoArr=ruleService.find(" code_udidoc='"+ICiMrDictCodeConst.SD_MR_TASK_EVENT_ZHUANCHU+"' and fg_active='Y' and id_mrcactm in (select id_mrcactm from bd_mrca_ctm where code_entp = '"+ciorderdo.getCode_entp()+"')  ", "", FBoolean.FALSE);
