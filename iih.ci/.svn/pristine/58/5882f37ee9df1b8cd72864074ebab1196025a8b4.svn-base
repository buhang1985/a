...
//			srvdo.setEu_sourcemd(Integer.parseInt(ctx.getEu_orsrcmdtp()));
	...
	protected void mergeOrderSrvInfo(CiEnContextDTO ctx,OrdSrvDO ordSrvInfo, Object uiModel) throws BizException {
		...
		ordSrvInfo.setDt_last_bl(srvdto.getDt_plan());//???
...
//		ordSrvInfo.setSd_hpsrvtp(Sd_hpsrvtp);
	...
	private void ordOpInfoFrom(EmsOpitemDO ordop,OrdApOpDO opInfo){
		...
		String sd_datelimittype ="";
		if (FBoolean.TRUE.equals(ordop.getFg_er_sug())) {
			sd_datelimittype="0";
		}
		if (FBoolean.TRUE.equals(ordop.getFg_xq_sug())) {
			sd_datelimittype="1";
		}
		if (FBoolean.TRUE.equals(ordop.getFg_zq_sug())) {
			sd_datelimittype="2";
		}
		opInfo.setSd_datelimittype(sd_datelimittype);
