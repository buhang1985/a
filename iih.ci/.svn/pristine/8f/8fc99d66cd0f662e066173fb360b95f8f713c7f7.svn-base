...
import iih.ci.event.ord.bps.sign.op.logicbp.OpConSignLogicDealWithBP;
 ...
 * @author F
 *
 * @date 2019年11月19日下午4:12:41
 *
 * @classpath iih.ci.event.ord.bps.sign.op.listenerbp.OpConSignStandardBP
 */
public class OpConSignStandardBP extends OPIPSendIECommonListener{
	
	@Override
	public boolean isMatchSourceIdAndEventType(IBusinessEvent event)throws BizException{
		OrdEventLogger.info(this.getClass().getSimpleName(), "BS318-门诊签署会诊判断事件源和事件类型");
		if(!(IOrdEventSources.OP_CON_SIGN.equals(event.getSourceID())&& IEventType.TYPE_UPDATE_AFTER.equals(event.getEventType()))) {
			return false;
		}
		return true;
	}
	...
	public BaseDTO[] assembleData(BaseDTO baseDTO) throws BizException {
		...
		OrdEventLogger.info(this.getClass().getSimpleName(), "BS318-进入组装门诊会诊签署数据方法，id_ors:"+firedto.getIdors());
		OpConSignLogicDealWithBP bp=new OpConSignLogicDealWithBP();
		return bp.exec(firedto);
	}

	@Override
	public void sendMessagesToIE(BaseDTO[] dtos) throws BizException {
		OrdEventLogger.info(this.getClass().getSimpleName(), "BS318-进入 门诊会诊签署 发往集成平台方法");
		for (BaseDTO baseDTO : dtos) {
			OrdEventLogger.info(this.getClass().getSimpleName(), "BS318-门诊 会诊 签署 msgDtoString:"+baseDTO.serializeJson()+",");
			//获取执行科室
			...
			OrdEventLogger.info(this.getClass().getSimpleName(), "BS318-门诊签署会诊 往集成平台每次发送的数据:"+userobj.serializeJson());
		}
