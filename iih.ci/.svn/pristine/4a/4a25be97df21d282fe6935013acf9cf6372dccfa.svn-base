...
public class JudgeOrderTemplateApplicationTable {
	...
	public void JudgeOrederTemplateAppTabel(CiEmsDTO[] ciEms, CiEnContextDTO ciEnContext) throws BizException {
		...
		if (ciEms != null && ciEms.length > 0) {
			...
			for (CiEmsDTO ciemsDTO : ciEms) {
				...
				}else if(EmsType.DIET == ciemsDTO.getEmstype()){
					//膳食
					getOrdApDietDO(ciemsDTO, ciemsDTO.getApplyno()==null?ApplyNo:ciemsDTO.getApplyno(), ciEnContext);
				}
			}
		}
	...
	private void getOrdApObsDO(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag) throws BizException {
		...
		obs.setDt_plan(ciemsDTO.getDt_begin());//计划检查日期
		...
		//obs.setDes_sympsign(CiOrdUtils.DiseaseDescription(ciemsDTO.getId_en()));//病情描述
		obs.setClinicalzztz(CiOrdUtils.DiseaseDescription(ciemsDTO.getId_en()));//临床症状和体征
		if(!CiOrdUtils.isEmpty(obs.getStr_name_di())){
			obs.setClinicalzztz((CiOrdUtils.isEmpty(obs.getClinicalzztz())?"":(obs.getClinicalzztz()+"\r\n"))+ "诊断：" + obs.getStr_name_di().replace("&", "；"));//拼接诊断
		}
		//拷贝原医疗单的数据
		String id_orsrc_main = ciemsDTO.getId_orsrc_main();
		OrdApObsDO sourceObs = getSourceApObs(id_orsrc_main);
		if(sourceObs!=null){
			obs.setFg_mp_bed(sourceObs.getFg_mp_bed());
			ciemsDTO.setFg_mp_bed(sourceObs.getFg_mp_bed());
			obs.setFg_urgent(sourceObs.getFg_urgent());
		}
		//检查目的名称
		...
		*/
		...
		MedSrvRisDO[] risDos = risService.findByAttrValString(MedSrvRisDO.ID_SRV, ciemsDTO.getId_srv());
		if(risDos!=null&&risDos.length>0){
			obs.setId_body(risDos[0].getId_body());
			obs.setSd_body(risDos[0].getSd_body());
			obs.setName_body(risDos[0].getName_body());
		}
	 ...
	 * @throws BizException 
	 */
	private OrdApObsDO getSourceApObs(String id_or) throws BizException{
		ICiorapprisRService service = ServiceFinder.find(ICiorapprisRService.class);
		OrdApObsDO[] obs = service.findByAttrValString(OrdApObsDO.ID_OR, id_or);
...
//		return clinicalzztzBuffer == null ? "" : clinicalzztzBuffer.toString();
	...
	private void getOrdApBtDO(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag) throws BizException {
		...
		bt.setDt_bt_pl(ciemsDTO.getDt_begin() + "");//计划输血日期
		//
	...
	private void getOrdAppBtUseDO(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag) {
		...
		bu.setDt_bu_plan(ciemsDTO.getDt_begin());//计划用血申请
	...
	private void getOrdApConsDO(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag, CiEnContextDTO ciEnContext) throws BizException {
		...
		cons.setDt_plan(ciemsDTO.getDt_begin());//计划会诊时间
		
		// 会诊地点 门诊为空， 住院是科室 病区 床位
		
		if(ciEnContext.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_OP)
				|| ciEnContext.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_ET_FLOW)
				 || ciEnContext.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_ET_FSTAID)){
			
			// 会诊地址，取当前就诊科室所在地址，如果为空，使用就诊科室名称
			String place = StringUtils.isNotBlank(ciEnContext.getAddr_dep_or()) ? ciEnContext.getAddr_dep_or() :  ciEnContext.getName_dep_or(); 
			cons.setPlace(place);//会诊地点
		}else if(ciEnContext.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_IP)
				 || ciEnContext.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_ET_OBS)){					
		...
		if (medSrvCons != null && medSrvCons.length > 0) {
			invite.setId_org(ciEnContext.getId_org());
			invite.setName_org(ciEnContext.getName_org());
		}
...
//		IMedSrvLisDORService imedSrvLisRService = (IMedSrvLisDORService) ServiceFinder.find(IMedSrvLisDORService.class);
		...
		*/ lab.setName_diag(diag[4]);//诊断名称显示
		...
		lab.setDt_plan(ciemsDTO.getDt_begin());//计划检验日期
		...
		//lab.setDes_sympsign(CiOrdUtils.DiseaseDescription(ciemsDTO.getId_en()));//病情描述
		lab.setClinicalzztz(CiOrdUtils.DiseaseDescription(ciemsDTO.getId_en()));//临床症状和体征
		if(!CiOrdUtils.isEmpty(lab.getStr_name_di())){
			lab.setClinicalzztz((CiOrdUtils.isEmpty(lab.getClinicalzztz())?"":(lab.getClinicalzztz()+"\r\n"))+ "诊断：" + lab.getStr_name_di().replace("&", "；"));//拼接诊断
		}
	...
	private void getOrdApOpDO(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag) throws BizException {
		...
		op.setId_di(diag[7]);//临床诊断主项目id
		op.setId_diitm(diag[3]);//主诊断id
		...
		op.setDt_plan(ciemsDTO.getDt_begin());//计划手术时间
...
//		agg.setOrdOpMmDO(new OrdOpMmDO[] { opmm });
	...
	private void getOrdAppOuthospDO(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag, CiEnContextDTO ciEnContext) {
		...
		ordApOut.setDt_timeout(ciemsDTO.getDt_begin());//离院时间
	 ...
	 * @param ciEnContext
	 */
	private void getOrdApDietDO(CiEmsDTO ciemsDTO, String ApplyNo, CiEnContextDTO ciEnContext) {

		OrdApDietDO ordApDietDO = new OrdApDietDO();
		ordApDietDO.setStatus(DOStatus.NEW);
		
		ordApDietDO.setId_or(ciemsDTO.getId_or());//医嘱id
		ordApDietDO.setNo_applyform(ApplyNo);//申请单号
		ordApDietDO.setCreatedby(ciemsDTO.getId_emp_phy());
		ordApDietDO.setCreatedtime(ciemsDTO.getDt_begin());
		ciemsDTO.setEmstype(EmsType.DIET);
		FMap map = new FMap();
		map.put(EmsType.DIET + "", ordApDietDO);
		ciemsDTO.setOrapplysheet(map);
	}
