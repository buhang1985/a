...
import iih.ci.event.ord.bps.revert.op.logicbp.OpBtRevertLogicDealWithBP;
 ...
 * @author F
 *
 * @date 2019年10月10日上午11:21:51
 *
 * @classpath iih.ci.event.ord.bps.revert.op.listenerbp.OpBtRevertStandardBP
 */
public class OpBtRevertStandardBP extends OPIPSendIECommonListener{

	@Override
	public boolean isMatchSourceIdAndEventType(IBusinessEvent event) throws BizException {
		OrdEventLogger.info(this.getClass().getSimpleName(), "BS008-门诊撤回备血判断事件源和事件类型");
		if(!(IOrdEventSources.OP_BT_REVERT.equals(event.getSourceID())&& IEventType.TYPE_UPDATE_AFTER.equals(event.getEventType()))) {
			return false;
		}
		return true;
	}
	...
	public BaseDTO[] assembleData(BaseDTO baseDTO) throws BizException {
		...
		OrdEventLogger.info(this.getClass().getSimpleName(), "BS008-进入组装门诊备血撤回数据方法，id_ors:"+firedto.getIdors());
		OpBtRevertLogicDealWithBP bp=new OpBtRevertLogicDealWithBP();
		return bp.exec(firedto);
	}

	@Override
	public void sendMessagesToIE(BaseDTO[] dtos) throws BizException {
		OrdEventLogger.info(this.getClass().getSimpleName(), "BS008-进入门诊备血撤回发往集成平台方法");
		for (BaseDTO baseDTO : dtos) {
			OrdEventLogger.info(this.getClass().getSimpleName(), "BS008-门诊 备血 撤回标注版 msgDtoString:"+baseDTO.serializeJson()+",");
			//获取执行科室
			...
			OrdEventLogger.info(this.getClass().getSimpleName(), "BS008-门诊撤回备血 往集成平台每次发送的数据:"+userobj.serializeJson());
		}
	}
