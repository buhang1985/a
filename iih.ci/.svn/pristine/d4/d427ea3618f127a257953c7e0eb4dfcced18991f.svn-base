...
public class DiagWFInfoMQListener implements IMQListener {
	...
	private String getCondFldStr(String[] idendiinfos) {
		String str=CiDiagDODesc.PRIMARYKEY_FIELDNAME;
		StringBuffer sql =new StringBuffer();
		SqlParam sqlParam=new SqlParam();
		sql.append(" select id_di from ci_di where id_en=?");
		sqlParam.addParam(idendiinfos[0]);
		try {
			List<String> ids = (List<String>)new DAFacade().execQuery(sql.toString(),sqlParam,new ColumnListHandler());
			str +=" in ("+CiOrdUtils.ListConveretCharacterString(ids)+") and "+CiDiagItemDO.ID_DIDEF;
		} catch (DAException e) {
			throw new BizRuntimeException("获取临床诊断失败！", e);
		}
		return str;
	}
