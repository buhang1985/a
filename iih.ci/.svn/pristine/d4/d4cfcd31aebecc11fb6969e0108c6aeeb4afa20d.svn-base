...
import iih.ci.ord.s.ems.op.ems.drugs.EmsDrugsLoadBP;
...
public class EmsHerbsLoadBP extends EmsDrugsLoadBP {
	...
	public EmsRstDTO[] load(CiEnContextDTO ctx, EmsLoadDTO[] args) throws BizException {
		...
		// 医疗单类型
		rst.getDriverInfo().setEmsBackendDriver(EmsType.HERB.toString());
		return rsts;
	}
	...
	protected EmsDrugItemDO modelFrom(CiEnContextDTO ctx, CiEmsDTO dto) throws BizException {
		...
		emsDrugItemInfo.setPurpose_or(dto.getPurpose_or());
		FArrayList drugList = emsDrugItemInfo.getEmsOrDrugListEx();
		
		emsDrugItemInfo.setEu_orhp(dto.getEu_orhp());
		emsDrugItemInfo.setEu_hpjudge(dto.getEu_hpjudge());
		...
		boolean isEnabledIndicVerify = ServiceFinder.find(ICiMiOrdRService.class).isEnabledIndicVerify(ctx);
		if(isEnabledIndicVerify && drugList.size() == 1  && HpJudgeRstEnum.SELFPAY.equals(drug.getEu_orsrvhp())){
			
			// 需要判断单方导致的自费
			
			emsDrugItemInfo.setEu_hpjudge(HpJudgeEnum.WAITING_JUDGE);
			emsDrugItemInfo.setEu_orhp(HpJudgeRstEnum.WAITING_JUDGE);

			
			drug.setEu_hpjudge(HpJudgeEnum.WAITING_JUDGE);
		...
		/* && HpIndicJudgeEnum.NONEEDJUDGE.equals(dto.getEu_hpindicjudge()) */) {
			...
			FBoolean hasPsdLimit = ServiceFinder.find(IBlHpOutQryService.class).hasPsdLimit(ctx.getId_hp(),
					drug.getId_srv());
