...
import iih.ci.ord.pub.listener.AbstractUdidocInsertAfterListener;
...
public class UdidocDiagTypeInsertAfterListener extends AbstractUdidocInsertAfterListener{
	...
	protected void doYourActionAfter(Object[] newObjs) throws BizException {
		...
		//根据事件类型编码组装数据  insert
		MSUdidocDiagTypeDTO[] msUdidocDiagTypeDTOs=udidocDiagTypeBP.assemableDatabyState(diagTypeDos,EventTypeEnum.INSERT_AFTER.getValueString());
		...
		for(MSUdidocDiagTypeDTO dto:msUdidocDiagTypeDTOs) {
			if(dto != null) {
				OrdBizLogger.info(null, "MS016-新增后-MSUdidocDiagTypeDTO:"+"["+i+"]"+dto.serializeJson());
			}
			//构建对象，并发送给集成组
			...
			CiOrdUtils.fireEvent(SourcesIdEnum.DIAG_TYPE_SOURCEID.getValueString(),//iih.ci.ord.ms.d.MSUdidocDiagTypeDTO
					IEventType.TYPE_INSERT_AFTER, new BaseDTO[] { domainBusinessUserObj });//1002
			if(domainBusinessUserObj != null) {
				OrdBizLogger.info(null, "MS016-新增后-发送数据DomainBusinessUserObj:"+"["+i+"]"+domainBusinessUserObj.serializeJson());
			}
			i++;
		}
	}
