...
public class GetOrdPresInfoQry implements ITransQry {
	...
	private String getQrySQlBaseStr(){
		return " select distinct A.Id_Pres, case when instr(ORD.sd_srvtp,'"+IBdSrvDictCodeConst.SD_SRVTP_HERBDRUG+"') = 1 then ORD.orders else null end orders, A.Id_Grp,A.ID_ORG,A.ID_PATI,B.Name_Pat as name_pati,C.Name as name_patica,A.id_entp,A.code_entp,A.id_en, "
				  ...
				  +" from ci_pres A "
				  +" inner join (select distinct id_pres from ci_or_srv ciorsrv,ci_order ciorder where ciorder.id_or = ciorsrv.id_or and (ciorder.eu_feereversetp is null or ciorder.eu_feereversetp != 1 )  %s) Q"
				  +" on Q.id_pres = A.id_pres %s "
				  ...
				  +" left join ci_or_srv SRV on SRV.id_pres=A.id_pres "
				  +" left join ci_order ORD on ORD.id_or=SRV.id_or ";
	}
	/**
	 * 就诊批量查询SQL
	 * 根据就诊
	...
	private String getQrySQl8IDEn(){
		String formatSql1 = " and ciorder.id_en = ? ";
		String formatSql2 = " and A.Id_En=? ";
		return String.format(getQrySQlBaseStr(), formatSql1,formatSql2) ;		
	}
	...
	private String getQrySql8IdPres(){
		...
		String formatStr = "";
		if(!CiOrdUtils.isEmpty(_id_en)){
			formatStr = " and ciorder.id_en = ?";
		}
		
		String id_pressql=CiOrdUtils.getSqlInStrsWithOutIn(_ids_orpres);
		
		if(!isIDs()){
			formatStr += " and ciorsrv.id_pres = ?";
		}else{
			formatStr += " and ciorsrv.id_pres in "+id_pressql;
		}
		
		return String.format(getQrySQlBaseStr(), formatStr) +getIdPresSqlStr();	
	}
