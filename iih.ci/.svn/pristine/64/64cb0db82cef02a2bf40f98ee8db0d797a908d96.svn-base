...
import iih.ci.ord.s.ems.op.orderv1.bp.ope.OrderSignOperateBP;
...
public class OrderSignBP implements IOrderSignBP {
	...
	private IOrderOperateBP signBP = new OrderSignOperateBP();
	...
	public OrderRstDTO sign(CiEnContextDTO ctx, OrderOperateDTO arg) throws BizException {
		...
		if (paramList != null && paramList.size() > 0 && extension != null) {
			...
			for (String key : extension.keySet()) {
				...
				// 获取开立状态的医嘱
				if ("N".equals((String) map.get("fg_sign"))) {
					lstIdors.add(key);
				}
			}

			return signBP.exec(lstIdors.toArray(new String[] {}), ctx, extension);
		}
