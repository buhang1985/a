...
import iih.ci.ord.s.ems.meta.OrderSavedRstInfo;
...
import iih.ci.ord.s.ems.op.base.EmsSetSaveBP;
import iih.ci.ord.s.ems.op.ems.deforder.DefaultLisCreateOrderInfo;
...
public class EmsLisSaveBP extends EmsSetSaveBP {
		...
		for (OrderPackageInfo opInfo : szOrderPackageInfo) {
			...
			if (!CiOrdUtils.isEmpty(opInfo.getOrderAppSheetList())) {
				...
				if (objAppSheetInfo instanceof OrdApLabDO) { // 检验申请单保存
					OrdApLabDO[] szOrdApLabDO = CiOrdAppUtils.getOrapplisService().save(opInfo.getOrderAppSheetList()
							.toArray(new OrdApLabDO[opInfo.getOrderAppSheetList().size()]));
	...
	protected OrderPackageInfo[] mergeOrderPakageInfo(CiEnContextDTO ctx, OrderPackageInfo[] szOrderPakageInfo)
		...
		for (OrderPackageInfo opInfo : orderPakageInfos) {
			String sd_samptp = "";
			if (opInfo.isNew()) {
				MedSrvLisDO[] srvlisdos = ServiceFinder.find(IMedSrvLisDORService.class).findByAttrValString("Id_srv",
						opInfo.getBdSrvList().get(0).getId_srv());
				if (!CiOrdUtils.isEmpty(srvlisdos)) {
					sd_samptp = srvlisdos[0].getSd_samptp();
				}

			} else {
				OrdApLabDO[] labdo = ServiceFinder.find(ICiorapplisRService.class).findByAttrValString("Id_or",
						opInfo.getOrderInfo().getId_or());
			...
			if (!CiOrdUtils.isEmpty(sd_samptp) && !CiOrdUtils.isEmpty(labdo)
					&& !sd_samptp.equals(labdo.getSd_samptp())) {
				OrderSrvMmList delsrvmmList = new OrderSrvMmList();
				for (OrderSrvMmInfo srvmmInfo : opInfo.getOrderSrvMmList()) {

					if (srvmmInfo.getOrderSrvInfo().getEu_sourcemd() == OrSrvSourceFromEnum.SPECIMENRELFEE || srvmmInfo
							.getOrderSrvInfo().getEu_sourcemd() == OrSrvSourceFromEnum.SPECIMENVESSELRELFEE) {
				...
				opInfo.getOrderSrvMmList().append(((DefaultLisCreateOrderInfo) this.getDefaultCreateOrderInfo())
						.specimenTpRelSrvHandle(opInfo, labdo.getSd_samptp(), labdo.getSd_contp(), ctx).asArray());
			}
		}
	...
	protected void mergeOrderInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, Object uiModel) throws BizException {
		...
		// 医嘱内容
		orderInfo.setContent_or(CiOrContentUtils
				.create(orderInfo.getSd_srvtp(), ems.getName_srv(), orderInfo.getRoute_name(), orderInfo.getFg_urgent())
				.toString());
		// 开立时间
		orderInfo.setDt_entry(ems.getDt_plan());
		//
		FDateTime[] dtSE = OrderUtils.getDtBeginEnd(ctx.getCode_entp(), ems.getDt_plan(), ems.getDt_plan(),
				ems.getUse_days());
		// 生效时间
		...
		// 医嘱描述
		orderInfo.setDes_or(ems.getDes_or());
		// 处理医嘱停止时间等相关信息
		...
		orderInfo.setNote_or(ems.getDes_or());
		orderInfo.setApplyno(ems.getApplyno());
		...
		Integer totalTimes = totalTimesBP.getTotalTimes(orderInfo.getDt_effe(),orderInfo.getId_freq(), ems.getUse_days());
		orderInfo.setTimes_cur(totalTimes);
		///////// 添加检验、检查、诊疗多剂量多次执行，杨敬本20171111
		orderInfo.setFg_quickwflow(ems.getFg_quickwflow());
	}
	...
	protected void mergeOrderSrvInfo(CiEnContextDTO ctx, OrdSrvDO ordSrvInfo, Object uiModel) throws BizException {
		...
		if (srvdto == null)
			return;
		if (ordSrvInfo.getStatus() == DOStatus.DELETED)
			return;
		...
		ordSrvInfo.setDt_last_bl(srvdto.getDt_begin_ui());// ???
		if (CiOrdUtils.isEmpty(srvdto.getPrice())) {
			// 映射折扣价，如果折扣价为空，则后台再查询折扣价
			CiOrSrvPriCalUtils.mappingPriceRateFromEmsToSrv(ctx.getCode_entp(), ordSrvInfo.getId_srv(),
					srvdto.getPrice().toString(), ordSrvInfo);
		} else {
	...
	protected void mergeOrderAppInfo(CiEnContextDTO ctx, Object[] appSheetList, Object[] extInfoList, Object uiModel)
		...
		for (Object obj : appSheetList) {
			...
			ordlis.setDes_sympsign(srvdto.getDes_or());// 病情描述
			...
			ordlis.setId_unit_quan(srvdto.getId_unit_sampcoltime());// 需求数量单位
																	// a0b14
			ordlis.setName_diag(srvdto.getName_diag());// 诊断名称
			ordlis.setId_sampcoltime(srvdto.getId_sampcoltime());// 标本采集时间 REF
																	// 标本
			...
			ordlis.setLen_sampcoltime(srvdto.getLen_sampcoltime());// 标本采集时长
																	// SINGLE
	...
	protected void afterMergeOrderSrvInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, OrdSrvDO srvInfo, Object uiModel)
		...
		srvInfo.setDt_last_bl(srvdto.getDt_plan());
		///////// 添加检验、检查、诊疗多剂量多次执行，杨敬本20171111
		// 频次
