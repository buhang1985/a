...
import iih.ci.ord.pub.listener.ConsultOrSignListener;
 ...
 * 下达会诊医嘱侦听器插件 产生病历任务
 */
public class MrConsultOrSignListener extends ConsultOrSignListener{
	...
	protected void doYourActionAfterOrSign(CiOrderDO[] ors) throws BizException {
		...
		for (int j = 0; j < ors.length; j++) {
			...
			MrTaskRuleDO[] rkRuleDoArr=ruleService.find(" code_udidoc='"+ICiMrDictCodeConst.SD_MR_TASK_EVENT_HUIZHEN+"' and fg_active='Y' and id_mrcactm in (select id_mrcactm from bd_mrca_ctm where code_entp = '"+ciorderdo.getCode_entp()+"')  ", "", FBoolean.FALSE);
