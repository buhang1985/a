...
public abstract class AbstractExcessiveDrugValidator extends AbstractDrugValidator<CiEmsDTO> {

	// 是否停止执行下一个校验器，如果有一小校验通过就用在执行下一个校验器
	private boolean isBreak;
	// 规则配置中的使用天数
	private int useDays;

	@Override
	public void setDefaultVal(CiEmsDTO ciEms) {

		super.setDefaultVal(ciEms);
...
//		}
	...
	protected boolean validate(CiEmsDTO ciEms) {
		...
		this.setDefaultVal(ciEms);
		String idExcessiveReasons = ciEms.getId_excessive_reason();
		...
		if (StringUtils.isBlank(idExcessiveReasons)) {
			if (ICiDictCodeConst.ID_EXCESSIVE_REASON_NORMAL.equals(this.getExcessiveReasonIdentity())) {
				return isEffectiveUseDays(ciEms);
			}
			return true;
		}
	 ...
	 * @return
	 */
	private boolean isEffectiveUseDays(CiEmsDTO ciEms) {

		// 如果规则不适用，直接返回true,进行下一个规则校验
		if (!isConformToTheRules(ciEms)) {
			return true;
		}

		boolean isEffective = this.useDays >= ciEms.getDays_or();
...
//			this.isBreak = true;
	 ...
	 * @return
	 */
	protected abstract boolean isConformToTheRules(CiEmsDTO ciEms);
