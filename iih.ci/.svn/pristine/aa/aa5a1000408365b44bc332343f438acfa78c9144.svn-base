...
public class RuleCfgChainFacory<T> {
	 ...
	 * @return 规则链中第一个规则
	 */
	public AbstractRuleExecutor<T> getRuleChain(RuleCfgParamDTO ruleCfgParam) {

		AbstractExecutorChain<T> executorChain = this.getExecutorChain(ruleCfgParam);
	 ...
	 * @return
	 */
	public AbstractRuleValidator<T> getRuleValidatorChain(RuleCfgParamDTO ruleCfgParam) {
	 ...
	 * @return
	 */
	private AbstractExecutorChain<T> getExecutorChain(RuleCfgParamDTO ruleCfgParam) {
		...
		try {
			// 获取规则配置结果，提取规则实体id
			List<CiRuleCfg> ruleCfgList = ciRuleCfgRService.getRuleCfgs(ruleCfgParam);
			if (ruleCfgList == null || ruleCfgList.size() == 0) {
				Logger.error("获取规则失败，传入参数check_point = '" + ruleCfgParam.getCheck_point() + "' , code_entp = '" + ruleCfgParam.getCode_entp() + "' ， 使用默认的规则实现类！");
				return null;
			}
