 ...
 * @author F
 *
 * @date 2019年8月27日上午11:41:11
 *
 * @classpath iih.ci.event.ord.bps.sign.ip.query.IpRisSignInfoQuery
 */
public class IpRisSignInfoQuery implements ITransQry{

	private String _id_ors;
    private String str_idor_ris;//普通检查
	private String str_idor_path;//病理检查
	
	public IpRisSignInfoQuery(String id_ors){
		this._id_ors = id_ors;
	}
	@Override
	 ...
	 */
	 private String getSql() throws BizException{//1.【IEOpRisOrEnDTO】+【IEOpRisIllHisDTO】 2.【IEOpRisOrDTO】 3.【IEOpRisOrItmDTO】
		 ...
		 if(str_idor_path.length()>0){
			 formatsql +=  new IpRisSignSql().getIpRisSignPathInfoSql( str_idor_path.substring(0, str_idor_path.lastIndexOf(",") ));//病理
		 }
		 if(str_idor_ris.length()>0){
			 if(formatsql.length()>0){
				 formatsql+=" Union All ";
			 }
			 formatsql +=  new IpRisSignSql().getIpRisSignRisInfoSql( str_idor_ris.substring(0, str_idor_ris.lastIndexOf(",")));//其他普通
		 }
		 return formatsql;
	 }
}
