...
public class MrInpatientStatisticQryBP {
	/**
	 * 评价分析表超时查询
	...
	public QaPatListDTO[] getInpatientStatistic(QryRootNodeDTO qryRootNodeDTO) throws BizException {
		IQrySQLGeneratorService service=ServiceFinder.find(IQrySQLGeneratorService.class);
 		...
 		sql.append("       WHERE "+sqlwhere+" and "+EnvContextUtil.processEnvContext("",new AMrDO(),false) +" and ee.fg_ip = 'Y' and ci_amr.code_entp = "+ICiMrDictCodeConst.SD_ENTP_ZHUYUAN);
 		//sql.append("	   AND ROUND(TO_NUMBER(sysdate - to_date(ee.dt_acpt,'yyyy-mm-dd,hh24:mi:ss')))>30");
 		sql.append("       AND  (SELECT COUNT(*) FROM CI_MR CM INNER JOIN BD_MRTP BM ON BM.ID_MRTP = CM.ID_MRTP WHERE CM.ID_ENT =ci_amr.id_ent AND BM.CD_STD ='"+IMrPubConst.DS_PJFX+"' AND CM.SD_SU_MR NOT IN ('"+ICiMrDictCodeConst.SD_SU_MR_NEW+"','"+ICiMrDictCodeConst.SD_SU_MR_REJECT+"')) >0 ");
