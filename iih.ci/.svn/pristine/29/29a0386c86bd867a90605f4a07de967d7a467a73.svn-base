...
public class OrDoctorCheckValidateBP {
	...
	public FArrayList exec(CiorderAggDO[] aggors){
		...
		for(CiorderAggDO agg : aggors){
			...
			if ((ord.getEu_hpindicjudge() == HpIndicJudgeEnum.WAITINGJUDGE || ord.getEu_uncporjudge() == HpIndicJudgeEnum.WAITINGJUDGE)
					&& !ord.getFg_pres_outp().booleanValue()) { // 0180088: 从临床路径开立的出院带药医嘱不需要进行临床路径审核
