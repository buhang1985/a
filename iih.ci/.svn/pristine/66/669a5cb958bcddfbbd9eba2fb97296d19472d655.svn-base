...
public class BDPsnInfoCache {
	...
	public static String GetNameOfBdPsnInfo(String id_psn) throws BizException {
		...
		if (null == psnInfo) {
			...
			if (null != szPsnDocDO && szPsnDocDO.length > 0){
				psnInfo = szPsnDocDO[0];
				ContextCache.Put(BDPsnInfoCache.class.getSimpleName(),DefaultContext_ID, DefaultContext_ID, BDPsnInfoCache_Key, id_psn, psnInfo);
			}
			
		}
		return psnInfo == null ? null : psnInfo.getName();
	}
