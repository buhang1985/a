 ...
 * @author F
 *
 * @date 2019年12月24日下午6:40:29
 *
 * @classpath iih.ci.event.ord.bps.diag.op.listenerbp.OpFlowDiagStandardBP
 */
public class OpFlowDiagStandardBP extends OpDiagEnterListener{
	...
	public boolean isOpData(CiDiagDO ciDiagDO)throws BizException{
		OrdEventLogger.info(IpDiagEnterListener.class.getSimpleName(), "BS301急诊流水诊断筛选流水数据");
		if(!OrdEventUtil.isEmpty(ciDiagDO)&&!OrdEventUtil.isEmpty(ciDiagDO.getCode_entp())&&OrdEventUtil.LS.equals(ciDiagDO.getCode_entp())) {
			return true;
		}
		return false;
	}
	...
	public IEOpPvDiagEnDTO[] assemableDate(CidiagAggDO aggDO, CiEnContextDTO ctx) throws BizException {
		OrdEventLogger.info(this.getClass().getSimpleName(), "BS301-进入组装流水诊断数据方法, CidiagAggDO aggDO"
	...
	public void sendMessagesToIE(IEOpPvDiagEnDTO[] OpPvDiagDTO) throws BizException {
		OrdEventLogger.info(this.getClass().getSimpleName(), "BS301-进入 流水诊断 发往集成平台方法");
		for (IEOpPvDiagEnDTO d : OpPvDiagDTO) {
			OrdEventLogger.info(this.getClass().getSimpleName(),"BS301-流水 诊断 IEOpPvDiagEnDTO:" + d.serializeJson() + ",");
			...
			OrdEventLogger.info(this.getClass().getSimpleName(), "BS301流水诊断每次发往集成平台数据:"+buo.serializeJson());
		}
	}
