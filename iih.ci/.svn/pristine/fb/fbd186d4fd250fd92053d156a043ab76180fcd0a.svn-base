...
/**
 * 住院 签署 检验 sql查询 BS006
 * @author F
 *
 * @date 2019年8月26日下午12:00:00
 *
 * @classpath iih.ci.event.ord.bps.sign.ip.sql.IpLisSignSql
 */
public class IpLisSignSql {

	/**
	 * BS006 
	 * 住院 检验 签署 所有信息查询sql //就诊1【IEOpLisOrEnDTO】 申请单2【IEOpLisOrDTO】 医嘱项目3【IEOpLisOrItmDTO】
	 * @param ids
	 * @return
	 */
	public String getIpLisSignAllInfoSql(String ids){
	    ...
	    stringBuilder.append(" (case when phy.code = '02' then phy.sv else '' end) as pregnancy_date, ");//2妊娠时间
	    stringBuilder.append(" C.Id_Ent as id_ent, ");//1就诊id
	    stringBuilder.append(" A.id_en as id_ielisoren, ");//1就诊id
	    stringBuilder.append(" B.code as patient_id, ");//1患者ID
	    stringBuilder.append(" B.CODE_AMR_IP as p_bar_code, ");//1就诊号 B.CODE_AMR_IP
	    ...
	    stringBuilder.append(" A.dt_chk as confirm_date, ");//1确认时间
	    ...
	    stringBuilder.append(" (case when F.Times_Ip is null then 0 else F.Times_Ip end) + (case when J.Times_Op is null then 0 else J.Times_Op end) as times, ");//1就诊次数
	    ...
	    stringBuilder.append(" Z4.price_ratio as test_price, ");//3测试项目价格
