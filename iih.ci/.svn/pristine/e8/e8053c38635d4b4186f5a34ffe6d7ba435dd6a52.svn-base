	 ...
	 */
	 private String getSql() throws BizException {
		 ...
		 if(str_idor_path.length()>0){
			 formatsql+=String.format(new CiIEMsgRelSqlConst().CI_IE_APPPATH_ALL_SQL(), "A.id_or"+CiOrdUtils.getSqlInOrEqualStrs(str_idor_path.substring(0, str_idor_path.length()-1)));
		 }
		 if(str_idor_ris.length()>0){
			 if(formatsql.length()>0)
				 formatsql+=" Union All ";
			 formatsql+=String.format(new CiIEMsgRelSqlConst().CI_IE_APPRIS_ALL_SQL(), "A.id_or"+CiOrdUtils.getSqlInOrEqualStrs(str_idor_ris.substring(0, str_idor_ris.length()-1)));
		 }
