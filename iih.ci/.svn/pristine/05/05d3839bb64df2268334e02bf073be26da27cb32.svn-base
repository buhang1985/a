...
/**
 * 用法关联剂型修改后，进行IC集成平台事件触发监听器 （仅集成平台事件发送用）
 */
public class OrderUseAssociatdeClear extends AbstractEvent4CIListener{

	@Override
	protected void sendIEMsg(Object[] newObjs, String eventType) throws BizException {

		XLogger.info("用法关联剂型修改 ，触发缓存清理-->触发点"+this.getClass().getName());
		//查找缓存清除类
	...
	protected String getEventSourceid() throws BizException {
		...
		return IBdSrvEventConst.USEASSOCIATED_ADDDELMOD_EVENT_SOURCEID;
	}
