package iih.ci.ord.s.bp.assi.tl.impl.cimes.base;
...
import iih.ci.ord.s.bp.assi.tl.impl.cimes.base.BaseCalCiEmsProperty;
...
public class OpBaseCopyCalCiEmsProperty extends BaseCalCiEmsProperty {
	...
	private List<String> getUsageScope() throws BizException {
		...
		// 常规在院执行药品给药途径范围设置
		String usageScope2 = ParamsetQryUtil.getParaString(orgId,
				ICiOrdNSysParamConst.SYS_PARAM_CiPharmMpInUsageScope);
		...
		if (StringUtils.isNotBlank(usageScope2)) {
			usageScopeList.addAll(Arrays.asList(usageScope2.split(",")));
		}
