...
public class PKUOutpatientPrescriptionTypeRule implements ICiOrPresSplitRule {
	...
	private void AnalyzeOrderPresSplitDTO(List<OrderPresSplitDTO> orderList) {
		...
		List<OrderPresSplitDTO> listObs = new ArrayList<OrderPresSplitDTO>();
		for (OrderPresSplitDTO dto : orderList){
			...
			if (IBdFcDictCodeConst.SD_CODE_ENTP_OP.equals(dto.getCode_entp())) {// 门诊
				...
				listES.add(dto);
			}else if(IBdFcDictCodeConst.SD_CODE_ENTP_ET_OBS.equals(dto.getCode_entp())){// 留观
				listObs.add(dto);
			}
		} 
		...
		if(listObs != null && listObs.size()>0 ){
			hpMap.put(IBdFcDictCodeConst.SD_CODE_ENTP_ET_OBS, listObs);
		}
	 }
