...
public class SkinTestOrSignBP {
	 ...
	 * @throws BizException
	 */
	public String exec(CiorderAggDO[] aggors) throws BizException{
		String rst = "";
		    	   ...
		    	   if(this.isSkinTestOr(ordSrvDO).booleanValue()){
		    		   ...
		    		   if(isHasSkinRet(ordSrvDO).booleanValue()){
		    				   ...
		    				   if(!CiOrdUtils.isEmpty(sd_rst)){
		    					   if(sd_rst.startsWith("1")){//阳性
		    						   ...
		    						   if(AllergicHandleModeEnum==0||AllergicHandleModeEnum==1){
		    							   rst = "医嘱"+aggor.getParentDO().getName_or() +"的皮试结果为阳性，禁止开立！";
		    							   return rst;
		    						   }
		    					   }
		    				   }
		    			   }else{
		    				   rst = "医嘱"+aggor.getParentDO().getName_or() +"的皮试医嘱不存在！";
		    				   return rst;
		    			   }
		    		   }else{
		    			   //关联的皮试医嘱
		    			   ...
		    			   if(CiOrdUtils.isEmpty(skintest)){
		    				   rst = "未检测到药品【"+ordSrvDO.getName_srv()+"】的过敏信息，请删除后重新开立！";
		    				   return rst;
		    			   }else{
		    				   if(!isAdmitDrugOrBeforeSkinTestRst(aggor.getParentDO().getId_org()).booleanValue()){
		    					   rst = "医嘱"+aggor.getParentDO().getName_or() +"的皮试结果未出！";
		    					   return rst;
		        			   }
		    			   }   
		    		   }
