...
import iih.ci.event.ord.bps.sign.ip.listenerbp.IpLisSignStandardBP;
 ...
 *
 */
public class IpReissueLisBP extends ReissueCommonBP{
	
	public IpReissueLisBP() {
		this.listener = new IpLisSignStandardBP();
	}
	@Override
	protected IEMsgReissueRstDTO assembleData(BaseDTO[] dtos) throws BizException {
		IEOpLisOrEnDTO[] ieOpLisOrEnDtos = (IEOpLisOrEnDTO[])dtos;
		try {
			...
			for(IEOpLisOrEnDTO ieOpLisOrEnDTO : ieOpLisOrEnDtos){
				...
				String xmlItms = new String(mappingService.message2XmlBytes("BS006", "02",fl ),"utf-8");
