 ...
 *
 */
public class EmsTreatCreateBP extends OpBaseEmsCreateBP {
	...
	public EmsRstDTO[] create(CiEnContextDTO ctx, EmsCrtDTO[] emsarray) throws BizException {
		...
		// 非缓存信息
		{
			//0179331: 【验证版】门急诊医生站，长期频次的非药品医嘱，开立和复制下，开始时间取值逻辑不一致
		...
		if (diagOutlineInfo != null) {
			...
			emsModel.setName_didef(diagOutlineInfo.getName_diag());
		}
		// 列表项目
		this.constructTableDatasource(ctx, emsModel, aggDO);
		...
		rst.getDriverInfo().setEmsBackendDriver(EmsType.COMMON.toString());
		rsts.add(rst);
	...
	private void constructTableDatasource(CiEnContextDTO ctx, EmsDrugItemDO drugItem, MedsrvAggDO medSrvAggInfo)
		...
		if (FBoolean.TRUE.equals(med.getIsmuldose())) {
			ems.setQuan_med(med.getQuan_med());
		} else {
			ems.setQuan_med(FDouble.ONE_DBL);
		}
		/*
		 * if(FBoolean.TRUE.equals(med.getIsmulexec())){ ems.set }
		 */
		...
		ems.setFg_ctm(med.getFg_ctm());
		CalQuantumBP quantumBP = new CalQuantumBP();
		ems.setQuan_cur(quantumBP.getUnMMQuantum(drugItem.getDt_begin_ui(),ems.getId_freq(), ems.getUse_days(), ems.getQuan_med()));

		// 获取执行科室
		OrWfDeptInfoDTO wf = DeptInfoUtils.GetOrWfDeptInfo(ctx, medSrvAggInfo.getParentDO().getId_srv(), "","","",medSrvAggInfo.getParentDO().getFg_long(),FBoolean.FALSE,drugItem.getDt_begin_ui());
		if (!CiOrdUtils.isEmpty(wf)) {
			...
			if ("1".equals(wf.getMp_source())) {
				drugItem.setStr_mp_dep_ids(null);
			} else {
				drugItem.setStr_mp_dep_ids(CiOrdUtils.isEmpty(wf.getId_mp_depts()) ? null : wf.getId_mp_depts());
			}
		...
		//诊断
		ems.setId_didef(drugItem.getId_didef());
		ems.setName_diag(drugItem.getName_didef());
		// 计算单价和总额
		ems.setPrice(calculatePrice(medSrvAggInfo.getParentDO(), medSrvAggInfo.getMedSrvSetItemDO(), ctx.getId_pripat())
				.getPrice_ratio());
		ems.setTotalprice(ems.getPrice().multiply(ems.getQuan_cur()));
		
		//参照唯一标识
        ems.setIdSrvIdMm(ems.getId_srv());
	...
	// return price;
	// }
	public MedSrvPriceDO calculatePrice(MedSrvDO medSrv, MedSrvSetItemDO[] szMedSrvSetItemDO, String id_pripat)
			throws BizException {
		...
		for (MedsrvAggDO agg : szMedsrvAggDO) {
			...
			if (CiOrdUtils.isTrue(medsrv.getFg_mm())) {
				...
				if (mms != null && mms.length > 0) {
					...
					MaterialUnitDTO[] materUnit = BDMmDoseUnitInfoCache.getMaterialUnitInfos(code_entp, new String[] {mms[0].getId_mm()});
