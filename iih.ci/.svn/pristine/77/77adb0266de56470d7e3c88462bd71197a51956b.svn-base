...
public class CiOPAgainPresBP {
      ...
      * @throws BizException
      */
	 public   CiOrderDO[] exec(CiEnContextDTO ctx, String id_dep_sign,String sd_entp)throws BizException{
		  // 判断 本次就诊，有可以撤回的医嘱吗
...
//		return CiOrderBackSign(id_en, id_dep_sign, sd_entp, rtns);
		return CiOrderBackSign(ctx, id_dep_sign);
	 ...
	 private String getIdOr(String iden)throws BizException{
		...
		sqlParam.addParam(ICiDictCodeTypeConst.ID_SU_BL_1);
		
		sqlb.append(" select a.id_or from ci_order a where a.id_en = ?");
		sqlb.append(" and a.id_su_bl = ? ");
		sqlb.append(" and rownum = 1 ");
		List<String> list = (List<String>) dafacade.execQuery(sqlb.toString(), sqlParam, new ColumnListHandler());
		String id_or = "";
	  ...
	  * @throws BizException
	  */
	private CiOrderDO[] CiOrderBackSign(CiEnContextDTO ctx, String id_dep_sign) throws BizException {

		CiOPAgainPresBPQry qry = new CiOPAgainPresBPQry(id_dep_sign,ctx.getCode_entp(),ctx.getId_en());
		...
		if (rtn != null && rtn.length > 0) {
			...
			//门诊重新分方入口-集成平台 直接替换之前的 因为之前的事件源下没有其他的类 只有分方的  所以直接替换了之前的事件源 并没有新加一个事件发送逻辑
			CiOrdUtils.fireBDEvent(IOrdEventSources.OP_REPRES_ENTER, IEventType.TYPE_UPDATE_AFTER,  listOrder.toArray(new CiOrderDO[0]));
	   ...
	   /* private List<CiOrPresSplitList> CiOrPresSplitDTO2CiOrPresSplitList( OrderPresSplitDTO[] orderpresdtos){
		...
		private List<CiOrPresSplitList> CiOrPresSplitDTO2CiOrPresSplitList(OrderPresSplitDTO[] orderpresdtos) {
			...
			for (OrderPresSplitDTO dto : orderpresdtos) {
				if (dto.getFg_or() != null && FBoolean.TRUE.equals(dto.getFg_or()))
					Dtolist.add(dto);
			}
			CiOrPresSplitList orderPres = new CiOrPresSplitList();
			orderPres.setOrderList(Dtolist);
