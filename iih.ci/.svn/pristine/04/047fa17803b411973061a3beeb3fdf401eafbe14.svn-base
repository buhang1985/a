...
public class CiOrSrvPriHelper {
	 ...
	 * @return
	 */
	public static FDouble getPriceSum(PriStdSrvDTO[] pridtos,Map<String,FDouble> setquanmap){
		if(ObjectUtils.isEmpty(pridtos))return null;
		FDouble rtn = FDouble.ZERO_DBL;
		for(int i=0;i<pridtos.length;i++){
			if(ObjectUtils.isEmpty(pridtos[i].getPrice()))continue;
			if(setquanmap!=null && setquanmap.size()>0 && setquanmap.containsKey(pridtos[i].getId_srv())){
				rtn=rtn.add(getPriStdSrvAmount(pridtos[i]).multiply(setquanmap.get(pridtos[i].getId_srv())));
			}else{
	...
	public static FDouble getPriStdSrvAmount(PriStdSrvDTO pristdsrvdto){
		if(ObjectUtils.isEmpty(pristdsrvdto))return new FDouble(0);
		FDouble quan=null,price=null;
		if(ObjectUtils.isEmpty(pristdsrvdto.getQuan()))
		{quan=new FDouble(1);}else{quan=pristdsrvdto.getQuan();}
		if(ObjectUtils.isEmpty(pristdsrvdto.getPrice()))
	...
	public static FDouble[] getPriceSum(SrvPricalRateAndPriceDTO[] pridtos,Map<String,FDouble> setquanmap){
		if(ObjectUtils.isEmpty(pridtos))return null;
		FDouble[] szRtn = new FDouble[]{FDouble.ZERO_DBL,FDouble.ZERO_DBL};
		for(int i=0;i<pridtos.length;i++){
			if(ObjectUtils.isEmpty(pridtos[i].getPrice()))continue;
			FDouble[] tp = getPriStdSrvAmount(pridtos[i]);
			if(setquanmap!=null && setquanmap.size()>0 && setquanmap.containsKey(pridtos[i].getId_srv())){
				szRtn[0] = szRtn[0].add(tp[0].multiply(setquanmap.get(pridtos[i].getId_srv())));
				szRtn[1] = szRtn[1].add(tp[1].multiply(setquanmap.get(pridtos[i].getId_srv())));
			}else{
				szRtn[0] = szRtn[0].add(tp[0]);
				szRtn[1] = szRtn[1].add(tp[1]);
			}
	...
	public static FDouble[] getPriStdSrvAmount(SrvPricalRateAndPriceDTO pristdsrvdto){
		if(ObjectUtils.isEmpty(pristdsrvdto))return new FDouble[]{FDouble.ZERO_DBL,FDouble.ZERO_DBL};
		FDouble quan=null;
		...
		if(ObjectUtils.isEmpty(pristdsrvdto.getPrice_ratio()))
