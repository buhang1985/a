...
public class MrmGenerateListBp {

	private String Name_src = "ci_mr_fp_pat";

	/**
	 * 通过前台传过来的条件，进行分页查询 分页需要页数和每页条数
	 */
	@SuppressWarnings("unchecked")
	public PagingRtnData<CiMrmDirHeaReportDTO> getReportList(QryRootNodeDTO qryRootNodeDTO, PaginationInfo paginationInfo) throws BizException {
		...
		MrmGenerateCSVBp cimrm = new MrmGenerateCSVBp();
		...
		List<PatiVisitDO> lstMrFpPat = (List<PatiVisitDO>) da.execQuery(sql, new BeanListHandler(PatiVisitDO.class));
	...
	private int getDataCount(String sqlWhere, String Name_src) throws BizException {
		...
		sql.append(sqlWhere);
		DAFacade da = new DAFacade();
		ArrayList<Map<String, Object>> lstBlStIpDO = 
				(ArrayList<Map<String, Object>>)da.execQuery(sql.toString(), new MapListHandler());
