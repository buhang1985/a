...
public class getClassMedSrvDOSBP {
	...
	public FMap2 exec(String code_entp, String condition, String orderColumn) throws BizException {
		...
		for (MedSrvDO medSrv : medSrvs) {
			...
			if (FBoolean.TRUE.equals(medSrv.getFg_mm()) && !CiOrdUtils.isEmpty(medSrv.getId_srv())) {
		...
		if (idSrvList.size() > 0) {
			...
			for (int i = 0; i < idSrvList.size(); i++) {
				...
				if ((i > 0 && i % 900 == 0) || i == idSrvList.size() - 1) {
					...
					String tempInSql = String.format(inSql, idMmBuffer.substring(1));
					sqlBuffer.append(tempInSql);
					idMmBuffer.setLength(0);
				}
			}
			
			String tempSql = String.format(sql, sqlBuffer.substring(3));			

			List<Map<String, String>> queryResult = (List<Map<String, String>>) new DAFacade()
					.execQuery(tempSql, new MapListHandler());
		...
		FMap2 validateRstMap = bp.exec(srvStatusInfoList.toArray(new OrdSrvChangedInfoDTO[srvStatusInfoList.size()]),
				code_entp);
		
		FMap2 srvStatusMap = new FMap2();
		for(String key : validateRstMap.keySet()){
			String[] keyArr = key.split(",");
			if(!srvStatusMap.containsKey(keyArr[0])){
				srvStatusMap.put(keyArr[0], validateRstMap.get(key));	
			}
		}
		
		CiOrdUtils.LogerOutInfo(" 类  getClassMedSrvDOSBP  方法  验证方法耗时  exec 耗时"+(System.currentTimeMillis()- startTime));
		medSrvMap.put("srvStatusMap",  srvStatusMap);
		Set<String> keys = medSrvMap.keySet();
		for(String key : keys)
		{
			
			Object o = medSrvMap.get(key);
			String s = "";
		}
		return medSrvMap;
	}
	 ...
	 * @throws BizException
	 */
	protected MedSrvDO[] sortPinyinCode(MedSrvDO[] medsrvs,FMap2 medSrvMap) throws BizException {
		
		Map<String, String> map = new HashMap<String, String>();
		List<MedSrvDO> list = new ArrayList<MedSrvDO>();
		FArrayList2 medSrvList = new FArrayList2();
