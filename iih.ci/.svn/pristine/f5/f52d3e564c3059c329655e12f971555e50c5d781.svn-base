 ...
 *
 */
public class TplTreatSaveBP extends BaseIpTplSaveBP {
	@Override
...
//			String code_or = CiOrdAppUtils.getIBillcodeManager().getPreBillCode_RequiresNew(CiOrderDODesc.CLASS_FULLNAME);
			...
			super.setEmsViewInfo(ctx, ortmpldo, ortplnitmdo, medSrvInfo, tpl.getDriverInfo(), treat, code_or, 1);
			// 计算物资流向科室
			this.setOrWfDeptInfo(ctx, medSrvInfo, treat);
			FArrayList docmuent = new FArrayList();
			docmuent.append(treat);
