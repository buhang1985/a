...
public class UdidocDiagTypeUpdateAfterListener extends AbstractUdidocUpdateAfterListener{
	...
	protected void doYourActionAfter(Object[] newObjs) throws BizException {
		...
		//根据事件类型编码组装数据    //update
		MSUdidocDiagTypeDTO[] msUdidocDiagTypeDTOs=udidocDiagTypeBP.assemableDatabyState(diagTypeDos,EventTypeEnum.UPDATE_AFTER.getValueString());
		...
		for(MSUdidocDiagTypeDTO dto:msUdidocDiagTypeDTOs) {
			if(dto != null) {
				OrdBizLogger.info(null, "MS016-修改后-MSUdidocDiagTypeDTO:"+"["+i+"]"+dto.serializeJson());
			}
			//构建对象，并发送给集成组
			...
			if(domainBusinessUserObj != null) {
				OrdBizLogger.info(null, "MS016-修改后-发送数据DomainBusinessUserObj:"+"["+i+"]"+domainBusinessUserObj.serializeJson());
			}
			i++;
		}
