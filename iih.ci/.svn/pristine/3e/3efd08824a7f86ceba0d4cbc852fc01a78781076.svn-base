...
public class CiOrdMedicalInsuranceServiceImpl implements ICiOrdMedicalInsuranceService {
	...
	public FMap getSpecialDiseaseJudgeRstMap(CiEnContextDTO ctx, String[] id_mms) throws BizException {
		...
		// 非医保或患者非特病直接返回
		if (!CiEnContextUtil.IsHpPat(ctx) || !FBoolean.TRUE.equals(ctx.getFg_hpspcl()))
			return judgeRstmap;

		// 根据物品获取特殊病判断结果集合
		List<CijudgeSpecillDTO> judgeSpecillList = blservice.JudgeSpecillFlag(ctx.getNo_hp(), ctx.getId_hp(),
				id_mms);
		...
		for (CijudgeSpecillDTO specill : judgeSpecillList) {
			// 如果是特殊病药
			if (FBoolean.TRUE.equals(specill.getFg_mmspecill())) {
				// 获取当前就诊诊断
				if (entDis == null) {
					entDis = ciordqryservice.getEntDiDOList(ctx.getId_en());
				}
