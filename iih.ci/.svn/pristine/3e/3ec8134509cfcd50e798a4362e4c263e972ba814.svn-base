...
public class CiOrCreateSrvMm8EmsBP {
	...
	public OrdSrvItemDatum exec(CiorderAggDO aggdo, CiEmsDTO ems, int index,
			Map<String, MedSrvSetItemDO> bdsrvinsetClinicalMap) throws BizException {
		CiOrderDO ordo = aggdo.getParentDO();
	...
	private void createOrSrvMmInfo(CiOrderDO ordo, CiEmsDTO ems, int index, OrdSrvItemDatum rtn) throws BizException {
		...
		if (!CiOrdUtils.isEmpty(srvdto.getBdHpIndication())) {
			...
			if (model == 2) {
				...
				if (StringUtils.isNotEmpty(bdHpIndication.getDes_hislimit())) {
					ciorsrvhpdo.setDes_hplimit(ciorsrvhpdo.getDes_hplimit() + "\n 院内限制条件："
							+ CiOrdUtils.IsNull(bdHpIndication.getDes_hislimit()));
				}
			} else if (model == 1) {
		...
		srvdo.setId_freq(srvdto.getId_freq()); // 2016-12-01 - mxj -
												// 修改ordo.getId_freq()--》srvdto.getId_freq()
												// 应该为业务传进的项目freq不应该是医嘱的freq，如果是的话应该走业务规则算法
		...
		srvdo.setEu_hpjudge(srvdto.getEu_hpjudge()); // 医保判断方式
		srvdo.setEu_orsrvhp(srvdto.getEu_orsrvhp()); // 医保判断结果
		...
		if(!CiOrdUtils.isEmpty(srvdto.getSd_nodispense())&&ICiDictCodeConst.SD_NODISPENSE_SELF.equals(srvdto.getSd_nodispense())){
			srvdo.setFg_self(FBoolean.TRUE);// 2020-3-25 李月龙 添加
			srvdto.setFg_self(FBoolean.TRUE);
		}else{
			srvdo.setFg_self(FBoolean.FALSE);
			srvdto.setFg_self(FBoolean.FALSE);
		}
		if (CiOrdUtils.isEmpty(srvdto.getSd_nodispense())) {
			srvdo.setFg_self(FBoolean.FALSE);
			srvdto.setFg_self(FBoolean.FALSE);
		}
		srvdo.setId_nodispense(srvdto.getId_nodispense());// 不计费类型ID
		srvdo.setSd_nodispense(srvdto.getSd_nodispense());// 不计费类型编码
		...
		if(!CiOrdUtils.isEmpty(srvdto.getSd_nodispense()) || FBoolean.TRUE.equals(srvdo.getFg_self())){
			srvdo.setFg_bl(FBoolean.FALSE);
			srvdto.setFg_bl(FBoolean.FALSE);
		}else{
		...
		srvdo.setFg_skintest(CiOrdUtils.nullHandle(srvdto.getFg_skintest()));// 2016-03-25
																				// 新增
		srvdo.setId_skintest_skip_reason(srvdto.getId_skintest_skip_reason());// 2016-03-25
																				// 新增
		srvdo.setSd_skintest_skip_reason(srvdto.getSd_skintest_skip_reason());// 2016-03-25
																				// 新增
		...
		// 住院毒麻药品也要算总量，郑州的需求-不合入稳定,自备药不需要计算总量,
		// 医技医生站也要计算总量
		if (!FBoolean.TRUE.equals(srvdo.getFg_self())
				&& (CiOrdUtils.isOpMode(ems.getCode_entp(), ems.getFg_pres_outp(), srvdo.getSd_srvtp()).booleanValue()
						|| !(srvdto.getSd_pois() == null || srvdto.getSd_pois().equals("0"))||ICiDoctorStationTypeConst.Type_Ip_MtDoctor_Station.equals(context.getStationType()))) {
			CiOrdUtils.setQuanTotalMeduToSrvDO(ems, srvdto, srvdo);
		}
		// if
		// (!ems.getCode_entp().equals(IEnDictCodeConst.SD_ENTP_INPATIENT)||(!CiOrdUtils.isEmpty(ems.getFg_pres_outp()))&&ems.getFg_pres_outp().booleanValue()){
		//
		// if(srvdto.getQuan_total_medu() !=
		// null&&srvdto.getQuan_total_medu().doubleValue()!=0){
		...
		srvdo.setFg_nothingwithorders(srvdto.getFg_nothingwithorders());// 与付数无关
		/*
		 * 存放高危药品值： 2018年8月20日 by GH
		 */
		srvdo.setFg_highrisk(srvdto.getFg_highrisk());
		rtn.setSrvdo(srvdo);
		...
		if (OrSrvSplitUtil.isTrue(srvdto.getFg_mm())) {
			if (!CiOrdUtils.isEmpty(srvdto.getId_mm())) {
				...
				if (!CiOrdUtils.isOpMode(ems.getCode_entp(), ems.getFg_pres_outp(), srvdo.getSd_srvtp()).booleanValue()
						&& (srvdto.getSd_pois() == null || srvdto.getSd_pois().equals("0"))) {
		...
		if ("0".equals(CiOrdUtils.GetDefaultSkinModel(ordo.getId_org()))) {
			FBoolean b = srvdto.getFg_skintest(); // 新建患者需要皮试 旧患者getFg_skintest
													// 为N
			if (FBoolean.TRUE.equals(srvdto.getFg_skintest()) && srvdto.getSd_reltp() != null
					&& srvdto.getSd_reltp().equals(ICiDictCodeConst.CODE_SKIN_SKIN)) {
	...
	private void createOrSrvMmInfo(CiOrderDO ordo, CiEmsDTO ems, int index, OrdSrvItemDatum rtn,
			Map<String, MedSrvSetItemDO> bdsrvinsetClinicalMap) throws BizException {
		OrdSrvDO srvdo = new OrdSrvDO();
		...
		ExeWhDeptDTO exeandwhdeptinfo = null; // 2016-07-28 新执行科室逻辑调整
												// 调整到新的时将如下语句进行对应处理
		if (srvdto.getId_dep() == null) {
			exeandwhdeptinfo = this.getMpDeptID(ordo, ems, bdsrvinsetClinicalMap.get(srvdo.getId_srv()), srvdo,
					srvdto.getId_mm());
			srvdo.setId_org_mp(exeandwhdeptinfo.getId_org_mp()); // 待打开
			srvdo.setId_dep_mp(exeandwhdeptinfo.getId_dep_mp()); // 待打开
			srvdo.setId_dep_wh(exeandwhdeptinfo.getId_dep_wh()); // 待打开
		...
		srvdo.setFg_nothingwithorders(srvdto.getFg_nothingwithorders());// 与付数无关
		srvdo.setId_nodispense(srvdto.getId_nodispense());// 不计费类型ID
		srvdo.setSd_nodispense(srvdto.getSd_nodispense());// 不计费类型SD
		srvdo.setName_nodispense(srvdto.getName_nodispense());// 不计费类型
		if (!CiOrdUtils.isEmpty(srvdo.getSd_nodispense())) {
			if (ICiDictCodeConst.SD_NODISPENSE_SELF.equals(srvdo.getSd_nodispense())) {
				srvdo.setFg_self(FBoolean.TRUE);// 自备标识
			} else if (ICiDictCodeConst.SD_NODISPENSE_ENTRUST.equals(srvdo.getSd_nodispense())
					|| ICiDictCodeConst.SD_NODISPENSE_FOREIGN.equals(srvdo.getSd_nodispense())) {
				srvdo.setFg_self(FBoolean.FALSE);
			}
		} else {
			srvdo.setFg_self(CiOrdUtils.nullHandle(srvdto.getFg_self()));
		...
		if(!CiOrdUtils.isEmpty(srvdo.getSd_nodispense()) ||  FBoolean.TRUE.equals(srvdo.getFg_self())){
			srvdo.setFg_bl(FBoolean.FALSE);// 费用标识
			srvdto.setFg_bl(FBoolean.FALSE);
		}else{
			srvdo.setFg_bl(CiOrdUtils.nullHandle(srvdto.getFg_bl()));
		}
		...
		if (OrSrvSplitUtil.isTrue(srvdto.getFg_mm())) {
			if (!CiOrdUtils.isEmpty(srvdto.getId_mm())) {
				OrdSrvMmDO ordsrvmmdo = createCiOrdSrvMmDO(srvdo, srvdto);
				// 住院就诊时候，不需要计算 Quan_cur 的值 -- 2016.12.28 by wangqzh
				if (!CiOrdUtils.isOpMode(ems.getCode_entp(), ems.getFg_pres_outp(), srvdo.getSd_srvtp())
						.booleanValue()) {
	 ...
	 * @throws BizException
	 */
	private void orSrvInfoHpHandle(CiorderAggDO aggdo, OrdSrvDO srvdo, CiEmsDTO ems, int index, OrdSrvItemDatum rtn)
			throws BizException {
		CiOrderDO ordo = aggdo.getParentDO();
		// CiEmsSrvDTO srvdto=(CiEmsSrvDTO)ems.getEmssrvs().get(index);
		if (!CiOrdUtils.isEmpty(ordo.getEu_hpindicjudge()) && ordo.getEu_hpindicjudge() == HpIndicJudgeEnum.SELFPAY) {
			...
			if (!CiOrdUtils.isEmpty(srvdo.getFg_hpindicjudged())
					&& srvdo.getFg_hpindicjudged() == HpIndicJudgeEnum.SELFPAY) {
	 ...
	 * @throws BizException
	 */
	private void setOrSrvMmModInfo(CiorderAggDO aggdo, OrdSrvDO srvdo, CiEmsDTO ems, int index, OrdSrvItemDatum rtn)
			throws BizException {
		if (CiOrdUtils.isEmpty(srvdo))
			return;
		CiOrderDO ordo = aggdo.getParentDO();
		CiEmsSrvDTO srvdto = (CiEmsSrvDTO) ems.getEmssrvs().get(index);
		if ("0".equals(CiOrdUtils.GetDefaultSkinModel(ordo.getId_org()))) {
		...
		CiOrSrvHpDO[] orSrvHpDos = iCiorsrvhpRService.find(" id_orsrv='" + srvdto.getId_orsrv() + "'", null,
				FBoolean.FALSE);
		HashMap<String, CiOrSrvHpDO> orSrvHpMap = new HashMap<String, CiOrSrvHpDO>();
		CiOrSrvHpDO ciorsrvhpdo = new CiOrSrvHpDO();
		if (!CiOrdUtils.isEmpty(orSrvHpDos))
			ciorsrvhpdo = orSrvHpDos[0];
		if (!CiOrdUtils.isEmpty(srvdto.getBdHpIndication())) {
			BdHpIndicationDTO bdHpIndication = (BdHpIndicationDTO) srvdto.getBdHpIndication().get(0);
			if (!CiOrdUtils.isEmpty(orSrvHpDos)) {
			}
			...
			int model = CiOrParamUtils.getMedInsuranceIndicInfoModelSet(CiOrdAppUtils.getEnvContext().getOrgId());
			if (model == 2) {
				ciorsrvhpdo.setDes_hplimit(" 医保限制条件：" + StringUtils.trimToEmpty(bdHpIndication.getDes_hplimit()));
				if (StringUtils.isNotEmpty(bdHpIndication.getDes_hislimit())) {
					ciorsrvhpdo.setDes_hplimit(ciorsrvhpdo.getDes_hplimit() + "\n 院内限制条件："
							+ StringUtils.trimToEmpty(bdHpIndication.getDes_hislimit()));
				}
			} else if (model == 1) {
				ciorsrvhpdo.setDes_hplimit(" 院内限制条件：" + StringUtils.trimToEmpty(bdHpIndication.getDes_hislimit()));
			} else {
				ciorsrvhpdo.setDes_hplimit(bdHpIndication.getDes_hplimit());
			}
			ciorsrvhpdo.setIndicitemid(srvdto.getIndicitemid());
		...
		if(!CiOrdUtils.isEmpty(srvdto.getSd_nodispense())){
			...
			srvdo.setFg_self(FBoolean.FALSE);
			srvdto.setFg_self(FBoolean.FALSE);
			srvdo.setSd_nodispense(srvdto.getSd_nodispense());
			srvdo.setId_nodispense(srvdto.getId_nodispense());
		}
		srvdo.setFg_pres_outp(CiOrdUtils.nullHandle(srvdto.getFg_outp()));
		...
		if (!CiOrdUtils.isEmpty(ordo.getEu_hpindicjudge()) && ordo.getEu_hpindicjudge() == 9) {
			...
			if (!CiOrdUtils.isEmpty(srvdto.getFg_hpindicjudged())
					&& srvdto.getFg_hpindicjudged() == HpIndicJudgeEnum.SELFPAY) {
				srvdo.setFg_hpindicjudged(null);// FIXME 编辑状态暂时置为null 去医保计算处重新计算
			} else {
		...
		srvdo.setFg_base(srvdto.getFg_base());
		// 个数加收时 这里会将要删除的费用项目 变为编辑状态 需要判断服务套计价模式特殊处理 by yzh 2017年12月27日 15:13:36
		if (ordo.getFg_set().booleanValue()) {
			CiEmsSrvDTO ciemssrv = getMainCiEmsSrv(ordo, ems.getEmssrvs());
			String id_primd = ciemssrv.getId_primd();
			if (!com.mysql.jdbc.StringUtils.isNullOrEmpty(id_primd)) {
				if (CiOrdUtils.isSrvsetMemCntAdditionalPrimd(id_primd)) {
	...
	private CiEmsSrvDTO getMainCiEmsSrv(CiOrderDO ordo, FArrayList emssrvs) {
		for (Object srv : emssrvs) {
			CiEmsSrvDTO ciemssrv = (CiEmsSrvDTO) srv;
			if (ciemssrv.getId_srv().equals(ordo.getId_srv()) && ciemssrv.getFg_set().booleanValue()) {
				return ciemssrv;
			}
		}
		return null;
	 ...
	 * @throws BizException
	 */
	private ExeWhDeptDTO getMpDeptID(CiOrderDO ordo, CiEmsDTO ems, MedSrvDO srvinsetdo, MedSrvSetItemDO tmpdo,
	...
	private ExeWhDeptDTO getMpDeptID(CiOrderDO ordo, CiEmsDTO ems, MedSrvSetItemDO tmpdo, OrdSrvDO srvdo,
			String srvrelmmdo) throws BizException {
	 ...
	 * @return
	 */
	private ExeDeptCalParamDTO getExeDeptCalParamDTO(CiOrderDO ordo, CiEmsDTO ems, MedSrvDO srvinsetdo,
			MedSrvSetItemDO tmpdo, MedSrvRelMmDTO srvrelmmdo) {
		...
		if (!CiOrdUtils.isEmpty(tmpdo)) {
			...
			paramdto.setDef3(tmpdo.getSd_mpdepcalmd() + CiOrdUtils.COMMA_STR + tmpdo.getId_dep_mp()
					+ CiOrdUtils.COMMA_STR + ordo.getId_dep_mp());
	...
	private ExeDeptCalParamDTO getExeDeptCalParamDTO(CiOrderDO ordo, CiEmsDTO ems, MedSrvSetItemDO tmpdo,
			OrdSrvDO srvdo, String id_mm) {
		ExeDeptCalParamDTO paramdto = new ExeDeptCalParamDTO();
		paramdto.setCode_entp(ems.getCode_entp());
		paramdto.setId_dep_en(ems.getId_dept_en());
		paramdto.setId_dep_or(ems.getId_dep_phy());
		...
		// if(!CiOrdUtils.isEmpty(id_mm)){ 0122115: 住院医生站开立皮试药品问题。
		// 修改判断，物品需要取得流向科室
	 ...
	 * @throws BizException
	 */
	private void srvMmRelSkinOrModHandle(OrdSrvDO srvdo, CiEmsSrvDTO srvdto, OrdSrvItemDatum rtn, CiEmsDTO ems)
			throws BizException {
	 ...
	 * @throws BizException
	 */
	private void orSrvMmInfoModHandle(CiorderAggDO aggdo, CiEmsDTO ems, int index, OrdSrvItemDatum rtn)
			throws BizException {
		CiOrderDO ordo = aggdo.getParentDO();
		CiEmsSrvDTO srvdto = (CiEmsSrvDTO) ems.getEmssrvs().get(index);
		if (CiOrdUtils.isDONew(srvdto)) {
			...
			if (CiOrdUtils.isDODel(srvdto)) {
				...
				if (CiOrdUtils.isEmpty(srvdo)) {
					throw new BizException("医嘱数据已被他人修改，请刷新界面重试！");
				}
	 ...
	 * @throws BizException
	 */
	private FDateTime getLastDt(String id_freq, FDateTime dt_effe, Integer firstdaytimes, FBoolean fg_long)
			throws BizException {
	...
	private boolean isOrSrvMmSame(String id_orsrv, String id_mm) throws BizException {
		...
		if (CiOrdUtils.isEmpty(orsrvmmdo)) {
			return false;
		}
		if (id_mm.equals(orsrvmmdo.getId_mm()))
			return true;
		return false;
	}
