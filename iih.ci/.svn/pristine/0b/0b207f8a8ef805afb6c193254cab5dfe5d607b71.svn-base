...
public class CiOrCreateSrvMm8CompPriSrv0BP {
	...
	public  ArrayList<OrdSrvDO> exec(CiorderAggDO aggdo,CiEmsDTO ems,Integer ipos)  throws BizException{
		...
		for(int i=0;i<pristdsrvdtos.length;i++){
			...
			if(ordsrvdo.getStatus()==DOStatus.NEW)//0123525: 住院医生在开立状态后，修改执行科室，通用医技执行无法检索到
	 ...
	 * @throws BizException
	 */
	public  ArrayList<OrdSrvDO> exec(CiorderAggDO aggdo,CiEmsDTO ems,Integer ipos,Hashtable ht)  throws BizException{
		 long startTime = System.currentTimeMillis();
		//有效性校验
		if(!validate(aggdo,ems,ipos))return null;
		...
		//遍历
		for(int i=0;i<pristdsrvdtos.length;i++){
			ordsrvdo=getSrvMm8CompPriSrv(aggdo,ems,ipos,pristdsrvdtos[i],medsrvdos[i],quan,ht);
			if(CiOrdUtils.isEmpty(ordsrvdo))continue;
			if(ordsrvdo.getStatus()==DOStatus.NEW)//0123525: 住院医生在开立状态后，修改执行科室，通用医技执行无法检索到
			rtnList.add(ordsrvdo);
		}
	 ...
	 * @throws BizException
	 */
	private OrdSrvDO getSrvMm8CompPriSrv(CiorderAggDO aggdo,CiEmsDTO ems,Integer ipos,PriStdSrvDTO prisrv,MedSrvDO prisrvdo,FDouble quan,Hashtable ht) throws BizException{
		CiOrCreateSrvMm8CompPriSrvNBP bp=new CiOrCreateSrvMm8CompPriSrvNBP();
		return bp.exec(aggdo, ems, ipos, prisrv,prisrvdo, quan, ht);
	}
