...
import iih.ci.event.ord.bps.sign.op.query.OpDrugSignEntInfoQuery;
import iih.ci.event.ord.bps.sign.op.query.OpDrugSignPressInfoQuery;
import iih.ci.event.ord.bps.sign.op.query.OpDrugSignPressPriceInfoQuery;
 ...
 * @author F
 *
 * @date 2019年8月27日下午3:48:36
 *
 * @classpath iih.ci.event.ord.bps.sign.op.logicbp.OpDrugSignLogicDealWithBP
 */
public class OpDrugSignLogicDealWithBP extends CommonParamBP{
	...
	public IEOpPharmOrEnDTO[] exec(OrdEventFireDTO firedto) throws BizException {
		...
		if ((OrdEventUtil.isEmpty(firedto.getIdwcors()) && OrdEventUtil.isEmpty(firedto.getIdherbors()))||
				OrdEventUtil.isEmpty(firedto.getIdent()))
			return null;
		
		//获得医嘱签署信息
		IEOpPharmOrEnDTO rtn = getIEMsg4OrSignInfo(firedto.getIdwcors(), firedto.getIdherbors());

		//获得处方信息
		String id_preses = pharmPresInfoHandle(firedto.getIdwcors(), firedto.getIdherbors(), rtn, firedto.getIdent());//"1001Z7100000000GWQMU"
	...
	private void ieOpPresItms4HerbHandle(String id_preses, IEOpPharmOrEnDTO rtn) throws BizException {
		...
		if (id_preses == null)
			return;
		//获得草药处方明细信息
		OpDrugSignHerbLogicDealWithBP bp = new OpDrugSignHerbLogicDealWithBP();
	...
	private void ieOpPresItms4DrugWcHandle(String id_preses, IEOpPharmOrEnDTO rtn) throws BizException {
		...
		if (id_preses == null)
			return;
		//获得西成药处方明细信息
		OpDrugSignWcLogicDealWithBP bp = new OpDrugSignWcLogicDealWithBP();
		Hashtable<String, FArrayList2> list = bp.exec(id_preses);
	...
	private IEOpPharmOrEnDTO getIEMsg4OrSignInfo(String id_wc_ors, String id_herb_ors) throws BizException {
		...
		//  就诊信息
		OpDrugSignEntInfoQuery qry = new OpDrugSignEntInfoQuery(id_or);
	...
	private String pharmPresInfoHandle(String id_wc_ors, String id_herb_ors, IEOpPharmOrEnDTO rtn, String id_en)throws BizException {
		String id_or = this.getOr(id_wc_ors, id_herb_ors);
		// 处方信息
		OpDrugSignPressInfoQuery qry = new OpDrugSignPressInfoQuery(id_or);
		IEOpPharmPresDTO[] predtos = (IEOpPharmPresDTO[]) AppFwUtil.getDORstWithDao(qry, IEOpPharmPresDTO.class);
		//查询处方金额 根据处方id进行拼接
		OpDrugSignPressPriceInfoQuery priceQry = new OpDrugSignPressPriceInfoQuery(id_en);
