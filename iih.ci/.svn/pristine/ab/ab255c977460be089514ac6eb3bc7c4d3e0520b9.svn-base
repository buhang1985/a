...
public class getOrTmplCaDOsQry implements ITransQry {
	...
	public SqlParam getQryParameter(StringBuffer arg0) {
		...
		sqlParam.addParam(id_grp);
		sqlParam.addParam(id_org);
		sqlParam.addParam(id_dep);
		sqlParam.addParam(id_emp);
		return sqlParam;
	}
	...
	public String getQrySQLStr() {
		...
		sb.append(" where (sd_srvorrt='0' and id_grp= ? and id_org= ? )");
		sb.append(" or (sd_srvorrt='1' and id_dep= ? ) or (sd_srvorrt='2' and id_emp= ? )");
