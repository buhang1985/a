		...
		if(agg != null  ){
		 CiordInviteConsDO[] inviteCons=agg.getCiordInviteConsDO();
		  for(CiordInviteConsDO item:inviteCons){
			  if(item.getId_dep() != null || StringUtils.isNotEmpty(item.getName_dep())){
				for(OrdSrvDO srvDO:orsrvdos){
					//String[] ids = new DBUtil().getOIDs(1);
					OrdSrvDO srv = null;
					if (item.getId_dep() != null) {
						srv=  this.createSrvDO(srvDO, item.getId_dep());
					} else {
						srv=  this.createSrvDO(srvDO, Context.get().getDeptId());
					}
					list.add(srv);
				 } 
