package iih.ci.ord.s.external.provide.bp.mpnr;
...
public class CiOrPharmOrRefoundBedMarginUpdateBP {
	 ...
	 * @throws BizException
	 */
	public String exec(String[] id_ors,String type) throws BizException {
		...
		// 4、累加使用量，余量校验
		validationQuan(quanRel, id_ors,type);

		// 5、扣除床旁余量
		quanReduce(quanRel,type);
	 ...
	 * @throws BizException
	 */
	private void validationQuan(QuanRel[] quanRel, String[] id_ors, String type) throws BizException {
		...
		if (quanRel != null && quanRel.length > 0) {
			...
			for (QuanRel quan : quanRel) {
				...
				if (quan.BedQuan_sum < quan.Quan_sum) {
					...
					throw new BizException("扣减床旁余量，医嘱项目【" + quan.getOrdSevDO().getName() + "】床旁余量不足,请领药！");
				}
			}
		}
	}
	 ...
	 * @throws BizException
	 */
	private void quanReduce(QuanRel[] quanRel, String type) throws BizException {
		...
		for (QuanRel quan : quanRel) {
			...
			if (quan.getOrdSrvMmDO() != null) {
				...
				Double bedQuan = (quan.getOrdSrvMmDO().getQuan_bed_medu()==null?0:quan.getOrdSrvMmDO().getQuan_bed_medu().toDouble());
				Double remain ;
				if("+".equals(type)){
					remain = bedQuan + quan.Quan_sum;
				}else{
					remain = bedQuan - quan.Quan_sum;
				}
