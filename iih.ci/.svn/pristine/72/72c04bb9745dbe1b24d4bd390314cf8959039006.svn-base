...
import iih.ci.ord.dto.ems.uiorderdto.d.FeeListViewDTO;
...
public class FeeListLoadBP extends ContextCache implements IFeeListLoadBP {
	...
	public FeeListRstDTO load(CiEnContextDTO ctx, FeeListLoadDTO ems) throws BizException {
		...
		List<FeeListViewDTO> ordFeeList = handleOrdFeeList(ordResult);
	...
	private String getOrdFeeLIstSql(String id_en, String code_entp, String OrderSequenceModel) {
		...
		sql.append(
				"  orsrv.fg_indic,orsrv.fg_selfpay,dep.name as name_dep,srv.des as des_srv,measdoc.name as name_unit_med,measdoc_mm.name as mm_name_unit_med,orsrvmm.code_mm,");
		sql.append(
	 ...
	 * @return
	 */
	private List<FeeListViewDTO> handleOrdFeeList(List<Map<String, Object>> ordFeeList) {
		List<FeeListViewDTO> ciOrdFeeList = new ArrayList<FeeListViewDTO>();
		for (Map<String, Object> ordFee : ordFeeList) {
			FeeListViewDTO ciOrdFeeSrvDTO = new FeeListViewDTO();
...
//			ciOrdFeeSrvDTO.setName_hp(CiOrdUtils.nullHandle(ordFee.get("name_hp")));
			ciOrdFeeSrvDTO.setQuan_total_sale(CiOrdUtils.isEmpty(ordFee.get("med_unit")) ? FDouble.ONE_DBL
			...
			ciOrdFeeSrvDTO.setName_unit_sale(CiOrdUtils.nullHandle(ordFee.get("name_unit_med")));
			...
			if (!CiOrdUtils.isEmpty(ordFee.get("sd_srvtp"))
				...
				ciOrdFeeSrvDTO.setQuan_total_sale(CiOrdUtils.isEmpty(ordFee.get("quan_drug")) ? FDouble.ONE_DBL
						: new FDouble(ordFee.get("quan_drug").toString()));
				ciOrdFeeSrvDTO.setName_unit_sale(CiOrdUtils.nullHandle(ordFee.get("mm_name_unit_med")));
				// ciOrdFeeSrvDTO.setCode_or(CiOrdUtils.nullHandle(ordFee.get("code_mm")));
			}
			ciOrdFeeSrvDTO.setCode_fee(CiOrdUtils.nullHandle(ordFee.get("fee_code_or")));
			...
			ciOrdFeeSrvDTO.setName_dep_mp(CiOrdUtils.nullHandle(ordFee.get("name_dep")));
