...
//		bpinv.exec();
		...
		if(!ciEnContextDTO.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_IP)){
		//药品毒麻信息审核
...
//		bpinv.exec();
	...
	private String judgeSpecillFlag(CiorderAggDO[] aggs,CiEnContextDTO ctx) throws BizException{
		...
		//return "西药，中药";
		//非医保或患者非特病返回false
		if(CiOrdUtils.isEmpty(ctx.getId_hp())||ctx.getFg_hpspcl()==null||!ctx.getFg_hpspcl().booleanValue()) return null;
		FArrayList idOrsrvs = new FArrayList();
		...
		List<CijudgeSpecillDTO> specils = blservice.JudgeSpecillFlag(ctx.getNo_hp(), ctx.getId_hp(),idMMs.toArray(new String[idMMs.size()]));
