 ...
 *
 */
public class EmsTreatLoadBP extends OpBaseEmsLoadBP {
	...
	public EmsRstDTO[] load(CiEnContextDTO ctx, EmsLoadDTO[] args) throws BizException {
		...
		EmsDrugItemDO ems = modelFrom(ctx, ciEmsInfo, medSrvAggInfo);
		// 医疗单对象
	...
	private void constructDrugInfo(EmsDrugItemDO emsDrugItemInfo, CiEmsDTO dto) {
		...
		// emsDrugItemInfo.setFirst_freq = dto.getTimes_firday_mp.ToString();
		// //首日执行次数 SINGLE
	 ...
	 * @throws BizException
	 */
	private EmsOrDrug drugItemFrom(CiEmsSrvDTO mainSrvInfo, EmsDrugItemDO emsDrugItemInfo, CiEmsDTO dto,
			MedsrvAggDO medSrvAggInfo) throws BizException {
		...
		if (!CiOrdUtils.isTrue(dto.getFg_set())) {
			...
			if (mainSrvInfo != null) {
				...
				ems.setQuan_cur(quantumBP.getUnMMQuantum(dto.getDt_begin(),ems.getId_freq(), ems.getUse_days(), ems.getQuan_med()));
			} else {
		...
		ems.setEu_orsrvhp(dto.getEu_orhp());
		//诊断信息
	 ...
	 * @throws BizException
	 */
	protected List<CiEmsSrvDTO> getSetClinicalSrvDO(MedsrvAggDO mainAggInfo, String code_entp, int Eu_sourcemd)
			throws BizException {
