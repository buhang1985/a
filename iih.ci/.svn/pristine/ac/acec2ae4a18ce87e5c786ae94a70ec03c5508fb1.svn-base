 ...
 *
 */
public class AutoQcForRegionIsHaveTimesMrBP {
			...
			for (int i = 0; i < times; i++) {
				...
				if (autoDo.getId_mr_types() != null) {
					...
					whereStr.append("   and  to_date(a0.date_submit,'yyyy-mm-dd hh24:mi:ss') between  (to_date('"
							...
							+ autoDo.getHours_dead() + "*" + i + ",'hour') )");
					whereStr.append("   and ( to_date('" + autoQcDTO.getDt_acpt()
							+ "','yyyy-mm-dd hh24:mi:ss')+numtodsinterval(" + autoDo.getHours_dead() + "*" + (i + 1)
							+ ",'hour') )");
				}
				...
				if (eventDo == null || autoQcDTO == null || autoDo.getTimes_region() == null)
					continue;
				if (mrArr == null || mrArr.length <= autoDo.getTimes_region()) {
