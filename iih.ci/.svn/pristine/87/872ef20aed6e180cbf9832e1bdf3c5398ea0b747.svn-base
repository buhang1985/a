...
import iih.ci.ord.s.ems.op.ems.deforder.DefaultRisCreateOrderInfo;
...
public class EmsRisSaveBP extends EmsSetSaveBP {
	...
	protected OrderSavedRstInfo[] handleSaveOrderPackageList(CiEnContextDTO ctx, OrderPackageInfo[] szOrderPackageInfo)
		...
		for (OrderPackageInfo opInfo : szOrderPackageInfo) {
			...
			if (!CiOrdUtils.isEmpty(opInfo.getOrderAppSheetList())) {
				...
				// ObjectList objAppSheetList = new
				// ObjectList(opInfo.getOrderAppSheetList());
				if (objAppSheetInfo instanceof OrdApObsDO) {// 检查申请单保存
					CiOrdAppUtils.getOrapprisService().save(opInfo.getOrderAppSheetList()
							.toArray(new OrdApObsDO[opInfo.getOrderAppSheetList().size()]));
				}
			}
		}
	...
	// @Override
	// protected OrderPackageInfo[] mergeOrderPakageInfo(CiEnContextDTO ctx,
	// OrderPackageInfo[] szOrderPakageInfo)
	// throws BizException {
	// // TODO Auto-generated method stub
	// OrderPackageInfo[] szOrderPackageInfo = super.mergeOrderPakageInfo(ctx,
	// szOrderPakageInfo);
	// for (OrderPackageInfo opInfo : szOrderPackageInfo) {
	...
	// }
	//
	// OrderSrvExtPackage srvext = ((DefaultLisCreateOrderInfo)
	// this.getDefaultCreateOrderInfo())
	// .CalcSrvFeesOrdSrvInfo(ctx, opInfo.getOrderInfo(), new
	// BdSrvMmInfo(opInfo.getBdSrvList().get(0)),
	// itemidlist.asArray());
	//
	...
	protected void mergeOrderInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, Object uiModel) throws BizException {
		...
		orderInfo.setDes_or(emsObsItemDO.getDes_or());
		...
		orderInfo.setFg_mp_in(OrderUtils.IsMpInpatent(ctx, orderInfo.getSd_srvtp(), orderInfo.getId_route(),
				orderInfo.getFg_mp_in()));
		...
		Integer totalTimes = totalTimesBP.getTotalTimes(orderInfo.getDt_effe(),orderInfo.getId_freq(), emsObsItemDO.getUse_days());
		orderInfo.setTimes_cur(totalTimes);
		///////// 添加检验、检查、诊疗多剂量多次执行，杨敬本20171111
		orderInfo.setFg_quickwflow(emsObsItemDO.getFg_quickwflow());
	}
	...
	protected void mergeOrderSrvInfo(CiEnContextDTO ctx, OrdSrvDO srvInfo, Object uiModel) throws BizException {
		...
		if (FBoolean.TRUE.equals(srvInfo.getFg_set())) {
			...
			if (CiOrdUtils.isEmpty(srvdto.getPrice())) {
				...
				CiOrSrvPriCalUtils.mappingPriceRateFromEmsToSrv(ctx.getCode_entp(), srvInfo.getId_srv(),
						srvdto.getPrice().toString(), srvInfo);
			} else {
	...
	protected void mergeOrderAppInfo(CiEnContextDTO ctx, Object[] appSheetList, Object[] extInfoList, Object uiModel)
		...
		for (Object obj : appSheetList) {
			...
			ordris.setDes_sympsign(srvdto.getDes_or());// 病情描述
	...
	// @Override
	// protected void mergeOrderSetInfo(CiEnContextDTO ctx, OrdSrvSetDO[]
	// setSrvList, Object uiModel) throws BizException {
	// // TODO 合并套内项目信息
