...
import iih.ci.ord.s.ems.meta.BdSrvInfoMap;
...
import iih.ci.ord.s.ems.op.ems.calcpri.NumACalcPriModeProxy;
import iih.ci.ord.s.ems.op.ems.calcpri.NumCalcPriModeProxy;
import iih.ci.ord.s.ems.op.ems.calcpri.SelfSrvCalcPriModeProxy;
...
public class DefaultSetCreateOrderInfo extends DefaultSingleSrvCreateOrderInfo {
	...
	public OrderSrvExtPackage CalcSrvFeesOrdSrvInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, BdSrvMmInfo bdSrvMmInfo,
		...
		MedSrvSetItemDO[] szMedSrvSetItemDO = BdSrvInfoUtils.QueryMedSrvSetItemBy(orderInfo.getId_srv(), szId_srv);
		...
		case IBdPrimdCodeConst.ID_PRI_SRV_COMP:// 组合定价
			ordSrvExtPackage = CalcPriStdSrvOfOrdSrvList(ctx, orderInfo, bdSrvMmInfo.getBdSrvInfo(),
					new CompCalcPriModeProxy(bdSrvMmInfo.getBdSrvInfo().getId_srv(),
							bdSrvMmInfo.getBdSrvInfo().getName(), szMedSrvSetItemDO.length),
					bdSrvMmInfo.getUiModel());
			// 组合计价本身构建
			ordSrvExtPackage.getOrderSrvMmList().add(getMainSetSrv(ctx, orderInfo, bdSrvMmInfo.getBdSrvInfo()));
			break;
		...
		case IBdPrimdCodeConst.ID_PRI_SRVSET_MU:// 套个数加收
			ordSrvExtPackage = CalcPriOfOrdSrvPackage(ctx, orderInfo, bdSrvMmInfo, szMedSrvSetItemDO,
					new NumACalcPriModeProxy(bdSrvMmInfo.getBdSrvInfo().getId_srv(),
							bdSrvMmInfo.getBdSrvInfo().getName(), szMedSrvSetItemDO.length));
			// TODO:套内非临床项目费用项错误，需要一个一个判断去获取费用项，现有逻辑未计算，送入的是主服务
			break;
		...
		for (int index = 0; index < szMedSrvSetItemDO.length; ++index) {
			...
			if (bdSrvInfoWithSetItemCache.containsKey(mssid.getId_srv_itm())) {
				...
				if (IBdPrimdCodeConst.ID_PRI_SRV_COMP.equals(srvInfo.getId_primd())) {
					...
					;
					OrderSrvExtPackage ordSrvExtPackage = CalcPriStdSrvOfOrdSrvList(ctx, orderInfo, srvInfo,
							new CompCalcPriModeProxy(srvInfo.getId_srv(), srvInfo.getName(), 0),
							bdSrvMmInfo.getUiModel());
					// 非套服务 - 组合定价服务信息（费用项目）计算
					if (!CiOrdUtils.isEmpty(ordSrvExtPackage.getOrderSrvMmList())) {
						OrderSrvMmList srvMmList = ordSrvExtPackage.getOrderSrvMmList();
						// 组合计价套内项数量需乘套内项数量
						for (OrderSrvMmInfo srvMmInfo : srvMmList) {
							srvMmInfo.getOrderSrvInfo().setQuan_medu(
									srvMmInfo.getOrderSrvInfo().getQuan_medu().multiply(mssid.getQuan_medu()));
						}
						osmil.addAll(ordSrvExtPackage.getOrderSrvMmList());
					}
				} else {
					BdSrvMmInfo setItmSrvMMInfo = new BdSrvMmInfo(srvInfo, mssid, bdSrvMmInfo.getUiModel());
					if (CiOrdUtils.isTrue(srvInfo.getFg_mm())) {
						setItmSrvMMInfo.createSrvMmInfo(srvInfo, ctx.getId_org(), ctx.getId_dep_or());
					}
					// 组织服务信息和物品信息
					OrderSrvMmInfo orderSrvMmInfo = createOrderSrvMmInfo(ctx, orderInfo, setItmSrvMMInfo,
							OrSrvSourceFromEnum.PHYSIAN);
					if (CiOrdUtils.isTrue(mssid.getFg_clinical())) {
						orderSrvMmInfo.getOrderSrvInfo().setFg_or(FBoolean.TRUE);// 套内临床项目fg_or=Y
					}
	...
	protected OrderSrvExtPackage CalcPriOfOrdSrvPackage(CiEnContextDTO ctx, CiOrderDO orderInfo,
		...
		OrderSrvExtPackage ordSrvExtPackage = CalcPriStdSrvOfOrdSrvList(ctx, orderInfo, bdSrvMmInfo.getBdSrvInfo(),
				iCalcPriModeProxy, bdSrvMmInfo.getUiModel());
	...
	public OrderSrvMmInfo getMainSetSrv(CiEnContextDTO ctx, CiOrderDO orderInfo, MedSrvDO medSrv) throws BizException {
		return createOrderSrvMmInfo(ctx, orderInfo, medSrv, OrSrvSourceFromEnum.PHYSIAN);
	}
