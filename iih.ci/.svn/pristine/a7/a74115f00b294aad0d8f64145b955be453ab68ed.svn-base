 ...
 * @author F
 *
 * @date 2019年11月27日下午7:04:14
 *
 * @classpath iih.ci.event.ord.listeners.refund.op.OpFstAidRefundSelfDefineStandardListener
 */
public class OpFstAidRefundSelfDefineStandardListener extends OpRefundEnterListener{
	...
	protected boolean isOpData(BlCgItmOepDO oepDO) {
		if(OrdEventUtil.isEmpty(oepDO.getCode_enttp())||!OrdEventUtil.QJ.equals(oepDO.getCode_enttp())) {//急诊抢救退费
			OrdEventLogger.info(this.getClass().getSimpleName(), "急诊抢救退费标准版,筛选急诊流水退费数据code_entp:"+oepDO.getCode_enttp());
			return false;
		}
		return true;
	}
	...
	public void fireMessageByCode_applytp(Hashtable<String, List<OpRefund4IpEsDTO>> hashtable) throws BizException {
		...
		OrdEventLogger.info(this.getClass().getSimpleName(), "急诊抢救退费标准版,数据筛选结果:"+hashtable.toString());
		//急诊抢救处方退费BS302
		// 01 处方(西药+草药)
