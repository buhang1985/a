...
import iih.ci.ord.s.ems.op.tmpl.apbt.TmplApBtAction;
import iih.ci.ord.s.ems.op.tmpl.cons.TmplConsAction;
import iih.ci.ord.s.ems.op.tmpl.drugs.TmplDrugsAction;
import iih.ci.ord.s.ems.op.tmpl.herbs.TmplHerbsAction;
import iih.ci.ord.s.ems.op.tmpl.lis.TmplLisAction;
import iih.ci.ord.s.ems.op.tmpl.meta.OrderTmplDetailList;
import iih.ci.ord.s.ems.op.tmpl.meta.OrderTmplDetailListMap;
import iih.ci.ord.s.ems.op.tmpl.meta.OrderTmplGroupList;
import iih.ci.ord.s.ems.op.tmpl.meta.OrderTmplGroupListMap;
import iih.ci.ord.s.ems.op.tmpl.opr.TmplOprAction;
import iih.ci.ord.s.ems.op.tmpl.pathgy.TmplPathgyAction;
import iih.ci.ord.s.ems.op.tmpl.ris.TmplRisAction;
import iih.ci.ord.s.ems.op.tmpl.treat.TmplTreatAction;
...
public class TmplSaveAction implements ITmplSaveBP, EmsType {
	...
	public TmplRstDTO save(CiEnContextDTO ctx, TmplSaveDTO arg) throws BizException {
		...
		OrderTmplGroupListMap stringOrTplNItmGroupListMap = handleDriverTmplGroupInfo(ctx,
				listGroupTplItems);
		...
		for (OrderTmplGroupList groupList : stringOrTplNItmGroupListMap.values()) {
			// 获取医疗单业务逻辑处理接口
			IEmsSaveBP saveBP = handleSaveTmplInfo(ctx, groupList);
			if (null != saveBP) {
				// 构建医疗单保存参数
...
//
//						// 保存模板
//						EmsRstDTO[] rst = saveBP.save(ctx, new EmsSaveDTO[]{saveEmsInfo});
//
//						// 处理返回值
