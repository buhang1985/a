...
public class CiOrRelAppSheetSaveBP {
	 ...
	 * @throws BizException
	 */
	public  void exec(Object orappmap,String id_or,Integer iemstp)  throws BizException{
		//有效性校验
		...
		if(EmsType.LIS.equals(iemstp)){
			...
			OrappdietSave((OrdApDietDO)orappmap,id_or);
		}
	}
	
	private void OrappdietSave(OrdApDietDO orappmap, String id_or) throws BizException {
		if(orappmap==null)return;
		if(CiOrdUtils.isEmpty(orappmap.getId_or())){orappmap.setId_or(id_or);}
		((ICiorappdietCudService)ServiceFinder.find(ICiorappdietCudService.class)).save(new OrdApDietDO[]{orappmap});
	}
