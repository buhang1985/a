 ...
 * 检验报告单录入节点自定义服务接口。
 * @author wu.junhui
 * @version ma.meng_2019年12月26日11:26:48_0181925：检验结果录入节点，录入检验结果后，没有回写CI_RPT_LAB_ITM.lis_code_srv，导致住院医生站趋势图无法显示
 * @version ma.meng_2019年12月27日10:01:58_0180180: 【验证版】住院医生站检验报告回写病历时,单位列没取值.
 */
public class CiRptLabServiceImpl implements ICiRptLabService {
	...
	public CirptlabAggDO getRptLabByReqNo(String reqNo) throws BizException {
		...
		if (labItems != null) {
			for (Map<String,Object> labItem:labItems) {
				...
				// 服务编码（解决0181925）
				labItemDO.setLis_code_srv((String)labItem.get("lis_code_srv"));
				//序号
				...
				//单位
				labItemDO.setId_unit((String)labItem.get("id_unit"));
				labItemDO.setName_unit((String)labItem.get("name_unit"));// 参照的名称Unit_name只在表单显示使用，使用name_unit存值
	...
	private List<Map<String,Object>> findLabItems(OrdApLabDO ordApLabDO) throws BizException {
		...
		sql.append("  srv.code as lis_code_srv,");// 服务编码（解决0181925）
		sql.append("  lab.std_lo,");
		sql.append("  lab.std_hi,");
		sql.append("  lab.id_unit_nuit as id_unit,");
		...
		if (!fgSet) {
			...
			sql.append("where ");
			sql.append("  srv.ds=0 ");
			sql.append("  and srv.id_srv= ? ");
		} else {
			sqlParam.addParam(srvId);
			sqlParam.addParam(orId);
			sql.append("from ");
			...
			sql.append("  and srv_set.id_srvsetdef= ? ");
			sql.append("  and srv_set.id_or = ? ");
		...
		List<Map<String, Object>> result = (List<Map<String, Object>>) new DAFacade()
				.execQuery(sql.toString(), sqlParam, new MapListHandler());
