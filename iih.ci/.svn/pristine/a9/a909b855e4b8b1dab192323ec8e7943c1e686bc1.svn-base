 ...
 *
 */
public class IpCiprnDeleteAppLisDataBP extends CiprnDeleteAppLisDataBP {
	...
	private FArrayList getSampOrders(String[] id_ors, String id_en) throws BizException {
		...
		String sql = " select distinct c2.id_or,c2.name_or " + "from ci_order c1 " + "  inner join bd_samp_pri pri1 "
				+ "      on c1.id_srv = pri1.id_srv " + "    and c1.id_or in ('" + inOrStrs + "') "
				+ "inner join bd_samp_pri_itm itm1 " + "  on itm1.id_samp_pri = pri1.id_samp_pri "
				+ " inner join ci_order c2 " + "on c2.id_srv=itm1.id_srv "
				+ "and  c2.id_en = ?  and c1.sd_srvtp like '03%' and c1.id_en=c2.id_en "
				+ "where itm1.eu_pritp = 1 and c2.fg_canc='N' and itm1.id_srv not in " + "     (select itm.id_srv "
				+ "       from ci_order ci " + "     inner join bd_samp_pri pri " + "    on ci.id_srv = pri.id_srv "
				+ "   inner join bd_samp_pri_itm itm " + "  on itm.id_samp_pri=pri.id_samp_pri "
				+ "  where ci.fg_canc = 'N' and ci.fg_sign='Y' " +
				// " and ci.sd_su_bl <> '2' "+
				"and itm.eu_pritp=1 " + "and ci.id_en = ? " + "and ci.id_or not in ('" + inOrStrs
