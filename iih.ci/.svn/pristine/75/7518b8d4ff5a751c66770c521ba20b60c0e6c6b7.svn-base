...
public class MrProcessQcQryBP {
	...
	public QaFltDTO[] getMrProcessQcNeedDefectFltList(String id_ent, String cqf_submit_user) throws BizException {
		...
		StringBuilder sql = new StringBuilder();
				...
				sql.append("   and ((ci_qa_flt.sd_qa_ty = '"+ICiMrDictCodeConst.SD_EXEC_SEGMENT+"' and ci_qa_flt.id_sbmt_user='"+ cqf_submit_user + "') or ci_qa_flt.sd_qa_ty = '"+ICiMrDictCodeConst.SD_EXEC_AUTOMATIC+"')"); 
	...
	public QaFltDTO[] getMrProcessQcDefectFltListByIdRevision(String id_revision) throws BizException {
		...
		StringBuilder sql = new StringBuilder();
				...
				sql.append(" WHERE "); 
				sql.append("    ci_qa_flt.id_revision = '"+id_revision+"'"); 
				sql.append("   and ci_qa_flt.sd_qa_ty in ('"+ICiMrDictCodeConst.SD_EXEC_SEGMENT+"','"+ICiMrDictCodeConst.SD_EXEC_AUTOMATIC+"')"); 
				sql.append("   AND ci_qa_flt.ds = '0'");  
				sql.append("   order by ci_qa_flt.ID_QA_FLT desc"); 
				String sqlStr=sql.toString();
	   @SuppressWarnings("unchecked")
...
//			sql.append("       on ci_qa_record.id_revision=cqrn.id_revision\n");
			...
			sql.append("        where ci_qa_record.id_ent= '"+qaPatListDto.getId_ent()+"' and "+EnvContextUtil.processEnvContext("",new QaRecordDO(),false)+" and  ee.code_entp='10'   and ci_qa_record.sd_qa_ty = '"+ICiMrDictCodeConst.SD_EXEC_SEGMENT+"' ORDER BY ci_qa_record.dt_plan DESC");
				String sqlStr=sql.toString();
       @SuppressWarnings("unchecked")
