...
public class AutoQcMethodForMOne {
	    	...
	    	for (int i = 0; i < times; i++) {
	    		if(autoDo.getId_mr_types()!=null){
		    		whereStr=" a0.id_ent='"+id_ent+"' and a0.id_mrtp in( '"+autoDo.getId_mr_types().replaceAll(",", "','")+"') and "
		    				+ " to_date(a0.date_submit,'yyyy-mm-dd hh24:mi:ss') between  (to_date('"+autoQcDTO.getDt_acpt()+"','yyyy-mm-dd hh24:mi:ss')+numtodsinterval("+autoDo.getHours_dead()+"*"+i+",'hour') ) and ( to_date('"+autoQcDTO.getDt_acpt()+"','yyyy-mm-dd hh24:mi:ss')+numtodsinterval("+autoDo.getHours_dead()+"*"+(i+1)+",'hour') )";
		    		if (autoDo.getId_mrtp_change()!=null) {
		    			whereStr="  a0.id_ent='"+id_ent+"' and a0.id_mrtp in( '"+autoDo.getId_mr_types().replaceAll(",", "','")+"') or  a0.id_mrtp in ( '"+autoDo.getId_mrtp_change().replaceAll(",", "','")+"')"
		    				+ " and to_date(a0.date_submit,'yyyy-mm-dd hh24:mi:ss') between  (to_date('"+autoQcDTO.getDt_acpt()+"','yyyy-mm-dd hh24:mi:ss')+numtodsinterval("+autoDo.getHours_dead()+"*"+i+",'hour') ) and ( to_date('"+autoQcDTO.getDt_acpt()+"','yyyy-mm-dd hh24:mi:ss')+numtodsinterval("+autoDo.getHours_dead()+"*"+(i+1)+",'hour') )";
		    		}
		    		String istimerStr=" a0.id_ent='"+id_ent+"' and a0.id_mrtp in( '"+autoDo.getId_mr_types().replaceAll(",", "','")+"') and a0.date_submit is not null ";
		    		if (autoDo.getId_mrtp_change()!=null) {
		    			istimerStr="  a0.id_ent='"+id_ent+"' and ( a0.id_mrtp in( '"+autoDo.getId_mr_types().replaceAll(",", "','")+"') or  a0.id_mrtp in ( '"+autoDo.getId_mrtp_change().replaceAll(",", "','")+"') ) and a0.date_submit is not null";
		    		}
		    		CiMrDO[] mrArr=mrService.find(whereStr, "", FBoolean.FALSE);
		    		CiMrDO[] timermrArr=mrService.find(istimerStr, "", FBoolean.FALSE);
