...
import iih.ci.ord.s.ems.op.orderv1.bp.ope.OrderDeleteHBIOperateBP;
 ...
 *
 */
public class OrderDeleteHBIBP implements IOrderDeleteHBIBP {

	private IOrderOperateBP deleteBP = new OrderDeleteOperateBP();
	private IOrderOperateBP cancelBP = new OrderCancelOperateBP();
	private IOrderOperateBP deleteHBIBP = new OrderDeleteHBIOperateBP();
	@Override
	public OrderRstDTO delete(CiEnContextDTO ctx, OrderOperateDTO arg) throws BizException {
		...
		if (paramList != null && paramList.size() > 0 && extension != null) {
			...
			List<String> lstIdorsHBI = new ArrayList<String>();
			//查询当前医嘱状态集合
			...
			for (String key : extension.keySet()) {
				...
				if (ICiDictCodeConst.SD_SU_OPEN.equals(extension.get(key)
						.toString())) {
					lstIdorsDel.add(key);// 获取开立状态的医嘱集合，医嘱删除
				} else if ("Y".equals((String) map.get("fg_sign"))) {
					if ("Y".equals(map.get("fg_feertnable"))) {
						lstIdorsHBI.add(key);// 高端商报退费
					} else {
			...
			if (lstIdorsDel.size() > 0) {
				rstDTO = deleteBP.exec(lstIdorsDel.toArray(new String[] {}), null, null);
			...
			if (lstIdorsHBI.size() > 0) {
				rstDTO = deleteHBIBP.exec(lstIdorsHBI.toArray(new String[] {}), ctx, null);
				rtn = rtn && rstDTO.getIsSuccess().booleanValue();
			}
