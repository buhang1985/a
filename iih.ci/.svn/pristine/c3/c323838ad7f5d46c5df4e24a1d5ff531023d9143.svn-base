...
import iih.ci.event.ord.bps.revert.op.query.OpTreatRevertEntInfoQuery;
import iih.ci.event.ord.bps.revert.op.query.OpTreatRevertOrdInfoQuery;
 ...
 * @author F
 *
 * @date 2019年9月11日下午1:11:55
 *
 * @classpath iih.ci.event.ord.bps.revert.op.logicbp.OpTreatRevertLogicDealWithBP
 */
public class OpTreatRevertLogicDealWithBP extends CommonParamBP{
	...
	public IEOpTreatOrEnDTO[] exec(OrdEventFireDTO firedto) throws BizException {
		...
		if (OrdEventUtil.isEmpty(rtn))
			return null;
		IEOpTreatOrDTO[] itmsDtos=getIEOpTreatOrDTOs(firedto.getIdors());//查询医嘱集合
		//组装数据
	...
	private IEOpTreatOrEnDTO[] getIEOpTreatOrEnDTOs(String id_ors) throws BizException {
		OpTreatRevertEntInfoQuery qry = new OpTreatRevertEntInfoQuery(id_ors);
	...
	private IEOpTreatOrDTO[] getIEOpTreatOrDTOs(String id_ors) throws BizException {
		...
		OpTreatRevertOrdInfoQuery qry=new OpTreatRevertOrdInfoQuery(id_ors);
		IEOpTreatOrDTO[] itms = (IEOpTreatOrDTO[]) AppFwUtil.getDORstWithDao(qry, IEOpTreatOrDTO.class);
		return itms;
	}
	/**
	 * 组装就诊和医嘱信息数据
