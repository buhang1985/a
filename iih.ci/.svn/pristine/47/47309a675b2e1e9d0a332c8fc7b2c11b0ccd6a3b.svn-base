...
//		}
		...
		//20190513将标本关联费用逻辑删除，因为此设计不符合实际需求。临床业务中是把多个检验合入一个单子中，标本费和采集费等应该是一个，这个逻辑是每个检验都会有标本费和采集费，不符合现实需求
		//现在已改为检验合管控制
		//标本类型  关联服务
	...
	private UsageRelFeeSrvDO[] routeRelFeeSrvHandle(CiEmsDTO ems,UsageRelFeeSrvDO[] rtn,String orgid) throws BizException{
		//不在院执行，不计算用法关联费用2019年9月9日新增逻辑
		if(CiOrdUtils.isEmpty(ems)||!FBoolean.TRUE.equals(ems.getFg_mp_in()))return rtn;
		// 获取当前需要去掉用法关联费用的就诊类型
		String orgId = CiOrdAppUtils.getEnvContext().getOrgId();
		...
		if(StringUtils.isNotEmpty(relUseageCodeEntp)){
			...
			if(codeEntpList.contains(ems.getCode_entp())){
				return rtn;
			}
		}		
		// 国际医院按照用法关联费用走 -- 铜陵 if(!CiOrdUtils.isTrue(ems.getFg_mp_in()))return rtn;  //暂时按只有在院执行时才进行费用关联来计算
