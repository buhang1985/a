...
public class UpdateOrdStatusInfo0BP {
	...
	public  OrSuModRtnInfoDTO[] exec(IdVProperty[] id_ors,FDateTime dt_end,String sd_su_or)  throws BizException{
		...
		if(ICiDictCodeConst.SD_SU_OPEN.equals(sd_su_or) || ICiDictCodeConst.SD_SU_CANCEL.equals(sd_su_or) || ICiDictCodeConst.SD_SU_DOCTORSTOP.equals(sd_su_or)){
			CiMsgSenderUtil.sendCiOrdConfirmMsg(ors);
			//0188501: 【住院医生站】检查和手术类医嘱在作废时取消申请单,应改为在护士站作废确认时处理
			// 撤销、删除、作废调用SC接口向sc回写手术申请撤回信息
			//CiOrSufInfo4ScAptApplBp bp = new CiOrSufInfo4ScAptApplBp();
			//CiEnContextDTO ctx = new CiEnContextDTO();
			//ctx.setId_psndoc(Context.get().getStuffId());
			//bp.sendSugInfo4ScAptAppl(ctx,ors);
		}
		//医嘱撤回时删除申请单
		if(ICiDictCodeConst.SD_SU_OPEN.equals(sd_su_or)||ICiDictCodeConst.SD_SU_CANCEL.equals(sd_su_or)){
			clearPrintData((String[])os[0]);
		}
		//医嘱及事件保存
...
//		}
        ...
        if(ICiDictCodeConst.SD_SU_DOCTORSTOP.equals(sd_su_or)){
			...
			nurMQBP.broadcastNurseConfirmMsg(null, ors);
        }
       ...
       if(ICiDictCodeConst.SD_SU_CANCEL.equals(sd_su_or)){
			...
			//0154388: 验证版，作废的医嘱，在护士站医嘱确认那里，没有弹提醒框
			CiOrderNurseMQBP nurMQBP = new CiOrderNurseMQBP();
			nurMQBP.broadcastNurseConfirmMsg(null, ors);
        }
       	//执行用血医嘱的回撤逻辑
       if(useBtrollBacks.size()>0){
		...
		if("1".equals(cfgParam)){
		  ...
		  sufInfo4ScAptApplBp.sendSugInfo4ScAptAppl(null, orders);
		}
       //执行患者核对信息的删除
...
//		String id_emp=CiOrdAppUtils.getEnvContext().getUserId();
		//0178590: 【南大象湖】输血接口，医嘱取消回写，调用接口，取消人现在默认成了管理员，取消科室也不对。
