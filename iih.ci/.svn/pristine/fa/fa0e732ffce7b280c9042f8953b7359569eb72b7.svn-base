 ...
 *
 */
public class DefaultApBtCreateOrderInfo extends DefaultSetCreateOrderInfo {
	...
	public OrderPackageInfo[] createOrderPakageInfo(CiEnContextDTO ctx, DefaultCreateParam[] szParam) throws BizException {
		...
		for(OrderPackageInfo info:szOrderPakageInfo){
			...
			agglist.add(assembleOrderBtAggInfp(info.getOrderInfo().getId_or()));
	...
	public CiorappbtAggDO assembleOrderBtAggInfp(String id_or){
		CiorappbtAggDO aggdo=new CiorappbtAggDO();
		aggdo.setParentDO(assembleOrderBtInfo(id_or));
		return aggdo;
	}

	public OrdApBtDO assembleOrderBtInfo(String id_or){
