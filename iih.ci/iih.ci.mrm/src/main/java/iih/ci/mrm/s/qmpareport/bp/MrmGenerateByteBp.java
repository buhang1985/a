...
public class MrmGenerateByteBp {
	...
	public static byte[] writeCSVByDTOs(List<CiMrmDirHeaReportDTO> lists,
			String[] fs) {
		ArrayList<Map<String, Object>> data = getKeysAndValuesFromDTO(lists
				.toArray(new CiMrmDirHeaReportDTO[0]));
		byte[] bytes = null;
		FileOutputStream outf = null;
		OutputStreamWriter outs = null;
		BufferedWriter out = null;
		...
		try {
			...
			for (Map<String, Object> map : data) {
				for (String strArName : fs) {
					out.write(map.get(strArName) == null ? "" : map.get(
							strArName).toString());
					out.write(",");
				}
				out.newLine();
	...
	public static byte[] writeDBF(List<CiMrmDirHeaReportDTO> lists, String[] fs) {
		...
		try {
			...
			for (Map<String, Object> m : data) {
				...
				int j = 0;
				for (String strArName : fs) {
					rowData[j] = m.get(strArName) == null
							|| m.get(strArName).toString().equals("") ? "" : m
							.get(strArName).toString();
					if (j == length - 1)
						break;
					j++;
				}
	...
	public static ArrayList<Map<String, Object>> getKeysAndValuesFromDTO(
			BaseDTO[] lists) {
		ArrayList<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		for (BaseDTO bas : lists) {
			...
			for (int i = 0; i < fs.length; i++) {
				try {
					String attarName = fs[i].substring(0, 1).toUpperCase()
							+ fs[i].substring(1);
