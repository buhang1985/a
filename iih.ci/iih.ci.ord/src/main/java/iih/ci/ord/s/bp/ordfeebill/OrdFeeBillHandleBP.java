...
public class OrdFeeBillHandleBP {
	public CiEmsSrvDTO[] save(CiEmsSrvDTO[] szSrv, CiEnContextDTO ctx) throws BizException {
		...
		for (CiEmsSrvDTO srv : szSrv) {
			...
			if(FBoolean.TRUE.equals(agg.getParentDO().getFg_sign()))continue;
		...
		validateTotalFees(orservice.findByBIds(getidors(szSrv), FBoolean.FALSE),ctx);
		return szSrv;
	}
	...
	protected void handleOrSrvDO(OrdSrvDO srvdo, CiOrderDO cider, CiEmsSrvDTO fee) throws BizException {
		...
		//0185719: 门急诊医生站---组合定价项目子项目数量可以修改明细
		//费用清单保存时，以下数据丢失
		srvdo.setEu_sourcemd(fee.getEu_sourcemd());
		srvdo.setEu_blmd(fee.getEu_blmd());
		srvdo.setPri_std(fee.getPri_std());
		srvdo.setPri_ratio(fee.getPri_ratio());
		srvdo.setId_pripat(fee.getId_pripat());
		srvdo.setEu_hpjudge(fee.getEu_hpjudge());
		srvdo.setEu_orsrvhp(fee.getEu_orsrvhp());
	}
	...
	private void validateTotalFees(CiorderAggDO[] szOrdAgg,CiEnContextDTO ctx) throws BizException{
		...
		if(aggdo != null && aggdo.getOrdSrvDO() != null && aggdo.getOrdSrvDO().length >0){
			 ...
			 for(OrdSrvDO srvdo: ordsrvDO){
				if(FBoolean.TRUE.equals(srvdo.getFg_bl()) && 
					...
					if (FBoolean.TRUE.equals(srvdo.getFg_mm()) && null != ordsrvmmRService)
					{
