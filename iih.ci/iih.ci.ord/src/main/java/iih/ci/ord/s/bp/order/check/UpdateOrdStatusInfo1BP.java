...
public class UpdateOrdStatusInfo1BP {
	 ...
	 * @throws BizException
	 */
	public  OrSuModRtnInfoDTO[] exec(CiOrderDO[] orders,FDateTime dt_end,FDateTime dt_chk_stop,String sd_su_or)  throws BizException{
		//有效性校验
		validateCheck(sd_su_or);
	
		//设置医嘱状态
		ArrayList<OrSuModRtnInfoDTO> rtninfos = new ArrayList<OrSuModRtnInfoDTO>();
		CiOrderDO[] ors = getValidOrders(orders, dt_end, dt_chk_stop, sd_su_or, rtninfos);
		
		//医嘱及事件保存
		CiOrderDO[] rtnors=ciOrStatusRelInfoSave(ors,sd_su_or);   //保存更新走相应的校验操作
		if(rtnors==null||rtnors.length==0){
			OrdBizLogger.info(null, "医嘱确认更新状态后医嘱状态：数量0");
		}else{
			OrdBizLogger.info(null, "医嘱确认更新状态后医嘱状态：数量"+ors.length+","+ors[0].getStatus()+","+ors[0].getId_or()+","+ors[0].getFg_chk());
		}
	 ...
	 * @throws BizException 
	 */
	private CiOrderDO[] getValidOrders(CiOrderDO[] orders, FDateTime dt_end, FDateTime dt_chk_stop, String sd_su_or,
		...
		for (int i = 0; i < orders.length; i++) {
			...
			if (ICiDictCodeConst.SD_SU_OPEN.equals(sd_su_or)) {//医嘱
				...
				if (or.getFg_chk_canc().isValue()) {
					...
					if(!FBoolean.TRUE.equals(or.getFg_canc())){
						or.setFg_canc(new FBoolean(true));
						or.setDt_canc(dtNow);
						or.setId_dep_canc(id_dep);
						or.setId_emp_canc(id_emp);
					}
					//liyue 0163189: 作废医嘱需要医生手动修改不打印医嘱单。
					try {
						FBoolean param =  ParamUtils.getOrgParamFBooleanValue("NX_CIOR0001");
						if(FBoolean.TRUE.equals(param)) {
							or.setFg_prn(FBoolean.FALSE);
						}
					} catch (Exception e) {
						 throw new BizException("参数NX_CIOR0001获取失败");
					}
					or.setFg_chk_canc(new FBoolean(true));
					or.setFg_uncancelable(new FBoolean(true));//不可作废标识
				...
				if (or.getFg_chk_stop().isValue()) {
				   ...
				   if(!or.getSd_su_or().equals(ICiDictCodeConst.SD_SU_FINISH)){//医嘱状态为完成不修改完成状态  li_cheng
					...
					or.setFg_stop(new FBoolean(true));
					if(!CiOrdUtils.isEmpty(dt_chk_stop)) {
						or.setDt_chk_stop(dt_chk_stop);
					}else
					{
		...
		OrdBizLogger.info(null, "医嘱确认,更改医嘱状态的医嘱数量【"+rtns.size()+"】");
	 ...
	 * @return
	 */
	public OrSuModRtnInfoDTO getOrderRelSuInfo(CiOrderDO or,String sd_su_or){
	 ...
	 * @throws BizException
	 */
	public CiOrderDO[] ciOrStatusRelInfoSave(CiOrderDO[] ors,String sd_su_or) throws BizException{
		if(ors==null||ors.length==0){
			OrdBizLogger.info(null, "医嘱确认更新状态前一刻的医嘱状态：数量为0");
		}else{
			OrdBizLogger.info(null, "医嘱确认更新状态前一刻的医嘱状态：数量"+ors.length+","+ors[0].getStatus()+","+ors[0].getId_or()+","+ors[0].getFg_chk());
		}
		CiOrderDO[] rtnors=CiOrdAppUtils.getOrService().update(ors); 

		String[] id_ors_temp = this.getIdOrdersListNotCancal(ors);
