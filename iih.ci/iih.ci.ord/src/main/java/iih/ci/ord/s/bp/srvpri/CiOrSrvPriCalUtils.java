...
public class CiOrSrvPriCalUtils {
	 ...
	 * @throws BizException
	 */
	public static void mappingPriceRateFromEmsToSrv(String code_entp, CiEmsSrvDTO srvdto, OrdSrvDO srvdo, Integer iN)
			throws BizException {
		// if (CiOrdUtils.isIpWf(code_entp)){//住院
		// FDouble price = FDouble.ZERO_DBL;
		// if (CiOrdUtils.isSrvMMPri8Id(srvdto.getId_primd())) {// 对应物品价格
		// price = CiOrdUtils.getOrSrvMMPrice(srvdto.getId_mm(),
		// srvdto.getId_unit_sale());
		// } else {
		...
		if (CiOrdUtils.isEmpty(srvdto.getPrice()) || CiOrdUtils.isEmpty(srvdto.getPri_std())
				|| CiOrdUtils.isEmpty(srvdto.getPri_ratio())) {
			FDouble[] priRats = getOrSrvPrice(code_entp, srvdto, iN);
			if (priRats != null) {
				srvdo.setPri(priRats[0]);
				srvdo.setPri_ratio(priRats[0]);
				srvdo.setPri_std(priRats[1]);
			}
		} else {
			CiOrdUtils.assignPriratToSrvDO(srvdto, srvdo);
		}
		OrdBizLogger.info(cienContext,
				String.format("计算服务[%s]价格，耗时:%s(ms)", srvdto.getName_srv(), System.currentTimeMillis() - tBeginTime));
	 ...
	 * @throws BizException
	 */
	public static void mappingRatioPriceRateFromEmsToSrv(String code_entp, CiEmsSrvDTO srvdto, OrdSrvDO srvdo, Integer iN)
			throws BizException {
	...
	public static void mappingPriceRateFromEmsToSrv(String code_entp, String id_srv, String id_primd, OrdSrvDO srvdo)
		...
		// FDouble price =
		// CiOrdAppUtils.getPriCalService().CalSingleSrvStdPrice(id_srv);
		// srvdo.setPri(price);
		// srvdo.setPri_std(price);
		// }
		// }else{
		...
		if (!CiOrdUtils.isEmpty(srvdo.getPri_std()) && srvdo.getPri_std().compareTo(FDouble.ZERO_DBL) != 0) {
			srvdo.setPri_ratio(srvdo.getPri().div(srvdo.getPri_std()));// 折扣系数
		}
		// }
	}
	...
	public static void mappingPriceRateFromEmsToSrv(String id_pripat, String code_entp, String id_srv, String id_primd,
		 ...
		 * update GH update date 2018-5-14 reason : 原方法代码中判断不准确
		 */
		// if (!CiOrdUtils.isEmpty(srvdo.getPri_std()) && srvdo.getPri_std() !=
		// FDouble.ZERO_DBL) {
		if (!CiOrdUtils.isEmpty(srvdo.getPri_std()) && srvdo.getPri_std().compareTo(FDouble.ZERO_DBL) != 0) {
			srvdo.setPri_ratio(srvdo.getPri().div(srvdo.getPri_std()));// 折扣系数
		}
	...
	public static PriStdSrvDTO[] CalSrvSetFIXPrices(String Id_srv_set, int num) throws BizException {
		...
		// if (CiOrdUtils.isIpWf(code_entp)){//住院
		// return
		// CiOrdAppUtils.getPriCalService().CalSrvSetFIXPrices(Id_srv_set, num);
		// }else{
		String id_pripat = cienContext.getId_pripat();
		...
		if(srvPricalRateAndPriceDTOMap == null || srvPricalRateAndPriceDTOMap.isEmpty()){
			return null;
		}
	...
	public static PriStdSrvDTO[] CalSrvSetMUPrices(String Id_srv_set, int num) throws BizException {
		...
		// String code_entp = cienContext.getCode_entp();
		// if (CiOrdUtils.isIpWf(code_entp)) {// 住院
		// return CiOrdAppUtils.getPriCalService().CalSrvSetMUPrices(Id_srv_set,
		// num);
		// } else {
		String id_pripat = cienContext.getId_pripat();
		Map<String, Map<String, SrvPricalRateAndPriceDTO>> priMap = CiOrdAppUtils.getPriCalService()
	...
	private static PriStdSrvDTO[] getPriStdSrvDTOs(Map<String, SrvPricalRateAndPriceDTO> srvPricalRateAndPriceDTOMap,
		...
		for (SrvPricalRateAndPriceDTO srvPricalRat : srvPricalRateAndPriceDTOMap.values()) {
			...
			priStdSrv.setId_mm(srvPricalRat.getId_mm());
			priStdSrv.setId_unit(srvPricalRat.getId_unit());
	 ...
	 * @throws BizException
	 */
	private static FDouble[] getOrSrvPrice(String code_entp, CiEmsSrvDTO emssrv, Integer iN) throws BizException {
		...
		if (FBoolean.TRUE.equals(emssrv.getFg_mm()) && !CiOrdUtils.isEmpty(emssrv.getPrice()))
		...
		CiOrBdSrvPriceCalBP bp = new CiOrBdSrvPriceCalBP();
		CiEnContextDTO cienContext = Context.get().getAttribute("CiEnContextDTO", CiEnContextDTO.class);
		// 0122141: 住院医生站，开立服务套部位加收模式的项目报错 将数量使用参数传入iN
		MedSrvPriceDO srvprice = bp.exec(getBdSrvPriCalParamDTOFrom(emssrv, iN), cienContext.getId_pripat());
		if (!CiOrdUtils.isEmpty(srvprice)) {
	...
	private static BdSrvPriCalParamDTO getBdSrvPriCalParamDTOFrom(CiEmsSrvDTO emssrv, Integer iN) {
		BdSrvPriCalParamDTO param = new BdSrvPriCalParamDTO();
