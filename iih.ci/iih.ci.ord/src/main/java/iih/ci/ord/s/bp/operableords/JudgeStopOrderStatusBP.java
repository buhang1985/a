...
public class JudgeStopOrderStatusBP{
	...
	protected void orStatusValidat(CiOrderDO[] orders) throws BizException {
		...
		for(CiOrderDO orderdo : ordersDb){
			if(!(orderdo.getSd_su_or() != ICiDictCodeConst.SD_SU_CHECKSTOP && FBoolean.TRUE.equals(orderdo.getFg_long()) && FBoolean.FALSE.equals(orderdo.getFg_canc())&& orderdo.getSd_su_or() != ICiDictCodeConst.SD_SU_FINISH && orderdo.getSd_su_or() != ICiDictCodeConst.SD_SU_CANCEL && orderdo.getSd_su_or() != ICiDictCodeConst.SD_SU_CHECKCANCEL)){
