...
public class DischargeCheckBP {
	...
	public String canDischarge(String entId) throws BizException{
		...
		errorMsg = this.getDischargeSrvNames(entId);
		if(!StringUtils.isEmpty(errorMsg)){
			errorMsg=("存在【"+errorMsg+"】有效医嘱");
		}
		 return errorMsg;
	/*	if(validCount>0){
			Integer dischargeCount = this.getDischarge(entId);
	...
	private String getDischargeSrvNames(String entId) throws DAException{
		StringBuilder builder = new StringBuilder();
		builder.append(" select listagg(or1.name_or,'，') within group (order by or1.name_or) ");
		builder.append(" from ci_order or1 ");
		//医嘱状态需要修改
		builder.append(" where (or1.fg_chk = 'N' or (or1.fg_canc ='Y' and or1.fg_chk_canc='N') or (or1.fg_long = 'Y' and or1.fg_canc ='N' and or1.fg_chk_stop='N')) and or1.id_en =? ");
		//builder.append(" where srv.sd_srvtp = ?  and or1.sd_su_or =?   and or1.id_en =? ");
		SqlParam param = new SqlParam();
		//param.addParam("1304");
		//param.addParam(ICiDictCodeConst.SD_SU_CHECKSTOP);
		param.addParam(entId);
		String result = (String) new DAFacade().execQuery(builder.toString(), param, new ColumnHandler());
		return result;
	}
	/**
	 * 获取未停止、且为作废的医嘱
