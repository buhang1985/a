...
public class SetMmInfoOfOrSrv8EmsBP {
	...
	private void createCiOrdSrvMmDO(OrdSrvMmDO srvmmdo,OrdSrvDO srvdo,CiEmsSrvDTO srvdto,int status) throws BizException{
		...
		srvmmdo.setName_pgku_cur(srvdto.getName_unit_sale());
		//如果价格为空，重新算一次
		...
		srvmmdo.setId_herbpreparationtp(srvdto.getId_herbpreparationtp());//草药制剂类型
		srvmmdo.setSd_herbpreparationtp(srvdto.getSd_herbpreparationtp());//草药制剂类型(0饮片；1配方颗粒)
		srvmmdo.setStatus(status);
		srvmmdo.setId_medu_cur(srvdto.getId_medu_cur());
		srvmmdo.setQuan_medu_cur(srvdto.getQuan_medu_cur());
		srvmmdo.setFactor_cur(srvdto.getFactor_medu_cur()==null?null:srvdto.getFactor_medu_cur().doubleValue()+"");
