package iih.ci.ord.s.external.bp.mr.gj;
...
public class getOrderFlush2MrDtoListBP {
	...
	public FMap2 getOrderFlushList(String id_ent, String code_entp, String id_psndoc, boolean isAll)
		...
		CiOrderDO[] ciors = CiOrdAppUtils.getOrQryService().find(whereFlushStr, "sortno", FBoolean.FALSE);
		if (ciors == null || ciors.length == 0)
	...
	private FMap2 DataHandle(CiOrderDO[] ciOrderDOs) throws BizException {
		...
		for (CiOrderDO ciOrderDO : ciOrderDOs) {
			...
			if (FBoolean.TRUE.equals(ciOrderDO.getFg_set()))
				lstIdsset.add(ciOrderDO.getId_or());
			else
				lstIds.add(ciOrderDO.getId_or());
		}
		...
		for (CiOrderDO ciOrderDO : ciOrderDOs) {
			...
			if (sd_srvtp != null) {
				...
				if (sd_srvtp.startsWith(IBdSrvDictCodeConst.SD_SRVTP_DRUG)) {
					...
					//拼接检查部位到医嘱名称中
					splicerBody2NameOr(ciOrderDO,lstRisBodyNamesSet, lstRisBodyNames);
	...
	/**
	 * 拼接检查部位到医嘱内容中，方便病历内容回写
	 * @param codo
	 */
	private void splicerBody2NameOr(CiOrderDO codo,List<UtilsDTO> lstRisBodyNamesSet,List<UtilsDTO> lstRisBodyNames) {
		boolean hasbody = false;
		if (FBoolean.TRUE.equals(codo.getFg_set())) {
			if (lstRisBodyNamesSet != null && lstRisBodyNamesSet.size() > 0) {
				for (UtilsDTO udto : lstRisBodyNamesSet) {
					if (codo.getId_or().equals(udto.getDef1())) {
						if (udto.getDef2() != null) {
							codo.setName_or(codo.getName_or() + "(" + udto.getDef2().toString() + ")");
							hasbody = true;
						}
					}
				}
	 ...
	 * @throws Exception
	 */
	private List<UtilsDTO> getRisBodyNames(String[] ids, boolean isSet) throws BizException {
		String str = "";
		if (ids.length > 0) {
			for (String id : ids) {
				str += "'" + id + "',";
			}
			return QryRisBodyName(str, isSet);
		}

		return null;
	}
	public static List<UtilsDTO> QryRisBodyName(String strSql, boolean isSet) throws BizException {
...
//				+ "replace(to_char(wmsys.wm_concat(udi.name||(case when udi1.name is null then '' else '-' end)||udi1.name) over (partition by orsrvset.id_or order by orsrvset.Sortno)),',','、') as def2 "
                + "replace(to_char(listagg(udi.name||(case when udi1.name is null then '' else '-' end)||udi1.name,',') within group( order by orsrvset.Sortno) over (partition by orsrvset.id_or ) ),',','、') as def2 "
				...
				+ "left outer join bd_udidoc udi1 on udi1.id_udidoc=cs.id_pos " 
				+ "where ord.id_or in  (" + strSql.substring(0, strSql.length() - 1) + ")";

		return (List<UtilsDTO>)new DAFacade().execQuery(sql, new BeanListHandler(UtilsDTO.class));
	}
}
