...
//			BdHpIndication.
		...
		//判断开立医嘱与子服务相同Sortno 置为1
		if(ordo.getId_srv().equals(srvdto.getId_srv())) {
			srvdo.setSortno(1);
		}else {
		...
		srvdo.setId_dsdef(srvdto.getId_dsdef()); // 病种ID
		srvdo.setCode_dsdef(srvdto.getCode_dsdef()); // 病种编码
		srvdo.setName_dsdef(srvdto.getName_dsdef()); // 病种名称
			
		if(!CiOrdUtils.isEmpty(srvdo.getEu_blmd()) && 
			srvdo.getEu_blmd()==MedSrvBlModeEnum.MANUALBL){
			// 此服务为手工计价
			...
			//自动划价也需要存  标准价和 价格系数
			if(CiOrdUtils.isEmpty(srvdto.getPri_std())||CiOrdUtils.isEmpty(srvdto.getPri_ratio())) {
				CiOrSrvPriCalUtils.mappingRatioPriceRateFromEmsToSrv(ordo.getCode_entp(), srvdto, srvdo,null);
			}
		}
		...
		if(FBoolean.TRUE.equals(srvdo.getFg_self())||FBoolean.TRUE.equals(srvdto.getFg_extdispense())) {//外配药时，费用标识为N,2020/7/2,LIYUELONG
...
//		}
		...
		srvdo.setId_srvca(srvdto.getId_srvca());
		srvdo.setFg_bl(CiOrdUtils.nullHandle(srvdto.getFg_bl()));
		if(FBoolean.TRUE.equals(srvdto.getFg_self())) {
			srvdto.setFg_bl(FBoolean.FALSE);
		}
		srvdo.setCode_srv(srvdto.getCode_srv());
		srvdo.setEu_sourcemd(srvdto.getEu_sourcemd());
...
//		}
		...
		//到此为止的sd_reltp是前台判断医嘱是否进行皮试的标识0需要皮试
		if(FBoolean.TRUE.equals(srvdto.getFg_skintest()) && srvdto.getSd_reltp()!=null && srvdto.getSd_reltp().equals(ICiDictCodeConst.CODE_SKIN_SKIN))
		{
			//在此重新为sd_reltp和id_reltp重新赋值，赋值为皮试医嘱关联的sd和id
			srvdto.setSd_reltp(ICiDictCodeConst.SD_RELTYPE_SKINTEST);
			srvdto.setId_reltp(ICiDictCodeConst.SD_RELTYPE_ID_SKINTEST);
	...
	private void setOrSrvMmModInfo(CiorderAggDO aggdo,OrdSrvDO srvdo,CiEmsDTO ems,int index,OrdSrvItemDatum rtn) throws BizException {
		...
		/*if(!CiOrdUtils.isEmpty(srvdto.getBdHpIndication())){
			...
			if(model==2){
				...
				if(StringUtils.isNotEmpty(bdHpIndication.getDes_hislimit())){
					ciorsrvhpdo.setDes_hplimit(ciorsrvhpdo.getDes_hplimit() +"\n 院内限制条件："+ StringUtils.trimToEmpty(bdHpIndication.getDes_hislimit()));
				}
			}else if(model==1){
...
//		CiOrdUtils.assignPriratToSrvDO(srvdto,srvdo);
		...
		srvdo.setNote_srv(srvdto.getDes_srv());
		srvdo.setId_srvca(srvdto.getId_srvca());
		if(FBoolean.TRUE.equals(srvdto.getFg_self())||FBoolean.TRUE.equals(srvdto.getFg_extdispense())) {// 是外配药则费用标识为N,2020/7/2,LIYUELONG
			srvdto.setFg_bl(FBoolean.FALSE);
		} else {
			srvdto.setFg_bl(FBoolean.TRUE);
		}
		srvdo.setFg_bl(CiOrdUtils.nullHandle(srvdto.getFg_bl()));
		srvdo.setCode_srv(srvdto.getCode_srv());
		srvdo.setEu_sourcemd(srvdto.getEu_sourcemd());
