...
public class OrdChangedSrvValidateBP {
	...
	public static String STR_SRV_BL_VAL = "【%s】费用项停用，不可开立";
	...
	public OrdChangedSrvValidateBP() throws BizException {
	...
	public FMap2 exec(OrdSrvChangedInfoDTO[] ordSrvChangedInfoDTOs, String code_entp) throws BizException {
		...
		judgeSrvActive();
		judgeSrvMMActive(srvMmMap, code_entp);
		judgeSrvSetActive();
		judgeSrvBL();
	...
	private void judgeSrvEntp(String code_entp) {
		...
		try {
			...
			for (String id_srv : new ArrayList<>(id_srvs)) {
				...
				//通用名模式下校验服务的适用就诊类型
				if (!((FBoolean.TRUE.equals(meSrvDO.getFg_mm()) && IBdMmParamValueConst.DRUGMCPROPMM_SRV.equals(drugManagementModel)) || !FBoolean.TRUE.equals(meSrvDO.getFg_mm()))) {
					continue;
				}
				...
				if (IBdFcDictCodeConst.SD_CODE_ENTP_OP.equals(code_entp) 
					...
					bEnable = true;
						...
						&& FBoolean.TRUE.equals(meSrvDO.getFg_use_pip())){// 预住院
					bEnable = true;
				}
	...
	protected void judgeSrvActive() throws BizException {
		...
		for (String id_srv : new ArrayList<>(id_srvs)) {
			...
			if (!FBoolean.TRUE.equals(meSrvDO.getFg_active())) {
				mapResult.put(id_srv, String.format(STR_SRV_ACTIVE_VAL+msg, meSrvDO.getName()));
				mapMeSrvDOs_Checked.remove(id_srv);
			}

			if (!FBoolean.TRUE.equals(meSrvDO.getFg_or())) {
					mapResult.put(id_srv, String.format(STR_SRV_OR_VAL+msg, meSrvDO.getName()));
					mapMeSrvDOs_Checked.remove(id_srv);
			}
		}
	}
	 ...
	 * 
	 * @param srvMmMap
	 *            服务物品关联集合key：id_srv value : id_mm 的List集合
	 * @param code_entp
	 *            就诊类型
	 * @throws BizException 
	 */
	private void judgeSrvMMActive(Map<String, List<String>> srvMmMap, String code_entp) throws BizException {

		if (null == mapMeSrvDOs_Checked || mapMeSrvDOs_Checked.size() <= 0 || srvMmMap.size() <= 0)
			return;
		String msg=getOpenItemMaintainerValidate();
		try {
			// 获取服务迭代器，构造服务id集合，根据id_srv集合获取物品信息
			Iterator<String> idSrvIterator  = mapMeSrvDOs_Checked.keySet().iterator();
			List<String> idSrvList = new ArrayList<String>();
			while (idSrvIterator.hasNext()) {
				idSrvList.add(idSrvIterator.next());
			}
			Map<String, List<MeterialDO>>  meterialMap = this.getMeterialMap(idSrvList);

			outer: for (String id_srv : idSrvList) {
				MedSrvDO meSrvDO = (MedSrvDO) mapMeSrvDOs_Checked.get(id_srv);

				if (FBoolean.TRUE.equals(meSrvDO.getFg_or()) && FBoolean.TRUE.equals(meSrvDO.getFg_mm())
						&& srvMmMap.containsKey(id_srv)) {

					// 执行绑定不对物品的停开，就诊类型是否匹配，关联物品信息是否正确进行校验
					if (ICiDictCodeConst.SD_MMBIND_EXECUTE.equals(getSd_mmbindType(code_entp, meSrvDO))) {
						continue outer;
					}

					List<MeterialDO> meterialList = meterialMap.get(id_srv);

					// 根据服务查询出对应物品的物品id集合，用于判断物品与服务关系是否有调整
					List<String> idMmList = new ArrayList<String>();

					for (MeterialDO meterial : meterialList) {

						idMmList.add(meterial.getId_mm());

						if (srvMmMap.get(id_srv).contains(meterial.getId_mm())) {
							// 物品停用
							if (!FBoolean.TRUE.equals(meterial.getFg_active())) {
								// 物品校验时对模板保存的校验，不是对服务基础数据的校验
								mapResult.put(id_srv + "," + meterial.getId_mm(),
										String.format(STR_SRV_MM_ACTIVE_VAL+msg, meSrvDO.getName(), meterial.getName()));
								continue;
							}

							// 物品停开
							if (FBoolean.TRUE.equals(meterial.getFg_out())) {
								// 物品校验时对模板保存的校验，不是对服务基础数据的校验
								mapResult.put(id_srv + "," + meterial.getId_mm(),
										String.format(STR_SRV_MM_OUT_VAL+msg, meSrvDO.getName(), meterial.getName()));
								continue;
							}
							//商品模式下校验物品的就诊类型
							if (IBdMmParamValueConst.DRUGMCPROPMM_DRUG.equals(drugManagementModel)) {
								judgeMmEntp(code_entp,id_srv,meSrvDO.getName(),meterial);
							}
						}
					}

					// 待校验数据中服务和物品关系
					List<String> waitValidateIdMmList = srvMmMap.get(id_srv);
					for (String id_mm : waitValidateIdMmList) {
						if (!idMmList.contains(id_mm)) {
							mapResult.put(id_srv + "," + id_mm, String.format(STR_SRV_MM_VAL+msg, meSrvDO.getName()));
						}
					}
				}
			}
	...
	private void judgeSrvSetActive() throws BizException {
		...
		String msg=getOpenItemMaintainerValidate();
		Set<String> id_srvs = mapMeSrvDOs_Checked.keySet();
		MedSrvSetItemDO[] allMedSrvSetItemDOs = this.getMedSrvSetItemDOs(new ArrayList<>(id_srvs));
		...
		outer: for (String id_srv : new ArrayList<>(id_srvs)) {
			MedSrvDO meSrvDO = (MedSrvDO) mapMeSrvDOs_Checked.get(id_srv);
			// 获取套内明细校验
			if (FBoolean.TRUE.equals(meSrvDO.getFg_or()) && FBoolean.TRUE.equals(meSrvDO.getFg_set())) {
						...
						for (MedSrvDO itmMeSrvDO : itmMeSrvDOs) {
							if (null == itmMeSrvDO.getFg_active() || !itmMeSrvDO.getFg_active().booleanValue()) {
								mapResult.put(id_srv,
										String.format(STR_SRV_SET_VAL+msg, meSrvDO.getName(), itmMeSrvDO.getName()));
								mapMeSrvDOs_Checked.remove(id_srv);
								continue outer;
							}
							//如果是组合定价时，判断明细项目的费用项是否启用
							if(IBdPrimdCodeConst.ID_PRI_SRV_COMP.equals(itmMeSrvDO.getId_primd())){
								msg = judgePriSrvComp(itmMeSrvDO.getId_srv());
								if(!StringUtils.isNullOrEmpty(msg)){
									mapResult.put(id_srv,
											String.format("【%s】套内明细【%s】,"+msg, meSrvDO.getName(), itmMeSrvDO.getName()));
									mapMeSrvDOs_Checked.remove(id_srv);
									continue outer;
								}
							}
