 ...
 *
 */
public class EmsDrugsValidate extends OpBaseEmsValidate {
	...
	public ErrorEmsList viewModelValidate(Object[] objEms, CiEnContextDTO ctx) throws BizException {
		...
		for (Object objInfo : objEms) {
			...
			if (emsdrugs == null || emssrvlist == null)
				return null;
	 ...
	 * @throws BizException
	 */
	private void checkSrvSkinLogic(EmsOrDrug emssrv, List<String> errorList, String id_pat, String id_org)
			throws BizException {
		if (!FBoolean.TRUE.equals(emssrv.getFg_skintest()))
			return;
		...
		if (piPatDO != null)
			param.setDt_birth(piPatDO.getDt_birth());
	...
	private void checkSrvFreqInfo(EmsOrDrug emsDTO, List<String> errors) {
		if (StringUtil.isEmpty(emsDTO.getId_freq()) || StringUtil.isEmpty(emsDTO.getName_freq())
	...
	private void checkSrvRouteInfo(EmsOrDrug emsDTO, List<String> errors) {
		if (StringUtil.isEmpty(emsDTO.getId_route()) || StringUtil.isEmpty(emsDTO.getName_route())) {
			errors.add("途径信息不完整");
		}
	}
	...
	private void checkMpInHospital(EmsDrugItemDO emsdrugs, List<String> errors) {
		if (FBoolean.TRUE.equals(emsdrugs.getFg_mp_in())
				&& (emsdrugs.getTimes_mp_in() == null || emsdrugs.getTimes_mp_in() <= 0)) {
	 ...
	 * @throws BizException
	 */
	private void checkDosageRouteInfo(List<String> dosageIds, List<String> errors, FArrayList srvList)
			throws BizException {
		...
		RouteDosageRefDO[] routeDosages = service
				.find(String.format("id_dosage in (%s)", ids.substring(0, ids.lastIndexOf(","))), "", FBoolean.FALSE);
		for (Object obj : srvList) {
			EmsOrDrug emsDTO = (EmsOrDrug) obj;
			List<RouteDosageRefDO> refsDo = new ArrayList<RouteDosageRefDO>();
			for (RouteDosageRefDO refdo : routeDosages) {
				if (refdo.getId_dosage().equals(emsDTO.getId_dosage())
						&& refdo.getId_route().equals(emsDTO.getId_route())) {
					refsDo.add(refdo);
				}
			}
			if (refsDo.size() == 0) {
