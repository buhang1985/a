...
public class OrSplitBP {
	 ...
	 * @param orsplitorder
	 * @return
	 * @version 2019年5月28日13:50:49 处理持续医嘱停止后，停止确认拆分医嘱拆分问题 fengjj
	 * @throws BizException
	 */
	private boolean specialCaseHandle(ArrayList<OrSplitOrderDTO> list, OrSplitOrderDTO orsplitorder, FDateTime[] validSE) throws BizException {
		...
		if (OrSrvSplitUtil.isAlwaysFreq1(orsplitorder.getSd_frequnit())) {
			...
			if (dts[0].getBeginDate().equals(orsplitorder.getDt_effe().getBeginDate())) {
				dts[0] = orsplitorder.getDt_effe();
			}
		...
		// 在院整领逻辑， 针对在院整领只拆分一次
		if (FBoolean.FALSE.equals((FBoolean) orsplitorder.getAttrVal("Fg_mp_in"))) {
			if (orsplitorder.getDt_effe().compareTo(validSE[0]) >= 0 && orsplitorder.getDt_effe().compareTo(validSE[1]) <= 0) {
				orsplitorder.setDt_mp_plan(orsplitorder.getDt_effe());
				list.add(orsplitorder);
				return true;
	...
	private void splitHandleByFreq(ArrayList<OrSplitOrderDTO> list, OrSplitOrderDTO orsplitorder, FDateTime[] validSE, Hashtable freqHt) throws BizException {
		...
		if (OrSrvSplitUtil.isTemporaryFreq(sd_frequnit)) {// 临时频次医嘱的处理逻辑
			...
			// 2017年12月20日18:56:47分钟间隔频次
			FreqDefDO freqDO = OrSrvSplitUtil.getFreqDefDO(id_freq);
			FDateTime[] dts = null;
			if (OrSrvSplitUtil.isApartFreq(freqDO))
				dts = getApartTimeDOs(orsplitorder.getDt_effe(), freqDO, validSE);// 隔分钟频次
			else
				dts = OrSrvSplitUtil.getDateArrayByMin(validSE[0], validSE[1], orsplitorder.getFrequnitcnt());// 只能是N分钟1次
			...
			FDateTime[] dts = null;
			if (OrSrvSplitUtil.isApartFreq(freqDO))
				dts = getApartTimeDOs(orsplitorder.getDt_effe(), freqDO, validSE);// 隔小时频次
			else
				dts = OrSrvSplitUtil.getDateArrayByHour(orsplitorder.getDt_effe(), validSE[0], validSE[1], orsplitorder.getFrequnitcnt());// 不会出现3小时2次的概念只能是N小时1次
			cloneAndAddToList(list, orsplitorder, dts);
			...
			if (DateTimeUtils.isSameDay(orsplitorder.getDt_effe(), orsplitorder.getDt_last_bl_or())) {
				FDateTime ent = new FDateTime(orsplitorder.getDt_last_bl_or().getDate().toLocalString() + " 00:00:00").addSeconds(24 * 3600);
				if (ent.compareTo(dt_effictive_end) > 0) {
					// 计算的首日拆分截止时间,与医嘱有效截止时间，取较小的一个
					// 确保首日在第一次全部拆分完
					if (orsplitorder.getDt_end().compareTo(ent) >= 0) {
						dt_effictive_end = ent;
					} else {
						dt_effictive_end = orsplitorder.getDt_end();
					}
				}
			}

			// 2017-09-15_xuxing 增加隔日执行逻辑
			FreqDefDO freqDO = OrSrvSplitUtil.getFreqDefDO(id_freq);
			FDateTime[] dts = null;
			if (OrSrvSplitUtil.isApartFreq(freqDO)) {
				dts = getApartTimeDOs(orsplitorder.getDt_effe(), freqDO, new FDateTime[] { validSE[0], dt_effictive_end });// 隔日执行频次
			} else {
				// 普通一天几次
