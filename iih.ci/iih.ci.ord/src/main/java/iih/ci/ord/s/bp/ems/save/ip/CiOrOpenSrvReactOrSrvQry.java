...
public class CiOrOpenSrvReactOrSrvQry implements ITransQry {
	...
	public String getQrySQLStr() {
		String sql= " select distinct A.id_or,A.Dt_Effe,A.Dt_End,B.Sd_Su_Mp,B.Id_Srv,B.Name as name_srv,A.Content_Or "
              +" from ci_order A "
              +" inner join ci_or_srv B on A.id_or=B.Id_Or "
              +" where A.Id_Pat=? and A.id_en=? "+getCond_IdOr()
              +"and A.ds=0  and A.Fg_Canc='N' and A.Fg_stop='N'  and (a.sd_su_or !='60' "   // or B.Sd_Su_Mp='1'  已执行的存在时  肯定可开新医嘱（原互斥执行医嘱护士逻辑执行完之后执行新医嘱！！）
              +" and (A.sd_srvtp like '1202%' or A.sd_srvtp like '1203%' or A.sd_srvtp like '1201%' or A.sd_srvtp like '04%')) and B.Id_Srv in (select id_srv from  bd_srv_react_itm where "+getCond_IdSrvReact()+") ";
		return sql;
	}
