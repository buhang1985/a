...
public class FireOpAgainNewEvent4IeBP extends AbstractOrSignListener {
	...
	private void fireEvent4Pharm(Hashtable<String, String> htors8ieevent, String ieEventType) throws BizException {
		...
		OrdBizLogger.info(ctx, "[FireOpAgainNewEvent4IeBP]药品医嘱重新分方集成平台事件逻辑开始");
		...
		BaseDTO[] msgdos = bp.exec(id_wc_ors, id_herb_ors,htors8ieevent.get(ICiIEEventConst.EVENT_IE_ID_ENT));
		...
		OrdBizLogger.info(ctx, String.format("药品医嘱重新分方集成平台事件完成,耗时:%s(ms)",System.currentTimeMillis()-startTime));
	}
