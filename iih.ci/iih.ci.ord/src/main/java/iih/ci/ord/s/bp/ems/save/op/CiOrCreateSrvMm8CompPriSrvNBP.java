...
public class CiOrCreateSrvMm8CompPriSrvNBP {
	...
	//	getMpDeptID(ordo,ems,setsrv,prisrvdo,null);  // 垃圾代码    2016 11-11 性能调优
		...
		FreqDefDO bdfreqInfo = BDFreqInfoCache.GetFreqInfo(ems.getId_freq());
		if(CiOrdUtils.isOpMode(ems.getCode_entp(), ems.getFg_pres_outp(), srvdo.getSd_srvtp()).booleanValue()
				|| (bdfreqInfo != null && bdfreqInfo.getSd_frequnitct() != null && IBdSrvDictCodeConst.SD_FREQNUNITCT_ONCE.equals(ems.getSd_frequnitct()))){
			...
			//	QuantumParamDTO quantum = new QuantumParamDTO(ems,setsrv);
