...
public class CiOrEmsSaveBP {
	...
	public  CiorderAggDO exec(CiEmsDTO ems)  throws BizException{
		...
		aggors=saveAggDON(ctx,oragginfo,skintestagginfos,null,ems.getEmstype());  //将替换上述一行代码
	 ...
	 * @throws BizException
	 */
	public  EmsRstDTO exec2(CiEmsDTO ems)  throws BizException{
		...
		if(aggors != null){
			if(aggors.length>1){
				...
				document.add(aggors[1].getParentDO());
			}else {
				document.add(aggors[0].getParentDO());
			}
		}else{
			document.add(new CiorderAggDO().getParentDO());
		}
		rst.setDocument(document);
	...
	private void handleSrvQuan(CiOrAggAndRelDatum oragginfo,CiEmsDTO ems) throws BizException{
		...
		if(ordo.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_RIS) 
				|| ordo.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_LIS) 
			    || ordo.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_NURSE)){
...
//					}
	 ...
	 * @throws BizException
	 */
	private CiorderAggDO[] saveAggDON(CiEnContextDTO ctx,CiOrAggAndRelDatum oragginfo,ArrayList<CiOrAggAndRelDatum> skintestagginfos,CiorderAggDO[] delskintests,Integer iemstp) throws BizException{
	...
	private ArrayList<CiOrAggAndRelDatum> getSkinTestOrInfos(CiOrAggAndRelDatum oragginfo) throws BizException{
		...
		//0186962: 治疗类需要计费的医嘱，费用不能为零-->0192960: 参数BLCG0040设置校验
		orPriValidate(oragginfo);
		//获得医嘱关联皮试医嘱数据信息
		Hashtable ht=oragginfo.getOrattachht();
	 ...
	 * @code 治疗类需要计费的医嘱，费用不能为零
	 * @param ems
	 */
	private void orPriValidate(CiOrAggAndRelDatum oragginfo) throws BizException {
