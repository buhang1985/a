...
public class CiOrSrvPriCalGetBP {
	...
	public Map<String,Map<String,SrvPricalRateAndPriceDTO>> exec(OrederPriCalParam[] pricalParams,String id_pripat) throws BizException{

		List<PriStdSrvDTO> srvSelfParamlist=new ArrayList<PriStdSrvDTO>();//本服务定价
		List<String> srvCompIdlist=new ArrayList<String>();//组合定价
		List<OrederPriCalParam> srvsetMemberSumIdlist=new ArrayList<OrederPriCalParam>();//套成员合计加
		List<OrederPriCalParam> srvsetMemCntAddIdlist=new ArrayList<OrederPriCalParam>();//套个数加收
		List<String> srvsetMemberCntIdlist=new ArrayList<String>();//套个数定价


		for( OrederPriCalParam param:pricalParams){
			...
			case IBdPrimdCodeConst.ID_PRI_SRV_COMP://组合定价
				...
				break;
			case IBdPrimdCodeConst.ID_PRI_SRVSET_AMOUNT://套成员合计加
		...
		if(srvsetMemberSumIdlist.size()>0){//套成员合计加
			...
			for(OrederPriCalParam param:srvsetMemberSumIdlist){
