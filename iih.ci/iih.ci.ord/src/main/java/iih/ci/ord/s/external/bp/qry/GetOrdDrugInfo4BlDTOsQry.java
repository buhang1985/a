...
public class GetOrdDrugInfo4BlDTOsQry implements ITransQry {
	...
	private FBoolean fg_cancel;

	public GetOrdDrugInfo4BlDTOsQry(String id_ent, String[] id_ors, String[] id_orsrvs,FBoolean fg_cancel, String... params) {
		this.id_ent = id_ent;
		this.id_ors = id_ors;
		this.id_orsrvs = id_orsrvs;
		this.fg_cancel = fg_cancel;
	}
...
//		}
	...
	public String getQrySQLStr() {
		...
		strSql.append("inner join ci_order ORD on ORD.id_or=ORSRV.id_or ");
		strSql.append("inner join ci_or_srv_mm ORSRVMM on ORSRVMM.id_orsrv=ORSRV.id_orsrv ");
		...
		String strWhere = "";
		if (!StringUtils.isNullOrEmpty(this.id_ent)){
			strWhere += " where ORD.id_en=? and ORSRV.fg_or='Y' and ORD.fg_sign='Y' and ORSRV.sd_srvtp like '01%' and ORD.sd_su_bl!=2 ";
			if(fg_cancel!=null){
				strWhere += " and ORD.fg_canc='N' ";
			}
		}
		if (id_ors != null && id_ors.length > 0){
			...
			strWhere += (SqlUtils.getInSqlByIds("ORD.id_or", this.id_ors));
		}
		if (id_orsrvs != null && id_orsrvs.length > 0){
			...
			strWhere += (SqlUtils.getInSqlByIds("ORSRV.id_orsrv", this.id_orsrvs));
		}
