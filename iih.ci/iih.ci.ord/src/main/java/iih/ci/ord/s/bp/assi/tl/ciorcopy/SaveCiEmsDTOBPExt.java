...
public class SaveCiEmsDTOBPExt {
	...
	public static MoreEmsParamDTO SaveCiEmsDTO(CiEnContextDTO envinfo, CiEmsDTO[] ciEms) throws BizException {
		...
		FArrayList errorCiEmsList = new FArrayList();
		String Errorinfo = "";
		int num = 1;
		if (ciEms != null && ciEms.length > 0) {
			...
			for (CiEmsDTO EmsDTO : ciEms) {
				try {
					...
					errorCiEmsList.add(EmsDTO); // 注意这行代码有错误：当两个医疗单发生相同的错误时候，最后一个发生错误的医疗单得到保存，前者会被覆盖
					continue;
				} finally {
		...
		// 错误信息
		int index =0;
		if(!CiOrdUtils.isEmpty(errorCiEmsList)){
			for(Object obj :errorCiEmsList){
				CiEmsDTO ems = (CiEmsDTO)obj;
				Errormap2.put(ems.getName()+"i"+index, ems);
				index++;
			}
