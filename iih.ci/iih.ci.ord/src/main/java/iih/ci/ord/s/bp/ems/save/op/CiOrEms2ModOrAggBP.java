...
/**
 * 根据临床Ems信息构造临床医嘱聚集数据操作BP 构造医疗单修改保存数据信息
 */
public class CiOrEms2ModOrAggBP {
	 ...
	 * @throws BizException
	 */
	public Hashtable exec(CiorderAggDO aggorder, CiEmsDTO ems, List<OrdSrvDO> orsetsrv) throws BizException {
		Hashtable ht = new Hashtable();
		long startTime = System.currentTimeMillis();
		// 有效性校验
		if (!isValidate(aggorder, ems))
			return null;
		...
		ciOrSrvInfoHandle(aggorder, orsrvs, ht, ems, orsetsrv);

		// 医嘱关联费用信息创建医嘱服务项目
		OrdSrvDO[] usgrelorsrvs = getCiOrRelUsgFeeOrSrvInfo(aggorder, ems, ht);
		ordSrvDOFreqMod(usgrelorsrvs, CiOrdUtils
				.getOrSrvSyncInfo(CiOrdUtils.getUsageRelFeeIdFreq(ciord), new FBoolean(false)));// 医嘱项目频次修正逻辑添加
																													// 2016-06-14
		...
		// 煎法要求关联费用创建医嘱服务项目
		ArrayList<OrdSrvDO> freList = boilRelationFee(aggorder, ems, orsrvs,ht);
		orsrvdos = merge(orsrvdos, freList);

		//药袋费    药品关联药袋费用派生费用创建医嘱服务项目
		ArrayList<OrdSrvDO> mmBagList = getMmbagSrvs(aggorder,ems,orsrvs,ht);
		orsrvdos=merge(orsrvdos,mmBagList);
		
		// 组合定价计价模式创建医嘱服务项目 2016-05-18新增逻辑分支
		ArrayList<OrdSrvDO> comppriorsrvs = getCiOrCompPriOrSrvs(aggorder, ems, orsrvdos, ht);
		ciOrSrvModFreq(comppriorsrvs, aggorder);
		orsrvdos = merge(orsrvdos, comppriorsrvs);

		orSrvDOSortNoHandle(ciord.getId_srv(),orsrvdos);
		if (!(orsrvdos == null || orsrvdos.length == 0)) {
			aggorder.setOrdSrvDO(orsrvdos);
		}
		CiOrdUtils.getlogger()
				.info("根据临床Ems信息构造临床医嘱聚集数据操作BP (CiOrEms2ModOrAggBP) 耗时" + (System.currentTimeMillis() - startTime));
		return ht;
	}

	private void ciOrSrvModFreq(ArrayList<OrdSrvDO> orsrvs, CiorderAggDO aggorder) {
		CiOrderDO ordo = aggorder.getParentDO();
		if (orsrvs != null) {
			for (OrdSrvDO ordSrvDO : orsrvs) {
				ordSrvDO.setId_freq(ordo.getId_freq());
			}
		}
	 ...
	 * @throws BizException
	 */
	private void ciOrSrvInfoHandle(CiorderAggDO aggdo, ArrayList<OrdSrvDO> orsrvs, Hashtable ht, CiEmsDTO ems,
			List<OrdSrvDO> orsetsrv) throws BizException {
		...
		if (!(srvsetitemindexs == null || srvsetitemindexs.length == 0)) {
			...
			MedSrvDO srvdo = CiOrdAppUtils.getMedsrvMDORService().findById(id_srv_set);
				...
				// 多剂量模式下 需要修改费用项目剂量
				if (FBoolean.TRUE.equals(srvdo.getIsmuldose()) || FBoolean.TRUE.equals(srvdo.getIsmulexec())) {
					CiEmsSrvDTO mainemssrv = (CiEmsSrvDTO) ems.getEmssrvs().get(0);
					if (CiOrdUtils.isSrvsetMemCntAdditionalPrimd(mainemssrv.getId_primd())) {

					} else if (CiOrdUtils.isSrvsetMemberCntPrimd(mainemssrv.getId_primd())) {

					} else {
						for (Object ciemssrv : ems.getEmssrvs()) {
							CiEmsSrvDTO ciemssrvdto = (CiEmsSrvDTO) ciemssrv;
							if (ciemssrvdto.getFg_or().equals(FBoolean.FALSE)
									&& ciemssrvdto.getFg_bl().equals(FBoolean.TRUE)) {
								if (srvdo.getIsmuldose().equals(FBoolean.TRUE)) {
									if(ciemssrvdto.getQuan_med() != null && ciord.getQuan_medu() != null){
										ciemssrvdto.setQuan_med(ciord.getQuan_medu().multiply(ciemssrvdto.getQuan_med()));
									}else{
										ciemssrvdto.setQuan_med(ciord.getQuan_medu());
									}
									ciemssrvdto.setQuan_total_medu(null);
								}
								if (srvdo.getIsmulexec().equals(FBoolean.TRUE)) {
									ciemssrvdto.setId_freq(ciord.getId_freq());
								}
								ciemssrvdto.setStatus(ems.getStatus());
							}
						}
					}
			...
			if (IBdSrvDictCodeConst.SD_SETORD_ORMODE.equals(srvdo.getSd_setord())) { // 医嘱模式
				ciOrSrvInfoHandleForOrMode(aggdo, orsrvs, ht, ems, srvsetitemindexs, srvdo, orsetsrv);
		...
		// 合并处理
		if (!(unsetsrvs == null || unsetsrvs.size() == 0)) {
			orsrvs.addAll(unsetsrvs);
		}
	}
	 ...
	 * @throws BizException
	 */
	private ArrayList<OrdSrvDO> getSrvMmInfo(CiorderAggDO aggdo, CiEmsDTO ems, Integer[] srvsetitemindexs, Hashtable ht)
			throws BizException {
		// 空校验
		...
		MedSrvSetItemDO[] bdsrvinset = CiOrdUtils.getBdSrvSetItems(srvdtos, srvsetitemindexs, FBoolean.FALSE);// 获得
																												// 非clinical服务项
		String id_srv = "";
		...
		for (int i = 0; i < srvdtos.size(); i++) {
			...
			if (CiOrdUtils.isInStr(CiOrdUtils.COMMA_STR + Integer.toString(i) + CiOrdUtils.COMMA_STR, matchstr)) {
				...
				if (!CiOrdUtils.isFeeItm4Set(setitmsrv, bdsrvinset, srvdtos, srvsetitemindexs)) {
					continue;
				} // 2016-08-31 新增判断逻辑
			}
			...
			if (datum.getSrvdo() == null) {
				...
				if (!CiOrdUtils.isEmpty(orssrvtmp)) {
					list.add(orssrvtmp);
				}
				continue;
			}
			...
			// 代办人的处理修改为，在签署的时候录入
			// 医嘱项目对应代办人记录处理
			// CiOrAttachInfoUtils.addOrSrvAgentInfoDO(ht, id_srv,
			// datum.getSrvagent());
			...
			// 医保多价格比例报销id
			if (!CiOrdUtils.isEmpty(datum.getCiorsrvhpdo()))
				orSrvHpList.add(datum.getCiorsrvhpdo());
		}
	...
	private OrdSrvDO[] getCiOrRelUsgFeeOrSrvInfo(CiorderAggDO aggdo, CiEmsDTO ems, Hashtable ht) throws BizException {
		if (CiOrdUtils.isHasPriSrvHandled(ems).booleanValue())
			return null;// 前台是否已经算过费用项目
		// 获得医嘱关联费用服务数据
		...
		if (!CiOrdUtils
				.isEmsRelSrvAllDeleted(ems.getEmssrvs(),
						aggdo.getParentDO().getId_srv())
				&& EmsHelper.isEmsSrvsGenerated(ems, aggdo.getParentDO().getId_srv(),
						new Integer[] { OrSrvSourceFromEnum.USAGERELFEE, OrSrvSourceFromEnum.BOILRELFEE,
								OrSrvSourceFromEnum.SPECIMENRELFEE, OrSrvSourceFromEnum.RELSRVTACTIC },
						null)) {
		...
		//赋值用法和煎法名称
		aggdo.getParentDO().setRoute_name(ems.getName_route());
		aggdo.getParentDO().setBoil_name(ems.getName_boil());
	 ...
	 * @param orsrvdos
	 */
	private void orSrvDOSortNoHandle(String id_srv,OrdSrvDO[] orsrvdos){
		if(CiOrdUtils.isEmpty(id_srv)) {
			CiOrdUtils.doSortNoHandle(orsrvdos, OrdSrvDO.SORTNO);
		}else {
			CiOrdUtils.rankSortno(id_srv, orsrvdos);
		}
	}
	/**
	 * 生成并设置医嘱项目信息处理（医嘱模式）
	 ...
	 * @throws BizException
	 */
	private void ciOrSrvInfoHandleForOrMode(CiorderAggDO aggdo, ArrayList<OrdSrvDO> rtn, Hashtable ht, CiEmsDTO ems,
			Integer[] srvsetitemindexs, MedSrvDO srvdo, List<OrdSrvDO> orsetsrv) throws BizException {
		// 套医嘱开立模式时套内服务项目的医嘱服务处理逻辑
		ArrayList<OrdSrvDO> setrelsrvs = srvSetRelSrvHandle4OrMode(aggdo, ems, srvsetitemindexs, rtn, srvdo, ht,
				orsetsrv);
		if (!(setrelsrvs == null || setrelsrvs.size() == 0)) {
			rtn.addAll(setrelsrvs);
		}
		// 医嘱相关信息套相关信息修改
	 ...
	 * @throws BizException
	 */
	private ArrayList<OrdSrvDO> srvSetRelSrvHandle4OrMode(CiorderAggDO aggdo, CiEmsDTO ems, Integer[] srvsetitemindexs,
			ArrayList<OrdSrvDO> rtn, MedSrvDO setsrvdo, Hashtable ht, List<OrdSrvDO> orsetsrv) throws BizException {
		CiOrSrvSetHandle4OrModeBP bp = new CiOrSrvSetHandle4OrModeBP();
		return bp.exec(aggdo, ems, srvsetitemindexs, setsrvdo, ht, orsetsrv);
	}
	 ...
	 * @throws BizException
	 */
	private ArrayList<OrdSrvDO> getCiOrCompPriOrSrvs(CiorderAggDO aggdo, CiEmsDTO ems, OrdSrvDO[] orsrvdos,
		...
		for (int i = 0; i < ems.getEmssrvs().size(); i++) {
			...
			if (!isSrvCompPriMd(emssrvdto) || isSetOrSrv(ems, emssrvdto))
				continue;
			if (emssrvdto.getStatus() == DOStatus.DELETED) {// 2011-11-15新增组合计价服务删除时的逻辑
															// 这里有一个漏洞
															// （一个医嘱有多个相同的组合计价项目时）
			...
			// if(EmsHelper.isEmsSrvsGenerated(ems, emssrvdto.getId_srv(), new
			// Integer[]{OrSrvSourceFromEnum.AGENTFROMCOMPPRIMD},null)){
			// 2016-11-15 新增下面这个判断逻辑 不过这个UI处理的假设前提不是特别的好 暂时先增加这个判断逻辑
			// if(emssrvdto.getStatus()==DOStatus.UPDATED &&
			// CiOrdUtils.isEmsRelSrvAllDeleted(ems.getEmssrvs(),emssrvdto.getId_srv())){
			// 这个情况应该是需要继续下去的
			// }else{
			// continue;
			// }
	 ...
	 * @throws BizException
	 */
	private ArrayList<OrdSrvDO> getSrvMm8CompPriSrv(CiorderAggDO aggdo, CiEmsDTO ems, Integer ipos, Hashtable ht)
			throws BizException {
	...
	private boolean isSetOrSrv(CiEmsDTO ems, CiEmsSrvDTO emssrvdto) throws BizException {
		...
		if (!CiOrdUtils.isEmpty(ems.getFg_set()) && ems.getFg_set().booleanValue()) {
			...
			if (CiOrdUtils.isSrvsetMemberCntPrimd(srvdo.getId_primd())
					|| CiOrdUtils.isSrvsetMemCntAdditionalPrimd(srvdo.getId_primd())) {
				if (id_srv_set.equals(emssrvdto.getId_srv_set()) && !CiOrdUtils.isEmpty(emssrvdto.getFg_or())
						&& emssrvdto.getFg_or().booleanValue()) {
					return true;
				}
			}
		}
	 ...
	 * @throws BizException
	 */
	private OrdSrvDO[] getDelOrSrvInfos(CiorderAggDO aggdo, UsageRelFeeSrvDO[] relfeesrvdos, Hashtable ht)
			throws BizException {
	 ...
	 * @throws BizException
	 */
	private ArrayList<OrdSrvDO> boilRelationFee(CiorderAggDO aggorder, CiEmsDTO ems, ArrayList<OrdSrvDO> orsrvs,Hashtable ht)
			throws BizException {
		CiOrHerbBoilDesRelationFeeBP bp = new CiOrHerbBoilDesRelationFeeBP();
		return bp.exec(aggorder, ems, orsrvs,ht);
