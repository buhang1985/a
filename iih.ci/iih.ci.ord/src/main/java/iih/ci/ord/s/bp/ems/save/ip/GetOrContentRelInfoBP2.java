...
/**
 * @author yzh 获得医嘱内容相关信息 code name content srvtp等
 */
public class GetOrContentRelInfoBP2 {
	 ...
	 * @throws BizException
	 */
	public CiOrIdentifcationInfo exec(CiEmsDTO ems, CiOrderDO order, OrdSrvDO[] ordsrvs, FMap mmMap)
			throws BizException {
	...
	private String getOrContent(CiEmsDTO ems, OrdSrvDO[] ordsrvs, FMap mmMap) throws BizException {
		...
		if (CiOrdUtils.isEmpty(srvlist)||ordsrvs==null)
			return null;
		for (OrdSrvDO ordSrvDO : ordsrvs) {
			...
			for (Object object : srvlist) {
				...
				if (ciemssrv.getId_srv().equals(ordSrvDO.getId_srv())) {
					if (!FBoolean.TRUE.equals(ciemssrv.getFg_self()) && ciemssrv.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_DRUG)
							&& !ciemssrv.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_HERBDRUG)) {
						...
						if (ordSrvDO.getPri() != null) {
							ciemssrv.setTotal_pri(
									new FDouble(CiOrdUtils.getPrettyNumber(ordSrvDO.getPri().toString())));
						}
					}
					continue;
				} else {
...
//		return CiOrdAppUtils.getIBillcodeManager().getPreBillCode_RequiresNew(CiOrderDODesc.CLASS_FULLNAME);
		/*
		 * String code=""; code=DateUtils.getDateTimeStr(new
		 * Date(),"yyyyMMddHHssSSS"); //暂时先这样吧
		 * //code=CiOrdUtils.getBillCode(CiOrderDODesc.CLASS_FULLNAME);
		 * //配置编码规则后可取该逻辑 return ICiDictCodeConst.IIH+code+getRandom();
		 */
	}
	...
	private OrdSrvDO getCiEmsSrv(CiOrderDO order, OrdSrvDO[] ordsrvs) {
		if (CiOrdUtils.isEmpty(ordsrvs))
			return null;
		for (OrdSrvDO ordSrvDO : ordsrvs) {
			if (order.getId_srv().equals(ordSrvDO.getId_srv()))
				return ordSrvDO;
		}
		return null;
	}
