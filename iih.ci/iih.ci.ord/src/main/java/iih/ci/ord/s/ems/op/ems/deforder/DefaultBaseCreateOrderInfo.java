...
//				CiOrdAppUtils.getIBillcodeManager().getPreBillCode_RequiresNew(CiOrderDODesc.CLASS_FULLNAME));
	 ...
	 * @throws BizException
	 */
	protected OrdSrvDO assembleOrderSrvInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, BdSrvMmInfo bdSrvMmInfo, int index)
			throws BizException {

		OrdSrvDO srvdo = new OrdSrvDO();
		OrWfDeptInfoDTO orWfDeptInfoDTO = DeptInfoUtils.GetOrWfDeptInfo(ctx, bdSrvMmInfo.getBdSrvInfo().getId_srv(),bdSrvMmInfo.getMmInfo()==null?"":bdSrvMmInfo.getMmInfo().getId_mm(),bdSrvMmInfo.getBdSrvInfo().getId_route(), null,orderInfo.getFg_long(),orderInfo.getFg_pres_outp(),CiOrdAppUtils.getServerDateTime());
		...
		srvdo.setQuan_total_medu(
				this.getQuantumUtils().getUnMMQuantum(orderInfo.getDt_effe(),srvdo.getId_freq(), nUseDays, srvdo.getQuan_medu()));// 计算服务总量
		...
		if (!CiOrdUtils.isEmpty(srvdo.getId_srv()) && !srvdo.getId_srv().equals(orderInfo.getId_srv()))
	 ...
	 * @throws BizException
	 */
	protected OrdSrvMmDO assembleSrvMmInfo(CiEnContextDTO ctx, OrdSrvDO orderSrvInfo, MeterialDO mmdo,
			MedSrvDrugDO medSrvDrugDO) throws BizException {
		if (mmdo == null)
			return null;
	 ...
	 * @throws BizException
	 */
	protected OrdSrvMmDO assembleSrvMmInfo(OrdSrvDO orderSrvInfo, MedSrvDrugDO drugdo, MeterialDO mmdo,
		...
		// srvmmdo.setQuan_cur(quantumBP.geteUnMMQuantum(orderSrvInfo.getId_freq(),
		// orderSrvInfo.getUse_days(), ems.getQuan_med()));
	 ...
	 * @throws BizException
	 */
	protected DefaultBaseCreateOrderInfo handleMpWhDeptInfo(CiEnContextDTO ctx, String id_dep_main, OrdSrvDO srvInfo,
			String id_mm,FBoolean isLong) throws BizException {
	 ...
	 * @throws BizException
	 */
	protected DefaultBaseCreateOrderInfo handleSrvMmInfo(CiEnContextDTO ctx, OrderPackageInfo orderPakageInfo,
		...
		MedSrvDrugDO[] szMedSrvDrugDO = ServiceFinder.find(IMedSrvDrugDORService.class).findByAttrValStrings("Id_srv",
				idSrvList.asArray());
		...
		for (OrderSrvMmInfo orderSrvMmInfo : listOrdSrvMmInfo) {
			orderSrvMmInfo.setOrderSrvMmInfo(assembleSrvMmInfo(ctx, orderSrvMmInfo.getOrderSrvInfo(),
	 ...
	 *            基础数据bd_srv数组
	 * @param eu_sourcemd
	 *            医嘱服务来源 OrSrvSourceFromEnum
	 * @return
	 * @throws BizException
	 */
	protected OrderSrvMmList createOrderSrvMmList(CiEnContextDTO ctx, CiOrderDO orderInfo, MedSrvDO[] szMedSrvInfo,
			int eu_sourcemd) throws BizException {
	 ...
	 *            基础数据bd_srv数据对象
	 * @param eu_sourcemd
	 *            服务来源 OrSrvSourceFromEnum
	 * @param index
	 *            排序号
	 * @return
	 * @throws BizException
	 */
	protected OrderSrvMmInfo createOrderSrvMmInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, MedSrvDO srvInfo,
			int eu_sourcemd, int index) throws BizException {
		BdSrvMmInfo bdSrvMmInfo = new BdSrvMmInfo(srvInfo);
		OrderSrvMmInfo ordSrvMmInfo = new OrderSrvMmInfo(assembleOrderSrvInfo(ctx, orderInfo, bdSrvMmInfo, index),
				null);
		ordSrvMmInfo.getOrderSrvInfo().setEu_sourcemd(eu_sourcemd); // 服务来源
		if (CiOrdUtils.isTrue(ordSrvMmInfo.getOrderSrvInfo().getFg_mm())
				&& bdSrvMmInfo.createSrvMmInfo(ctx.getId_org(), ctx.getId_dep_or())) {
			...
			ordSrvMmInfo.setOrderSrvMmInfo(assembleSrvMmInfo(ctx, ordSrvMmInfo.getOrderSrvInfo(),
					bdSrvMmInfo.getMmInfo(), bdSrvMmInfo.getSrvDrugInfo()));
		}
		return ordSrvMmInfo;
	}
	...
	@Override
	public OrderSrvMmInfo createOrderSrvMmInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, MedSrvDO srvInfo,
			int eu_sourcemd) throws BizException {
		// TODO Auto-generated method stub
		return null;
	}
	...
	@Override
	public OrderSrvMmInfo createOrderSrvMmInfo(CiEnContextDTO ctx, CiOrderDO orderInfo, String id_srv, String id_mm,
			int eu_sourcemd) throws BizException {
		// TODO Auto-generated method stub
		return null;
	}
