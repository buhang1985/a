...
public class OrSrvSplitUtil {
	...
	public static boolean isDtValidate(FDateTime dtstart, FDateTime dtend, FDateTime dt) {
		...
		if (day0 >= 0 && day1 > 0)//2018年6月5日17:40:12，记头不记尾
			return true;
		return false;
	}
	...
	public static FreqDefDO getFreqDefDO(String id_freq) throws BizException {
		...
		if(fredo == null) return null;
		//持续性和计划性频次不需校验周期数
		if (StringUtils.isNotEmpty(fredo.getSd_frequnitct()) && (IBdSrvDictCodeConst.SD_FREQNUNITCT_ALWAYS.equals(fredo.getSd_frequnitct()) || IBdSrvDictCodeConst.SD_FREQNUNITCT_PLAN.equals(fredo.getSd_frequnitct())))
			return fredo;
		if (fredo.getFrequnitct() == null || fredo.getFrequnitct() < 1) 
			throw new BizException("频次【" + fredo.getName() + "】周期内次数【" + fredo.getFrequnitct() + "】非法！");
		return fredo;
	}
...
//			return true;
	 ...
	 * @throws BizException
	 */
	public static boolean isPeriodMinFreq(String sd_frequnit) throws BizException {
		if (isEmpty(sd_frequnit))
			return false;
		if (IBdSrvDictCodeConst.SD_FREQNUNITCT_MINUTE.equals(sd_frequnit))
			return true;
		return false;
	}
	 ...
	 * @return
	 */
	public static boolean isApartFreq(FreqDefDO freqDO){
//		if (freqDO.getFrequnitct()>1) {
//			return true;
//		}
		if (FBoolean.TRUE.equals(freqDO.getFg_interval())) {
			return true;
		}
		return false;
	}
	 ...
	 * @return
	 */
	public static FDateTime[] getDateArrayByMin(FDateTime dstart, FDateTime dtend, int iperiods) {
		int mins = FDateTime.getMinutesBetween(dstart, dtend);
		int[] divremains = getDivAndRemain(mins, iperiods);//0：取整值，1：余数值
		if (divremains[0] == 0)
			return null;
		FDateTime[] rtns = new FDateTime[divremains[0]];
		for (int i = 1; i <= divremains[0]; i++) {
			rtns[i-1]=dstart.addSeconds(i*iperiods*60);
		}
		return rtns;
	}
	 ...
	 * @return
	 */
	public static FDateTime[] getDateArrayByHour(FDateTime dt_effe,FDateTime dstart, FDateTime dtend, int iperiods) {		
		List<FDateTime> rtns = new ArrayList<FDateTime>();
		// 游标，以医嘱生效时间为起点
		// 此逻辑目前是记尾不计头
		FDateTime dt_plan = (FDateTime) dt_effe.clone();
		while (dtend.compareTo(dt_plan) >= 0) {
			// 依次增加周期数
			dt_plan = DateUtils.getDateTimeAfter(dt_plan, iperiods);
			// 校验时间点是否在指定请求的区间
			if (isDtValidate(dstart, dtend, dt_plan))
				rtns.add(dt_plan);
		}
		if (rtns.size() > 0)
			return rtns.toArray(new FDateTime[0]);
		return null;

		//原有逻辑，每次以dt_last_bl为起点计算执行点，dt_last_bl是动态变化的，导致遗漏执行点
