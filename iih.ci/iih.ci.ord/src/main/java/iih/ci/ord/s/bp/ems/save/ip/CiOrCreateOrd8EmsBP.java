...
public class CiOrCreateOrd8EmsBP {
	 ...
	 * @throws BizException
	 */
	public void exec(CiorderAggDO aggdo, CiEmsDTO ems, Hashtable ht) throws BizException {
		if (ems == null)
			return;
		setCommonData();
		if (CiOrdUtils.isDONew(aggdo.getParentDO())) {// 新增处理
    		 // 预住院导入特殊处理
            if (ems.getEu_orsrcmdtp() != null && (OrSourceFromEnum.IIHPREIPAPP.equals(ems.getEu_orsrcmdtp())||OrSourceFromEnum.IIHORRELCONTP.equals(ems.getEu_orsrcmdtp())||OrSourceFromEnum.IIHORRELSAMPPRI.equals(ems.getEu_orsrcmdtp())||OrSourceFromEnum.IIHORRELAUTOGEN.equals(ems.getEu_orsrcmdtp()))) {
                curtime = CiOrdAppUtils.getServerDateTime();
                depid = ems.getId_dep_phy(); // 科室id
                empid = ems.getId_emp_phy(); // 人员id
            }
	...
	private String SetNameOR(CiEmsDTO ems) {
		...
		if (ems != null && ems.getEmssrvs() != null && ems.getEmssrvs().size() > 0) {
			if (ems.getSd_srvtp().equals(IBdSrvDictCodeConst.SD_SRVTP_WESTDRUG_ZX)
					|| ems.getSd_srvtp().equals(IBdSrvDictCodeConst.SD_SRVTP_WESTDRUG_DSY)) {
				FArrayList list = ems.getEmssrvs();
				for (int i = 0; i < list.size(); i++) {
					CiEmsSrvDTO srv = (CiEmsSrvDTO) list.get(i);
					name_or += "," + srv.getName_srv();
				}
			} else if (ems.getSd_srvtp().equals(IBdSrvDictCodeConst.SD_SRVTP_HERBDRUG)) {
				name_or = ",草药";
			} else {
	 ...
	 * @throws BizException
	 */
	private void createOrderDOInfo(CiorderAggDO aggdo, CiEmsDTO ems, Hashtable ht) throws BizException {
		CiEnContextDTO context = (CiEnContextDTO) Context.get().getAttribute("CiEnContextDTO");
		if(null==ems.getFg_urgent()){//2020-01-14liyuelong,医嘱模板，服务分类，就诊历史开立时，不经过医疗单，加急标志由允许加急和默认加急标识决定
			ems.setFg_urgent(CiOrdUtils.getFgUrgent(ems.getId_srv(), ems.getCode_entp()));
		}
		CiOrIdentifcationInfo oridrelinfo = getOrIdRelInfo(ems);
		CiEmsSrvDTO srvdto = (CiEmsSrvDTO) ems.getEmssrvs().get(0);
		CiOrderDO ordo = new CiOrderDO();
		// ordo.setId_or(ems.getId_or());
		// 草药是否院内制剂 by yzh 2018年4月10日 10:00:24
		ordo.setFg_hospital_herbpres(ems.getFg_hospital_herbpres());
		...
		ordo.setEu_hpjudge(ems.getEu_hpjudge());
		ordo.setEu_orhp(ems.getEu_orhp());
		...
		if (StringUtils.isEmpty(ordo.getCode_or())) {
		...
		// 护嘱，存在一个bug，医生有妇科一和妇科二两个权限。患者是妇科二，通过模板开立后，开单科室是妇科一
		// 无法定位原因，所以，护嘱保存时，开单科室重新查一遍
		if (ICiDoctorStationTypeConst.Type_Ip_NurDoctor_Station.equals(context.getStationType())) {
			ordo.setId_dep_or(CiOrdUtils.getIdDepNur(ems.getId_en()));
		} else {
			ordo.setId_dep_or(this.depid);
		}
		ordo.setId_dep_nur_or(context.getId_dep_ns());
		 ...
		 */
		if (ems.getFg_long() != null && ems.getFg_long().booleanValue() && ems.getDt_end() != null
		...
		// 20181008，张万青与黄军昊讨论后，发现，dt_last_bl应该直接等于开始时间dt_effe，不用计算。因为，dt_last_bl为最后拆分时间，开立时等于开始时间即可
		// ordo.setDt_last_bl(getLastDt(srvdto.getId_freq(), ordo.getDt_effe(),
		// ems.getTimes_firday_mp(), ordo.getFg_long()));
		//20190118，张万青与徐星讨论后，发现，dt_last_bl受首日执行次数的影响。当首日执行次数医生没有修改或者医生+1，此时不用计算dt_last_bl,直接等于dt_effe即可；
		//当首日执行次数医生修改减少时，需要计算，例如：tid，当前时间为10:28分，正常首日为2次。医生改为1时，dt_last_bl应该为16:00
		if(CiOrdUtils.isIpWf(ordo.getCode_entp())&&FBoolean.TRUE.equals(ordo.getFg_pres_outp())){
			ordo.setDt_last_bl(ordo.getDt_effe());
		}else{
			ordo.setDt_last_bl(getLastDt(srvdto.getId_freq(), ordo.getDt_effe(),ems.getTimes_firday_mp(), ordo.getFg_long()));
		}
		...
		if (ems.getId_dep_mp() == null) {// 2016-07-28 新执行科室逻辑调整
											// 调整到新的时将如下语句进行对应处理
		...
		ems.setId_dep_mp(ordo.getId_dep_mp());// 2016-10-27 新增逻辑 主要是为传递执行科室用
												// 应该不会引起大的变化
		ems.setId_dep_mp(ordo.getId_dep_mp());// 2016-10-27 新增逻辑 主要是为传递执行科室用
												// 应该不会引起大的变化
		ordo.setFg_urgent(ems.getFg_urgent());// 医嘱的加急标志2016-06-30张万青
		ordo.setFg_infect_dep_cons(ems.getFg_infect_dep_cons());// 是否感染科会诊2018-09-27
		...
		if (!CiOrdUtils.isEmpty(ems.getOrapplysheet())) {// 申请单处理逻辑
			...
			if(EmsType.CONS.equals(ems.getEmstype())&&ems.getOrapplysheet()!=null && !ems.getOrapplysheet().isEmpty()){
				Object[] os = ems.getOrapplysheet().values().toArray();
				if (!(os == null || os.length == 0)) {
					CiorappconsultAggDO apConsAgg = (CiorappconsultAggDO)os[0];
					if(apConsAgg != null) {
						apConsAgg.getParentDO().setNo_applyform(CiOrdUtils.GetConsApplyNo());
						ordo.setApplyno(apConsAgg.getParentDO().getNo_applyform());
					}
				}
			}
		}
		...
		ordo.setId_excessive_reason(ems.getId_excessive_reason());// 药品超量开单原因
		ordo.setSd_excessive_reason(ems.getSd_excessive_reason());// 药品超量开单原因
		ordo.setMore_money_reason(ems.getMore_money_reason());// 超额原因
		ordo.setName_dripspeed(ems.getName_dripspeed());
		ordo.setId_dripspeed(ems.getId_dripspeed());// 滴速
		if (StringUtils.isBlank(ems.getId_orsrcfun())) {
			ordo.setId_orsrcfun(ICiDictCodeConst.ID_ORDSRCFUN_DOC);
			ordo.setSd_orsrcfun(ICiDictCodeConst.SD_ORDSRCFUN_DOC);
		} else {
			ordo.setId_orsrcfun(ems.getId_orsrcfun());
			ordo.setSd_orsrcfun(ems.getSd_orsrcfun());
		}
		ordo.setIsmuldose(ems.getIsmuldose());
		ordo.setIsmulexec(ems.getIsmulexec());
		ordo.setSd_totalopenmode(ems.getSd_totalopenmode());
		//在院整领标识和领药方式
		ordo.setFg_wholepack(ems.getFg_wholepack());
		ordo.setId_drugdeliverymethod(ems.getId_drugdeliverymethod());
		ordo.setSd_drugdeliverymethod(ems.getSd_drugdeliverymethod());
		ordo.setName_drugdeliverymethod(ems.getName_drugdeliverymethod());
		if(ICiDictCodeConst.SD_DRUG_DELIVERY_METHOD_PRES_BACK.equals(ems.getSd_drugdeliverymethod())){
			ordo.setFg_back(FBoolean.TRUE);// 退药标识
			if(StringUtils.isNotEmpty(srvdto.getId_or_src())){
				ordo.setId_or_src(srvdto.getId_or_src());//退药关联原医嘱ID
			}
		}
	...
	private void orAppSheetHandle(FMap appsheet, int iemstp, Hashtable ht) {
		...
		if (os == null || os.length == 0)
			return;
		CiOrAttachInfoUtils.addOrAppDatumObj(ht, iemstp, appsheet.values().toArray()[0]);
	}
	 ...
	 * @throws BizException
	 */
	private void setOrderDOInfo(CiorderAggDO aggdo, CiEmsDTO ems, Hashtable ht) throws BizException {
		...
		ordo.setId_dep_mp(ems.getId_dep_mp());
		ordo.setFg_urgent(ems.getFg_urgent());// 医嘱的加急标志2016-06-30张万青
		ordo.setFg_infect_dep_cons(ems.getFg_infect_dep_cons());// 医嘱的加急标志2016-06-30张万青
		ordo.setId_unit_med(ems.getId_unit_med());// 2016-07-21 新增 以下三个属性（含本条）
		ordo.setQuan_medu(ems.getQuan_medu());
		ordo.setId_orpltp(getOrCLoopTpId(ordo));
		...
		ordo.setId_excessive_reason(ems.getId_excessive_reason());// 药品超量开单原因
		ordo.setSd_excessive_reason(ems.getSd_excessive_reason());// 药品超量开单原因
		ordo.setMore_money_reason(ems.getMore_money_reason());// 超额开单原因
		ordo.setName_dripspeed(ems.getName_dripspeed());
		ordo.setId_dripspeed(ems.getId_dripspeed());// 滴速    LiYue 19-04-12
		ordo.setFg_wholepack(ems.getFg_wholepack());
		ordo.setId_drugdeliverymethod(ems.getId_drugdeliverymethod());
		ordo.setSd_drugdeliverymethod(ems.getSd_drugdeliverymethod());
		ordo.setName_drugdeliverymethod(ems.getName_drugdeliverymethod());
		ordo.setPurpose_or(ems.getPurpose_or());//中医治法
		ordo.setId_didef(ems.getId_didef());
		ordo.setName_didef(ems.getName_didef());
	 ...
	 * @throws BizException
	 */
	private CiOrIdentifcationInfo getOrIdRelInfo(CiEmsDTO ems) throws BizException {
	 ...
	 * @throws BizException
	 */
	private void ordFreqExTimeHandle(CiorderAggDO aggdo, FArrayList orfreqextimes) throws BizException {
		// CiOrFreqTimeDTO
	 ...
	 * @throws BizException
	 */
	private void ordFreqExTimeHandle4Mod(CiorderAggDO aggdo, FArrayList orfreqextimes) throws BizException {
		OrdFreqTimeDO[] orfreqtimedos = aggdo.getOrdFreqTimeDO();
		if (CiOrdUtils.isEmpty(orfreqextimes) && CiOrdUtils.isEmpty(orfreqtimedos))
			return;
	...
	private ExeDeptCalParamDTO getExeDeptCalParamDTO(CiOrderDO ordo, CiEmsDTO ems) {
		...
		if (ordo.getFg_long() == null) {
			// 全部计算
			paramdto.setRecurstr(String.valueOf(OrRecurTypeEnum.NULL));
		} else if (ordo.getFg_long().booleanValue()) {
			// 长期
