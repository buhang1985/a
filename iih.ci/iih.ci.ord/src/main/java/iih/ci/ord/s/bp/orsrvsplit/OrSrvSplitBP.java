...
public class OrSrvSplitBP {
	...
	private boolean specialCaseHandle(ArrayList<SrvSplitOrderDTO> list, SrvSplitOrderDTO srvsplitorder, FDateTime[] validSE, boolean isLogicSplit) throws BizException {
		...
		if (isLogicSplit) {
			...
			// 在院整领逻辑
			if (FBoolean.FALSE.equals((FBoolean) srvsplitorder.getAttrVal("Fg_mp_in"))) {
				srvsplitorder.setDt_mp_plan(null);
				list.add(srvsplitorder);
				return true;
			}
			...
			// 持续逻辑
			// 持续逻辑修改，之前持续医嘱停止确认的时候不进拆分 2019年5月29日12:05:11
			if (OrSrvSplitUtil.isAlwaysFreq1(srvsplitorder.getSd_frequnit())) {
				FDateTime[] dts = OrSrvSplitUtil.getDateArray(validSE[0], validSE[1]);
				if (dts[0].getBeginDate().equals(srvsplitorder.getDt_effe().getBeginDate())) { // 判断是否一日内情形
					dts[0] = srvsplitorder.getDt_effe();
				}
				//2019年7月23日13:09:33，迭代服务的持续频次拆分，与医嘱拆分保持一致
				alwaysFreqHandle(list, srvsplitorder, dts, validSE);
				return true;
				//for (FDateTime dt : dts) {
				//	SrvSplitOrderDTO addItm = (SrvSplitOrderDTO) srvsplitorder.clone();
				// srvsplitorder.setQuan_medu(new
				// FDouble(OrSrvSplitUtil.getDateTimeLengthBySeconds(validSE[0],
				// validSE[1])));
				// OrSrvSetQuanMeduBp bp = new OrSrvSetQuanMeduBp();//
				// 根据计量单位，换算医学数量 2016-05-07 xuxing
				// bp.exec(srvsplitorder);
				//	addItm.setDt_mp_plan(dt);
				//	list.add(addItm);
				//}
			}
		}
	...
	private void splitHandleByFreq(ArrayList<SrvSplitOrderDTO> list, SrvSplitOrderDTO srvsplitorder, FDateTime[] validSE, Hashtable freqHt) throws BizException {
		...
		if (OrSrvSplitUtil.isTemporaryFreq(sd_frequnit)) {// 临时频次医嘱的处理逻辑
			...
			FDateTime[] dts = OrSrvSplitUtil.getDateArrayByHour(srvsplitorder.getDt_effe(), validSE[0], validSE[1], srvsplitorder.getFrequnitcnt());
	 ...
	 * @throws BizException
	 */
	private void alwaysFreqHandle(ArrayList<SrvSplitOrderDTO> list, SrvSplitOrderDTO srvsplitorder, FDateTime[] dts, FDateTime[] validSE) throws BizException {
		int iLen = dts.length;
		if (iLen == 1) {
			alwaysFreqLen1Handle(list, srvsplitorder, validSE);
			return;
		} // 一日内情形的处理
		Object[] o = null;
		...
		for (int i = 0; i < iLen; i++) {
			...
			// 创建新的拆分对象并赋值执行相关信息
			SrvSplitOrderDTO tmp = (SrvSplitOrderDTO)srvsplitorder.clone();
	 ...
	 * @throws BizException
	 */
	private void alwaysFreqLen1Handle(ArrayList<SrvSplitOrderDTO> list, SrvSplitOrderDTO orsplitorder, FDateTime[] validSE) throws BizException {
		// 获得持续时间
