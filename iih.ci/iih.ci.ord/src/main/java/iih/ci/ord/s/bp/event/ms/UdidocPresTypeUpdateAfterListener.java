...
import iih.ci.ord.pub.listener.AbstractUdidocUpdateAfterListener;
...
public class UdidocPresTypeUpdateAfterListener extends AbstractUdidocUpdateAfterListener{
	...
	protected void doYourActionAfter(Object[] newObjs) throws BizException {
		...
		//根据事件类型编码组装数据    //update
		MSUdidocPresTypeDTO[] msUdidocPresTypeDTOs=udidocPresTypeBP.assemableDatabyState(PresTypeDos,EventTypeEnum.UPDATE_AFTER.getValueString());
		...
		for(MSUdidocPresTypeDTO dto:msUdidocPresTypeDTOs) {
			if(dto != null) {
				OrdBizLogger.info(null, "MS095-修改后-MSUdidocPresTypeDTO:"+"["+i+"]"+dto.serializeJson());
			}
			//构建对象，并发送给集成组
			...
			CiOrdUtils.fireEvent(SourcesIdEnum.PRES_TYPE_SOURCEID.getValueString(),//iih.ci.ord.ms.d.MSUdidocPresTypeDTO
					IEventType.TYPE_UPDATE_AFTER, new BaseDTO[] { domainBusinessUserObj });//1004
			if(domainBusinessUserObj != null) {
				OrdBizLogger.info(null, "MS095-修改后-发送数据DomainBusinessUserObj:"+"["+i+"]"+domainBusinessUserObj.serializeJson());
			}
			i++;
		}
