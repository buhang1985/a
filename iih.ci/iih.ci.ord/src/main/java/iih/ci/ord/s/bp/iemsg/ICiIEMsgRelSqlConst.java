...
public interface ICiIEMsgRelSqlConst {
	...
	public static final String CI_IE_ORCONFIRM_SQL=
			" select A.id_en as id_iepharmoren,B.code as patientid,F.code_amr_ip as patientseqnum,F.Times_Ip as admiss_times,"
		   +" D.CODE as deptcode,D.Name as deptname,E.code as wardcode,E.name as wardname,F.NAME_BED as bedcode,"
		   +" C.Name_Pat as name,C.Sd_Sex_Pat as sexid,B.DT_BIRTH_HMS as birthdaydate,null as age,%1$s as confirm_date,"
		   +" G.Code as confirm_nurse_code,G.name as confirm_nurse_name,H.orgcode as orgcode,H.name as orgname, "
		   +" C.code as visite_seq_no,(case when C.code_entp='00' then '01' when C.code_entp='10' then '03' end) as visite_type_code,BD.name as visite_type_name "
		   +" from ci_order A"
		   ...
		   +"      left outer join BD_ENTP BD ON C.CODE_ENTP = BD.CODE "
		   +" where A.id_or='%3$s'"; 
	/**
	 * 西成药医嘱数据信息SQL串 20170517yjb
	 */
	public static final String CI_IE_ORDRUGWC_SQL=
			" select A.code_or as order_group_id, A.Id_Or as id_iepharmwcor,A.Id_En as id_iepharmoren,A1.Id_Orsrv as xy_zcy_order_code,"
		   +" (case when substr(A.sd_srvtp,0,6)='010101'  then 'c' else 'f' end) as xy_zcy_order_type_code,A3.name as xy_zcy_order_type_name,A.Des_Or as xy_zcy_yz_memo,A4.Code as xy_zcy_fre_code,"
		   ...
		   +" A2.Quan_Cur as xy_zcy_zyl, A7.Name as xy_zcy_zylunit, "
		   ...
		   +" A9.Code as xy_zcy_yz_old_dept_code,A9.Name as xy_zcy_yz_old_dept_name,"
		   +" A10.Code as xy_zcy_yz_old_ward_code,A10.name as xy_zcy_yz_old_ward_name,mm.Code as xy_zcy_yw_code,mm.name as xy_zcy_yw_name, "
		   +" (case when mm.id_unit_pkgbase = A2.id_pgku_cur then  '00' ELSE '01' END ) as xy_zcy_bz_no,A1.Sd_Hpsrvtp as xy_zcy_yw_yb_code,A12.name as xy_zcy_yw_yb_name,A.Dt_Entry as xy_zcy_yz_date,"
		   ...
		   +" '' as xy_zcy_hc_order_type_name,'' as xy_zcy_fybj_code, A8.code  as xy_zcy_visit_ord_no,"
		   +" '' as xy_zcy_fybj_name,A1.NOTE_SRV as xy_zcy_jt,(case when A1.Fg_Indic='N' then 'false' else 'true' end ) as xy_zcy_is_sy,(case when A1.Fg_Indic='Y' then '适应' else '非适应' end) as xy_zcy_is_syresult,(case when A1.Fg_Skintest='N' then 'false' else 'true' end ) as xy_zcy_is_ps,"
		   ...
		   +" (case when A4.Sd_Frequnitct='0' and A4.Code='ST' then 'true' else 'false' end) as xy_zcy_is_jj,'' as xy_zcy_is_jjresult,null as xy_zcy_is_yg,null as xy_zcy_is_ygresult,"
		   +" null as xy_zcy_lyl,null as xy_zcy_lylunit,A.dt_effe as xy_zcy_exe_date,A.Dt_End as xy_zcy_stop_date,A.Orders as cy_fs_count,null as cy_fs_unit,null as route_code,null as route_no, "
		   +" '1' as xy_zcy_order_seq_num,(case when A.fg_long = 'Y' then '长期' when A.fg_long = 'N' then '临时' end) as xy_zcy_ord_date_type_name,(case when A.fg_long = 'Y' then '1' when A.fg_long = 'N' then '0' end) as xy_zcy_ord_date_type_code "
		   
		   +" from ci_order A "
		   +"      inner join ci_or_srv A1 ON A.Id_Or=A1.Id_Or"
		   +"      left outer join ci_or_srv_mm A2 ON A1.Id_Orsrv=A2.Id_Orsrv"
		   +"      left outer join bd_mm mm on A2.id_mm = mm.id_mm" //查出bd_mm.id_unit_pkgbase 判断 ci_or_srv_mm.id_pgku_cur是否等于bd_mm.id_unit_pkgbase 等于 取小包装 00 否则为大包装01
	...
	public static final String CI_IE_ORDRUGHERB_SQL=	
		   ...
		   " select A.code_or as order_group_id,A.Id_Or as id_iepharmor,A.Id_En as id_iepharmoren,A.Id_Or as xy_zcy_order_code,"
		   +" 'o' as xy_zcy_order_type_code,A3.name as xy_zcy_order_type_name,A.Des_Or as xy_zcy_yz_memo,A4.Code as xy_zcy_fre_code,"
		   +" A4.name as xy_zcy_fre_name,A5.Code as xy_zcy_yytj_code,A5.Name as xy_zcy_yytj_name,"
		   +" A9.Code as xy_zcy_yz_old_dept_code,A9.Name as xy_zcy_yz_old_dept_name,"
		   +" A10.Code as xy_zcy_yz_old_ward_code,A10.name as xy_zcy_yz_old_ward_name,'04' as xy_zcy_yw_code,"
		   +" null as xy_zcy_yw_yb_code,null as xy_zcy_yw_yb_name,A.Dt_Entry as xy_zcy_yz_date,"
		   +" A13.code as xy_zcy_yz_doctor_no,A13.Name as xy_zcy_yz_doctor_name,A.Dt_Sign as xy_zcy_yz_confirm_date,A14.Code as xy_zcy_yz_confirm_code,"
		   ...
		   +" '' as xy_zcy_fybj_name,null as xy_zcy_jt,null as xy_zcy_is_sy,null as xy_zcy_is_syresult,'flase' as xy_zcy_is_ps,"
		   +" '' as xy_zcy_is_psresult,(case when A4.Sd_Frequnitct='0' and A4.Code='ST' then 'true' else 'false' end) as xy_zcy_is_jj,'' as xy_zcy_is_jjresult,null as xy_zcy_is_yg,null as xy_zcy_is_ygresult,"
		   +" null as xy_zcy_lyl,null as xy_zcy_lylunit,A.dt_effe as xy_zcy_exe_date,A.Dt_End as xy_zcy_stop_date,A.Orders as cy_fs_count,'付' as cy_fs_unit, "
		   +" (case when A.fg_long = 'Y' then '长期' when A.fg_long = 'N' then '临时' end) as xy_zcy_ord_date_type_name,(case when A.fg_long = 'Y' then '1' when A.fg_long = 'N' then '0' end) as xy_zcy_ord_date_type_code "
	...
	public static final String CI_IE_ORDRUGHERB_MM_SQL=	
		   ...
		   +" B1.Name as cy_weightunit,mm.Code as cy_yp_code,A2.name_mm as cy_yp_name ,(case when mm.id_unit_pkgbase = A2.id_pgku_cur then  '00' ELSE '01' END ) as cy_bz_no,B2.Code as cy_tsyf_code,B2.Name as cy_tsyf,"
		   +" 'false' as cy_fswgbj,'' as cy_fswgbj_result,A1.Sd_Hpsrvtp as cy_ywyb_type_code,B3.Name as cy_ywyb_type_name,'1' as cy_ord_seq_no"
		   ...
		   //+"	   LEFT OUTER JOIN BD_MM_PKGU Z ON (Z.ID_MM = A2.ID_MM AND Z.ID_UNIT_PKG=A2.ID_PGKU_CUR)"
		   //+"	   LEFT OUTER JOIN BD_MM_PKGUTP Z1 ON Z.ID_MMPKGUTP=Z1.ID_MMPKGUTP"
	...
	public static final String CI_IE_APPLIS_ALL_SQL_bak=	
		   ...
		   +" B.Id_Code as idcard,C.Telno_Pat as telephone,C.Name_Pat as name,C.Sd_Sex_Pat as sexid,B.DT_BIRTH_HMS as birthdaydate,'' as age, "
	...
	public static final String CI_IE_APPLIS_OP_ALL_SQL=	
			" select A.id_en as id_ielisoren,B.code as patient_id,B.BARCODE_CHIS as p_bar_code,E.code as ward_code,E.name as ward_code_name,F.NAME_BED as bed_no, "
					   ...
					   +" (case when A.fg_long = 'Y' then '长期' when A.fg_long = 'N' then '临时' end) as Ordertimetypename,(case when A.fg_long = 'Y' then '1' when A.fg_long = 'N' then '0' end) as Ordertimetype, "
					   +" C.code as Visitordno,(case when C.code_entp='00'then '01'when C.code_entp='10'then '03' end) as Visittype,BD.name as Visittypename"
...
//					   +"		left outer join ci_or_srv Z2 ON A.Id_or=Z2.Id_or "
	...
	public static final String CI_IE_APPRIS_ALL_SQL=
					   ...
					   +" S.CODE as ybcode,S.Sd_Patcardtp,C.Sd_Sex_Pat as sexid,B.DT_BIRTH_HMS as birthdaydate,'' as age,B.CODE_AMR_IP as patientseqnum, "
					   +" (case when F.Times_Ip is null then 0 else F.Times_Ip end) +(case when J.Times_Op is null then 0 else J.Times_Op end) as admiss_times "
	 ...
	 * 套时部位得单独求取
	 */
	public static final String CI_IE_APPRIS_IP_ALL_SQL_1=
...
//					   +"		left outer join ci_or_srv Z2 ON A.Id_or=Z2.Id_or "
	...
	public static final String CI_IE_APPRIS_OP_ALL_SQL=
					   ...
					   +" Q.Id_Orobs as id_ierisoritm,A.Id_Or as id_ierisor,Q.NO_APPLYFORM as exam_serial,P.code as exam_sub_type,P.name as exam_sub_type_name,"
					   +" '' as order_pri,A.dt_effe as dt_effe,A.dt_end as dt_end,"
					   +" Z1.Code as yz_frequency,Z1.Name as yz_frequency_name,'' as exam_sub_fftype,'' as exam_sub_fftype_name,(case when P.fg_set='N' then V.Sd_Body else  Z4.code end) as exam_region,"
					   +" (case when P.fg_set='N' then Z.name else Z4.name end) as exam_region_name ,"
					   +" '' as isstest, '' as stest, A.fg_urgent as is_em, '' as is_em_r,'' as isyg, '' as yg,'' as vip_val, "
					   +" P.sd_iemsgca as iemsgca_code,(case when Z3.name='未记账' then '099' when Z3.name='已记账' then  P1.name end) as iemsgca_name , P1.ctrl1 as iemsgca_typename,"
					   +" (case when A.fg_long = 'Y' then '长期' when A.fg_long = 'N' then '临时' end) as Ordertimetypename,(case when A.fg_long = 'Y' then '1' when A.fg_long = 'N' then '0' end) as Ordertimetype, "
					   +" C.code as Visitordno,(case when C.code_entp='00'then '01'when C.code_entp='10'then '03' end) as Visittypecode,BD.name as Visittypename"
					   +" from ci_ap_obs Q left outer join ci_order A on Q.Id_Or=A.Id_Or "
					   +"       left outer join pi_pat B ON A.Id_Pat=B.Id_Pat "
					   +"       left outer join en_ent C ON A.Id_En=C.Id_Ent "
					   +"       left outer join bd_dep D ON C.Id_Dep_Phy=D.ID_DEP " // D就诊科室
...
//					   +"		left outer join ci_or_srv Z2 ON A.Id_or=Z2.Id_or "
					   +"		left outer join bd_udidoc Z3 ON (Z3.id_udidoc ='"+ICiDictCodeTypeConst.ID_SU_BL_0+"'and Z3.id_udidoclist='"+ICiDictCodeTypeConst.SD_SU_BL+"')"
					   +"		left outer join bd_udidoc Z4 ON (Z4.Id_Udidoc in (select Z5.id_body from ci_or_srv_set Z5 where Z5.id_or = A.id_or and Z5.fg_clinical = 'Y' and Z5.id_body is not null ))"
					   +"       left outer join BD_ENTP BD ON C.CODE_ENTP = BD.CODE "
	   				   +" where %1$s";
...
//			   +"		left outer join ci_or_srv Z2 ON A.Id_or=Z2.Id_or "
	 ...
	 * 套时部位得单独求取
	 */
	public static final String CI_IE_APPPATH_IP_ALL_SQL=
...
//					   +"		left outer join bd_udidoc P1 ON P.Code_rel7 = P1.Id_Udidoc	 "
	...
	public static final String CI_IE_APPPATH_OP_ALL_SQL=
		   ...
		   +" '' as id_ierisoritm,Q.NO_APPLYFORM as id_ierisor,Q.NO_APPLYFORM as exam_serial,P.code as exam_sub_type,P.name as exam_sub_type_name,"
		   + " '' as order_pri,A.dt_effe as dt_effe,A.dt_end as dt_end,"
		   + " Y.Code as yz_frequency,Y.Name as yz_frequency_name,'' as exam_sub_fftype,'' as exam_sub_fftype_name,V.Sd_Body as exam_region,Z.Name as exam_region_name, "
		   + " '' as isstest, '' as stest, A.fg_urgent as is_em, '' as is_em_r,'' as isyg, '' as yg,'' as vip_val, "
		   + " P.sd_iemsgca as iemsgca_code,(case when Z3.name='未记账' then '099' when Z3.name='已记账' then  P1.name end) as iemsgca_name  , P1.ctrl1 as iemsgca_typename, "
		   + " (case when A.fg_long='Y' then '1' else '0' end )as orderTimeType,(case when A.fg_long='Y' then '长期' else '临时' end)as orderTimeTypeName, "
		   +" C.code as Visitordno,(case when C.code_entp='00'then '01'when C.code_entp='10'then '03' end) as Visittypecode,BD.name as Visittypename"
...
//					   +"		left outer join bd_udidoc P1 ON P.Code_rel7 = P1.Id_Udidoc	 "
	...
	public static final String CI_IE_ORPHARM_OP_PRES8IDEN_SQL=
			"select A.ID_PRES as id_iepharmpres,A.Id_En as id_iepharmoren,null as id_iepharmwcors,A.id_pres as orderno , "//A.Code as orderno  | F.code||'_'||G.times_op||'_'||A.SD_PRESTP||'_'||A.code as orderno
			+ "A.Sd_Prestp as ordertypecode,B.name as ordertypename,C.Code as doctorcode,C.Name as doctorname,A.Dt_Entry as ordertime, "
		    + "D.Code as deptcode,D.Name as deptname,E.code as checkdoctorcode,E.name as checkdoctorname,A.Dt_Entry as checktime "
	...
	public static final String CI_IE_ORDRUGWC_OP_SQL=
		    ...
		    +" V.Quan_Cur as amount,W.Name as amountunit,V.Quan_Cur as getamount,W.Name as getunit,A3.Code as ypcode,V.name_mm as ypname,(case when mm.id_unit_pkgbase = V.id_pgku_cur then  '00' ELSE '01' END ) as packserial,'' as parentno,A1.NOTE_SRV as advice, "
		    ...
		    +" '' as is_yg,'' as is_ygresult, (case when A.fg_long = 'Y' then '长期' when A.fg_long = 'N' then '临时' end) as temporaryflagname,"
		    + "(case when A.fg_long = 'Y' then '1' when A.fg_long = 'N' then '0' end) as temporaryflagcode,'1' as Orderseqnum,Z3.name as payflag "
		  ...
		  // +"	   LEFT OUTER JOIN BD_MM_PKGUTP Z1 ON Z.ID_MMPKGUTP=Z1.ID_MMPKGUTP "
		   ...
		   +"	   left OUTER join bd_mm A3 ON A3.ID_MM=V.ID_MM"
		   +"      left outer join bd_mm mm on V.id_mm = mm.id_mm"
		   +"	    left outer join bd_udidoc Z3 ON (Z3.id_udidoc ='"+ICiDictCodeTypeConst.ID_SU_BL_1+"'and Z3.id_udidoclist='"+ICiDictCodeTypeConst.SD_SU_BL+"')"
	...
	public static final String CI_IE_ORDRUGHERB_OP_SQL=
			" select A1.Id_Or as id_iepharmor,A1.Id_Pres as id_iepharmpres,null as id_iepharmormms,A1.id_or as yzno,'04' as medicine_type, "
		    ...
		    +" (case when A1.Fg_Skintest = 'Y' then 'true' else 'flase' end) as is_ps,(case when A1.FG_SKINTEST_RST='Y' then (select t1.name from ci_skin_test t left outer join bd_udidoc t1 on t.id_rst_skintest=t1.id_udidoc where id_or=A1.Id_Or) else '' end) as is_psresult, "
		    +" (case when A1.Fg_Indic = 'Y' then 'true' else 'flase' end) as is_syz,(case when A1.Fg_Indic='Y' then '适应' else '非适应' end) as is_syzresult,"
		    +" (case when S.Sd_Frequnitct='0' and S.Code='ST' then 'true' else 'false' end) as is_jj,(case when A.Fg_Urgent='Y' then '加急' when A.Fg_Urgent='N' then '非加急' else '' end) as is_jjresult, "
		    +" '' as is_yg,'' as is_ygresult,(case when A.fg_long = 'Y' then '长期' when A.fg_long = 'N' then '临时' end) as temporaryflagname,"
		    + "(case when A.fg_long = 'Y' then '1' when A.fg_long = 'N' then '0' end) as temporaryflagcode,Z3.name as payflag "
		   
		   +" from ci_or_srv A1 "
		   +"      inner join ci_order A ON A.Id_Or=A1.Id_Or and A.id_srv = A1.id_srv "
		   +"      left outer join ci_pres P ON P.Id_Pres=A1.Id_Pres "
		   +"      left outer join bd_udidoc Q ON Q.Id_Udidoc=P.Id_Prestp "
		   +"      left outer join bd_freq S ON S.Id_Freq=A1.Id_Freq  "
		   +"      left outer join bd_dep X ON X.ID_DEP=A1.Id_Dep_Wh "
	...
	public static final String CI_IE_ORDRUGHERB_MM_OP_SQL=
			" select A2.Id_Orsrvmm as id_iepharmormm,A1.ID_OR as id_iepharmor,A2.Quan_Cur as weight,T.Name as weightunit,A3.CODE as ypcode,A2.name_mm as ypname, "
		   +" (case when Z1.CODE='1000' then '00' when Z1.CODE='1001' THEN '01' ELSE '' END ) as packserial,U.Code as methocode,U.name as methoname,'07' as nmwamount,"
		   + "'true' as isnmwamount,A1.Sd_Hpsrvtp as ybtypecode,R.Name as ybtypename ,'1' as orderseqnum"
		   ...
		   +"	   LEFT OUTER JOIN BD_MM_PKGU Z ON (Z.ID_MM = Y.ID_MM AND Z.ID_UNIT_PKG=Y.ID_PGKU_CUR) "
		   +"	   LEFT OUTER JOIN BD_MM_PKGUTP Z1 ON Z.ID_MMPKGUTP=Z1.ID_MMPKGUTP "
		   +"	   left join bd_mm A3 ON A3.ID_MM=A2.ID_MM"
	...
	public static final String CI_IE_ORCANCEL_OP_SQL=
		   ...
		   +"  (case when F.Times_Ip is null then 0 else F.Times_Ip end) +(case when J.Times_Op is null then 0 else J.Times_Op end) as times "
		   +"  ,A.ID_OR as id_ieorcancstp,A.Id_En as id_ieorcancstpen,A.Code_Or as order_id,(case when substr(A.sd_srvtp,0,2)='04' then '"+IBdSrvIEEventConst.SD_SRVTP_SXL+"' else N.Code end) as order_type,'' as sample_no, "
//		   +" A.Dt_Canc as cancel_date,K.Code as cancel_opera,K.Name as cancel_opera_name,' ' as cancel_reason,'' as mutex_order_no,'' as mutex_order_type,'' as mutex_order_name, "
		   +" A.sv as cancel_date,K.Code as cancel_opera,K.Name as cancel_opera_name,' ' as cancel_reason,'' as mutex_order_no,'' as mutex_order_type,'' as mutex_order_name, "
		   //IE集成平台差异对比新增 2017-06-22 18:04:40
		   +"	P.code as dept_code,P.name as dept_name,O.orgcode as org_code,O.name as org_name,'1' as sequence_number,S.sd_iemsgca as Iemsgca_code, "//服务类型编码
		   //end
		   +" C.code as visitOrdNo,(case when c.code_entp='00' then '01' when c.code_entp='10' then '03' end) as visitType,bd.name as visitTypeName,N.name as order_type_name"
		   +" from ci_order A  "
		   +"      left outer join pi_pat B ON A.Id_Pat=B.Id_Pat "
		   +"      left outer join en_ent C ON A.Id_En=C.Id_Ent  "
		   +"      left outer join en_ent_ip F ON C.Id_Ent=F.Id_Ent "
...
//		   +"      left outer join bd_psndoc K ON K.Id_Psndoc=A.Id_Emp_Canc"
		   ...
		   +"	   LEFT OUTER JOIN bd_srv S ON S.Id_Srv=A.Id_Srv  "
		   +"	   left outer join BD_ENTP BD ON C.CODE_ENTP = BD.CODE  "
		   //end
		   +" where  %1$s ";  //A.Id_or
	/**
	...
	public static final String CI_IE_ORCANCEL_SQL=
			...
			" select A.Id_En as id_ieorcancstpen,B.code as patientid,X.Code as exec_dept_code,X.Name as exec_dept_name,'01' as domain_id,n.code as order_type_code, "
		   +" (case when F.Times_Ip is null then 0 else F.Times_Ip end) as admiss_times, X.Code as exec_unit,X.Name as exec_unit_name,bu.name as ordertypename,"
		   +" A.ID_OR as id_ieorcancstp,(case when substr(A.sd_srvtp,0,2) = '02' then Q.NO_APPLYFORM when substr(A.sd_srvtp,0,2) = '03' then H.NO_APPLYFORM"
		   + " when substr(A.sd_srvtp,0,4) = '0103' then A.Id_Or when substr(A.sd_srvtp,0,4) = '0101' then A1.id_orsrv ELSE A.code_or end) as orderno,'' as bbno, "
		   +" (case when A.Fg_Canc='Y' then A.dt_canc else A.Dt_Stop end) as canceltime, "
		   ...
		   +" (case when substr(A.sd_srvtp,0,2)='02' then '"+IBdSrvIEEventConst.SD_SRVTP_JCL+"' "
		   + " when substr(A.sd_srvtp,0,2) ='03' then '"+IBdSrvIEEventConst.SD_SRVTP_HYL+"'"
		   + " when substr(A.sd_srvtp,0,2)='04' then '"+IBdSrvIEEventConst.SD_SRVTP_SXL +"'"
		   + " when substr(A.sd_srvtp,0,2)='05' then '"+IBdSrvIEEventConst.SD_SRVTP_ZLL+"' "
		   + " when substr(A.sd_srvtp,0,6)='140101' then '"+IBdSrvIEEventConst.SD_SRVTP_SXSQ+"' "
		   + " when substr(A.sd_srvtp,0,6)='140102' then '"+IBdSrvIEEventConst.SD_SRVTP_QXSQ+"'"
		   + " when substr(A.sd_srvtp,0,6) in ('010102','010199') then '"+IBdSrvIEEventConst.SD_SRVTP_ZXL+"' "
		   + " when substr(A.sd_srvtp,0,6) in ('010103', '010104','010105','010106') then '"+IBdSrvIEEventConst.SD_SRVTP_ZJYP+"' "
		   + " when substr(A.sd_srvtp,0,2)='06' then '"+IBdSrvIEEventConst.SD_SRVTP_HLL+"' "
		   + " when substr(A.sd_srvtp,0,6) ='010101' then '"+IBdSrvIEEventConst.SD_SRVTP_YP+"' "
		   + " when substr(A.sd_srvtp,0,4) = '0103' then '"+IBdSrvIEEventConst.SD_SRVTP_CYL+"' else '"+IBdSrvIEEventConst.SD_SRVTP_QTL+"' end) as ordertype,"
		   +" '' as yz_cancel_yy,'' as hc_order_no,'' as hc_order_type_no,'' as hc_order_type_name, "
		   //IE集成平台差异对比后新增 begin 2017-05-18 19:39:08
		   +"	P.code as dept_code,P.name as dept_name,O.orgcode as org_code,O.name as org_name,'1' as sequence_number,d.SD_IEMSGCA as iemsgca_code,"
		   +" C.code as visit_sort_no,(case when c.code_entp='00' then '01' when c.code_entp='10' then '03' end) as visit_type_code,bd.name as visit_type_name"
		   +" from ci_order A  "
		   +"       inner join ci_or_srv A1 on A.id_or = A1.id_or"
		   +"       left outer join ci_ap_lab H on H.Id_Or=A.Id_Or"//检验
		   +"       left outer join ci_ap_obs Q on Q.Id_Or=A.Id_Or"//检查
		   +"       left outer join pi_pat B ON A.Id_Pat=B.Id_Pat "
		   +"       left outer join en_ent C ON A.Id_En=C.Id_Ent  "
		   +"       left outer join en_ent_ip F ON C.Id_Ent=F.Id_Ent "
		   +"       left outer join en_ent_op J ON A.Id_En=J.Id_Ent "
		   ...
		   +"		LEFT OUTER JOIN bd_dep P ON C.id_dep_phy =P.id_dep  "
		   +"		LEFT OUTER JOIN bd_org O ON A.id_org_or=O.id_org  "	
		   +"       left outer join bd_srv D ON a.Id_Srv=D.Id_Srv "
		   +"	    left outer join BD_ENTP BD ON C.CODE_ENTP = BD.CODE  "
		   +"       left outer join bd_udidoc bu on bu.id_udidoc = A.id_srvtp "
		   //end
		   +" where A1.Fg_Or='Y' and %1$s ";  //A.Id_or	
	 /**
	 * 住院手术医嘱数据信息SQL串
	 * 仅住院手术医嘱使用
	 */
	public static final String CI_IE_OROP_SQL=
			" select Q.Id_Apop as id_ieoporen,B.code as patientid,C.Name_Pat as name,C.Sd_Sex_Pat as sexid,B.DT_BIRTH_HMS as birthdaydate,'' as age,B.Id_Code as idcard,C.Telno_Pat as telephone,C.ADDR_PAT as homeaddress "
...
+"       left outer join en_ent_di M1 ON Q.Id_Di=M1.ID_entdi "//在检查医嘱属性表里ci_ap_sug存储的id_di是就诊诊断表en_ent_di里的ID_entdi，与其他诊断表里的ID_DI不一致，此处临时更改
	...
	public static final String CI_IE_ORTREAT_CONFIRM_SQL=
			" select A.Id_En as id_ietreatoren,B.code as patientid,C.Name_Pat as name,C.Sd_Sex_Pat as sexid,'' as age,B.DT_BIRTH_HMS as birthdaydate,B.BARCODE_CHIS as patientseqnum, "
...
//		   +"	   left outer join ci_pres C1 ON A1.id_pres =C1.id_pres"
	...
	public static final String CI_IE_OROTH_CONFIRM_SQL=
			" select A.Id_En as id_ieothoren,C.Name_Pat as name,C.Sd_Sex_Pat as sexid,'' as age,B.DT_BIRTH_HMS as birthdaydate,B.code_amr_ip as patientseqnum, "
		   +"   (case when H.Times_Ip is null then 0 else H.Times_Ip end) +(case when J.Times_Op is null then 0 else J.Times_Op end) as admiss_times, B.code as patientid,"
		   +" I.Code as deptcode,I.Name as deptname,G.Code as wardcode,G.Name as wardname,H.NAME_BED as bedcode,A.Dt_Chk as confirm_date,R.Code as confirm_nurse_code,R.Name as confirm_nurse_name,'01' as domain_id, "
		   //IE集成平台差异比对 新增 begin 2017-05-19 13:56:55
		   +"	O.orgcode AS org_code,O. NAME AS org_name,C.code as visit_sort_no, (case when C.code_entp='00' then '01' when C.code_entp='10' then '03' end) as visit_type_code,BD.name as visit_type_name "
		   //end
		   +" from ci_order A "
		   +"       left outer join pi_pat B ON A.Id_Pat=B.Id_Pat "
		   +"       left outer join en_ent C ON A.Id_En=C.Id_Ent "
	...
	public static final String CI_IE_OROTH_ITM_SQL=
		   ...
		   +" K.code as yz_confirm_doctor_code,K.name as yz_confirm_doctor_name,L.code as exe_dept_code,L.name as exe_dept_name,'' as f_order_code,'' as hc_order_type_code,'' as hc_order_type_name, "
		   ...
		   +" O.orgcode as org_code,O.name as org_name, '1' as order_sort_no,(case when A.fg_long = 'Y' then '长期' when A.fg_long = 'N' then '临时' end) as visit_time_type_name,"
		   +" (case when A.fg_long = 'Y' then '1' when A.fg_long = 'N' then '0' end) as visit_time_type_code, "
		   +" A.des_or as yz_memo "
		   //end
		   +" from ci_or_srv A1 left outer join ci_order A  ON A1.Id_Or=A.Id_Or "
		   +"      left outer join pi_pat B ON A.Id_Pat=B.Id_Pat "
	...
	public static final String CI_IE_ORNS_CONFIRM_SQL=
			" select A.Id_En as id_iensoren,B.code as patientid,C.Name_Pat as name,C.Sd_Sex_Pat as sexid,'' as age,B.DT_BIRTH_HMS as birthdaydate,B.code_amr_ip as patientseqnum, "
		   +" (case when H.Times_Ip is null then 0 else H.Times_Ip end)+(case when J.Times_Op is null then 0 else J.Times_Op end) as admiss_times, "
		   +" I.Code as deptcode,I.Name as deptname,G.Code as wardcode,G.Name as wardname,H.NAME_BED as bedcode,A.Dt_Chk as confirm_date,"
		   +" R.Code as confirm_nurse_code,R.Name as confirm_nurse_name,'02' as domain_id, D.orgcode as orgcode,D.name as orgname,C.code as visit_sort_no,"
		   +" (case when C.code_entp='00' then '01' when C.code_entp='10' then '03' end) as visit_type_code,BD.name as visit_type_name   "
		   +"     from ci_order A "
		   +"       left outer join pi_pat B ON A.Id_Pat=B.Id_Pat "
		   +"       left outer join en_ent C ON A.Id_En=C.Id_Ent "
		   +"       left outer join en_ent_ip H ON C.Id_Ent=H.Id_Ent  "
	...
	public static final String CI_IE_ORNS_ITM_SQL=
		   ...
		   + "L.code as exe_dept_code,L.name as exe_dept_name,'' as jt,'' as hc_order_type_code,'' as hc_order_type_name,'' as f_order_code, "
		   ...
		   + "(case when A1.Fg_Skintest='Y' then 'true' else 'false' end) as is_ps,(case when A1.Fg_Skintest='Y' then '需皮试' when A1.Fg_Skintest='N' then '不需皮试' else '' end) as is_psresult,"
		   + "(case when A1.Fg_Indic='Y' then 'true' else 'false' end) as is_syz,(case when A1.Fg_Indic='Y' then '适应症' when A1.Fg_Indic='N' then '非适应症' else '' end) as is_syzresult, "
		   + "(case when G.Sd_Frequnitct='0' and G.Code='ST' then 'true' else 'false' end) as is_jj,(case when A.Fg_Urgent='Y' then '加急' when A.Fg_Urgent='N' then '非加急' else '' end) as is_jjresult, "
		   + "'' as is_yg,'' as is_ygresult,null as route_code,null as route_no,(case when A.fg_long = 'Y' then '长期' when A.fg_long = 'N' then '临时' end) as order_time_type_name, "
		   + "(case when A.fg_long = 'Y' then '1' when A.fg_long = 'N' then '0' end) as order_time_type,  "
		   + "A.des_or as yz_memo  "
		   +" from ci_or_srv A1 left outer join ci_order A  ON A1.Id_Or=A.Id_Or "
		   +"      left outer join pi_pat B ON A.Id_Pat=B.Id_Pat "
		   +"      left outer join en_ent C ON A.Id_En=C.Id_Ent "
	...
	public static final String CI_IE_ORBT_MAIN_SQL=
		   ...
		   +" B.code as patientid,C.Name_Pat as name,C.Sd_Sex_Pat as sexid,'' as age,B.DT_BIRTH_HMS as birthdaydate,B.BARCODE_CHIS as patientseqnum "
		   +" ,(case when H.Times_Ip is null then 0 else H.Times_Ip end) +(case when J.Times_Op is null then 0 else J.Times_Op end) as admiss_times "
	...
	public static final String CI_OP_LISRIS_IE_SQL=
			...
			+" C.code as visitOrdNo,(case when c.code_entp='00' then '01' when c.code_entp='10' then '03' end) as visitType,bd.name as visitTypeName,b.name as order_type_name"
		    ...
		    +" left outer join en_ent C ON cior.Id_En=C.Id_Ent"
		    +" left outer join BD_ENTP BD ON C.CODE_ENTP = BD.CODE  "
		    +" left outer join bd_udidoc b on b.id_udidoc = cior.id_srvtp"
		    +" where cior.id_or %s";
	/**
	 ...
	 * 查询退费处方号
	 */
	public static final String CI_IE_ORPHARM_OP_REFUNG_IDPRES_SQL=
			"select A.Code||'_'||A.id_pres as orderno from  ci_pres A where A.Id_Pres in (%1$s)";//A.Code as orderno  | F.code||'_'||G.times_op||'_'||A.SD_PRESTP||'_'||A.code as orderno
	 ...
	 * 
	 */
	public static final String CI_IE_ORBT_VIEWITEM_OP_SQL=
			...
			+"	from ci_ap_bt_viewitm A "
			+"	left outer join bd_measdoc B ON A.id_unit=B.id_measdoc "
			+"	left outer join bd_srv C ON A.id_srv=C.id_srv"
			+"	where %1$s ";
	/**
	 ...
	 * ABO Rh血型得单独服务求取
	 */
	public static final String CI_IE_ORBU_IP_SQL=
			" select A1.id_or as id_iebtuseoren,B.code as patient_id,B.code_amr_ip as p_bar_code,C.Name_Pat as name,C.Sd_Sex_Pat as sex,B.DT_BIRTH_HMS as brithday,"
		   + "'' as age,I.code as apply_unit,I.name as apply_unit_name,G.code as  ward_code,G.name as ward_code_name,H.NAME_BED as bed_no,S.OrgCode as hospital_code,S.Name as hospital_name, "
		   +" (case when H.Times_Ip is null then 0 else H.Times_Ip end) +(case when J.Times_Op is null then 0 else J.Times_Op end) as times,U.code as use_amount_code,  "
		   +" btor.applyno as bill_id,Q.no_applyform as use_sn,'1' as get_times,A.Code_or as yz_no,substr(T.code,7) as blood_class,T.name as blood_class_name,A1.Quan_Medu as use_amount,  "
		   +" Q.Dt_Bu_Plan as use_date,bt.sd_bloodtype_ap as abo_type,aboudi.name as abo_type_name,bt.sd_rhd_ap as rh_d,rhudi.name as rh_d_name,'' as is_lg,'' as is_lg_r,W.code as app_doctor,"
		   +" W.name as app_doctor_name,W.MOBILE as app_tel,'' as op_loc,'' as op_loc_name,'' as op_tel,'' as visit_type,'01' as domain_id,t.SD_IEMSGCA as iemsgca_code,"
		   +" t.SD_IEMSGCA as iemsgca_code,C.code as visit_ord_no,BD.name as visit_type_name,(case when A.Code_Entp='00'then '01' when A.Code_Entp ='10'then '03' end )as visit_type"
		   +"  from ci_or_srv A1   "
		   +"       left outer join ci_order A ON A1.Id_Or=A.Id_Or  "
		   +"       left outer join ci_ap_bu Q ON Q.id_or=A.Id_Or  "
		   +"       left outer join pi_pat B ON A.Id_Pat=B.Id_Pat  "
		   ...
		   +"       left outer join bd_org S ON S.Id_Org=A.Id_Org  "
		   +"       left outer join bd_measdoc U ON U.Id_Measdoc=A1.Id_Medu  "
		   +"       left outer join ci_order btor on A.id_or_rel=btor.id_or"
		   +"       left outer join ci_ap_bt bt on btor.id_or=bt.id_or"
		   +"       left outer join bd_udidoc aboudi on aboudi.id_udidoc=bt.id_bloodtype_ap"
		   +"       left outer join bd_udidoc rhudi on rhudi.id_udidoc=bt.id_rhd_ap"
		   +"       left outer join bd_srv T ON T.Id_Srv=btor.Id_Srv  "
		   +"		left outer join BD_ENTP BD ON C.CODE_ENTP = BD.CODE "	 	  
		   +"		 where A1.Sd_Srvtp like '14%' and ";	 //A1.id_or	 
	/**
	 * 住院治疗医嘱信息SQL串 20170517yjb
	 * 治疗处置
	 * 
	 */
	public static final String CI_IE_ORTREAT_ITM_IP_SQL=
			"select A1.Id_Orsrv as id_ietreator,A.Id_En as id_ietreatoren,A1.id_pres as order_no,A.code_or as yz_no,"
			+ "D.Code as order_type,D.Name as order_type_name,A1.Note_Srv as yz_comment,E.Code as yz_code,E.Name as yz_name,F.Name as pack_serial, "
		    + "A.Quan_Medu as charge_amount,F.Name as spe_external,G.code as freq_code,G.name as freq_code_name,H.code as yz_olddept_code,H.name as yz_olddept_name,"
		    + "I.code as yz_oldward_code,I.name as yz_oldward_name,A.Dt_Entry as enter_date,J.Code as doctor_code,J.Name as doctor_name, "
		    + "A.Dt_Sign as confirm_date,K.code as confirm_opera,K.name as confirm_opera_name,L.code as exec_sn,L.Name as exec_name,A1.Id_Orsrv as parent_no, "
            ...
            +"   (case when H1.Times_Ip is null then 0 else H1.Times_Ip end) +(case when J1.Times_Op is null then 0 else J1.Times_Op end) as admiss_times, "
            //end
            + "'' as is_yg,'' as is_ygresult,null as route_code,null as route_no,'1' as Orderseqnum, "
            + "(case when A.fg_long = 'Y' then '长期' when A.fg_long = 'N' then '临时' end) as Ordertimetypename,(case when A.fg_long = 'Y' then '1' when A.fg_long = 'N' then '0' end) as Ordertimetype  "
...
//		   +"	   left outer join ci_pres C1 ON A1.id_pres =C1.id_pres"
		   //end
	   	   +"      where %1$s and A1.Fg_or='Y'";
	/**
	 * 住院治疗医嘱就诊信息SQL串 20170517yjb
	 * 治疗处置
	 * 
	 */
	public static final String CI_IE_ORTREAT_CONFIRM_IP_SQL=
			"select A.Id_En as id_ietreatoren,'' as ietreatfees,B.code as patient_id,B.code_amr_ip as p_bar_code, "
           +" C.Name_Pat as name,C.Sd_Sex_Pat as sex,'' as age,B.DT_BIRTH_HMS as birthday,I.code as apply_unit,I.name as apply_unit_name, "
		   +" (case when H.Times_Ip is null then 0 else H.Times_Ip end)+(case when J.Times_Op is null then 0 else J.Times_Op end) as admiss_times, "
		   +" G.code as ward_code,G.Name as ward_code_name,H.NAME_BED as bed_no,A.Dt_Chk as confrim_date, "
		   +" R.Code as confirm_opera,R.Name as confrim_opera_name,'02' as domain_id,D.orgcode as orgcode,D.name as orgname, "
		   +" C.code as Visitordno, (case when C.code_entp='00' then '01' when C.code_entp='10' then '03' end) as Visittype,BD.name as Visittypename "
		   +" from ci_order A "
		   +"       left outer join pi_pat B ON A.Id_Pat=B.Id_Pat "
		   +"       left outer join en_ent C ON A.Id_En=C.Id_Ent "
