...
import iih.ci.ord.s.external.provide.bp.mr.PatOrderInfoBP;
...
public class CiOrdMrServiceImpl implements ICiOrdMrService {
	...
	public String getDiagList(String id_ent) throws BizException {
		...
		sql.append(" select max(DI.dt_di) from ci_di DI where DI.id_en=?");
		param.addParam(id_ent);
		String maxdt_di=(String)new DAFacade().execQuery(sql.toString(),param,new ColumnHandler());
		if(StringUtils.isEmpty(maxdt_di))return null;
		
		String condition = String.format("dt_di='%s'", maxdt_di);
	...
	private String getPropertyVal(DIDTO obj, String propertyName) throws BizException {
		...
		if (isNaN) {
			...
			if (valueObj instanceof String || valueObj instanceof FDateTime) {// 字符串类型
				...
				valStr = FBoolean.TRUE.equals((FBoolean) valueObj) ? "Y" : "N";
			} else {
				valStr = "";
			}
			return valStr;
	...
	private Map<String, List<DIDTO>> reOrderDIDTO(DIDTO[] diDTOs, List<String> zdtxList) {
		...
		if (zdtxList != null && zdtxList.size() > 0 && diDTOs != null && diDTOs.length > 0) {
			...
			for (DIDTO diDTO : diDTOs) {
				...
				// 是否为主诊断，主诊断放第一位
				if (FBoolean.TRUE.equals(diDTO.getFg_majdi())) {
...
//			zdtxList = new ArrayList<String>();
	 ...
	 */
	@Override
	public FDateTime getDeathDateTime(String id_en) throws BizException {

		PatOrderInfoBP patOrderInfo = new PatOrderInfoBP();		
		return  patOrderInfo.getPatDeathTime(id_en);
	}
	 ...
	 */
	@Override
	public List<CiOrdInfoDTO> getCiOrdInfoByIden(String id_en) throws BizException {
		// TODO Auto-generated method stub
		PatOrderInfoBP patOrderInfo = new PatOrderInfoBP();	
		return patOrderInfo.geCiOrdInfoDTOs(id_en);
	}
