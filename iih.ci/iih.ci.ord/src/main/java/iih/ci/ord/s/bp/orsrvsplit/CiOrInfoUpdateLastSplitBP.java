...
import iih.ci.ord.s.bp.orsrvsplit.help.ArrayGroupingBp;
...
public class CiOrInfoUpdateLastSplitBP {
	...
	private void updateOrder(String[] idOrKeys, FDateTime Dt_last_bl) throws BizException {
		if (idOrKeys != null && idOrKeys.length > 0) {
			...
			if (idOrKeys.length > 500) {
				// key数量大于500的更新，采用分组处理
				ArrayGroupingBp bp = new ArrayGroupingBp(idOrKeys);
				while (true) {
					String[] paramKeys = bp.nextArrayGroup();
					if (paramKeys == null || paramKeys.length < 1)
						break;
					CiOrderSplitUpdateQry Sql = new CiOrderSplitUpdateQry(paramKeys, Dt_last_bl);
	...
	private void updateOrderSrv(String[] idOrSrvKeys, FDateTime Dt_last_bl) throws BizException {
		if (idOrSrvKeys != null && idOrSrvKeys.length > 0) {
			...
			if (idOrSrvKeys.length > 500) {
				// key数量大于500的更新，采用分组处理
				ArrayGroupingBp bp = new ArrayGroupingBp(idOrSrvKeys);
				while (true) {
					String[] paramKeys = bp.nextArrayGroup();
					if (paramKeys == null || paramKeys.length < 1)
						break;
					CiOrderSrvSplitUpdateQry Sql = new CiOrderSrvSplitUpdateQry(paramKeys, Dt_last_bl);
					new DAFacade().execUpdate(Sql.getQrySQLStr(), Sql.getQryParameter(null));
				}
			} else {
				// 小于500的直接更新
