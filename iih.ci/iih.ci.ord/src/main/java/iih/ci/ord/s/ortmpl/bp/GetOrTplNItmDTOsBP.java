 ...
 *
 */
public class GetOrTplNItmDTOsBP {
	...
	public FMap getOrderTemplateDTO(String[] idortmpls) throws BizException {
		return getOrderTemplateDTO(idortmpls, ctx.getId_pripat(), 
				ctx.getId_hp_default(), ctx.getCode_entp());
	}
		...
		if(orTmplDO != null){
			...
			for (OrTplNItmDO item : orTplItem) {
				if (OrTplItmTypeEnum.ORTMPL.equals(item.getEu_ortplitmtp())) {
					...
					if(!StringUtils.isBlank(item.getName_srv())){
						template.setName(item.getName_srv());
					}
	...
	private String getFG_setRadio(String id)throws BizException{
		MedSrvDO medsrvDO = CiOrdAppUtils.getMedsrvMDORService().findById(id);
	...
	private void getOrTplNItmDTOsXCY(OrTmplDO orTmplDO, FArrayList lstOrTplNItmDTOs, Boolean isRef) throws BizException {
		...
		if(orTmplDO != null && orTplItem != null && orTplItem.size() > 0){
			...
			for (OrTplNItmDO item : orTplItem) {
				...
				FBoolean fg_highrisk = BDSrvDrugInfoCache.GetBdSrvDrugInfo(null, item.getId_srv()).getFg_highrisk();
				if(FBoolean.TRUE.equals(fg_highrisk)) {
					item.setOrtplnitm_mm_name("【高危】" + item.getOrtplnitm_mm_name());
					item.setOrtplnitm_srv_name("【高危】" + item.getOrtplnitm_srv_name());
				}
				if (num == 2) {
	...
	private void setOrTplNItmDO(FArrayList list, MedSrvSetItemDO[] itmDOs, OrTplNItmDO ortmpl) {
		if (itmDOs != null && itmDOs.length > 0) {
			for (MedSrvSetItemDO medSrvset : itmDOs) {
				...
				item.setId_ortmplitm(ortmpl.getId_ortmplitm());//套单选时使用使用模板项目主键
	...
	private void getMedicareCataLogSrv(String id_hp, List<String> id_srvs) throws DAException {
		if(StringUtils.isNotEmpty(id_hp)) {
			String sql = "select bd_hp_srvorca.id_srv,bd_udidoc.name from bd_hp_srvorca,bd_udidoc "
					+ " where bd_hp_srvorca.id_hpsrvtp=bd_udidoc.id_udidoc and bd_hp_srvorca.id_hp=?"
					+ " and bd_hp_srvorca.id_srv in ("
					+ CiOrdUtils.ListConveretCharacterString(id_srvs)
					+")";
			srvMapMedical = new HashMap<String, String>();
			SqlParam sqlParam = new SqlParam();
			sqlParam.addParam(id_hp);
			List<Map<String, Object>> priMapList = (List<Map<String, Object>>) new DAFacade().execQuery(sql, sqlParam,new MapListHandler());
	...
	private void getMedicareCataLogMm(String id_hp, List<String> id_mms) throws DAException {
		if(StringUtils.isNotEmpty(id_hp)) {
			String sql = "select bd_hp_srvorca.id_mm,bd_udidoc.name from bd_hp_srvorca,bd_udidoc "
					+ " where bd_hp_srvorca.id_hpsrvtp = bd_udidoc.id_udidoc and bd_hp_srvorca.id_hp=?"
					+ " and bd_hp_srvorca.id_mm in ("
					+CiOrdUtils.ListConveretCharacterString(id_mms)
					+")";
			mmMapMedical = new HashMap<String, String>();
			List<Map<String, Object>> priMapList = null;
			SqlParam sqlParam = new SqlParam();
			sqlParam.addParam(id_hp);
			priMapList = (List<Map<String, Object>>) new DAFacade().execQuery(sql, sqlParam,new MapListHandler());
			if (priMapList != null && priMapList.size() > 0) {
				for (Map<String, Object> priObjMap : priMapList) {
