 ...
 *
 */
public class FeeListLoadBP extends ContextCache implements IFeeListLoadBP {
	...
	public FeeListRstDTO load(CiEnContextDTO ctx, FeeListLoadDTO ems) throws BizException {
		...
		// 查询医嘱费用数据
		List<Map<String, Object>> ordResult = (List<Map<String, Object>>) new DAFacade().execQuery(ordSql,
				new MapListHandler());
		if (CiOrdUtils.isEmpty(ordResult))
			return feeListRstDTO;
		List<CiOrdFeeSrvDTO> ordFeeList = handleOrdFeeList(ordResult);
		if (CiOrdUtils.isEmpty(ordFeeList))
			return feeListRstDTO;
		// 将费用数据组织为 UI 数据
	...
	private String getOrdFeeLIstSql(String id_en, String code_entp, String OrderSequenceModel) {
		...
		sql.append("orsrv.id_srv,orsrv.id_orsrv,orsrv.eu_sourcemd,");
		sql.append("  orsrv.pri as price,round(orsrv.pri*orsrv.quan_total_medu,4) as amt_total,");
		sql.append(
				"  orsrv.fg_indic,orsrv.fg_selfpay,orsrv.eu_orsrvhp,dep.name as name_dep,srv.des as des_srv,measdoc.id_measdoc as id_unit_med,measdoc.name as name_unit_med,measdoc_mm.id_measdoc as mm_id_unit_med, measdoc_mm.name as mm_name_unit_med,orsrvmm.code_mm,");
		sql.append(
				"  orsrv.sd_srvtp,a0.orders,orsrvmm.quan_cur as quan_drug,round(orsrv.pri*orsrvmm.quan_cur,4) as amt_total_drug,");
		sql.append(
				"  orsrv.fg_bl as orsrv_fg_bl,orsrv.fg_mm as orsrv_fg_mm,(case when orsrv.fg_mm='Y' then bdmm.code   when orsrv.fg_bl = 'Y' then srv.code  else '' end) as fee_code_or");
		...
		sql.append("  left outer join bd_dep dep on orsrv.id_dep_mp=dep.id_dep");
	...
	private List<CiOrdFeeSrvDTO> handleOrdFeeList(List<Map<String, Object>> ordFeeList) {
		...
		for (Map<String, Object> ordFee : ordFeeList) {
			...
			if (CiOrdUtils.isEmpty(ordFee))
				continue;
			ciOrdFeeSrvDTO.setId_or(CiOrdUtils.nullHandle(ordFee.get("id_or")));
			ciOrdFeeSrvDTO.setId_srv(CiOrdUtils.nullHandle(ordFee.get("id_srv")));
			ciOrdFeeSrvDTO.setId_orsrv(CiOrdUtils.nullHandle(ordFee.get("id_orsrv")));
			ciOrdFeeSrvDTO.setEu_sourcemd(CiOrdUtils.isEmpty(ordFee.get("eu_sourcemd")) ? null : Integer.valueOf(ordFee.get("eu_sourcemd").toString()));
			ciOrdFeeSrvDTO.setCode_or(CiOrdUtils.nullHandle(ordFee.get("code_or")));
			ciOrdFeeSrvDTO.setDt_effe(
					CiOrdUtils.isEmpty(ordFee.get("dt_effe")) ? null : new FDateTime((String) ordFee.get("dt_effe")));
			...
			ciOrdFeeSrvDTO.setQuan_total_medu(CiOrdUtils.isEmpty(ordFee.get("med_unit")) ? FDouble.ONE_DBL
					: new FDouble(ordFee.get("med_unit").toString()));
			ciOrdFeeSrvDTO.setPrice(CiOrdUtils.isEmpty(ordFee.get("price")) ? FDouble.ONE_DBL
					: new FDouble(ordFee.get("price").toString()));
			ciOrdFeeSrvDTO.setAmt_total(CiOrdUtils.isEmpty(ordFee.get("amt_total")) ? FDouble.ONE_DBL
					: new FDouble(ordFee.get("amt_total").toString()));
			ciOrdFeeSrvDTO.setId_unit_med(CiOrdUtils.nullHandle(ordFee.get("id_unit_med")));
			...
			if (!CiOrdUtils.isEmpty(ordFee.get("sd_srvtp"))
					&& ordFee.get("sd_srvtp").toString().startsWith(IBdSrvDictCodeConst.SD_SRVTP_DRUG)) {
				ciOrdFeeSrvDTO.setAmt_total(CiOrdUtils.isEmpty(ordFee.get("amt_total_drug")) ? FDouble.ONE_DBL
						: new FDouble(ordFee.get("amt_total_drug").toString()));
				ciOrdFeeSrvDTO.setQuan_total_medu(CiOrdUtils.isEmpty(ordFee.get("quan_drug")) ? FDouble.ONE_DBL
						: new FDouble(ordFee.get("quan_drug").toString()));
				ciOrdFeeSrvDTO.setId_unit_med(CiOrdUtils.nullHandle(ordFee.get("mm_id_unit_med")));
			...
			ciOrdFeeSrvDTO.setFg_indic(CiOrdUtils.isEmpty(ordFee.get("fg_indic")) ? FBoolean.FALSE
					: new FBoolean(ordFee.get("fg_indic").toString()));
			ciOrdFeeSrvDTO.setFg_selfpay(CiOrdUtils.isEmpty(ordFee.get("fg_selfpay")) ? FBoolean.FALSE
					: new FBoolean(ordFee.get("fg_selfpay").toString()));
			...
			ciOrdFeeSrvDTO.setFg_bl((CiOrdUtils.nullHandle(ordFee.get("orsrv_fg_bl"))==null||"N".equals(CiOrdUtils.nullHandle(ordFee.get("orsrv_fg_bl"))))?FBoolean.FALSE:FBoolean.TRUE);
			// 添加医嘱状态和医嘱开立医生用于费用清单的新增编辑时使用
			// ciOrdFeeSrvDTO.setSd_su_or(CiOrdUtils.nullHandle(ordFee.get("sd_su_or")));
			// ciOrdFeeSrvDTO.setId_emp_or(CiOrdUtils.nullHandle(ordFee.get("id_emp_or")));
			ciOrdFeeSrvDTO.setEu_orsrvhp(CiOrdUtils.nullHandle(ordFee.get("eu_orsrvhp")));
