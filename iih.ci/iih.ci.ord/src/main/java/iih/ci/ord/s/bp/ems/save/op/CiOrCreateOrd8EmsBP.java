...
public class CiOrCreateOrd8EmsBP {
	...
	private void createOrderDOInfo(CiorderAggDO aggdo, CiEmsDTO ems, Hashtable ht) throws BizException {
		...
		// 草药是否院内制剂 by yzh 2018年4月9日 19:22:16
		...
		ordo.setFg_orhp(ems.getFg_orhp());
		...
		ordo.setId_excessive_reason(ems.getId_excessive_reason());// gh 超量原因
		ordo.setSd_excessive_reason(ems.getSd_excessive_reason());// gh 超量原因
		ordo.setId_dripspeed(ems.getId_dripspeed());// 滴速 liYue 2019.04.12
		...
		// {{
		// if (ordo.getDt_end().compareTo(ordo.getDt_effe()) > 0 &&
		// CiOrdUtils.isOpUrgentWf(ems.getCode_entp())) {
		// ordo.setFg_long(FBoolean.TRUE);
		// }
		// 临时医嘱开始时间等于结束时间
		if (ordo.getFg_long() == null || !ordo.getFg_long().booleanValue()) {
			ordo.setDt_end(ordo.getDt_effe());
		}
		// }}
	...
	private void setOrderDOInfo(CiorderAggDO aggdo, CiEmsDTO ems, Hashtable ht) throws BizException {
		...
		// ordo.setId_entp(ems.getId_entp());
		// ordo.setCode_entp(ems.getCode_entp());
		// 草药是否院内制剂 by yzh 2018年4月9日 19:22:52
		ordo.setFg_hospital_herbpres(ems.getFg_hospital_herbpres());
		...
		ordo.setCode_didef(ems.getCode_didef());
		ordo.setName_didef(ems.getName_didef());
		ordo.setPurpose_or(ems.getPurpose_or());
