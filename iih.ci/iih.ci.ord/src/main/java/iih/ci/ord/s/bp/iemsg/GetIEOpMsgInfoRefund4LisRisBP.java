...
public class GetIEOpMsgInfoRefund4LisRisBP extends GetIEOpMsgInfoRefundAbstractBP{
	...
	private List<Map<String, Object>> getLisRisIEData(String id_ors) throws BizException {
		// 查询出检查和检验的集成平台数据
		String sql = String.format(new CiIEMsgRelSqlConst().CI_OP_LISRIS_IE_SQL(), CiOrdUtils.getSqlInOrEqualStrs(id_ors));
	...
	private void setIeOpOrCancStpEnDTO(IEOpOrCancStpEnDTO ieOpOrCancStpEnDTO, Map<String, Object> map) {
		...
		ieOpOrCancStpEnDTO.setVisitordno(CiOrdUtils.nullHandle(map.get("visitordno")));
		ieOpOrCancStpEnDTO.setVisittype(CiOrdUtils.nullHandle(map.get("visittype")));
		ieOpOrCancStpEnDTO.setVisittypename(CiOrdUtils.nullHandle(map.get("visittypename")));
	}
	...
	private void setIEOpOrCancStpDTO(IEOpOrCancStpDTO iEOpOrCancStpDTO, Map<String, Object> map,
		...
		iEOpOrCancStpDTO.setOrder_type(CiOrdUtils.nullHandle(map.get("order_type")));//医嘱类型
		iEOpOrCancStpDTO.setOrder_type_name(CiOrdUtils.nullHandle(map.get("order_type_name")));//医嘱的类型编码
		//执行科室编码
		iEOpOrCancStpDTO.setExec_unit(CiOrdUtils.nullHandle(map.get("exec_unit")));
		//执行科室名称
		iEOpOrCancStpDTO.setExec_unit_name(CiOrdUtils.nullHandle(map.get("exec_unit_name")));
