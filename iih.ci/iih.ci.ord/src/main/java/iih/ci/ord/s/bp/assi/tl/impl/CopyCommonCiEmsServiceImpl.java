package iih.ci.ord.s.bp.assi.tl.impl;
...
import iih.ci.ord.s.ems.utils.SrvMatchEmsUtils;
...
public class CopyCommonCiEmsServiceImpl extends AbstractAssiCopy<CiOrTmplDTO, CiEmsDTO> {
	//医疗单匹配工具类
	private SrvMatchEmsUtils srvMatchEmsUtils = new SrvMatchEmsUtils();
	@Override
	public void copyPropertys(CiOrTmplDTO ciOrTmplDTO, CiEmsDTO ciEmsDto) throws BizException {
		...
		ciEmsDto.setDt_begin(CiOrdUtils.getCompareFormatServerDateTime(ciEmsDto.getId_en()));// 开始日期,取的格式化的系统开始时间

//		// 门诊不使用后台匹配医疗单
//		EmsMatchRstDTO emsmatch = CiOrdUtils.getFuncClassStr(envinfo, medSrvDO);
		SrvMatchEmsRstDTO matchEms = srvMatchEmsUtils.matchEms(envinfo,medSrvDO.getId_srv(),medSrvDO.getSd_srvtp());
		if (matchEms == null) {
			...
			ciEmsDto.setFuncclassstr(matchEms.getFuncclassstr());// 医疗单URL
			ciEmsDto.setId_srvof(matchEms.getId_ems());// 医疗单
			ciEmsDto.setFg_quickwflow(matchEms.getFg_quickwflow());
//			try {
//				if (!CiOrdUtils.isEmpty(matchEms.getFuncclassstr())) {
//					EmsFunclassKVDTO kv = ServiceFinder.find(IEmsManagementService.class)
//							.getEmsFunclassKVDTO(matchEms.getFuncclassstr());
//					ciEmsDto.setEmstype(Integer.parseInt(kv.getEmstype()));// 医疗单类型
//				}
//			} catch (Exception ex) {
...
//			}
		...
		/*
		 * TODO 移动到计算类中 if (ciEmsDto.getFg_mp_in() == FBoolean.TRUE) { // 在院执行时
		 * ciEmsDto.setTimes_mp_in(ciEmsDto.getTimes_cur());//
		 * 如果是在院执行，助手中的构造的医嘱等于总次数
		 * 
		 ...
		 */
		/*
		 * TODO 移动到 计算属性的类中 if (ciEmsDto.getSd_srvtp() != null &&
		 * ciEmsDto.getSd_srvtp().startsWith(IBdSrvDictCodeConst.
		 * SD_SRVTP_HERBDRUG)) { if (tmplSrvDTO != null &&
		 * tmplSrvDTO.getQuan_med() != null) { ciEmsDto.setNote(" " +
		 * ciEmsDto.getName_boil() + " " + tmplSrvDTO.getQuan_med() + " " +
		 * medSrvDO.getMed_name() + "," + ciEmsDto.getName_freq() + "," +
		 * ciEmsDto.getName_route()); } else { ciEmsDto.setNote(" " +
		 * ciEmsDto.getName_boil() + " " + medSrvDO.getQuan_med() + " " +
		 * medSrvDO.getMed_name() + "," + ciEmsDto.getName_freq() + "," +
		 * ciEmsDto.getName_route()); } } else { // ciEmsDto.setNote(); }
		 */
		ciEmsDto.setFg_mp_bed(FBoolean.FALSE);// 床边执行标识
												// 只有检查涉及到床旁执行，是否床旁执行属性通过OpCopyRisPropertyImpl中设置
		...
		ciEmsDto.setId_or_rel(ciOrder.getId_or_rel());
		// 存在保外诊断时，医保就诊状态调整为不需要判断医保状态（非医保状态）
		/*
		 * 添加到读取上下文环境 CopyContextCiEmsServiceImpl if
		 * (HpBeyondEnum.HPEXTERNALDIAG.equals(envinfo.getEu_hpbeyond())) {
		 * ciEmsDto.setEu_hpindicjudge(HpIndicJudgeEnum.NONEEDJUDGE); }
		 * 
		 * ciEmsDto.setBhpjudgerst(envinfo.getBhpjudgerst()); // 基本医保判断结果数据信息
		 * ciEmsDto.setDes_bhpjudgerst(envinfo.getDes_bhpjudgerst()); //
		 * 基本医保判断结果数据信息描述
		 */
		/*
		 * 移动到抽象类中 if (OrSourceFromEnum.IIHCLINICALKITHELPER ==
		 * ciOrTmplDTO.getEu_orsrcmdtp() && ciOrTmplDTO.getFg_set() ==
		 * FBoolean.TRUE) {// 来源组套 this.setMkrSetProperty(ciEmsDto, envinfo,
		 * ciOrTmplDTO, medSrvDO); } else { // 创建套内项目集合
		 * ciEmsDto.setSrvsetitms(this.createCiEmsSrvSetItems(ciOrTmplDTO));
		 * 
		 * // 医嘱项目 FArrayList list = this.getEmssrvs(envinfo, ciOrTmplDTO,
		 * medSrvDO); ciEmsDto.setEmssrvs(list);// 医疗单项目集合 }
		 * 
		 * // 设置服务价格 this.setCiEmsPrice(ciEmsDto, medSrvDO);
		 * 
		 * FArrayList emssrvList = ciEmsDto.getEmssrvs(); for (Object obj :
		 * emssrvList) { CiEmsSrvDTO srvDTO = (CiEmsSrvDTO) obj; if
		 * (srvDTO.getFg_skintest() == FBoolean.TRUE) {
		 * ciEmsDto.setFg_skintest(FBoolean.TRUE); break; } }
		 */
