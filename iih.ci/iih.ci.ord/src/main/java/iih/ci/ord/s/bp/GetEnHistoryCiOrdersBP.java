	...
	private FMap2 getCiorderAggMap(String code_entp, CiorderAggDO[] ciOrdAggs) throws BizException {
		...
		for (CiorderAggDO ciorderAgg : ciOrdAggs) {
			...
			for (OrdSrvDO ordSrv : ordSrvs) {
				...
				if (FBoolean.TRUE.equals(ordSrv.getFg_or()) && FBoolean.TRUE.equals(ordSrv.getFg_mm())&&!FBoolean.TRUE.equals(ordSrv.getFg_self())) {
				...
				if (!FBoolean.TRUE.equals(ordSrv.getFg_mm()) && ordSrv.getPri() != null
						&& ordSrv.getQuan_total_medu() != null) {
					FDouble srv_pri=ordSrv.getPri().multiply(ordSrv.getQuan_total_medu());
					FDouble or_total_pri= CiOrdUtils.CalculationPriRounding(2,srv_pri);
					// 计算非物品服务价格
					srvprice = srvprice.add(or_total_pri);
				}
			}
...
//		orderPriMap.putAll(mmPriMap);
		...
		this.getMmPriMap2(idOrdMap, ordSrvMmMap,orderPriMap);
	 ...
	 * @throws BizException
	 */
	private void getMmPriMap2(Map<String, String> idOrdMap, Map<OrdSrvMmDO, String> ordSrvMmMap,FMap2 ordSrvMmPriMap)
			throws BizException {
		...
		for (OrdSrvMmDO ordSrvMm : ordSrvMms) {
			...
			// 获取缓存的map结构中的物品医嘱价格
			if (ordSrvMmPriMap.containsKey(idOr)) {
				Object val=ordSrvMmPriMap.get(idOr);
				currPrice = (FDouble)val;
			...
			if (ordSrvMm.getPrice_pgku_cur() != null && ordSrvMm.getQuan_cur() != null) {
				FDouble srv_pri=ordSrvMm.getPrice_pgku_cur().multiply(ordSrvMm.getQuan_cur());
				FDouble or_total_pri= CiOrdUtils.CalculationPriRounding(2,srv_pri);
				currPrice = currPrice.add(or_total_pri);
			}
