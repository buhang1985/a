 ...
 *
 */
public class EmsOprCreateBP extends OpBaseEmsCreateBP {
	...
	public EmsRstDTO[] create(CiEnContextDTO ctx, EmsCrtDTO[] emsarray) throws BizException {
		...
		// 合成主UI模型对象
		EmsOpitemDO emsModel = BDEmsInfoCache.GetCache( ctx, ems.getDriverInfo().getId_ems(),
				ems.getId_srv());
		;
		if (null == emsModel) {
			emsModel = emsModelFrom(aggDO.getParentDO());
			//0155311: 在手术的诊疗单的动态指标项目设置为不允许为空，医生开立手术时，不填写指标项目依然能够签署
			emsModel.setId_srvof(ems.getDriverInfo().getId_ems());
		...
		if (null != ems.getDriverInfo()) {
			OrdApSugViewItemDO[] szOrdApSugViewItemDO = this.ordApSugViewItemListFrom(ctx,
	...
	protected EmsOpitemDO emsModelFrom(MedSrvDO medSrv) throws BizException {
		...
		ems.setNo_applyform(CiOrdUtils.getApplyNo(medSrv.getSd_srvtp())); // 申请单号
																			// SINGLE
																			// String
																			// 50
		...
		// ems.setSortno //排序号
		// String // 50
		ems.setEu_blmd(medSrv.getEu_blmd());
		return ems;
	}
