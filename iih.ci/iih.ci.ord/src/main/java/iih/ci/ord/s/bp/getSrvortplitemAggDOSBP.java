...
import iih.ci.ord.s.ems.cache.BDBoilInfoCache;
import iih.ci.ord.s.ems.cache.BDFreqInfoCache;
import iih.ci.ord.s.ems.cache.BDMeasInfoCache;
import iih.ci.ord.s.ems.cache.BDRouteInfoCache;
 ...
 *          北大医疗信息技术有限责任公司
 */
public class getSrvortplitemAggDOSBP {
	...
	public FMap getNewOrderTemplateDTO2(String Id_pripat, String id_hp, String code_entp) throws BizException {
			  ...
			  this.infoList = new ArrayList<OrdSrvChangedInfoDTO>();
		String whereStr = " 1=1";
		if (getEnTp(code_entp) != null) {
			whereStr += "  and " + getEnTp(code_entp);
			  }
		OrtmplAggDO[] aggdo = CiOrdAppUtils.getIOrtmplRService().find(whereStr, OrTmplDODesc.PRIMARYKEY_FIELDNAME,
				FBoolean.FALSE);
	...
	public FMap getNewOrderTemplateDTO2() throws BizException {
		...
		return HandleOrTmplate(aggdo, ctx.getId_pripat(), ctx.getId_hp(), ctx.getCode_entp());
	 ...
	 * 
       ...
       * @throws BizException
       */
	public FMap getNewOrderTemplateDTO2(String[] id_ortmpl, String Id_pripat, String id_hp, String code_entp)
			throws BizException {
			long start = System.currentTimeMillis();
			FMap  map = new FMap();
		    this.templateList = new FArrayList();
			  this.infoList = new ArrayList<OrdSrvChangedInfoDTO>();
		String whereStr = OrTmplDODesc.PRIMARYKEY_FIELDNAME + " in ( "
				+ CiOrdUtils.IdsConveretCharacterString(id_ortmpl) + " )";
		if (getEnTp(code_entp) != null) {
			whereStr += "  and " + getEnTp(code_entp);
			  }
		...
		CiOrdUtils.LogerOutInfo(
				" CiOrdAppUtils.getIOrtmplRService().find 模板项目耗时:" + (System.currentTimeMillis() - start2));
	...
	public FMap getNewOrderTemplateDTO2(String[] id_ortmpl) throws BizException {
		return getNewOrderTemplateDTO2(id_ortmpl, ctx.getId_pripat(), ctx.getId_hp(), ctx.getCode_entp());
	  }
	...
	public OrTplNItmDO[] selectSort(OrTplNItmDO[] ortmplItem) {
	        for (int x = 0; x < ortmplItem.length - 1; x++) {
				...
				if (ortmplItem[x].getSortno() == null)
					break;
	 ...
	 * 
	   ...
	   * @throws BizException
	   */
	private FMap HandleOrTmplate(OrtmplAggDO[] aggdo, String Id_pripat, String id_hp, String code_entp)
			throws BizException {
		  long start = System.currentTimeMillis();
		FMap map = new FMap();
		int num = 0;
	...
	private void HandleFMap2(FMap map, String Id_pripat, String id_hp, String code_entp) throws BizException {
		if (map != null && map.size() > 0) {
			  this.Id_mmMap = new HashMap<String, String>();
			  this.Id_srvMap = new HashMap<String, String>();
			...
			if (this.infoList != null && this.infoList.size() > 0) {
				this.ChangedSrvValidate(this.infoList.toArray(new OrdSrvChangedInfoDTO[this.infoList.size()]),
						code_entp);
				}
	...
	private void HandlOrTplNItmDOlist(FArrayList list) {
		if (list != null && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				...
				if (itemDo.getSd_srvtp() != null
						&& itemDo.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_DRUG)) {
					...
					if (!CiOrdUtils.isEmpty(itemDo.getId_srv_set())
							&& !this.Id_srvMap.containsKey(itemDo.getId_srv_set())) {
	...
	private void HandlOrTplNItmDOSetValue(FArrayList list, NewOrderTemplateDTO dto) {
		if (list != null && list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				if (itemDo.getSd_srvtp() != null
						&& itemDo.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_DRUG)) {
					if (this.mmMapPrice != null
							&& this.mmMapPrice.containsKey(itemDo.getId_srv() + "," + itemDo.getId_mm())) {
						itemDo.setPrice(
								formatePriceLength(this.mmMapPrice.get(itemDo.getId_srv() + "," + itemDo.getId_mm())));
					   }
					...
					if (this.bdsrvUserMap != null && this.bdsrvUserMap.containsKey(itemDo.getId_srv())) {
						...
						if (dto.getUi_flag() != null && dto.getUi_flag() == "1") {
							dto.setFg_active(FBoolean.FALSE);// 套的不可启用
																// （套有一个项目停用的
																// 套就停用 王琪）
	...
	private void OrtmplateConvertTONewTmplateDTO(OrtmplAggDO aggdo) throws BizException {
		...
		// Ui_flag 1 :套 , 2:(多药品)药品, 3:(单一药品), 5 草药显示, 6 诊疗 , 4 :其它
		// (NewTemplteDTO 的 str 赋值显示子项没有checkbox)
		  // 模板类型
			/// 成组西成药
	...
	private void MedicareCataLogMm(String id_hp, List<String> id_mms) {
		...
		if (id_hp == null || id_hp == "") {
			...
			// select
			// bd_hp_srvorca.id_hp,bd_hp_srvorca.id_srv,bd_hp_srvorca.id_mm,
			// bd_udidoc.name
	...
	private void getTemplateType(String id_ortmpl) throws BizException {
		...
		/*
		 * for(OrTplNItmDO ortmpl: orTplItem){ NewOrderTemplateDTO template =
		 * new NewOrderTemplateDTO(); template.setFreqdefdo(freqMap);
		 * template.setMeasdocdo(measDocMap); template.setRoutedo(routeMap);
		 * template.setBoildo(boilMap);
		 * template.setTemplatetype(FBoolean.FALSE);// 其它模板时N ，草药模板时 Y
		 * 页面显示控件判断使用 this.setNewTemplateDTO(template, ortmpl,aggdo);
			...
			for (OrTplNItmDO item : orTplItem) {
				if (OrTplItmTypeEnum.ORTMPL.equals(item.getEu_ortplitmtp())) {
					...
					if (item.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_WESTDRUG)
						...
						// NewOrderTemplateDTO template = new
						// NewOrderTemplateDTO();
					...
					// template.setItem(Item);
					 ...
					 template.setId_srv(item.getId_srv());
					template.setStr3(getFgsetradio(item.getId_srv()));// 套是否单选模式
					template.setTemplatetype(FBoolean.FALSE);// 其它模板时N ，草药模板时 Y
																// 页面显示控件判断使用
					 this.templateList.append(template);
					 this.NewTmplateList.append(template);
	...
	private String getFgsetradio(String id_srv) throws BizException {
		  MedSrvDO medsrvDO = CiOrdAppUtils.getMedsrvMDORService().findById(id_srv);
		if (medsrvDO != null && FBoolean.TRUE.equals(medsrvDO.getFg_setradio())) {
			  return "Y";
		  }
		return  "N";
	  }
	...
	private void setOrTemplateCYFJ(OrtmplAggDO aggdo) throws BizException {
	  ...
	  /**
	   *  
	 * Ui_flag 1 :套 , 2:(多药品)药品, 3:(单一药品), 5 草药显示, 4 :其它 (NewTemplteDTO 的 str
	 * 赋值显示子项没有checkbox) 判断医嘱模板的类型
	 * 
	   * @param template
	   * @param ortmpl
	   */
	private void setNewTemplateDTO(NewOrderTemplateDTO template, OrTplNItmDO ortmpl, OrtmplAggDO aggdo)
			throws BizException {
		if (ortmpl != null) {
			...
			if (OrTplItmTypeEnum.ORTMPL.equals(ortmpl.getEu_ortplitmtp())) {
				...
				/*
				 * 01 成组西成药 映射 2 多药品 02 草药方剂 映射 4 str 赋值 11 复合模板 继续拆分
				 */
				...
				if (ortmpl.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_WESTDRUG)
						|| ortmpl.getSd_srvtp().startsWith(IBdSrvDictCodeConst.SD_SRVTP_CYDRUG)) {
					 template.setUi_flag("3");
					 template.setId(ortmpl.getId_ortmpl());
					 list.add(ortmpl);
					 template.setItemlist(list);
	...
	private void MappingOrItem(FArrayList list, MedSrvSetItemDO[] rtn, OrTplNItmDO ortmpl) {
		if (rtn != null && rtn.length > 0) {
			for (MedSrvSetItemDO medSrvset : rtn) {
				...
				item.setOrtplnitm_mm_name(ortmpl.getOrtplnitm_srv_name());//// 借用这个字段存套的
																			//// 名称
																			//// 需求还在
																			//// ，待优化
	...
	private void getOrTplNItmDOFArrayList(String id_ortmpl, NewOrderTemplateDTO template) throws BizException {
		...
		OrTplNItmDO[] temp = CiOrdAppUtils.getIOrTplNItmDORService().find(whereStr, OrTplNItmDO.ID_ORTMPL,
				FBoolean.FALSE);
	...
	public OrderTemplateDTO getSrvortplitemAggDOS(String id_srvortpl) throws BizException {
		if (id_srvortpl == null)
			throw new BizException("parameter :id_srvortpl is null ");
		 OrderTemplateDTO  orderTemplateDto = new OrderTemplateDTO();
		 FMap  srvoritemAgg = new FMap();
		SrvortplitemAggDO[] srvortplItem = CiOrdAppUtils.getISrvortplitemRService()
				.find("a0.ds ='0' and  a0.id_srvortpl ='" + id_srvortpl + "'", "id_srvortpl", FBoolean.FALSE);
	...
	private FArrayList srvOrTplItem(SrvortplitemAggDO[] srvortplItem) throws BizException {
		...
		for (SrvortplitemAggDO item : srvortplItem) {
			...
			// 1 :套
			if (item.getParentDO().getFg_set() != null && item.getParentDO().getFg_set().booleanValue()) {
			...
			// 2:(多药品)药品,
			else if (item.getParentDO().getSd_srvtp() != null && item.getParentDO().getSd_srvtp().startsWith("01")
  ...
  // 服务套 项目
	private void getBdSrvSetItem(SrvortplitemAggDO item) throws BizException {
		if (item != null && item.getParentDO().getId_srv() != null && item.getParentDO().getFg_set().booleanValue()) {
   ...
   */
	...
	public FMap getFreqMap() throws BizException {
		FreqDefDO[] szFreqInfo = BDFreqInfoCache.GetBdFreqInfos(ctx);

		if (null == szFreqInfo || szFreqInfo.length == 0) {
			FMap rstFreqInfoMap = new FMap();
			FArrayList freqdo = new FArrayList();
			for (FreqDefDO freq : szFreqInfo) {
	...
	public FMap getBoilMap() throws BizException {
		CHerbBoilMdDO[] szCHerbBoilMdDO = BDBoilInfoCache.GetBdBoilInfos(ctx);
		if (null != szCHerbBoilMdDO){
			FMap rstBoilInfoMap = new FMap();
	        	FArrayList measlist =  new FArrayList();
			for (CHerbBoilMdDO item : szCHerbBoilMdDO) {
	...
	public FMap getMeasDocMap() throws BizException {
		MeasDocDO[] szMeasInfo = BDMeasInfoCache.GetBdMeasInfos(ctx);
		if (null != szMeasInfo){
			FMap rstMeasDocMap = new FMap();
	        	FArrayList measlist =  new FArrayList();
			for (MeasDocDO item : szMeasInfo) {
	...
	public FMap getRouteMap() throws BizException {
		MedUsageDO[] szMedUsageDO = BDRouteInfoCache.GetBdRouteInfos(ctx);
		if (null != szMedUsageDO){
		    	    
			FMap rstRouteInfoMap = new FMap();
		        	FArrayList measlist =  new FArrayList();
				for (MedUsageDO medUsage : szMedUsageDO) {
