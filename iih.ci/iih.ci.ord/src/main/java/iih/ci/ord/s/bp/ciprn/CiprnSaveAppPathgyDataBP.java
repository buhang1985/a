...
public class CiprnSaveAppPathgyDataBP {
	...
	private CiapppathgysheetAggDO[] getCiapppathgysheetAggDOs(CiOrderDO[] ciors, Map<String, CiorapppathgyAggDO> map, CiOrSessionDO session) throws BizException{
		...
		String[] pathgyNOs = CiOrdUtils.generateNormNOs(ciors.length, ciAppRisSheetDO);
		int i = 0;
		...
		for (CiOrderDO cior : ciors) {
			...
			if (context != null
					&& (context.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_OP) 
				...
				pathgySheetDO.setHecominsurinfo(context.getName_pat() + ":" + CiOrdAppUtils.getServerDateTime());
			} else {
