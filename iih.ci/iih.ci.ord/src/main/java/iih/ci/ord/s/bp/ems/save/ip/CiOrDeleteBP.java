...
public class CiOrDeleteBP {
	...
	public void exec(String id_or) throws BizException {
		// 有效性校验
		if (OrSrvSplitUtil.isEmpty(id_or))
			return;
		...
		// 医嘱对应的检验申请相关信息的删除
		if (emstype.equals(EmsType.LIS)) {
			ciOrdApLabDODelete(id_or);
		}

		// 医嘱对应的检查申请相关信息的删除
		if (emstype.equals(EmsType.RIS)) {
			ciOrdApObsDODelete(id_or);
		}

		// 医嘱对应的会诊申请相关信息的删除
		if (emstype.equals(EmsType.CONS)) {
			ciOrdApConsDODelete(id_or);
		}
		...
		// 备血对应的申请单相关信息的删除
		if (emstype.equals(EmsType.BT)) {
			ciOrdApbtDODelete(id_or);
		}

		// 手术对应申请单相关的信息的删除
		if (emstype.equals(EmsType.OPER)) {
			ciOrdApOpDODelete(id_or);
		}

		// 病理对应的申请单相关的信息的删除
		if (emstype.equals(EmsType.PATHGY)) {
			ciOrdApPathgyDODelete(id_or);
		}

		// 用血对应的申请单相关信息的删除
		if (emstype.equals(EmsType.BTUSE)) {
			ciOrdApbuDODelete(id_or);
		}
		;

		// 转科对应的申请单相关信息的删除
		if (emstype.equals(EmsType.TRANSDEPT)) {
			ciOrdAptransdeptDODelete(id_or);
		}
		;

		// 出院对应的申请单相关信息的删除
		if (emstype.equals(EmsType.OUTHOSP)) {
			ciOrdApoutDODelete(id_or);
		}
		;
		...
		// 记录删除日志
		CiOrdUtils.log(new CiorderAggDO[] { aggdo });
		// 处方相关数据的删除 暂不做处理
	}
	/**
	 * 强删医嘱
	 ...
	 * @throws BizException
	 */
	public void forceDelete(String[] id_ors) throws BizException {
	...
	public void forceDelete(String id_or) throws BizException {
		// 有效性校验
		if (OrSrvSplitUtil.isEmpty(id_or))
			return;
		...
		if (aggdo == null)
			return;
		// 兰陵BUG：已经计费的医嘱 不能删除
		if (ICiDictCodeConst.SD_SU_BL_ACCOUNT.equals(aggdo.getParentDO().getSd_su_bl()))
			throw new BizException("已经计费的医嘱 不能删除");
		Integer emstype = getEmsType(aggdo.getParentDO().getSd_srvtp());
		if (aggdo == null)
			return;
		//删除处方
		...
		// 临床医嘱数据信息删除
	    aggdo = CiOrdAppUtils.getOrAggQryService().findById(id_or);
		ciOrDelete(aggdo);
		// 记录删除日志
		CiOrdUtils.log(new CiorderAggDO[] { aggdo });
	}
	 ...
	 * @throws BizException
	 */
	private void delPres(String id_or) throws BizException {
		// 清医嘱项目中处方标识
		MOdOrdsrv4PresBP bpMOdOrdsrv = new MOdOrdsrv4PresBP();		
//		bpMOdOrdsrv.delPres(new String[]{id_or});
		bpMOdOrdsrv.exec(new String[]{id_or});
	}
	...
	private void ciOrSkinTestRstDelete(CiorderAggDO aggdo) throws BizException {
		...
		// 有效性判断
		if (!ICiDictCodeConst.SD_RELTYPE_SKINTEST.equals(ordo.getSd_reltp()))
			return;
		if (CiOrdUtils.isEmpty(ordo.getId_reltp()))
			return;
