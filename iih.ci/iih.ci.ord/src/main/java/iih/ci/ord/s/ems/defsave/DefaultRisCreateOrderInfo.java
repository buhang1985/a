...
public class DefaultRisCreateOrderInfo extends DefaultSetCreateOrderInfo {
	...
	public OrderPackageInfo[] createOrderPakageInfo(CiEnContextDTO ctx, DefaultCreateParam[] szParam) throws BizException {
		...
		MedSrvRisDO[] srvrisdos= ServiceFinder.find(IMedSrvRisDORService.class).findByAttrValStrings("Id_srv", id_srvs);

		if(srvrisdos!=null && srvrisdos.length>0){
			for(MedSrvRisDO srvris:srvrisdos){
				ordApLabDOMap.put(srvris.getId_srv(), assembleOrderRisInfo(srvris));
			}
		}else{
			for(String id_srv:id_srvs){
				ordApLabDOMap.put(id_srv, assembleOrderRisInfo());
			}
		}
	...
	public OrdApObsDO assembleOrderRisInfo(){
		OrdApObsDO ordris=new OrdApObsDO();
		ordris.setStatus(DOStatus.NEW);
		ordris.setId_orobs(this.generatePks());//医嘱检查申请单主键标识	
		//		ordris.setId_or(orderInfo.getId_or());//医嘱	
