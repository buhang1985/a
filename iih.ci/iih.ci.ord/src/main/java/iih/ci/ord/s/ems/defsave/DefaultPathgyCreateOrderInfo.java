...
public class DefaultPathgyCreateOrderInfo extends DefaultSingleSrvCreateOrderInfo {
	...
	public OrderPackageInfo[] createOrderPakageInfo(CiEnContextDTO ctx, DefaultCreateParam[] szParam) throws BizException {
		...
		MedSrvLisDO[] srvpathdos= ServiceFinder.find(IMedSrvLisDORService.class).findByAttrValStrings("Id_srv",id_srvs);
		if(srvpathdos!=null && srvpathdos.length>0){
			...
			for(String id_srv:id_srvs){
				...
				OrdApPathgyDO pathgydo=new OrdApPathgyDO();
				pathgydo.setStatus(DOStatus.NEW);
				aggdo.setParentDO(pathgydo);
				consMap.put(id_srv, aggdo);
			}
		}
		...
		for(OrderPackageInfo info:szOrderPakageInfo){
			...
			if(consMap.containsKey(id_srv)){
				consMap.get(id_srv).getParentDO().setId_or(info.getOrderInfo().getId_or());
	...
	private CiorapppathgyAggDO assembleOrderPathgyAggInfo(MedSrvLisDO srvris){
		CiorapppathgyAggDO aggdo=new CiorapppathgyAggDO();
...
//		ordpath.setNo_applyform();
		...
		ordpath.setQuan(srvris.getQuan()==null?0:Integer.parseInt(srvris.getQuan().toString()));
