...
import iih.ci.ord.s.ems.defsave.DefaultApBuCreateOrderInfo;
...
import iih.ci.ord.s.ems.meta.StringObjectMap;
import iih.ci.ord.s.ems.op.base.OpBaseEmsSaveBP;
import iih.ci.ord.s.ems.op.ems.apbu.EmsApBuValidate;
...
public class TmplBaseApBuSaveBP extends OpBaseEmsSaveBP {
	...
	protected DefaultCreateParam[] defaultCreateParamFrom(List<Object> listUiModel) throws BizException{
		...
		for (Object uiModel : listUiModel){
			EmsBuItemDO doInfo = (EmsBuItemDO)uiModel;
	...
	protected OrderKey2UiModelMap assembleOrderKey2UiModelMap(List<Object> listUiModel) {
		...
		for (int index = 0; index < listUiModel.size(); ++index) {
			EmsBuItemDO itemDO = (EmsBuItemDO) listUiModel.get(index);
	...
	protected void mergeOrderInfo(CiEnContextDTO ctx,CiOrderDO ordo, Object uiModel) throws BizException{
		EmsBuItemDO srvInfo = (EmsBuItemDO)uiModel;
		CiOrderDO btord=getApBtSrv(srvInfo.getId_or());
		ordo.setId_srv(srvInfo.getId_srv());

		ordo.setContent_or(CiOrContentUtils.create(srvInfo.getName_srv(),srvInfo.getName_route(),srvInfo.getQuan_medu_bu(),srvInfo.getName_unit()).toString());
		...
		if(btord!=null){
			...
			ordo.setId_or_rel(srvInfo.getId_or());
		}
	...
	protected void mergeOrderSrvInfo(CiEnContextDTO ctx,OrdSrvDO srvdo, Object uiModel) throws BizException {
		...
		srvdo.setQuan_medu(srvInfo.getQuan_medu_bu());
		...
		srvdo.setId_org_mp(CiOrdUtils.getOrgOfDept(srvInfo.getId_dep_mp()));
		...
		srvdo.setId_dep_mp(srvInfo.getId_dep_mp());
	...
	protected void mergeOrderAppInfo(CiEnContextDTO ctx, List<Object> appSheetList,List<Object> extInfoList,Object uiModel)throws BizException{
		EmsBuItemDO srvdto=(EmsBuItemDO)uiModel;
	...
	private void ordApBuInfo(EmsBuItemDO srvInfo,OrdAppBtUseDO ordbu){
		ordbu.setId_apbu(srvInfo.getId_apbu());
//		ordbu.setId_or(srvInfo.getid);
		ordbu.setDt_bu_plan(srvInfo.getDt_pl_bu());
		ordbu.setNo_applyform(srvInfo.getNo_applyform_bu());
