...
public class GetIEOpMsgInfo4RisBP {
	...
	public BaseDTO[] exec(String id_ors) throws BizException{
		...
		if(StringUtils.isNotEmpty(sql)) {
			...
			OrdBizLogger.info(ctx, "++++++++检查收费 sql:"+sql);
		}
		//查询联系人集合
		...
		for (Map<String, Object> map2 : maps) {
			...
			ierisdto.setAge(AgeCalcUtil.getAge(new FDate(ierisdto.getBirthday().toString())));
			//赋值联系人集合
	...
	private void setRisEnDto8Map(IEOpRisOrEnDTO ieoprisdto, Map<String, Object> map) {
		...
		ieoprisdto.setVisitordno(CiOrdUtils.nullHandle(map.get("visitordno")));
		ieoprisdto.setVisittypecode(CiOrdUtils.nullHandle(map.get("visittypecode")));
		ieoprisdto.setVisittypename(CiOrdUtils.nullHandle(map.get("visittypename")));
		//健康卡号
