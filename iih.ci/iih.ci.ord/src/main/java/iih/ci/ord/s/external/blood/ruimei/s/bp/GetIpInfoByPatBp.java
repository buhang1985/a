...
public class GetIpInfoByPatBp {
	


	public String exec(PatcodeParam param) throws BizException {
...
//		String rtn = getXmlStr(document);
	 ...
	 * @throws BizException
	 */
	private void validation(PatcodeParam param, List<String> listMsg) {

		if (param == null) {
			listMsg.add("参数空异常");
		}

		if (StringUtil.isEmptyWithTrim(param.getPatcode())) {
			listMsg.add("“患者编码”没被赋值");
		}
		if (StringUtil.isEmptyWithTrim(param.getTimes_ip())) {
			listMsg.add("“住院次数”没被赋值");
		}
	 ...
	 * @throws BizException
	 */
	private Patientinfo[] getIpInfo(PatcodeParam param) throws BizException {
		QueryPatientInfoBp bp = new QueryPatientInfoBp();
		return bp.exec(null,param,null);
	}
