 ...
 */
public class GetLatelySkinTestRelInfoBP {
	 ...
	 * @throws BizException
	 */
	public SkinTestRstDTO[] getPiValidSkinTestOrInfo(String id_pi,FDateTime[] secttime) throws BizException{
		GetPiValidSkinTestOrInfoBP bp=new GetPiValidSkinTestOrInfoBP();
		return bp.exec(id_pi, secttime);
	}
	/**
	 * 需皮试用药的药品存在有效的已执行用药记录判断
	 * @param id_pi
	...
	/**
	 * 需皮试用药的药品存在有效的已执行用药记录
	 * @param skintestparam
	 * @return
	 * @throws BizException
	 */
	public SkinTestUsePharmRstDTO existValidisNeedSkinExOr(SkinTestParamDTO skintestparam) throws BizException{
		if(skintestparam.getId_org()==null)skintestparam.setId_org(Context.get().getOrgId());
		//获得有效天数  如：1或3天
		int days=getDays4SkinTestInfo(skintestparam.getId_org(),skintestparam.getDt_birth());
		...
		//需皮试用药的药品存在有效的已执行用药记录判断
		if(existValidisNeedSkinExOr(skintestparam.getId_pi(),skintestparam.getId_srv(),skintestparam.getId_mm(),dts).booleanValue())
			return getSkinTestUsePharmRst(NDaysValidThingEnum.WITHEXECUSEPHARM,null,null);
		return null;
	}
	/**
	 * 获得有效期内全部的有效皮试结果记录
	 ...
	 * @throws BizException
	 */
	public SkinTestRstDTO[] getPiValidSkinTestOrInfo(SkinTestParamDTO skintestparam) throws BizException{
		if(skintestparam.getId_org()==null)skintestparam.setId_org(Context.get().getOrgId());
		//获得有效天数  如：1或3天
		int days=getDays4SkinTestInfo(skintestparam.getId_org(),skintestparam.getDt_birth());
		...
		//获得患者有效皮试医嘱数据信息
		SkinTestRstDTO[] validskinorinfo=getPiValidSkinTestOrInfo(skintestparam.getId_pi(),dts);
		if(!CiOrdUtils.isEmpty(validskinorinfo)) return validskinorinfo;
		return null;
	}
}
