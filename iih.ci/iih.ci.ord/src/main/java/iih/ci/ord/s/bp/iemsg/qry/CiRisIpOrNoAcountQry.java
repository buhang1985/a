 ...
 *
 */
public class CiRisIpOrNoAcountQry implements ITransQry {

	private String _id_ors;
    private String str_idor_ris;//普通检查
	private String str_idor_path;//病理检查
	
	public CiRisIpOrNoAcountQry(String id_ors){
		this._id_ors = id_ors;
	}
	@Override
	 ...
	 */
	 private String getSql() throws BizException{
		 ...
		 if(str_idor_path.length()>0){
			 formatsql +=  new Pat4IESql().CiIeApppathAllSql( str_idor_path.substring(0, str_idor_path.lastIndexOf(",") ));//病理
		 }
		 if(str_idor_ris.length()>0){
			 ...
			 formatsql +=  new Ris4IESql().CiIeAppRisAllSql( str_idor_ris.substring(0, str_idor_ris.lastIndexOf(",")));//其他普通
