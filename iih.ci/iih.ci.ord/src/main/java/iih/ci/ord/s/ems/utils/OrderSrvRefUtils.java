...
public class OrderSrvRefUtils {
	...
	public static OrdSrvDO[] RouteRelFeeSrvs(CiEnContextDTO ctx, CiOrderDO orderInfo) throws BizException{
		...
		for (UsageRelFeeSrvDO feeSrvDO : feeSrvDOs) {
			...
			srvdo.setId_emp_srv(CiOrdAppUtils.getEnvContext().getStuffId());
			...
			srvdo.setDt_last_bl(orderInfo.getDt_last_bl());
			...
			//执行科室赋值
			OrWfDeptInfoDTO wf = DeptInfoUtils.GetOrWfDeptInfo(ctx, srvdo.getId_srv(),null, srvdo.getId_route(), srvdo.getId_dep_mp(),orderInfo.getFg_long(),orderInfo.getFg_pres_outp(),orderInfo.getDt_effe());
			if (wf != null) {
	...
	public static OrdSrvDO[] BoilRelFeeSrvs(CiEnContextDTO ctx, CiOrderDO orderInfo) throws BizException{
		//Ent4BannerDTO banner = ctx.getEnt4BannerDTO();
		UsageRelFeeSrvDO[] feeSrvDOs = CiOrdUtils.getPriBoilRelFeeSrvInfo(ctx.getId_org(), orderInfo.getId_boil(),orderInfo.getOrders_boil());
		if(CiOrdUtils.isEmpty(feeSrvDOs)) 
			return null;
		List<String> relSrvIds = new ArrayList<String>();
		for (UsageRelFeeSrvDO feeSrvDO : feeSrvDOs) {
