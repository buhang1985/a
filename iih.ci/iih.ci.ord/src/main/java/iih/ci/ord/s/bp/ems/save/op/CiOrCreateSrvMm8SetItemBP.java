...
//		srvdo.setQuan_medu(tmpdo.getQuan_medu());
		//套内项目剂量按照医嘱上的剂量取值 by yzh 2017-12-04 09:07:26
		...
		if(OrSrvSplitUtil.isTrue(srvdo.getFg_mm())){
			materials = BDMmDoseUnitInfoCache.getMaterialUnitInfos(IBdFcDictCodeConst.SD_CODE_ENTP_OP, new String[] {srvrelmmdo.getId_mm()});
		}
		if(CiOrdUtils.isOpMode(ems.getCode_entp(), ems.getFg_pres_outp(),srvdo.getSd_srvtp()).booleanValue()){
			...
			if(!CiOrdUtils.isEmpty(srvinsetdo.getFg_mm())&&srvinsetdo.getFg_mm().booleanValue()){
				if(!CiOrdUtils.isEmpty(materials)){
					String sd_mutp = "";
					if(IBdFcDictCodeConst.SD_CODE_ENTP_OP.equals(ems.getCode_entp())) {
						sd_mutp = srvrelmmdo.getSd_opmutp();
					} else if(IBdFcDictCodeConst.SD_CODE_ENTP_ET_FLOW.equals(ems.getCode_entp())) {
						sd_mutp = srvrelmmdo.getSd_ermutp();
					}  else if(IBdFcDictCodeConst.SD_CODE_ENTP_ET_FSTAID.equals(ems.getCode_entp())) {
						sd_mutp = srvrelmmdo.getSd_er1mutp();
					}
					
					srvdo.setQuan_total_medu(bp.getMMQuantum(sd_mutp, srvinsetdo.getQuan_med().multiply(srvdo.getQuan_medu()), srvrelmmdo.getFactor_mb(), materials[0].getFactor(), ems.getTimes_cur()));
				}
				...
				srvdo.setQuan_total_medu(bp.getQuantum(srvinsetdo.getQuan_med().multiply(srvdo.getQuan_medu()), ems.getTimes_cur()));
			}
		}
