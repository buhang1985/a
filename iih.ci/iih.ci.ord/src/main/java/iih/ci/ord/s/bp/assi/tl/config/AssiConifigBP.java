package iih.ci.ord.s.bp.assi.tl.config;
...
public class AssiConifigBP {
	 ...
	 *            执行点
	 * @param envinfo
	 *            当前上下文就诊环境， 取其中的 code_entp 就诊类型,orSrouceFrom 数据来源
	 * @param sd_srvtp
	 *            服务类型
	 * @return
	 * @throws BizException
	...
	private static AssiDynamicConfigDTO getMatchConfig(List<AssiDynamicConfigDTO> matchConfigList, String sd_srvtp,
		...
		for (AssiDynamicConfigDTO dynamicConfig : matchConfigList) {
			...
			if (isAllowed(sd_srvtp, allowedSdSrvtp)) {
				...
				} else if (sd_srvtp.startsWith(dynamicConfig.getSd_srvtp())) {
	...
	private static Map<String, List<AssiDynamicConfigDTO>> getConfigResult() {
		...
		config.setSd_srvtp(IBdSrvDictCodeConst.SD_SRVTP_HERBDRUG);
		...
		configBaseIP.setFullClassPath("iih.ci.ord.s.bp.assi.tl.impl.cimes.base.IpBaseCopyCalCiEmsProperty");
		...
		configIp.setSd_srvtp(IBdSrvDictCodeConst.SD_SRVTP_HERBDRUG);
		configIp.setAllowed_sd_srvtp(IBdSrvDictCodeConst.SD_SRVTP_HERBDRUG);
		configIp.setExclude_sd_srvtp("");
		configIp.setFullClassPath("iih.ci.ord.s.bp.assi.tl.impl.cimes.op.OpCopyHerbsPropertyImpl");
		...
		config0506Ip.setFullClassPath("iih.ci.ord.s.bp.assi.tl.impl.cimes.op.OpCopyTreatSkinTestPropertyImpl");
		...
		config02Ip.setFullClassPath("iih.ci.ord.s.bp.assi.tl.impl.cimes.op.OpCopyRisPropertyImpl");
		...
		config03Ip.setFullClassPath("iih.ci.ord.s.bp.assi.tl.impl.cimes.op.OpCopyLisPropertyImpl");
		...
		config05Ip.setFullClassPath("iih.ci.ord.s.bp.assi.tl.impl.cimes.op.OpCopyTreatPropertyImpl");
		listIp.add(config05Ip);

		// 护理
		AssiDynamicConfigDTO config06Ip = new AssiDynamicConfigDTO();
		config06Ip.setExtensionPoint(ExtensionPointEu.EndCiEms);// 执行的操作切入点。
		config06Ip.setCode_entp(IBdFcDictCodeConst.SD_CODE_ENTP_IP);
		config06Ip.setSd_srvtp(IBdSrvDictCodeConst.SD_SRVTP_NURSE);
		config06Ip.setAllowed_sd_srvtp("");
		config06Ip.setExclude_sd_srvtp("");
		config06Ip.setFullClassPath("iih.ci.ord.s.bp.assi.tl.impl.cimes.op.OpCopyTreatPropertyImpl");
		listIp.add(config06Ip);
		...
		config08Ip.setFullClassPath("iih.ci.ord.s.bp.assi.tl.impl.cimes.base.OpBaseCopyCalCiEmsProperty");
		...
		config1401Ip.setFullClassPath("iih.ci.ord.s.bp.assi.tl.impl.cimes.ip.IpCopyApbtPropertyImpl");
		...
		config1402Ip.setFullClassPath("iih.ci.ord.s.bp.assi.tl.impl.cimes.ip.IpCopyApbuPropertyImpl");
