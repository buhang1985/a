...
public class GetPatiAllergicHisBP {
	...
	public PiPatAlDO[] exec(String id_empi, String id_pharm_master, String dt_min, String dt_max) throws BizException {
		...
		// 查询结果返回
		return CiOrdAppUtils.getIPiPatAlDORService().find(whereStr,
				PiPatAlDODesc.TABLE_ALIAS_NAME + CiOrdUtils.FULLSTOP_STR + PiPatAlDO.DT_ACT + " desc", FBoolean.FALSE);
	}
	/**
	 * 获得患者药品过敏史数据信息，根据过敏分类进行判断
	 * @param id_empi
	 * @param dt_min
	 * @param dt_max
	 * @return
	 * @throws BizException
	 */
	public PiPatAlDO[] exec(String id_empi, String dt_min, String dt_max) throws BizException {
		// 有效性判断
		if (CiOrdUtils.isEmpty(id_empi))
			return null;
		...
		// 获得条件串
		String tblalias = PiPatAlDODesc.TABLE_ALIAS_NAME + CiOrdUtils.FULLSTOP_STR;
		String whereStr = getWhereStr(id_empi, dt_min, dt_max, tblalias);
	 ...
	 * @return
	 */
	private String getWhereStr(String id_empi, String dt_min, String dt_max, String tblalias) {
		String whereStr = tblalias + PiPatAlDO.ID_PAT + CiOrdUtils.EQUAL_STR + "'" + id_empi + "'";
		if (!CiOrdUtils.isEmpty(dt_min)) {
			whereStr += CiOrdUtils.AND_STR + tblalias + PiPatAlDO.DT_ACT + CiOrdUtils.GREATTHAN_EQUAL_STR + "'" + dt_min
					+ "'";
		...
		if (!CiOrdUtils.isEmpty(dt_max)) {
			whereStr += CiOrdUtils.AND_STR + "substr(dt_act,0,10)" + CiOrdUtils.LESSTHAN_EQUAL_STR + "'" + dt_max + "'";
		}
		return whereStr;
	}
}
