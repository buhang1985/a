 ...
 *
 */
public class EmsConsLoadBP extends OpBaseEmsLoadBP {
	...
	public EmsRstDTO[] load(CiEnContextDTO ctx, EmsLoadDTO[] args) throws BizException {
		...
		EmsConsItemDO ems = modelFrom(ctx, ciEmsInfo, medSrvAggInfo);
		// 医疗单对象
		...
		// 获取执行科室
		OrWfDeptInfoDTO wf = DeptInfoUtils.GetOrWfDeptInfo(ctx, medSrvAggInfo.getParentDO().getId_srv(),null,ciEmsInfo.getId_route(),null,ciEmsInfo.getFg_long(),ciEmsInfo.getFg_pres_outp(),ciEmsInfo.getDt_begin());
		if (null == wf) {
			// 封装错误信息
	...
	public EmsConsItemDO modelFrom(CiEnContextDTO ctx, CiEmsDTO dto, MedsrvAggDO medSrvAggInfo) throws BizException {
		...
		if (dto.getOrapplysheet() != null) {
			CiorappconsultAggDO ciOrAppConsultAggInfo = (CiorappconsultAggDO) dto.getOrapplysheet()
					.get(EmsType.CONS.toString());
	 ...
	 * @throws BizException
	 */
	private void ConstructApConsInfo(EmsConsItemDO ems, CiorappconsultAggDO ciOrAppConsultAggInfo, CiEmsDTO dto,
			CiEmsSrvDTO mainSrvInfo) throws BizException {

		OrdApConsDO ordApConsInfo = ciOrAppConsultAggInfo.getParentDO();

		ems.setId_emsconsitem(ordApConsInfo.getId_apcons()); // 主键 SINGLE String
																// 50
	...
	private void constructConsEmpInfo(EmsConsItemDO ems, CiorappconsultAggDO agg) {
		...
		for (CiordInviteConsDO item : agg.getCiordInviteConsDO()) {
			...
			emsItemInCons.setId_emsitemincons(item.getId_invitecons());// 主键 主键
																		// SINGLE
			...
			emsItemInCons.setName_dep_emp(item.getName_dep());// 受邀科室 SINGLE
																// String
			emsItemInCons.setSd_emp_title(item.getSd_emp_title());// 受邀职称编码
																	// SINGLE St
			emsItemInCons.setId_emp_title(item.getId_emp_title());// 受邀职称id REF
																	// 人员聘任职
			emsItemInCons.setName_emp_title(item.getName_emp_title());// 受邀职称
																		// SINGLE
																		// String
			emsItemInCons.setId_emp_doctor(item.getId_emp());// 受邀人id REF 用户 20
			emsItemInCons.setName_emp_doctor(item.getName_emp());// 受邀人 SINGLE
																	// String 50
			emsItemInCons.setDt_response(item.getDt_response());// 应答时间 SINGLE
																// FDateT
			emsItemInCons.setFg_response(item.getFg_response());// 应答标志 SINGLE
																// FBoole
			emsItemInCons.setId_emp_response(item.getId_emp_response());// 应答人id
																		// REF
																		// 用户 20
			emsItemInCons.setName_emp_response(item.getName_emp_respon());// 应答人
																			// SINGLE
																			// String
																			// 50
			...
			emsItemInCons.setName_constp(cons.getName_constp());// 会诊类型 SINGLE
																// String
			emsItemInCons.setSv(item.getSv()); // ????? 后加
			
			if(index==0){
				ems.setId_dep_emp(item.getId_dep());
				ems.setName_dep_emp(item.getName_dep());
				ems.setId_emp_doctor(item.getId_emp());
				ems.setName_emp_doctor(item.getName_emp());
            }
