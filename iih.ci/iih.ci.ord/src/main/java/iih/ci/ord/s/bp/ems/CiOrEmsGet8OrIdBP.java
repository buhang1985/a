...
import iih.ci.ord.s.bp.ems.save.ip.CiOrappbtGetBP;
import iih.ci.ord.s.bp.ems.save.ip.CiOrappbuGetBP;
import iih.ci.ord.s.bp.ems.save.ip.CiOrappconsultGetBP;
import iih.ci.ord.s.bp.ems.save.ip.CiOrappoptGetBP;
import iih.ci.ord.s.bp.ems.save.ip.CiOrappoutGetBP;
import iih.ci.ord.s.bp.ems.save.ip.CiOrapppathgyGetBP;
import iih.ci.ord.s.bp.ems.save.ip.CiOrapptransdepGetBP;
...
public class CiOrEmsGet8OrIdBP {
	 ...
	 */
	@Deprecated
	public CiEmsDTO exec(String id_or) throws BizException {
		FMap ciEmsMap = this.exec(new String[] { id_or });
		return (CiEmsDTO) ciEmsMap.get(id_or);
	}
	...
	private CiEmsDTO createCiEmsDTO8AggDO(CiorderAggDO aggdo) throws BizException {
		...
		// 草药是否院内制剂 by yzh 2018年4月9日 19:24:03
		emsdto.setFg_hospital_herbpres(ordo.getFg_hospital_herbpres());
		...
		emsdto.setFg_infect_dep_cons(ordo.getFg_infect_dep_cons());// 是否感染会诊标识
		...
		// 超量开单原因
		emsdto.setId_excessive_reason(ordo.getId_excessive_reason());
		emsdto.setSd_excessive_reason(ordo.getSd_excessive_reason());
		emsdto.setName_excessive_reason(ordo.getName_excessive_reason());
		// 超额原因
		emsdto.setMore_money_reason(ordo.getMore_money_reason());
		// 滴速
		emsdto.setId_dripspeed(ordo.getId_dripspeed());
		emsdto.setName_dripspeed(ordo.getName_dripspeed());
		// 医保标识
		emsdto.setFg_orhp(ordo.getFg_orhp()); // 该标识废弃
		
		emsdto.setEu_hpjudge(ordo.getEu_hpjudge());
		emsdto.setEu_orhp(ordo.getEu_orhp());
		...
		//诊断信息
		emsdto.setId_didef(ordo.getId_didef());
		emsdto.setCode_didef(ordo.getCode_didef());
		emsdto.setName_didef(ordo.getName_didef());
		//在院整领标识和领药方式
		emsdto.setFg_wholepack(ordo.getFg_wholepack());
		emsdto.setId_drugdeliverymethod(ordo.getId_drugdeliverymethod());
		emsdto.setSd_drugdeliverymethod(ordo.getSd_drugdeliverymethod());
		emsdto.setName_drugdeliverymethod(ordo.getName_drugdeliverymethod());
		...
		if (!OrSrvSplitUtil.isEmpty(ordo.getId_srv()) && OrSrvSplitUtil.isTrue(ordo.getFg_set())) {
			// MedSrvSetItemDO[]
			// setitemdos=CiOrdUtils.getBdSrvSetItems(ordo.getId_srv(), null);
			OrdSrvSetDO[] ordsrvsetDO = CiOrdUtils.getIOrdsrvsetRService().find("a0.id_or ='" + ordo.getId_or() + "'",
					"a0.id_or", FBoolean.FALSE);
	...
	private FArrayList createEmsSrvs8AggDo(CiorderAggDO aggdo) throws BizException {
		...
		// 根据医嘱项目集合创建医疗单项目数据集
		FArrayList arrylist = createEmsSrvs8OrSrvDOs(aggdo.getParentDO(), orsrvdos);
	...
	private void createEmsSrvs8OrSrvSetDOs(CiorderAggDO aggdo, FArrayList arrylist) throws BizException {
		...
		if (orsrvsetdos == null || orsrvsetdos.length == 0)
			return;
	 ...
	 * @throws BizException
	 */
	private FArrayList createEmsSrvs8OrSrvDOs(CiOrderDO order, OrdSrvDO[] orsrvdos) throws BizException {
		FArrayList arrylist = new FArrayList();
		FMap blsrvFMap = getBlSrvFMap(orsrvdos);
		// 批量查询物品信息
		Map<String, OrdSrvMmDO> ordSrvMmMap = getSrvMMInfoMap(orsrvdos);
		if (!CiOrdUtils.isEmpty(orsrvdos)) {
			for (int i = 0; i < orsrvdos.length; i++) {
				OrdSrvMmDO srvMm = ordSrvMmMap.get(orsrvdos[i].getId_orsrv());
				arrylist.add(createEmsSrv8OrSrvDO(order, orsrvdos[i], blsrvFMap, srvMm));
			}

			// 获取自备药中的预防和治疗用药标识
			List<String> idSrvList = new ArrayList<String>();
			Map<String, CiEmsSrvDTO> ciEmsSrvMap = new HashMap<String, CiEmsSrvDTO>();
			for (Object obj : arrylist) {

				CiEmsSrvDTO ciEmsSrv = (CiEmsSrvDTO) obj;
				if (FBoolean.TRUE.equals(ciEmsSrv.getFg_self())) {
					idSrvList.add(ciEmsSrv.getId_srv());
					ciEmsSrvMap.put(ciEmsSrv.getId_srv(), ciEmsSrv);
				}
			}

			if (idSrvList.size() > 0) {

				IMedSrvDrugDORService medSrvDrugRService = CiOrdAppUtils.getIMedSrvDrugDORService();
				MedSrvDrugDO[] medSrvDrugs = medSrvDrugRService.findByAttrValStrings(MedSrvDrugDO.ID_SRV,
						idSrvList.toArray(new String[idSrvList.size()]));

				if (medSrvDrugs != null && medSrvDrugs.length > 0) {
					for (MedSrvDrugDO medSrvDrug : medSrvDrugs) {

						CiEmsSrvDTO ciEmsSrv = ciEmsSrvMap.get(medSrvDrug.getId_srv());
						ciEmsSrv.setId_anti(medSrvDrug.getId_anti());
						ciEmsSrv.setSd_anti(medSrvDrug.getSd_anti());
						// 设置治疗和预防标识
						if (FBoolean.FALSE.equals(medSrvDrug.getFg_anti())
								&& !IBdMmDictCodeConst.SD_ANTI_NULL.equals(medSrvDrug.getSd_anti())) {
							ciEmsSrv.setFg_propc(null);
						}
					}
				}
			}
	 ...
	 * @return
	 */
	private Map<String, OrdSrvMmDO> getSrvMMInfoMap(OrdSrvDO[] orsrvdos) {

		FArrayList srvIdList = new FArrayList();
		Map<String, OrdSrvMmDO> srvMmMap = new HashMap<String, OrdSrvMmDO>();
		...
		if (srvIdList.size() > 0) {
			try {
				...
				e.printStackTrace();
				throw new BizRuntimeException("查询医嘱服务项目物品异常");
			}
		}
		return srvMmMap;
	}
	 ...
	 * @throws BizException
	 */
	private CiEmsSrvDTO createEmsSrv8OrSrvDO(CiOrderDO order ,OrdSrvDO orsrvdo, FMap blsrvFMap, OrdSrvMmDO srvMm) throws BizException {
		CiEmsSrvDTO emssrvmm = new CiEmsSrvDTO();
		...
		emssrvmm.setQuan_total_medu(orsrvdo.getQuan_total_medu()); // add by qz
																	// 2016-8-22
		...
		emssrvmm.setSd_frequnitct(orsrvdo.getSd_frequnitct());// add by qz
																// 2016-8-22
		...
		if(FBoolean.TRUE.equals(emssrvmm.getFg_self())) {
			emssrvmm.setFg_bl(FBoolean.FALSE);
		}
		...
		emssrvmm.setFg_selfpay(orsrvdo.getFg_selfpay()); // 废弃20191120
		// 医保标识
		emssrvmm.setEu_hpjudge(orsrvdo.getEu_hpjudge());
		emssrvmm.setEu_orsrvhp(orsrvdo.getEu_orsrvhp());
		emssrvmm.setId_dsdef(orsrvdo.getId_dsdef());
		emssrvmm.setCode_dsdef(orsrvdo.getCode_dsdef());
		emssrvmm.setName_dsdef(orsrvdo.getName_dsdef());		
		// emssrvmm.setFg_treat();
		...
		emssrvmm.setName_dep_wh(orsrvdo.getName_dep_wh());
		...
		emssrvmm.setFg_nothingwithorders(orsrvdo.getFg_nothingwithorders());// 与付数无关
		// 根据医嘱项目对应物品数据信息创建医疗单服务物品信息
		if (OrSrvSplitUtil.isTrue(emssrvmm.getFg_mm())) {
			setEmsSrvMmInfo(emssrvmm, srvMm);
		}

		// 设置治疗和预防标识
		if (!StringUtils.isEmpty(emssrvmm.getSd_anti())
				&& emssrvmm.getSd_anti().equals(IBdMmDictCodeConst.SD_ANTI_NULL)) {
		...
		if (!CiOrdUtils.isEmpty(blsrvFMap) && blsrvFMap.containsKey(orsrvdo.getId_srv())) {
			...
			if (!CiOrdUtils.isEmpty(medsrv)) {
				emssrvmm.setDes_bl(medsrv.getDes());
			}
		}
		emssrvmm.setFg_highrisk(orsrvdo.getFg_highrisk());
		// 不计费类型
		if(!StringUtils.isEmpty(orsrvdo.getSd_nodispense())){
			if(ICiDictCodeConst.SD_NODISPENSE_SELF.equals(orsrvdo.getSd_nodispense())){// 自费
				emssrvmm.setId_nodispense(ICiDictCodeConst.ID_NODISPENSE_SELF);
				emssrvmm.setSd_nodispense(ICiDictCodeConst.SD_NODISPENSE_SELF);
				emssrvmm.setName_nodispense(ICiDictCodeConst.NAME_NODISPENSE_SELF);
			}else if(ICiDictCodeConst.SD_NODISPENSE_ENTRUST.equals(orsrvdo.getSd_nodispense())){// 嘱托
				emssrvmm.setId_nodispense(ICiDictCodeConst.ID_NODISPENSE_ENTRUST);
				emssrvmm.setSd_nodispense(ICiDictCodeConst.SD_NODISPENSE_ENTRUST);
				emssrvmm.setName_nodispense(ICiDictCodeConst.NAME_NODISPENSE_ENTRUST);
			}else if(ICiDictCodeConst.SD_NODISPENSE_FOREIGN.equals(orsrvdo.getSd_nodispense())){// 外配
				emssrvmm.setId_nodispense(ICiDictCodeConst.ID_NODISPENSE_FOREIGN);
				emssrvmm.setSd_nodispense(ICiDictCodeConst.SD_NODISPENSE_FOREIGN);
				emssrvmm.setName_nodispense(ICiDictCodeConst.NAME_NODISPENSE_FOREIGN);
			}
		}
		
		// 退药关联医嘱
		if(order.getId_srv().equals(orsrvdo.getId_srv())){
			emssrvmm.setId_or_src(order.getId_or_src());
		}
	...
	private void setEmsSrvMmInfo(CiEmsSrvDTO emssrvmm, OrdSrvMmDO srvmmdo) throws BizException {
		...
		emssrvmm.setName_dosage(srvmmdo.getDs_name());//剂型名称
		...
		emssrvmm.setId_herbpreparationtp(srvmmdo.getId_herbpreparationtp());//草药制剂类型
		emssrvmm.setSd_herbpreparationtp(srvmmdo.getSd_herbpreparationtp());//草药制剂类型(0饮片；1配方颗粒)
		//增加当前剂量，当前剂量单位，当前剂量换算系数
		emssrvmm.setId_medu_cur(srvmmdo.getId_medu_cur());
		emssrvmm.setQuan_medu_cur(srvmmdo.getQuan_medu_cur());
		if(srvmmdo.getFactor_cur()!=null&&srvmmdo.getFactor_cur().trim().length()>0)
		emssrvmm.setFactor_medu_cur(new FDouble(srvmmdo.getFactor_cur()));
		// 物品删除字段报错 todo
	...
	private void createEmsSrv4SetOrOpen(CiOrderDO ordo, FArrayList arrylist) {
		...
		if (!OrSrvSplitUtil.isTrue(ordo.getFg_set()))
			return;
		CiEmsSrvDTO emssrvmm = new CiEmsSrvDTO();
		// emssrvmm.setId_orsrv();
	...
	private void orAppInfoHandle(String id_or, CiEmsDTO emsdto) throws BizException {
		...
		if (EmsType.LIS.equals(iemstp)) {
			...
			getOrappdietInfo(id_or, emsdto, iemstp);
		}
	}
	/**
	 * 膳食申请单
	 ...
	 * @throws BizException 
	 */
	private void getOrappdietInfo(String id_or, CiEmsDTO emsdto, int iemstp) throws BizException {
		OrdApDietDO[] apDiets = ((ICiorappdietRService)ServiceFinder.find(ICiorappdietRService.class)).findByAttrValString(OrdApDietDO.ID_OR, id_or);
		if(apDiets!=null&& apDiets.length>0){
			CiOrdUtils.setEmsAppDatum(apDiets, emsdto, iemstp);
		}
	}
	 ...
	 * @throws BizException
	 */
	private void getOrapptransdepInfo(String id_or, CiEmsDTO emsdto, int iemstp) throws BizException {
