 ...
 *
 */
public class TplTransDeptSaveBP extends BaseIpTplSaveBP {
	@Override
	public EmsRstDTO[] save(CiEnContextDTO ctx, EmsSaveDTO[] szEms) throws BizException {
		EmsSaveDTO tpl = szEms[0];
		FArrayList transDeptList = tpl.getDocument();
		if (CiOrdUtils.isEmpty(transDeptList)) 
			return null;
		for (Object object : transDeptList) {
//			String code_or = CiOrdAppUtils.getIBillcodeManager().getPreBillCode_RequiresNew(CiOrderDODesc.CLASS_FULLNAME);
			...
			EmsTransDeptViewDTO transDept = new EmsTransDeptViewDTO();
			super.setEmsViewInfo(ctx, ortmpldo, ortplnitmdo, medSrvInfo, tpl.getDriverInfo(), transDept, code_or, 1);
			// 计算物资流向科室
			this.setOrWfDeptInfo(ctx, medSrvInfo, transDept);
			FArrayList docmuent = new FArrayList();
			docmuent.append(transDept);
