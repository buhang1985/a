 ...
 *
 */
public abstract class AbstractValidator<T> implements ICiRuleValidator<T> {
	...
	public List<RuleMsgDTO> startValidate(CiEnContextDTO ctxDTO, T t) {
		...
		if (nextValidator != null) {
			nextValidator.request = this.request;
			nextValidator.response = this.response;

			List<RuleMsgDTO> msgList = nextValidator.startValidate(ctxDTO, t);
