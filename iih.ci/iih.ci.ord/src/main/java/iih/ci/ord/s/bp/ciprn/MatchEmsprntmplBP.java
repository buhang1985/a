...
public class MatchEmsprntmplBP {
	...
	private void getPresAndInjectData() throws BizException
    {
        ...
        for (CiOrderDO order:this.ciOrderDOs)
        {
            ...
            //皮试医嘱为套时，下面是有可能挂着药品的
            if(IBdSrvDictCodeConst.SD_SRVTP_TREAT_SKINMINGANTEST.equals(order.getSd_srvtp()))
            {
            	 strIdors += ",'" + order.getId_or() + "'";
                 if (lstIdroutes.contains(order.getId_route()))
                 {
	...
	private Map<String,List<String>> matchUrlIDs(EmsprntmplAggDO[] emsprntmplAggDOs){
		...
		for (EmsprntmplAggDO aggDo:emsprntmplAggDOs){
            ...
            if (ICiDictCodeConst.SD_CIPRNSHEETTP_INJECAPP.equals(aggDo.getParentDO().getSd_ciprintsheettp()))//注射治疗单
            {
                matchInjc(lstParamIds);
            }
            else if (ICiDictCodeConst.SD_CIPRNSHEETTP_COSTBILL.equals(aggDo.getParentDO().getSd_ciprintsheettp()))//诊疗费用清单
            {
                matchFee(lstParamIds);
            }
            else
            {
                if (!("Y".equals(aggDo.getParentDO().getFg_mm_ciprnsheettp()) ? matchPres(aggDo, lstParamIds) : matchApp(aggDo, lstParamIds)))
                    continue;
            }
