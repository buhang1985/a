...
import iih.ci.ord.hp.tl.HpMedicalInsurance;
...
public class CiOrEmsGetOrAggAndRelInfoBP {
	...
	public  CiOrAggAndRelInfo exec(CiEmsDTO ems)  throws BizException{
		...
		if(FBoolean.TRUE.equals(CiOrdUtils.IsMedicalInsuranceEnable())){
			FArrayList2 diagList  = CiOrdUtils.getDiagList(orandreldatum.getOraggdo().getParentDO().getId_en());
	...
	private FMap getOrSrvMmDOMap(FMap mmmap){
		...
		String key="";
		int iN=mmmap.size();
		OrdSrvMmDO[] orsrvmmdos=new OrdSrvMmDO[iN];
		...
		for(int i = 0; i<iN; i++)
		{
			...
			ArrayList<OrdSrvMmDO> mmlist = (ArrayList<OrdSrvMmDO>)entry.getValue();
			if(mmlist!=null && mmlist.size()>0){
				orsrvmmdos[i] =mmlist.get(0);
			}				
		}
