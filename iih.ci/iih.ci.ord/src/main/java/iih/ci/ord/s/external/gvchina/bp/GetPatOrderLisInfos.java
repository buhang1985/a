 ...
 *
 */
public class GetPatOrderLisInfos extends BaseGetPatInfo {
	public GvLisOrderRstInfo[] exec(String code_en,String code_dept) throws BizException{
		// 有效性判断
				...
				sb.append(" ord.id_srv code,ord.name_or name,ord.dt_effe appTime ")
						.append(" from ci_order ord ")
						.append(" left join ci_ap_obs obs on ord.id_or = obs.id_or ")
						.append(" left join bd_dep dep on ord.id_dep_or = dep.id_dep ")
						.append(" where ord.fg_sign = 'Y' and ord.fg_canc = 'N' and ord.sd_su_bl != '2' and ord.ds = 0 and ord.sd_srvtp like '03%' and ")
				...
				// 查询数据库
				List<GvLisOrderRstInfo> listGvLisOrderRstInfo = (List<GvLisOrderRstInfo>) new DAFacade()
						.execQuery(sb.toString(), sqlParam,new BeanListHandler(GvLisOrderRstInfo.class));
				if (null != listGvLisOrderRstInfo) {
					// 填充患者基本信息
					for (GvLisOrderRstInfo info : listGvLisOrderRstInfo) {
				...
				return listGvLisOrderRstInfo == null || listGvLisOrderRstInfo.size() == 0 ? null
						: listGvLisOrderRstInfo.toArray(new GvLisOrderRstInfo[listGvLisOrderRstInfo.size()]);
	}
}
