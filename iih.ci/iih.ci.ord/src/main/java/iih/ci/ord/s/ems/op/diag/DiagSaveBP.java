 ...
 * Package Name:iih.ci.ord.s.ems.op.diag  
...
package iih.ci.ord.s.ems.op.diag;
...
import iih.ci.ord.s.ems.op.base.OpBaseDiagSaveBP;
import iih.ci.ord.s.ems.utils.DiagFireEventUtils;
 ...
 * @since JDK 1.8
 */
public class DiagSaveBP extends OpBaseDiagSaveBP {
	/**
	 * 诊断体系缓存map
	 */
	...
	public DiagRstDTO save(CiEnContextDTO ctx, DiagSaveDTO diSaveInfo) throws BizException {
		...
		if (doucumentlist == null || doucumentlist.size() < 0)
			return null;
		// 诊断体系
		if (diagSysMap == null) {
			getUdidocMap();
		...
		// 创建诊断AggDO
		CidiagAggDO aggdo = creatDiAggInfo(doucumentlist, ctx);
		// 诊断保存
		CidiagAggDO[] ret = getICidiagCudService().save(new CidiagAggDO[] { aggdo });

		// 删除就诊域的原由的诊断
		EntDiDO[] entDiDOs = getIEntdiRService().find("a0.id_ent ='" + aggdo.getParentDO().getId_en() + "'", "id_ent",
				FBoolean.FALSE);
		...
		if (entdidos != null)
			getIEntdiCudService().save(entdidos);
		// 诊断的集成平台事件
	...
	private CidiagAggDO creatDiAggInfo(FArrayList doucumentlist, CiEnContextDTO ctx) throws BizException {
		...
		if (StringUtils.isNotEmpty(diDTO.getId_didef())) {
			...
			if (itemlist.size() > 0)
				agg.setCiDiagItemDO(itemlist.toArray(new CiDiagItemDO[itemlist.size()]));
			// 签署
	...
	private void setDiagItemHpTp(Map<String, FBoolean> map, String[] idDiDefList, CiDiagItemDO[] itemList,
			CiEnContextDTO ctx, String entp) throws BizException {
		// 获取保外诊断判断结果，key值诊断定义id， value true 保内诊断，Y 保外诊断
			...
			if (map != null && map.size() > 0) {
				for (CiDiagItemDO item : itemList) {
					if (map.containsKey(item.getId_didef())) {
						if (FBoolean.FALSE.equals(map.get(item.getId_didef()))) {
	...
	private List<CiDiagDO> getListCiDiagDO(String id_en) throws BizException {
		...
		String sql = " dt_sign in (" + " select max(dt_sign) from ci_di where id_en ='" + id_en
				...
				+ ICiDictCodeConst.SD_SUPPLY + "'" + " and dt_sign>(" + " select max(dt_sign) from ci_di where id_en ='"
				+ id_en + "' and  fg_sign ='Y' and sd_ditp !='" + ICiDictCodeConst.SD_SUPPLY + "'" + " ))";
