...
public class JudgeOrderTemplateApplicationTable {
	 ...
	 * @param ciEms
	 */
	public void JudgeOrederTemplateAppTabel(CiEmsDTO[] ciEms, CiEnContextDTO ciEnContext, String clinicalzztz) throws BizException {
		...
		if (ciEms != null && ciEms.length > 0) {
			...
			for (CiEmsDTO ciemsDTO : ciEms) {
				...
				if (EmsType.PATHGY == ciemsDTO.getEmstype()) {
					...
					// 检查
					getOrdApObsDO(ciemsDTO, ciemsDTO.getApplyno() == null ? ApplyNo : ciemsDTO.getApplyno(), diag, clinicalzztz);
				...
				} else if (EmsType.COMMON == ciemsDTO.getEmstype() || EmsType.CONS == ciemsDTO.getEmstype()) {
					// 会诊
					getOrdApConsDO(ciemsDTO, ciemsDTO.getApplyno() == null ? ApplyNo : ciemsDTO.getApplyno(), diag,
							ciEnContext);
					...
					// 出院
					getOrdAppOuthospDO(ciemsDTO, ciemsDTO.getApplyno() == null ? ApplyNo : ciemsDTO.getApplyno(), diag,
							ciEnContext);
				}
			}
		}
	 ...
	 * @throws BizException
	 */
	private void getOrdApObsDO(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag, String clinicalzztz) throws BizException {
		...
		// 0155084:
		// 门诊医生站开检查项目时，表单中的临床症状及体征要必填，在没有写病历的情况下，医生手写临床症状及体征后，再开下一条检查医嘱时，临床症状及体征能自动带过来，不需要医生多次填写
		if (clinicalzztz != null && !"".equals(clinicalzztz)) {
			obs.setClinicalzztz(clinicalzztz);// 临床症状及体征
		} else {
			clinicalzztz = this.getClinicalzztz(ciemsDTO);
			if (clinicalzztz != null && !"".equals(clinicalzztz)) {
				obs.setClinicalzztz(clinicalzztz);// 临床症状及体征
			} else {
				OrdApObsDO ordApObsDO = getOrdApObsDO("id_en", ciemsDTO.getId_en());
				if (ordApObsDO != null && ordApObsDO.getClinicalzztz() != null) {
					obs.setClinicalzztz(ordApObsDO.getClinicalzztz());
				}
			}
		}
		if(ciemsDTO.getFg_set()==null||!ciemsDTO.getFg_set().booleanValue()) {
			if(ciemsDTO.getEmssrvs()!=null&&ciemsDTO.getEmssrvs().size()>0) {
				CiEmsSrvDTO srvdto=(CiEmsSrvDTO)ciemsDTO.getEmssrvs().get(0);
				obs.setId_body(srvdto.getId_body());
				obs.setName_body(srvdto.getBody_name());
				obs.setSd_body(srvdto.getSd_body());
				obs.setId_pos(srvdto.getId_pos());// 体位
				obs.setSd_pos(srvdto.getSd_pos());// 体位编码
				obs.setName_pos(srvdto.getPos_name());// 体位名称
			}
		}
	...
	private String getClinicalzztz(CiEmsDTO ciemsDTO) throws BizException {
		return CiOrdUtils.DiseaseDescription(ciemsDTO.getId_en());
		//
		// // 临床症状和体征
		// StringBuffer clinialBuffer = null;
		// String clinicalInfo =
		// ParamsetQryUtil.getParaString(Context.get().getOrgId(),
		// ICiOrdNSysParamConst.SYS_PARAM_ClinicalInfo);
		// if (clinicalInfo == null) {// 未配置参数提取主诉和先病史
		// IMrdocrefvalueRService mrdocservice =
		// ServiceFinder.find(IMrdocrefvalueRService.class);
		// String sql = String.format("id_ent='%s' and code_element in
		// ('%s','%s','%s')", ciemsDTO.getId_en(),
		// ICiDictCodeConst.CODE_ELEMENT_MAINSUIT,
		// ICiDictCodeConst.CODE_ELEMENT_NOWDISEASE,
		// ICiDictCodeConst.CODE_ELEMENT_TEST);
		//
		// MrDocRefValueDO[] szMrDocRefValueDO = mrdocservice.find(sql, "",
		// FBoolean.FALSE);
		// if (szMrDocRefValueDO != null && szMrDocRefValueDO.length > 0) {
		// clinialBuffer = new StringBuffer();
		// MrDocRefValueDO itemMainSuit = null, itemNowDise = null, itemTest =
		// null;
		// for (MrDocRefValueDO o : szMrDocRefValueDO) {
		// // 主诉
		// if (itemMainSuit == null &&
		// o.getCode_element().equals(ICiDictCodeConst.CODE_ELEMENT_MAINSUIT)) {
		// itemMainSuit = o;
		// }
		// // 现病史
		// if (itemNowDise == null &&
		// o.getCode_element().equals(ICiDictCodeConst.CODE_ELEMENT_NOWDISEASE))
		// {
		// itemNowDise = o;
		// }
		// // 体格检查
		// if (itemTest == null &&
		// o.getCode_element().equals(ICiDictCodeConst.CODE_ELEMENT_TEST)) {
		// itemTest = o;
		// }
		// }
		// if (itemMainSuit != null &&
		// !CiOrdUtils.isEmpty(itemMainSuit.getContent())) {
		// clinialBuffer.append("以" + itemMainSuit.getContent() +
		// "之主诉就诊。").append(System.lineSeparator());
		// }
		//
		// if (itemNowDise != null &&
		// !CiOrdUtils.isEmpty(itemNowDise.getContent())) {
		...
		// // 根须序号排序
		// TreeMap<String, String> num_elecode = new TreeMap<String, String>(new
		// Comparator<String>() {
		// @Override
		...
		// String num_content = split[i].substring(0, split[i].length() -
		// elecode.length());
		...
		// }
		// ICiMrOutQryServices iCiMrOutQryServices =
		// ServiceFinder.find(ICiMrOutQryServices.class);
		// MrDocRefValueDO[] mrdocs =
		// iCiMrOutQryServices.getCimrPhaseByElemCode(ciemsDTO.getId_en(),
		// ElemCodes);
		// Map<String, MrDocRefValueDO> teamMr = new HashMap<String,
		// MrDocRefValueDO>();
		...
		// if (teamMr.containsKey(key) && teamMr.get(key) != null &&
		// teamMr.get(key).getContent() != null) {
		// clinical = contents.get(key) + teamMr.get(key).getContent() + "." +
		// clinical;
		// }
		// }
		// return clinical;
	 ...
	 * @param ciemsDTO
	 */
	private void getOrdApConsDO(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag, CiEnContextDTO ciEnContext)
			throws BizException {
		...
		// cons.setDt_constimelimit(ciemsDTO.getDt_constimelimit());//会诊失效时间
		// todo
		...
		if (ciEnContext.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_OP)
				|| ciEnContext.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_ET)) {

			// 会诊地址，取当前就诊科室所在地址，如果为空，使用就诊科室名称
			String place = StringUtils.isNotBlank(ciEnContext.getAddr_dep_or()) ? ciEnContext.getAddr_dep_or()
					: ciEnContext.getName_dep_or();
			cons.setPlace(place);// 会诊地点
		} else if (ciEnContext.getCode_entp().equals(IBdFcDictCodeConst.SD_CODE_ENTP_IP)) {
			cons.setPlace(ciEnContext.getName_dep_en() + ciEnContext.getName_dep_ns() + ciEnContext.getName_bed());// 会诊地点
		}
	 ...
	 * @throws BizException
	 */
	private void getciorapppathgy(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag) throws BizException {
		...
		if (!CiOrdUtils.isEmpty(lisdo)) {
			...
			if (!CiOrdUtils.isEmpty(lisdo.getQuan())) {
		...
		// 0186038: 启用批量查良开单原因：开立病理类医嘱，诊断回写错误。  2020-03-11 LiYue
		if( !CiOrdUtils.isEmpty( ciemsDTO.getName_didef())) {
			pathgy.setName_diag(ciemsDTO.getName_didef());
		}else {
		...
		// 0155084:
		// 门诊医生站开检查项目时，表单中的临床症状及体征要必填，在没有写病历的情况下，医生手写临床症状及体征后，再开下一条检查医嘱时，临床症状及体征能自动带过来，不需要医生多次填写
		String clinicalzztz = this.getClinicalzztz(ciemsDTO);
		if (clinicalzztz != null && !"".equals(clinicalzztz)) {
			pathgy.setClinicalzztz(clinicalzztz);// 临床症状及体征
		} else {

			OrdApPathgyDO ordApPathgyDO = getOrdApPathgyDO("id_en", ciemsDTO.getId_en());
			if (ordApPathgyDO != null) {
				pathgy.setClinicalzztz(ordApPathgyDO.getClinicalzztz());
			}
		}
...
//		}
		...
		OrgDO orgdo = CiOrdUtils.getOrgDO(Orgid);
		if (!CiOrdUtils.isEmpty(orgdo)) {
		   pathgy.setOrg_pathgy_old(orgdo.getName());// 既往检查医院
		}
		...
		// 0171423:
		// 门急诊医生站：开立内镜病理签署报没有标本信息不可签署。全选开立医嘱后取消勾选内镜病理医嘱签署报内镜病理没有标本信息。在医嘱模板中开立内镜病理医嘱签署时报内镜病理没有标本信息、正常开立可直接签署。
		// 没有标本信息也能保存病理
	...
	public OrdApPathgyDO getOrdApPathgyDO(String column, String columnValue) throws BizException {
		...
		OrdApPathgyDO ordDO = (OrdApPathgyDO) new DAFacade().execQuery(sql, sqlParam,
				new BeanHandler(OrdApPathgyDO.class));
		System.out.println("ordDo:" + (ordDO == null ? "null" : ordDO.toString()));
		return ordDO;
	}
	...
	private void getOrdApLabDO(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag) throws BizException {
		...
		/*
		 * String id_srv = null; if (ciemsDTO.getFg_set() == FBoolean.TRUE) {
		 * FMap map = ciemsDTO.getSrvsetitms(); FArrayList setItems =
		 * (FArrayList) map.get(ciemsDTO.getId_srv()); OrdSrvSetDO ordSrvSet =
		 * (OrdSrvSetDO) setItems.get(0); id_srv = ordSrvSet.getId_srvset();
		 * 
		 * } else { ciemsDTO.getId_srv(); }
		 * 
		 * MedSrvLisDO[] medSrvLiss =
		 * imedSrvLisRService.findByAttrValString(MedSrvLisDO.ID_SRV, id_srv);
		 * if (medSrvLiss == null || medSrvLiss.length == 0) { throw new
		 * BizException("检验服务【" + ciemsDTO.getName() + "】 未找到对应定义态检验属性数据，请核对！");
		...
		// 临床诊断子项（新增）
		/*
		 * lab.setDef8(ciemsDTO.getDef8());// lab.setDef9(ciemsDTO.getDef9());//
		...
		if (FBooleanUtils.isTrue(ciemsDTO.getFg_urgent())) {
			lab.setFg_urgent(FBoolean.TRUE);// 加急标识
		} else {
			lab.setFg_urgent(FBoolean.FALSE);
		}
	...
	private void getOrdApOpDO(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag) throws BizException {
		...
		if(medSrvOpDOArr != null && medSrvOpDOArr.length>0) {
		MedSrvOpDO medSrvOpDO = medSrvOpDOArr[0];
		if(StringUtils.isNotEmpty(medSrvOpDO.getId_opclass())){
			op.setId_lvlsug(medSrvOpDO.getId_opclass()); 	//手术等级
			op.setSd_lvlsug(medSrvOpDO.getSd_opclass());
		}
		if(StringUtils.isNotEmpty(medSrvOpDO.getId_incitp())){
			op.setId_incitp(medSrvOpDO.getId_incitp()); 		//切口等级
			op.setSd_incitp(medSrvOpDO.getSd_incitp());
		}
		}
	...
	private void getOrdAppOuthospDO(CiEmsDTO ciemsDTO, String ApplyNo, String[] diag, CiEnContextDTO ciEnContext) {
		...
		if (CiOrdUtils.isCapitalInStr(ciemsDTO.getSd_srvtp(), IBdSrvDictCodeConst.SD_SRVTP_PATIMAN_CLIDEATH)) {
			...
			ordApOut.setId_outtp(IBdFcDictCodeConst.ID_LVHOS_WITHCI);// 离院方式
																		// 医嘱离院
			ordApOut.setSd_outtp(IBdFcDictCodeConst.SD_LVHOS_WITHCI);// 离院方式编码
																		// 医嘱离院
		...
		ordApOut.setName_dep_out(ciEnContext.getName_dep_en());
		ordApOut.setId_dep_nur_out(ciemsDTO.getId_dept_ns());// 离院时病区
		ordApOut.setName_dep_nur_out(ciEnContext.getName_dep_ns());
		ordApOut.setId_bed_out(ciEnContext.getId_bed());// 离院时床位
		ordApOut.setName_bed_out(ciEnContext.getName_bed());
		 ...
		 * ordApOut.setDes_again31();//31日内再入院目的 ordApOut.setDef1();//客户扩展字段1
		 * ordApOut.setDef2();//客户扩展字段2 ordApOut.setDef3();//客户扩展字段3
		 * ordApOut.setDef4();//客户扩展字段4 ordApOut.setDef5();//客户扩展字段5
		 * ordApOut.setDef6();//客户扩展字段6 ordApOut.setDef7();//客户扩展字段7
		 * ordApOut.setDef8();//客户扩展字段8 ordApOut.setDef9();//客户扩展字段9
		 * ordApOut.setDef10();//客户扩展字段10 ordApOut.setCreatedby();//创建人
		 * ordApOut.setCreatedtime();//创建时间 ordApOut.setModifiedby();//最后修改人
