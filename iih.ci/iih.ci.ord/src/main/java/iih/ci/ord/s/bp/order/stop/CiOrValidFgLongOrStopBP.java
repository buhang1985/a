package iih.ci.ord.s.bp.order.stop;
...
//			cior.setId_emp_chk_stop(cior.getId_emp_chk());
//			cior.setDt_chk_stop(CiOrdAppUtils.getServerDateTime());
	 ...
	 * @throws BizException
	 */
	public ArrayList<CiOrderDO> exec4Ip(ReactExtOrsAndStopOrsDO reactandstopors) throws BizException{
		//有效性校验
		if (CiOrdUtils.isEmpty(reactandstopors)
		...
		setCommonData();
		ArrayList<CiOrReactLogDO> reactlogs=new ArrayList<CiOrReactLogDO>();
		for(CiOrderDO cior:reactandstopors.getStopordos()){
			dt_end=getOrDtEnd(cior.getDt_last_mp(), reactandstopors.getReactextdo().getParentDO().getDt_effe());
			if(OrSrvSplitUtil.isTrue(cior.getFg_sign()) && 
			   !OrSrvSplitUtil.isTrue(cior.getFg_chk()) && OrSrvSplitUtil.isTrue(cior.getFg_long())){//仅修改医嘱截止时间  
				...
				//String id_emp=CiOrdAppUtils.getEnvContext().getUserId();
				String id_emp =CiOrdAppUtils.getEnvContext().getStuffId();
				cior.setDt_stop(reactandstopors.getReactextdo().getParentDO().getDt_effe());
				cior.setId_dep_stop(id_dep);
				cior.setId_emp_stop(id_emp);	
				AuditInfoUtil.updateData(cior);
				cior.setStatus(DOStatus.UPDATED);
