...
public class HpQryCiorderBP {
	...
	private List<OrdSrvDO> getOrdsrvList(List<String> idOrList) throws DAException {
		...
		sqlBuffer.append("select * from ci_or_srv srv where (");
		
		sqlBuffer.append(SqlUtils.getInSqlByBigIds("srv.id_or",idOrList.toArray(new String[0])));
		
		sqlBuffer.append(" ) ");
		// by lim 过滤掉自费项目
//		sqlBuffer.append(" and srv.fg_indic = 'Y'");
