 ...
 *
 */
public class OpBaseEmsSaveBP extends ContextCache implements IEmsSaveBP, IOrderHpInfoBP {
	...
	public EmsRstDTO[] save(CiEnContextDTO ctx, EmsSaveDTO[] szEms) throws BizException {
		List<EmsRstDTO> listRstInfo = new ArrayList<EmsRstDTO>();
		for (EmsSaveDTO ems : szEms ){
			...
			if (!CiOrdUtils.isEmpty(errorMsg)) {
				...
				return listRstInfo.toArray(new EmsRstDTO[listRstInfo.size()]);
			}
	...
	protected OrderPackageInfo[] orderPakageInfoArrayFrom(OrderKey2UiModelMap oium) throws BizException {
		...
		for (CiorderAggDO orderAggInfo : szCiorderAggInfo) {
			...
			// MedSrvDO[] szMedSrvInfo = BdSrvInfoUtils.QueryBdSrvInfo(new
			// String[]{orderInfo.getId_srv()});
			// opi.setBdSrvList(new BdSrvInfoList(szMedSrvInfo));
			// }
			// else{
			//
			// MedSrvDO[] szMedSrvInfo =
			// BdSrvInfoUtils.QueryBdSrvInfo(ordSrvMmInfoList.asSrvKeyArray());
			// opi.setBdSrvList(new BdSrvInfoList(szMedSrvInfo));
			// }
	...
	// 国际 处理逻辑
	@Override
	public CiErrorDTO handleOrderHpInfo(CiEnContextDTO ctx, OrderPackageInfo orderPackageInfo) throws BizException {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public CiErrorDTO handleHpVerfyInfo(CiEnContextDTO enContextInfo, OrderPackageInfo orderPakageInfo)
			throws BizException {
		// TODO Auto-generated method stub
		return null;
	}
	...
	@Override
	public CiErrorDTO handleVerfyHpIndic(CiEnContextDTO ctx, OrderPackageInfo orderPackageInfo) throws BizException {
		// TODO Auto-generated method stub
		return null;
	}
