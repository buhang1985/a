...
public class CiEnContextUtil {
	...
	public static CiEnContextDTO getCiEnContext(Ent4BannerDTO ent4BannerDTO) throws BizException {
		...
		conetxtDTO.setName_pat(ent4BannerDTO.getName_pat());
		conetxtDTO.setDt_birth(ent4BannerDTO.getDt_birth());
		...
		conetxtDTO.setNo_hp(ent4BannerDTO.getNo_hp()); // 医保账号
		conetxtDTO.setSd_hptp(ent4BannerDTO.getSd_hptp()); // 医保类型
		...
		conetxtDTO.setFg_hpspcl(ent4BannerDTO.getFg_hpspcl());// 医保特病
		conetxtDTO.setFg_gcvip(ent4BannerDTO.getFg_gcvip());// vip
		...
		conetxtDTO.setTel_dep_or(dept.getTel());
		conetxtDTO.setAddr_dep_or(dept.getId_plc());
//		conetxtDTO.setDept(dept);
		...
		conetxtDTO.setName_org(org.getName()); // 所属组织
//		conetxtDTO.setOrg(org);
		...
		conetxtDTO.setName_grp(group.getName()); // 所属组织
//		conetxtDTO.setGroup(group);
		...
		UserDO user = userRService.findById(Context.get().getUserId());
//		conetxtDTO.setUser(user);
		...
		// 基本医保判断结果数据信息
		String bhpjudgerst = (FBoolean.TRUE.equals(conetxtDTO.getFg_hpfundpay()) ? "1" : "0") + conetxtDTO.getEu_inhpbbr()
	...
	public static void SetHpCiDiagItem(CiEnContextDTO conetxtDTO, CiDiagItemDO[] ciDiagItems) {
		...
		if (!FBoolean.TRUE.equals(conetxtDTO.getFg_hpfundpay()) || ciDiagItems == null || ciDiagItems.length == 0) {
			return;
		}
	...
	public static boolean IsHpPat(Ent4BannerDTO ent4BannerDTO) {
		...
		if (ent4BannerDTO != null && StringUtils.isNotEmpty(ent4BannerDTO.getId_hp())
				...
				&& FBoolean.TRUE.equals(ent4BannerDTO.getFg_hpfundpay())) {
			return true;
		}
	...
	public static boolean IsHeComInsurAllowedAccountPat(Ent4BannerDTO ent4BannerDTO) {
		// 商保用户 并且 可医保记账
		if (isHeComInsurPat(ent4BannerDTO) && FBoolean.TRUE.equals(ent4BannerDTO.getFg_hpcg())) {
			return true;
		}
	...
	/**
	 * 通过就诊id_ent,获取bannerDTO
	 * @param id_ent
	 * @return
	 * @throws BizException
	 */
	public static CiEnContextDTO getCiEnContext(String id_ent) throws BizException{
		IEnOutQryService service = ServiceFinder.find(IEnOutQryService.class);
		Ent4BannerDTO[] banners = service.getEntInfo4Banner("", String.format("enent.id_ent='{0}'", id_ent), "");
		if(banners == null || banners.length==0){
			return null;
		}
		return getCiEnContext(banners[0]);
	}
}
