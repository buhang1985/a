...
import iih.ci.ord.s.ems.ip.assi.tpl.pathgy.TplPathgyDriver;
 ...
 *
 */
public class AssiPathgyDriver extends BaseEmsDriver {
	@Override
	public EmsRstDTO[] save(CiEnContextDTO ctx, EmsSaveDTO[] ems) throws BizException {
		...
		if(OrSourceFromEnum.IIHORTMPLHELPER.equals(ctx.getEu_orsrcmdtp())){
			// 医嘱模板逻辑
			return new TplPathgyDriver().save(ctx, ems);
		}
		else if(OrSourceFromEnum.IIHCPHELPER.equals(ctx.getEu_orsrcmdtp())){
			// 临床路径
//			return new CpPathgyDriver().save(ctx, ems);
		}
