...
public class GetExeDepts4CiOrSrvBP {
	...
	public  OrWfExDeptDTO[] exec(OrWfExDeptParamDTO param)  throws BizException{
		...
		if(CiOrdUtils.isIpWf(param.getCode_entp())){
			...
			if(!CiOrdUtils.isEmpty(deptinfos) && deptinfos[0].equals("1")){
				if(!CiOrdUtils.isEmpty(deptinfos[1])){
					...
					if(!CiOrdUtils.isEmpty(exdeptdto)){
						List<OrWfExDeptDTO> whDeptList = new ArrayList<OrWfExDeptDTO>();
						if(EnumFlow.SUPPLIESFLOW.equals(param.getEu_wftp())){
							//获得服务执行科室
							OrWfExDeptDTO[] exdeptinfo=CiOrdAppUtils.getBdFcQryQryService().getOrExDeptMainBP(param);
							if(!CiOrdUtils.isEmpty(exdeptinfo)){
								for(OrWfExDeptDTO orWf : exdeptinfo){
									if(EnumFlow.SUPPLIESFLOW.equals(orWf.getEu_wftp())){
										whDeptList.add(orWf);
									}
								}
							}
						}
						whDeptList.add(exdeptdto);
						return whDeptList.toArray(new OrWfExDeptDTO[0]);
					}
				}
				return getIpExDept(param,EnumFlow.EXECUTEFLOW);
			}
			...
			//返回病区或就诊科室
			return getIpExDept(param,EnumFlow.NULL);
		}else{
			//参数设置
			String relsrvsetinfo=param.getReserv1();  //套id,套分类,套类型,套用法,分类内码
			FBoolean isDeptFixed=FBoolean.FALSE;
			...
			if(exdeptinfo==null || exdeptinfo.length==0){
				...
				boolean hasMpDept = false;
				for (int i = 0; i < exdeptinfo.length; i++) {
					if(exdeptinfo[i].getEu_wftp()== EnumFlow.EXECUTEFLOW){
						hasMpDept = true;
					}
				}
				if (!hasMpDept) {
					OrWfExDeptDTO[] mprtn = getExDept(param, EnumFlow.EXECUTEFLOW);
					return merge(exdeptinfo, mprtn);
				}
				return  merge(exdeptinfo,exdeptdto);
			}
	...
	private OrWfExDeptDTO[] getExDept(OrWfExDeptParamDTO param,Integer eu_wftp) throws BizException{
		//有效性校验
		if(param==null)return null;
		String sysparam = SysParamUtils.getSysParam().getSYS_PARAM_CiOrExFlowDeptHandleMode().getOrgParam();
	 ...
	 * @throws BizException 
	 */
	private OrWfExDeptDTO[] getIpExDept(OrWfExDeptParamDTO param,Integer eu_wftp) throws BizException{
		//有效性校验
		if(param==null)return null;
			//获得科室id
			String	id_dep=param.getId_dept_ns();
			//特别注意这句话为临时的  最后需要注释掉
			//理论角度讲如果Banner上下文中正确的话这句话也不会执行的  也没有问题
			if(CiOrdUtils.isEmpty(id_dep)){id_dep=param.getId_dept_or();}   
	...
	private OrWfExDeptDTO getOrWfExDept(String id_dep,Integer eu_wftp) throws BizException{
		...
		//0来源于查询流向配置 1来源于默认值 CIOR0170 配置为1时
		deptdto.setMp_source("1");
	 ...
	 * @return
	 */
	private OrWfExDeptDTO[] merge(OrWfExDeptDTO[] wfrtn,OrWfExDeptDTO[] mprtn){
		if(CiOrdUtils.isEmpty(wfrtn)) return mprtn;
		if(CiOrdUtils.isEmpty(mprtn)) return wfrtn;
		
		int wfL=wfrtn.length;
		int mpL=mprtn.length;
		OrWfExDeptDTO[] rtn=new OrWfExDeptDTO[wfL+mpL];
		for (int i = 0; i < wfrtn.length; i++) {
			rtn[i] = wfrtn[i];
		}
		for (int i = 0; i < mprtn.length; i++) {
			rtn[wfL+i] = mprtn[i];
		}
		return rtn;
	}
