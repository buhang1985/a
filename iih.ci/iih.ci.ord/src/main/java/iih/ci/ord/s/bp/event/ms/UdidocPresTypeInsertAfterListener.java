...
public class UdidocPresTypeInsertAfterListener extends AbstractUdidocInsertAfterListener{
	...
	protected void doYourActionAfter(Object[] newObjs) throws BizException {
		...
		//根据事件类型编码组装数据  insert
		MSUdidocPresTypeDTO[] msUdidocPresTypeDTOs=udidocPresTypeBP.assemableDatabyState(PresTypeDos,EventTypeEnum.INSERT_AFTER.getValueString());
		...
		for(MSUdidocPresTypeDTO dto:msUdidocPresTypeDTOs) {
			if(dto != null) {
				OrdBizLogger.info(null, "MS095-新增后-MSUdidocPresTypeDTO:"+"["+i+"]"+dto.serializeJson());
			}
			//构建对象，并发送给集成组
			...
			if(domainBusinessUserObj != null) {
				OrdBizLogger.info(null, "MS095-新增后-发送数据DomainBusinessUserObj:"+"["+i+"]"+domainBusinessUserObj.serializeJson());
			}
			i++;
		}
	}
