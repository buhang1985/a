...
import iih.bd.base.util.EntpUtils;
import iih.bd.bc.udi.pub.IBdFcDictCodeConst;
import iih.bd.utils.BdEnvContextUtil;
import iih.ci.ord.ciorder.d.CiOrderDO;
import iih.ci.ord.ciorder.d.OrdSrvDO;
import iih.ci.ord.dto.blexorder.d.MpInEnum;
 ...
 *  @version xuxing_拆分查询条件增加在院患者条件
 *  @version ma.meng_0163517: 建议两个条件合并，使用least
 *  @version xuxing_修正least使用不正确,导致加锁数量太大引起的或锁失败
 */
public class GetOrSrvSplitCondSqlBP {
	...
	public String exec(OrSrvSplitParamDTO param) throws BizException {
		...
		// 在院整领标识条件处理fg_mp_in,
		Integer fg_mp_in = param.getFg_mp_in();
		appendSqlStr(rtn, getFgMpInCondStr(fg_mp_in));
	...
	private void baseWhereHandle(StringBuffer sb, Integer orgensplittp) {
		...
		sb.append(" WHERE ci_order.Code_Entp in(" + EntpUtils.getIpEntpSqlStrs() + ")"); // 住院和留观
		sb.append(" and ci_order.Fg_Chk='Y' and ci_order.Fg_Canc='N' and ci_order.Fg_Pmor='N' and Y.fg_ip = 'Y' "); // 已核对未作废非备用
	...
	private String getIdDeptMpCondStr(String id_dept) {
		String fldname = IOrAndSrvSplitConst.OR_TABLE_ALIAS + ".id_dep_mp";
	 ...
	 * @return
	 */
	private String getFgMpInCondStr(Integer fg_mp_in) {
		String fldname = IOrAndSrvSplitConst.OR_TABLE_ALIAS + ".fg_mp_in";
		if (MpInEnum.MPIN.equals(fg_mp_in)) {
			return " and " + fldname + "='Y'";
		} else if (MpInEnum.UNMPIN.equals(fg_mp_in)) {
			return " and " + fldname + "='N'";
		} else if (MpInEnum.ALL.equals(fg_mp_in)) {
			return "";
		}
		return "";
	}
	...
	private String getDtRelSplitCondStr(FDateTime dt_split_start, FDateTime dt_split_end) {
		...
		//return " and (" + sfldname + "<='" + e + "' and " + sfldname + "<=" + efldname + ")";
		return " and " + sfldname + "<= least(" + efldname + ",'" + e + "')";
