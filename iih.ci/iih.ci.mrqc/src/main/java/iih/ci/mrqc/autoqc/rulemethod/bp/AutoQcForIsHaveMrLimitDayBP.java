 ...
 *
 */
public class AutoQcForIsHaveMrLimitDayBP {
	public void exec(String id_ent, QcEventDO eventDo, AutoItmDO autoDo, Boolean isDocAutoQcSelf) throws BizException {
		...
		for (AutoQcDTO autoQcDTO : autoQcDtoLists) {
			...
			for (int i = 0; i < times; i++) {
				if(visitDO == null || new FDateTime().before(new FDateTime(visitDO.getDt_acpt().getMillis()+ICalendar.MILLIS_PER_HOUR*(i+1)* autoDo.getHours_dead()))){
					continue;
				}
				...
				if (autoDo.getId_mr_types() != null) {
					...
					whereStr.append("   and  to_date(a0.date_submit,'yyyy-mm-dd hh24:mi:ss') between  (to_date('"
							+ autoQcDTO.getDt_acpt() + "','yyyy-mm-dd hh24:mi:ss')+numtodsinterval("
							+ autoDo.getHours_dead() + "*" + i + "-" + "24" +",'hour') )");
					whereStr.append("   and ( to_date('" + autoQcDTO.getDt_acpt()
							+ "','yyyy-mm-dd hh24:mi:ss')+numtodsinterval(" + autoDo.getHours_dead() + "*" + (i)
							+ ",'hour') )");
				}
				...
				if (eventDo == null || autoQcDTO == null)
					continue;
