...
public class MrDeptQcQryBP {
	...
	public QaFltDTO[] getMrDeptQcNeedDefectFltList(String id_ent, String cqf_submit_user) throws BizException {
		...
		StringBuilder sql = new StringBuilder();
				...
				sql.append("   and ((ci_qa_flt.sd_qa_ty = '"+ICiMrDictCodeConst.SD_EXEC_DEPARTMENT+"' and ci_qa_flt.id_sbmt_user='"+ cqf_submit_user + "') or ci_qa_flt.sd_qa_ty = '"+ICiMrDictCodeConst.SD_EXEC_AUTOMATIC+"')"); 
				sql.append("   AND ci_qa_flt.id_ent = '"+ id_ent + "'"); 
				sql.append("   AND ci_qa_flt.ds = '0'"); 
				//sql.append("   and ci_qa_flt.id_sbmt_user='"+ cqf_submit_user + "'"); 
				sql.append("   order by ci_qa_flt.dt_sbmt desc"); 
	...
	public QaFltDTO[] getMrDeptQcDefectFltListByIdRevision(String id_revision) throws BizException {
		...
		StringBuilder sql = new StringBuilder();
				...
				sql.append(" WHERE "); 
				sql.append("    ci_qa_flt.id_revision = '"+id_revision+"'"); 
				sql.append("   and ci_qa_flt.sd_qa_ty in ('"+ICiMrDictCodeConst.SD_EXEC_DEPARTMENT+"','"+ICiMrDictCodeConst.SD_EXEC_AUTOMATIC+"')"); 
				sql.append("   AND ci_qa_flt.ds = '0'");  
				sql.append("   order by ci_qa_flt.dt_sbmt desc"); 
				String sqlStr=sql.toString();
	   @SuppressWarnings("unchecked")
...
//			sql.append("       on ci_qa_record.id_revision=cqrn.id_revision\n");
			...
			sql.append("       where ci_qa_record.id_ent= '"+qaPatListDto.getId_ent()+"' and "+EnvContextUtil.processEnvContext("",new QaRecordDO(),false)+" and  ee.code_entp='10'   and ci_qa_record.sd_qa_ty = '"+ICiMrDictCodeConst.SD_EXEC_DEPARTMENT+"'");
	...
	public ItmDTO[] getMrQcDeptItmList(QaPatListDTO qaPatDto) throws BizException {
    	...
    	StringBuilder sql = new StringBuilder();
				...
				if (qaPatDto!=null&&qaPatDto.getId_revision()!=null) {
					...
					sql.append("       (select count(*) from ci_qa_flt  ci_qa_flt where ci_qa_flt.id_qa_itm=ci_qa_itm.id_qa_itm and ci_qa_flt.id_revision='~' and ci_qa_flt.id_ent='"+qaPatDto.getId_ent()+"' and  ((ci_qa_flt.sd_qa_ty='"+ICiMrDictCodeConst.SD_EXEC_DEPARTMENT+"' and ci_qa_flt.id_sbmt_user='"+Context.get().getUserId()+"') or ci_qa_flt.sd_qa_ty='"+ICiMrDictCodeConst.SD_EXEC_AUTOMATIC+"')) as deduct_scr_times,");
				}
				...
				sql.append("       on ci_qa_itm_fst.id_fst = ci_qa_itm.id_fst");
				sql.append("    where ci_qa_itm.sd_qa like '%"+ICiMrDictCodeConst.SD_EXEC_DEPARTMENT+"%'"); 
