...
public class GetInvolvePatSql implements ITransQry {
	String sd_attribute;
	String startTime;
	String endTime;
	String sd_coll_ref;
	...
	public GetInvolvePatSql(String sd_attribute, String startTime, String endTime, String sd_coll_ref, String id_org,
	...
	public String getQrySQLStr() {
		...
		if (GetConstant.getOccurDate().equals(sd_coll_ref)) {
			sqlStr.append(" ev.dateTime dateTime                            ");
			sqlStr.append(" from bd_udidoc udi_imprd                                                ");
			sqlStr.append(" left outer join (select aerimprd.id_sfda_imprd_tp,aerev.dt_occur dateTime          ");
			sqlStr.append(" from SFDA_AER_IMPRD aerimprd         ");
			sqlStr.append(" left outer join SFDA_AER_EV aerev on aerimprd.id_aer_ev = aerev.id_aer_ev        ");
			...
			sqlStr.append(" ev.dateTime dateTime                         ");
			sqlStr.append(" from bd_udidoc udi_imprd                                                ");
			sqlStr.append(" left outer join (select aerimprd.id_sfda_imprd_tp,aerev.createdtime dateTime          ");
			sqlStr.append(" from SFDA_AER_IMPRD aerimprd         ");
			sqlStr.append(" left outer join SFDA_AER_EV aerev on aerimprd.id_aer_ev = aerev.id_aer_ev        ");
			sqlStr.append(" left outer join bd_psndoc psndoc on psndoc.id_psndoc = aerev.id_psndoc                  ");
