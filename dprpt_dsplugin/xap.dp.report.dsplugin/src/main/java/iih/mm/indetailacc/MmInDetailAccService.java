...
public class MmInDetailAccService {
	//包装单位性质(零售包装)
	...
	public List<?> fillBeanList(String stDeSchemeID, String whID, String yearmonth1, String yearmonth2,
			String mmpkguType, String mmcaInnercode, String cusMmcaInnercode, String metrialID, String supID) throws Exception {
		...
		for(StockDealTypeConfigItmDO stoDealTyConItemDO : stockDealTypeConfigItmDOS) {
			List<MmInDetailAccDTO> mmInDetailAccDTOS = this.getMmInDetailAccCount(stoDealTyConItemDO.getId_mmstca(), stDeSchemeID, whID, yearmonth1, yearmonth2, 
					mmpkguType, mmcaInnercode, supID, metrialID);
			
			if(mmInDetailAccDTOS == null || mmInDetailAccDTOS.size() <= 0) {
				String stcaCode = this.getMmStcaCode(stoDealTyConItemDO.getId_mmstca());
				mmInDetailAccDTOS = this.getMmInDetailInfoFix(whID, yearmonth1, yearmonth2, mmpkguType, mmcaInnercode, supID, metrialID, 
						stcaCode, stoDealTyConItemDO.getDisplay_nm());
			} else {
				mmInDetailAccDTOS = this.getMmInDetailInfoAuto(stoDealTyConItemDO.getId_mmstca(), stDeSchemeID, whID, yearmonth1, yearmonth2, 
						mmpkguType, mmcaInnercode, supID, metrialID);
			}		
			...
			for(MmInDetailAccDTO mmInDetailAccDTO : mmInDetailAccDTOS) {
				result.add(mmInDetailAccDTO);
			}
		}
	...
	private List<MmInDetailAccDTO> getMmInDetailInfoAuto(String id_mmstca,
			String stDeSchemeID, String whID, String yearmonth1,
			String yearmonth2, String mmpkguType,
		...
		strSql.append(" BD_MM.Code ONLY_CODE, ");
		strSql.append(" BD_MM_STCA.Code STCA_CODE, MM_SDT_CONFIG_ITM.Display_Nm STCA_NAME, ");
		strSql.append(" sum(IN_ITEM.Quan_Actual * ACT_PKGU.Factor / NVL(USER_PKGU.FACTOR, DEF_PKGU.FACTOR)) IN_QUAN, ");
		strSql.append(" sum(IN_ITEM.Amt) IN_AMT ");
		...
		strSql.append(" left outer join BD_MM_PKGU ACT_PKGU on ACT_PKGU.ID_MMPKGU = IN_ITEM.ID_MMPKGU_ACTUAL ");
		strSql.append(" and ACT_PKGU.Id_Mm = IN_ITEM.ID_MM ");
		strSql.append(" left outer join BD_MM_PKGU USER_PKGU on USER_PKGU.ID_MM = IN_ITEM.ID_MM ");
		...
		strSql.append(" left outer join BD_MM_PKGU DEF_PKGU on DEF_PKGU.ID_MM = IN_ITEM.ID_MM ");
		...
		strSql.append(" NVL(USER_PKGU.MMPKGUNAME, DEF_PKGU.MMPKGUNAME),BD_MM_STCA.Code,MM_SDT_CONFIG_ITM.Display_Nm ");
		
		return this.getResultBySql(strSql.toString());
	}

	private List<MmInDetailAccDTO> getMmInDetailInfoFix(String whID,
			String yearmonth1, String yearmonth2, String mmpkguType,
			String mmcaInnercode, String supID, String metrialID,
	...
	private List<MmInDetailAccDTO> getMmInDetailAccCount(String mmstcaID, String stDeSchemeID,
			String whID, String yearmonth1, String yearmonth2, String mmpkguType,
			String mmcaInnercode, String supID, String metrialID) throws Exception {
		StringBuffer strSql = new StringBuffer();
