 ...
 *
 */
public class CiorSrvCostPrintService {
	 ...
	 * @throws Exception
	 */
	public List<CiorMainDTO> exec(String id_ors) throws Exception{
		...
		if (patEntPrnDTOs == null || patEntPrnDTOs.size() == 0)
			return null;
		List<PatEntPrnDTO> rstBlY = new ArrayList<PatEntPrnDTO>();
		List<PatEntPrnDTO> rstBlN = new ArrayList<PatEntPrnDTO>();
...
//			rstBlY.add(patEntPrnDTOs.get(i));
			if ("Y".equals(patEntPrnDTOs.get(i).getFg_blsettled())) {
				rstBlY.add(patEntPrnDTOs.get(i));
			}else{
				rstBlN.add(patEntPrnDTOs.get(i));
			}
			
		}
		
		List<CiorMainDTO> rtn = new ArrayList<CiorMainDTO>();
		if (rstBlY.size() > 0) {
			CiorMainDTO dto = this.getCiorMainDTO(rstBlY);
			rtn.add(dto);
		}
		if (rstBlN.size() > 0) {
			CiorMainDTO dto = this.getCiorMainDTO(rstBlN);
			rtn.add(dto);
		}
	...
	public List<CiorMainDTO> exec(String ids, String blstatetp) throws Exception{
	 ...
	 * @return
	 */
	private CiorMainDTO getCiorMainDTO(List<PatEntPrnDTO> rstBl) {
		CiorMainDTO dto = new CiorMainDTO();
		FDouble amt = new FDouble("0.00");
		for (PatEntPrnDTO prndto : rstBl) {
			amt = amt.add(prndto.getTotalPrice());
		}
		dto.setTotalPrice(amt);
		dto.setBarcode(rstBl.get(0).getBarcode());
		dto.setFg_blsettled(rstBl.get(0).getFg_blsettled());
		dto.setPatEntPrnDTOList(rstBl);
		return dto;
	}
