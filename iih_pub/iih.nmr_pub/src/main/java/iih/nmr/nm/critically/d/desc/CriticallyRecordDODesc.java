...
package iih.nmr.nm.critically.d.desc;
...
public class CriticallyRecordDODesc extends DODesc {
	public static final String CLASS_FULLNAME = "iih.nmr.nm.critically.d.CriticallyRecordDO";
	...
	private void init(){		
		...
		this.add(getName_diaADesc(tblDesc));
	 ...
	 * @return
	 */
	private IAttrDesc getName_diaADesc(ITableDesc tblDesc){
		Attr attrDesc=new Attr("Name_dia",  getName_diaCDesc(tblDesc), this);
		attrDesc.setJavaType(FType.String);
		...
		attrDesc.setSerializable(true);
		attrDesc.setStatic(false);
		attrDesc.setRefTblInfos(new String[]{"bd_dica a0b12","name_diagnosis=id_dica","name"});
		attrDesc.setRefType(false);
		return attrDesc;
	}
	/**
	 ...
	 * @return
	 */
	private IColumnDesc getName_diaCDesc(ITableDesc tblDesc){
		ColumnDesc column=new ColumnDesc(tblDesc,"Name_dia");
        column.setSqlType(Types.VARCHAR);
		column.setNullable(true);
