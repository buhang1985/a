package iih.mp.mb.i;

import java.util.List;

import org.codehaus.jackson.annotate.JsonProperty;

public class DCDoctorAdvice
{
    @JsonProperty(value = "FreTimes")
    private Integer FreTimes;
    @JsonProperty(value = "FreDur")
    private Integer FreDur;
    @JsonProperty(value = "FreDurUnit")
    private String FreDurUnit;
    @JsonProperty(value = "PatientId")
    private String PatientId;
    @JsonProperty(value = "PatientName")
    private String PatientName;
    @JsonProperty(value = "BedNo")
    private String BedNo;
    @JsonProperty(value = "PatientVisitId")
    private String PatientVisitId;
    @JsonProperty(value = "ID")
    private String ID;
    @JsonProperty(value = "OrderID")
    private String OrderID;
    @JsonProperty(value = "IsLongTerm")
    private String IsLongTerm;
    @JsonProperty(value = "StartOrStopIndicator")
    private String StartOrStopIndicator;
    @JsonProperty(value = "State")
    private String State;
    @JsonProperty(value = "StateString")
    private String StateString;
    @JsonProperty(value = "OrderClass")
    private String OrderClass;
    @JsonProperty(value = "OrderClassString")
    private String OrderClassString;
    @JsonProperty(value = "Content")
    private String Content;
    @JsonProperty(value = "TotalDosage")
    private String TotalDosage;
    @JsonProperty(value = "TotalDosageUnit")
    private String TotalDosageUnit;
    @JsonProperty(value = "Dosage")
    private String Dosage;
    @JsonProperty(value = "Duration")
    private String Duration;
    @JsonProperty(value = "DurationUnit")
    private String DurationUnit;
    @JsonProperty(value = "Frequency")
    private String Frequency;
    @JsonProperty(value = "FrequencyCode")
    private String FrequencyCode;
    @JsonProperty(value = "FreqCounter")
    private String FreqCounter;
    @JsonProperty(value = "BillingAttr")
    private String BillingAttr;
    @JsonProperty(value = "DrugBillingAttr")
    private String DrugBillingAttr;
    @JsonProperty(value = "FreqInterval")
    private String FreqInterval;
    @JsonProperty(value = "FreqIntervalUnit")
    private String FreqIntervalUnit;
    @JsonProperty(value = "DosageUnit")
    private String DosageUnit;
    @JsonProperty(value = "Schedule")
    private String Schedule;
    @JsonProperty(value = "OrderCode")
    private String OrderCode;
    @JsonProperty(value = "DeptCode")
    private String DeptCode;
    @JsonProperty(value = "TestNo")
    private String TestNo;
    @JsonProperty(value = "RelativeNo")
    private String RelativeNo;
    @JsonProperty(value = "Performance")
    private String Performance;
    @JsonProperty(value = "CreateDoctorName")
    private String CreateDoctorName;
    @JsonProperty(value = "CreateDateTime")
    private String CreateDateTime;
    @JsonProperty(value = "CheckNurseName")
    private String CheckNurseName;
    @JsonProperty(value = "CheckDateTime")
    private String CheckDateTime;
    @JsonProperty(value = "EndDoctorName")
    private String EndDoctorName;
    @JsonProperty(value = "EndDateTime")
    private String EndDateTime;
    @JsonProperty(value = "RelatedOrderNo")
    private String RelatedOrderNo;
    @JsonProperty(value = "RelatedOrderSubNo")
    private String RelatedOrderSubNo;
    @JsonProperty(value = "OrderPerformanceStatus")
    private String OrderPerformanceStatus;
    @JsonProperty(value = "OrderTypeName")
    private String OrderTypeName;
    @JsonProperty(value = "StartExectueTime")
    private String StartExectueTime;
    @JsonProperty(value = "StopExecuteTime")
    private String StopExecuteTime;
    @JsonProperty(value = "LatestExecuteDateTime")
    private String LatestExecuteDateTime;
    @JsonProperty(value = "Effective")
    private String Effective;
    @JsonProperty(value = "Send")
    private String Send;
    @JsonProperty(value = "ComNO")
    private String ComNO;
    @JsonProperty(value = "Summary")
    private String Summary;
    @JsonProperty(value = "ScheduleExecuteTime")
    private String ScheduleExecuteTime;
    @JsonProperty(value = "qty")
    private String qty;
    @JsonProperty(value = "ExecStatus")
    private String ExecStatus;
    @JsonProperty(value = "ColorValue")
    private String ColorValue;
    @JsonProperty(value = "InputType")
    private String InputType;
    @JsonProperty(value = "SubAdviceList")
    private ArrayOfDCDoctorAdvice SubAdviceList;
    @JsonProperty(value = "SendRecordList")
    private ArrayOfDCDoctorAdviceSendRecord SendRecordList;
    @JsonProperty(value = "PriceDetail")
    private ArrayOfDCDoctorAdvicePriceDetail PriceDetail;
    @JsonProperty(value = "ExecutionList")
    private ArrayOfDCDoctorAdviceExecutionRecord ExecutionList;

    public void setFreTimes(Integer FreTimes)
    {
        this.FreTimes = FreTimes;
    }
    public Integer getFreTimes()
    {
        return this.FreTimes;
    }
    public void setFreDur(Integer FreDur)
    {
        this.FreDur = FreDur;
    }
    public Integer getFreDur()
    {
        return this.FreDur;
    }
    public void setFreDurUnit(String FreDurUnit)
    {
        this.FreDurUnit = FreDurUnit;
    }
    public String getFreDurUnit()
    {
        return this.FreDurUnit;
    }
    public void setPatientId(String PatientId)
    {
        this.PatientId = PatientId;
    }
    public String getPatientId()
    {
        return this.PatientId;
    }
    public void setPatientName(String PatientName)
    {
        this.PatientName = PatientName;
    }
    public String getPatientName()
    {
        return this.PatientName;
    }
    public void setBedNo(String BedNo)
    {
        this.BedNo = BedNo;
    }
    public String getBedNo()
    {
        return this.BedNo;
    }
    public void setPatientVisitId(String PatientVisitId)
    {
        this.PatientVisitId = PatientVisitId;
    }
    public String getPatientVisitId()
    {
        return this.PatientVisitId;
    }
    public void setID(String ID)
    {
        this.ID = ID;
    }
    public String getID()
    {
        return this.ID;
    }
    public void setOrderID(String OrderID)
    {
        this.OrderID = OrderID;
    }
    public String getOrderID()
    {
        return this.OrderID;
    }
    public void setIsLongTerm(String IsLongTerm)
    {
        this.IsLongTerm = IsLongTerm;
    }
    public String getIsLongTerm()
    {
        return this.IsLongTerm;
    }
    public void setStartOrStopIndicator(String StartOrStopIndicator)
    {
        this.StartOrStopIndicator = StartOrStopIndicator;
    }
    public String getStartOrStopIndicator()
    {
        return this.StartOrStopIndicator;
    }
    public void setState(String State)
    {
        this.State = State;
    }
    public String getState()
    {
        return this.State;
    }
    public void setStateString(String StateString)
    {
        this.StateString = StateString;
    }
    public String getStateString()
    {
        return this.StateString;
    }
    public void setOrderClass(String OrderClass)
    {
        this.OrderClass = OrderClass;
    }
    public String getOrderClass()
    {
        return this.OrderClass;
    }
    public void setOrderClassString(String OrderClassString)
    {
        this.OrderClassString = OrderClassString;
    }
    public String getOrderClassString()
    {
        return this.OrderClassString;
    }
    public void setContent(String Content)
    {
        this.Content = Content;
    }
    public String getContent()
    {
        return this.Content;
    }
    public void setTotalDosage(String TotalDosage)
    {
        this.TotalDosage = TotalDosage;
    }
    public String getTotalDosage()
    {
        return this.TotalDosage;
    }
    public void setTotalDosageUnit(String TotalDosageUnit)
    {
        this.TotalDosageUnit = TotalDosageUnit;
    }
    public String getTotalDosageUnit()
    {
        return this.TotalDosageUnit;
    }
    public void setDosage(String Dosage)
    {
        this.Dosage = Dosage;
    }
    public String getDosage()
    {
        return this.Dosage;
    }
    public void setDuration(String Duration)
    {
        this.Duration = Duration;
    }
    public String getDuration()
    {
        return this.Duration;
    }
    public void setDurationUnit(String DurationUnit)
    {
        this.DurationUnit = DurationUnit;
    }
    public String getDurationUnit()
    {
        return this.DurationUnit;
    }
    public void setFrequency(String Frequency)
    {
        this.Frequency = Frequency;
    }
    public String getFrequency()
    {
        return this.Frequency;
    }
    public void setFrequencyCode(String FrequencyCode)
    {
        this.FrequencyCode = FrequencyCode;
    }
    public String getFrequencyCode()
    {
        return this.FrequencyCode;
    }
    public void setFreqCounter(String FreqCounter)
    {
        this.FreqCounter = FreqCounter;
    }
    public String getFreqCounter()
    {
        return this.FreqCounter;
    }
    public void setBillingAttr(String BillingAttr)
    {
        this.BillingAttr = BillingAttr;
    }
    public String getBillingAttr()
    {
        return this.BillingAttr;
    }
    public void setDrugBillingAttr(String DrugBillingAttr)
    {
        this.DrugBillingAttr = DrugBillingAttr;
    }
    public String getDrugBillingAttr()
    {
        return this.DrugBillingAttr;
    }
    public void setFreqInterval(String FreqInterval)
    {
        this.FreqInterval = FreqInterval;
    }
    public String getFreqInterval()
    {
        return this.FreqInterval;
    }
    public void setFreqIntervalUnit(String FreqIntervalUnit)
    {
        this.FreqIntervalUnit = FreqIntervalUnit;
    }
    public String getFreqIntervalUnit()
    {
        return this.FreqIntervalUnit;
    }
    public void setDosageUnit(String DosageUnit)
    {
        this.DosageUnit = DosageUnit;
    }
    public String getDosageUnit()
    {
        return this.DosageUnit;
    }
    public void setSchedule(String Schedule)
    {
        this.Schedule = Schedule;
    }
    public String getSchedule()
    {
        return this.Schedule;
    }
    public void setOrderCode(String OrderCode)
    {
        this.OrderCode = OrderCode;
    }
    public String getOrderCode()
    {
        return this.OrderCode;
    }
    public void setDeptCode(String DeptCode)
    {
        this.DeptCode = DeptCode;
    }
    public String getDeptCode()
    {
        return this.DeptCode;
    }
    public void setTestNo(String TestNo)
    {
        this.TestNo = TestNo;
    }
    public String getTestNo()
    {
        return this.TestNo;
    }
    public void setRelativeNo(String RelativeNo)
    {
        this.RelativeNo = RelativeNo;
    }
    public String getRelativeNo()
    {
        return this.RelativeNo;
    }
    public void setPerformance(String Performance)
    {
        this.Performance = Performance;
    }
    public String getPerformance()
    {
        return this.Performance;
    }
    public void setCreateDoctorName(String CreateDoctorName)
    {
        this.CreateDoctorName = CreateDoctorName;
    }
    public String getCreateDoctorName()
    {
        return this.CreateDoctorName;
    }
    public void setCreateDateTime(String CreateDateTime)
    {
        this.CreateDateTime = CreateDateTime;
    }
    public String getCreateDateTime()
    {
        return this.CreateDateTime;
    }
    public void setCheckNurseName(String CheckNurseName)
    {
        this.CheckNurseName = CheckNurseName;
    }
    public String getCheckNurseName()
    {
        return this.CheckNurseName;
    }
    public void setCheckDateTime(String CheckDateTime)
    {
        this.CheckDateTime = CheckDateTime;
    }
    public String getCheckDateTime()
    {
        return this.CheckDateTime;
    }
    public void setEndDoctorName(String EndDoctorName)
    {
        this.EndDoctorName = EndDoctorName;
    }
    public String getEndDoctorName()
    {
        return this.EndDoctorName;
    }
    public void setEndDateTime(String EndDateTime)
    {
        this.EndDateTime = EndDateTime;
    }
    public String getEndDateTime()
    {
        return this.EndDateTime;
    }
    public void setRelatedOrderNo(String RelatedOrderNo)
    {
        this.RelatedOrderNo = RelatedOrderNo;
    }
    public String getRelatedOrderNo()
    {
        return this.RelatedOrderNo;
    }
    public void setRelatedOrderSubNo(String RelatedOrderSubNo)
    {
        this.RelatedOrderSubNo = RelatedOrderSubNo;
    }
    public String getRelatedOrderSubNo()
    {
        return this.RelatedOrderSubNo;
    }
    public void setOrderPerformanceStatus(String OrderPerformanceStatus)
    {
        this.OrderPerformanceStatus = OrderPerformanceStatus;
    }
    public String getOrderPerformanceStatus()
    {
        return this.OrderPerformanceStatus;
    }
    public void setOrderTypeName(String OrderTypeName)
    {
        this.OrderTypeName = OrderTypeName;
    }
    public String getOrderTypeName()
    {
        return this.OrderTypeName;
    }
    public void setStartExectueTime(String StartExectueTime)
    {
        this.StartExectueTime = StartExectueTime;
    }
    public String getStartExectueTime()
    {
        return this.StartExectueTime;
    }
    public void setStopExecuteTime(String StopExecuteTime)
    {
        this.StopExecuteTime = StopExecuteTime;
    }
    public String getStopExecuteTime()
    {
        return this.StopExecuteTime;
    }
    public void setLatestExecuteDateTime(String LatestExecuteDateTime)
    {
        this.LatestExecuteDateTime = LatestExecuteDateTime;
    }
    public String getLatestExecuteDateTime()
    {
        return this.LatestExecuteDateTime;
    }
    public void setEffective(String Effective)
    {
        this.Effective = Effective;
    }
    public String getEffective()
    {
        return this.Effective;
    }
    public void setSend(String Send)
    {
        this.Send = Send;
    }
    public String getSend()
    {
        return this.Send;
    }
    public void setComNO(String ComNO)
    {
        this.ComNO = ComNO;
    }
    public String getComNO()
    {
        return this.ComNO;
    }
    public void setSummary(String Summary)
    {
        this.Summary = Summary;
    }
    public String getSummary()
    {
        return this.Summary;
    }
    public void setScheduleExecuteTime(String ScheduleExecuteTime)
    {
        this.ScheduleExecuteTime = ScheduleExecuteTime;
    }
    public String getScheduleExecuteTime()
    {
        return this.ScheduleExecuteTime;
    }
    public void setQty(String qty)
    {
        this.qty = qty;
    }
    public String getQty()
    {
        return this.qty;
    }
    public String getExecStatus() {
		return ExecStatus;
	}
	public void setExecStatus(String execStatus) {
		ExecStatus = execStatus;
	}
	public String getColorValue() {
		return ColorValue;
	}
	public void setColorValue(String colorValue) {
		ColorValue = colorValue;
	}
	public void setInputType(String InputType)
    {
        this.InputType = InputType;
    }
    public String getInputType()
    {
        return this.InputType;
    }
    public void setSubAdviceList(ArrayOfDCDoctorAdvice SubAdviceList)
    {
        this.SubAdviceList = SubAdviceList;
    }
    public ArrayOfDCDoctorAdvice getSubAdviceList()
    {
        return this.SubAdviceList;
    }
    public void setSendRecordList(ArrayOfDCDoctorAdviceSendRecord SendRecordList)
    {
        this.SendRecordList = SendRecordList;
    }
    public ArrayOfDCDoctorAdviceSendRecord getSendRecordList()
    {
        return this.SendRecordList;
    }
    public void setPriceDetail(ArrayOfDCDoctorAdvicePriceDetail PriceDetail)
    {
        this.PriceDetail = PriceDetail;
    }
    public ArrayOfDCDoctorAdvicePriceDetail getPriceDetail()
    {
        return this.PriceDetail;
    }
    public void setExecutionList(ArrayOfDCDoctorAdviceExecutionRecord ExecutionList)
    {
        this.ExecutionList = ExecutionList;
    }
    public ArrayOfDCDoctorAdviceExecutionRecord getExecutionList()
    {
        return this.ExecutionList;
    }
}
