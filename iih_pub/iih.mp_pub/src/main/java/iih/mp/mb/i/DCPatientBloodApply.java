package iih.mp.mb.i;


import java.util.List;

import org.codehaus.jackson.annotate.JsonProperty;

public class DCPatientBloodApply
{
    @JsonProperty(value = "ApplyID")
    private String ApplyID;
    @JsonProperty(value = "PatientHID")
    private String PatientHID;
    @JsonProperty(value = "PatientID")
    private String PatientID;
    @JsonProperty(value = "PatientName")
    private String PatientName;
    @JsonProperty(value = "PatientSex")
    private String PatientSex;
    @JsonProperty(value = "PatientBirthDate")
    private String PatientBirthDate;
    @JsonProperty(value = "ApplyDeptCode")
    private String ApplyDeptCode;
    @JsonProperty(value = "ApplyDeptName")
    private String ApplyDeptName;
    @JsonProperty(value = "BloodPaper")
    private String BloodPaper;
    @JsonProperty(value = "FeeType")
    private String FeeType;
    @JsonProperty(value = "PatientSource")
    private String PatientSource;
    @JsonProperty(value = "BloodInuse")
    private String BloodInuse;
    @JsonProperty(value = "BloodDiagnose")
    private String BloodDiagnose;
    @JsonProperty(value = "BloodTaboo")
    private String BloodTaboo;
    @JsonProperty(value = "BloodHistory")
    private String BloodHistory;
    @JsonProperty(value = "LastBloodDate")
    private String LastBloodDate;
    @JsonProperty(value = "Cyesis")
    private String Cyesis;
    @JsonProperty(value = "BirthTo")
    private String BirthTo;
    @JsonProperty(value = "GatherDate")
    private String GatherDate;
    @JsonProperty(value = "BloodPurpose")
    private String BloodPurpose;
    @JsonProperty(value = "Hematin")
    private String Hematin;
    @JsonProperty(value = "Platelet")
    private String Platelet;
    @JsonProperty(value = "Leucocyte")
    private String Leucocyte;
    @JsonProperty(value = "PatBloodGroup")
    private String PatBloodGroup;
    @JsonProperty(value = "Rh")
    private String Rh;
    @JsonProperty(value = "BloodSum")
    private String BloodSum;
    @JsonProperty(value = "ApplyDate")
    private String ApplyDate;
    @JsonProperty(value = "Doctor")
    private String Doctor;
    @JsonProperty(value = "Director")
    private String Director;
    @JsonProperty(value = "Physician")
    private String Physician;
    @JsonProperty(value = "Price")
    private String Price;
    @JsonProperty(value = "HCT")
    private String HCT;
    @JsonProperty(value = "ALT")
    private String ALT;
    @JsonProperty(value = "HBSAG")
    private String HBSAG;
    @JsonProperty(value = "HCV")
    private String HCV;
    @JsonProperty(value = "HIV")
    private String HIV;
    @JsonProperty(value = "ANTI_MD")
    private String ANTI_MD;
    @JsonProperty(value = "DCPatientBloodApplyItemList")
    private ArrayOfDCPatientBloodApplyItem DCPatientBloodApplyItemList;

    public void setApplyID(String ApplyID)
    {
        this.ApplyID = ApplyID;
    }
    public String getApplyID()
    {
        return this.ApplyID;
    }
    public void setPatientHID(String PatientHID)
    {
        this.PatientHID = PatientHID;
    }
    public String getPatientHID()
    {
        return this.PatientHID;
    }
    public void setPatientID(String PatientID)
    {
        this.PatientID = PatientID;
    }
    public String getPatientID()
    {
        return this.PatientID;
    }
    public void setPatientName(String PatientName)
    {
        this.PatientName = PatientName;
    }
    public String getPatientName()
    {
        return this.PatientName;
    }
    public void setPatientSex(String PatientSex)
    {
        this.PatientSex = PatientSex;
    }
    public String getPatientSex()
    {
        return this.PatientSex;
    }
    public void setPatientBirthDate(String PatientBirthDate)
    {
        this.PatientBirthDate = PatientBirthDate;
    }
    public String getPatientBirthDate()
    {
        return this.PatientBirthDate;
    }
    public void setApplyDeptCode(String ApplyDeptCode)
    {
        this.ApplyDeptCode = ApplyDeptCode;
    }
    public String getApplyDeptCode()
    {
        return this.ApplyDeptCode;
    }
    public void setApplyDeptName(String ApplyDeptName)
    {
        this.ApplyDeptName = ApplyDeptName;
    }
    public String getApplyDeptName()
    {
        return this.ApplyDeptName;
    }
    public void setBloodPaper(String BloodPaper)
    {
        this.BloodPaper = BloodPaper;
    }
    public String getBloodPaper()
    {
        return this.BloodPaper;
    }
    public void setFeeType(String FeeType)
    {
        this.FeeType = FeeType;
    }
    public String getFeeType()
    {
        return this.FeeType;
    }
    public void setPatientSource(String PatientSource)
    {
        this.PatientSource = PatientSource;
    }
    public String getPatientSource()
    {
        return this.PatientSource;
    }
    public void setBloodInuse(String BloodInuse)
    {
        this.BloodInuse = BloodInuse;
    }
    public String getBloodInuse()
    {
        return this.BloodInuse;
    }
    public void setBloodDiagnose(String BloodDiagnose)
    {
        this.BloodDiagnose = BloodDiagnose;
    }
    public String getBloodDiagnose()
    {
        return this.BloodDiagnose;
    }
    public void setBloodTaboo(String BloodTaboo)
    {
        this.BloodTaboo = BloodTaboo;
    }
    public String getBloodTaboo()
    {
        return this.BloodTaboo;
    }
    public void setBloodHistory(String BloodHistory)
    {
        this.BloodHistory = BloodHistory;
    }
    public String getBloodHistory()
    {
        return this.BloodHistory;
    }
    public void setLastBloodDate(String LastBloodDate)
    {
        this.LastBloodDate = LastBloodDate;
    }
    public String getLastBloodDate()
    {
        return this.LastBloodDate;
    }
    public void setCyesis(String Cyesis)
    {
        this.Cyesis = Cyesis;
    }
    public String getCyesis()
    {
        return this.Cyesis;
    }
    public void setBirthTo(String BirthTo)
    {
        this.BirthTo = BirthTo;
    }
    public String getBirthTo()
    {
        return this.BirthTo;
    }
    public void setGatherDate(String GatherDate)
    {
        this.GatherDate = GatherDate;
    }
    public String getGatherDate()
    {
        return this.GatherDate;
    }
    public void setBloodPurpose(String BloodPurpose)
    {
        this.BloodPurpose = BloodPurpose;
    }
    public String getBloodPurpose()
    {
        return this.BloodPurpose;
    }
    public void setHematin(String Hematin)
    {
        this.Hematin = Hematin;
    }
    public String getHematin()
    {
        return this.Hematin;
    }
    public void setPlatelet(String Platelet)
    {
        this.Platelet = Platelet;
    }
    public String getPlatelet()
    {
        return this.Platelet;
    }
    public void setLeucocyte(String Leucocyte)
    {
        this.Leucocyte = Leucocyte;
    }
    public String getLeucocyte()
    {
        return this.Leucocyte;
    }
    public void setPatBloodGroup(String PatBloodGroup)
    {
        this.PatBloodGroup = PatBloodGroup;
    }
    public String getPatBloodGroup()
    {
        return this.PatBloodGroup;
    }
    public void setRh(String Rh)
    {
        this.Rh = Rh;
    }
    public String getRh()
    {
        return this.Rh;
    }
    public void setBloodSum(String BloodSum)
    {
        this.BloodSum = BloodSum;
    }
    public String getBloodSum()
    {
        return this.BloodSum;
    }
    public void setApplyDate(String ApplyDate)
    {
        this.ApplyDate = ApplyDate;
    }
    public String getApplyDate()
    {
        return this.ApplyDate;
    }
    public void setDoctor(String Doctor)
    {
        this.Doctor = Doctor;
    }
    public String getDoctor()
    {
        return this.Doctor;
    }
    public void setDirector(String Director)
    {
        this.Director = Director;
    }
    public String getDirector()
    {
        return this.Director;
    }
    public void setPhysician(String Physician)
    {
        this.Physician = Physician;
    }
    public String getPhysician()
    {
        return this.Physician;
    }
    public void setPrice(String Price)
    {
        this.Price = Price;
    }
    public String getPrice()
    {
        return this.Price;
    }
    public void setHCT(String HCT)
    {
        this.HCT = HCT;
    }
    public String getHCT()
    {
        return this.HCT;
    }
    public void setALT(String ALT)
    {
        this.ALT = ALT;
    }
    public String getALT()
    {
        return this.ALT;
    }
    public void setHBSAG(String HBSAG)
    {
        this.HBSAG = HBSAG;
    }
    public String getHBSAG()
    {
        return this.HBSAG;
    }
    public void setHCV(String HCV)
    {
        this.HCV = HCV;
    }
    public String getHCV()
    {
        return this.HCV;
    }
    public void setHIV(String HIV)
    {
        this.HIV = HIV;
    }
    public String getHIV()
    {
        return this.HIV;
    }
    public void setANTI_MD(String ANTI_MD)
    {
        this.ANTI_MD = ANTI_MD;
    }
    public String getANTI_MD()
    {
        return this.ANTI_MD;
    }
    public void setDCPatientBloodApplyItemList(ArrayOfDCPatientBloodApplyItem DCPatientBloodApplyItemList)
    {
        this.DCPatientBloodApplyItemList = DCPatientBloodApplyItemList;
    }
    public ArrayOfDCPatientBloodApplyItem getDCPatientBloodApplyItemList()
    {
        return this.DCPatientBloodApplyItemList;
    }
}
