package iih.cssd.dpp.dfpcanc.d;

import xap.sys.appfw.orm.desc.DescManager;
import xap.mw.coreitf.c.GlobalConst;
import xap.mw.coreitf.i.IDODesc;
import xap.mw.coreitf.d.*;
import xap.mw.core.data.*;
import iih.cssd.dpp.dfpcanc.d.desc.DfpCancDODesc;
import java.math.BigDecimal;

/**
 * 消毒包撤销 DO数据 
 * 
 */
public class DfpCancDO extends BaseDO {
	private static final long serialVersionUID = 1L;

	//撤销主键标识
	public static final String ID_DFP_CANC= "Id_dfp_canc";
	//组织
	public static final String ID_ORG= "Id_org";
	//集团
	public static final String ID_GRP= "Id_grp";
	//消毒包
	public static final String ID_DFP= "Id_dfp";
	//撤销科室
	public static final String ID_DEP_CANC= "Id_dep_canc";
	//撤销人
	public static final String ID_EMP_CANC= "Id_emp_canc";
	//撤销时间
	public static final String DT_CANC= "Dt_canc";
	//撤销请领
	public static final String ID_DFP_APP= "Id_dfp_app";
	//原请领发放
	public static final String IF_DFP_SEND_APP= "If_dfp_send_app";
	//撤销拒发
	public static final String IF_DFP_SEND_CANC= "If_dfp_send_canc";
	//创建人
	public static final String CREATEDBY= "Createdby";
	//创建时间
	public static final String CREATEDTIME= "Createdtime";
	//最后修改人
	public static final String MODIFIEDBY= "Modifiedby";
	//最后修改时间
	public static final String MODIFIEDTIME= "Modifiedtime";
	public static final String DS = "Ds";
	public static final String SV = "Sv";
	
	/**
	 * 撤销主键标识
	 * @return String
	 */
	public String getId_dfp_canc() {
		return ((String) getAttrVal("Id_dfp_canc"));
	}	
	/**
	 * 撤销主键标识
	 * @param Id_dfp_canc
	 */
	public void setId_dfp_canc(String Id_dfp_canc) {
		setAttrVal("Id_dfp_canc", Id_dfp_canc);
	}
	/**
	 * 组织
	 * @return String
	 */
	public String getId_org() {
		return ((String) getAttrVal("Id_org"));
	}	
	/**
	 * 组织
	 * @param Id_org
	 */
	public void setId_org(String Id_org) {
		setAttrVal("Id_org", Id_org);
	}
	/**
	 * 集团
	 * @return String
	 */
	public String getId_grp() {
		return ((String) getAttrVal("Id_grp"));
	}	
	/**
	 * 集团
	 * @param Id_grp
	 */
	public void setId_grp(String Id_grp) {
		setAttrVal("Id_grp", Id_grp);
	}
	/**
	 * 消毒包
	 * @return String
	 */
	public String getId_dfp() {
		return ((String) getAttrVal("Id_dfp"));
	}	
	/**
	 * 消毒包
	 * @param Id_dfp
	 */
	public void setId_dfp(String Id_dfp) {
		setAttrVal("Id_dfp", Id_dfp);
	}
	/**
	 * 撤销科室
	 * @return String
	 */
	public String getId_dep_canc() {
		return ((String) getAttrVal("Id_dep_canc"));
	}	
	/**
	 * 撤销科室
	 * @param Id_dep_canc
	 */
	public void setId_dep_canc(String Id_dep_canc) {
		setAttrVal("Id_dep_canc", Id_dep_canc);
	}
	/**
	 * 撤销人
	 * @return String
	 */
	public String getId_emp_canc() {
		return ((String) getAttrVal("Id_emp_canc"));
	}	
	/**
	 * 撤销人
	 * @param Id_emp_canc
	 */
	public void setId_emp_canc(String Id_emp_canc) {
		setAttrVal("Id_emp_canc", Id_emp_canc);
	}
	/**
	 * 撤销时间
	 * @return FDateTime
	 */
	public FDateTime getDt_canc() {
		return ((FDateTime) getAttrVal("Dt_canc"));
	}	
	/**
	 * 撤销时间
	 * @param Dt_canc
	 */
	public void setDt_canc(FDateTime Dt_canc) {
		setAttrVal("Dt_canc", Dt_canc);
	}
	/**
	 * 撤销请领
	 * @return String
	 */
	public String getId_dfp_app() {
		return ((String) getAttrVal("Id_dfp_app"));
	}	
	/**
	 * 撤销请领
	 * @param Id_dfp_app
	 */
	public void setId_dfp_app(String Id_dfp_app) {
		setAttrVal("Id_dfp_app", Id_dfp_app);
	}
	/**
	 * 原请领发放
	 * @return String
	 */
	public String getIf_dfp_send_app() {
		return ((String) getAttrVal("If_dfp_send_app"));
	}	
	/**
	 * 原请领发放
	 * @param If_dfp_send_app
	 */
	public void setIf_dfp_send_app(String If_dfp_send_app) {
		setAttrVal("If_dfp_send_app", If_dfp_send_app);
	}
	/**
	 * 撤销拒发
	 * @return String
	 */
	public String getIf_dfp_send_canc() {
		return ((String) getAttrVal("If_dfp_send_canc"));
	}	
	/**
	 * 撤销拒发
	 * @param If_dfp_send_canc
	 */
	public void setIf_dfp_send_canc(String If_dfp_send_canc) {
		setAttrVal("If_dfp_send_canc", If_dfp_send_canc);
	}
	/**
	 * 创建人
	 * @return String
	 */
	public String getCreatedby() {
		return ((String) getAttrVal("Createdby"));
	}	
	/**
	 * 创建人
	 * @param Createdby
	 */
	public void setCreatedby(String Createdby) {
		setAttrVal("Createdby", Createdby);
	}
	/**
	 * 创建时间
	 * @return FDateTime
	 */
	public FDateTime getCreatedtime() {
		return ((FDateTime) getAttrVal("Createdtime"));
	}	
	/**
	 * 创建时间
	 * @param Createdtime
	 */
	public void setCreatedtime(FDateTime Createdtime) {
		setAttrVal("Createdtime", Createdtime);
	}
	/**
	 * 最后修改人
	 * @return String
	 */
	public String getModifiedby() {
		return ((String) getAttrVal("Modifiedby"));
	}	
	/**
	 * 最后修改人
	 * @param Modifiedby
	 */
	public void setModifiedby(String Modifiedby) {
		setAttrVal("Modifiedby", Modifiedby);
	}
	/**
	 * 最后修改时间
	 * @return FDateTime
	 */
	public FDateTime getModifiedtime() {
		return ((FDateTime) getAttrVal("Modifiedtime"));
	}	
	/**
	 * 最后修改时间
	 * @param Modifiedtime
	 */
	public void setModifiedtime(FDateTime Modifiedtime) {
		setAttrVal("Modifiedtime", Modifiedtime);
	}

	public Integer getDs() {
		return ((Integer) getAttrVal(GlobalConst.DELETE_SIGN));
	}
	
	public void setDs(Integer ds) {
		setAttrVal(GlobalConst.DELETE_SIGN, ds);
	}
	
	public FDateTime getSv() {
		return ((FDateTime) getAttrVal(GlobalConst.SYSTEM_VERSION));
	}
	
	public void setSv(FDateTime sv) {
		setAttrVal(GlobalConst.SYSTEM_VERSION, sv);
	}

//	@Override
//	public java.lang.String getParentPKFieldName() {
//		return null;
//	}
  
	@Override
	public String getPKFieldName() {
		return "Id_dfp_canc";
	}
	
	@Override
	public String getTableName() {	  
		return "cssd_dfp_canc";
	}
	
	@Override
	public IDODesc getDODesc() {
		return DescManager.getInstance().getDODesc(DfpCancDODesc.class);
	}
	
}