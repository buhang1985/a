...
public class NmnurlvListener  implements IWfBillExtHandler{
	@Override
	public WfFormInfoCtx[] write(WfFlowInfoCtx flwInfoCtx, WfFormInfoCtx... frmInfoCtx) {
		...
		INmnurlvCudService psnshiftCudService = ServiceFinder.find(INmnurlvCudService.class);
		try {
			NmNurLvDO DO= billDo.getTransDo();
			psnshiftCudService.update(new NmNurLvDO[] { billDo.getTransDo() });
