...
public class NmpsnshiftListener implements IWfBillExtHandler {
	...
	public WfFormInfoCtx[] write(WfFlowInfoCtx flwInfoCtx, WfFormInfoCtx... frmInfoCtx) {
		...
		INmpsnshiftCudService psnshiftCudService = ServiceFinder.find(INmpsnshiftCudService.class);
		try {
			psnshiftCudService.update(new NmNurTransDO[] { billDo.getTransDo() });
