...
public abstract class AbstractUdidocUpdateAfterListener extends AbstractUdidocListener implements IBusinessListener{
	
	@Override
	public void doAction(IBusinessEvent event) throws BizException {
		//事件源及事件类型判断 
		if(!(UdidocDODesc.CLASS_FULLNAME.equals(event.getSourceID()) && IEventType.TYPE_UPDATE_AFTER.equals(event.getEventType()))) {
			return;
		}
		//获取数据
