 ...
 * @author chenxiang
 *
 * @date 2019年9月6日下午1:33:07
 *
 * @classpath iih.hp.cp.hpcp.i.HpcpExportAuditPsn
 *
 * 	临床路径医务部审核人员
 *
 */
public class HpcpExportAuditPsn implements IActorListener {
	
	private static final long serialVersionUID = -6750912712578882407L;

	@SuppressWarnings("unchecked")
	@Override
	public String[] getActors(WfFormInfoCtx wfFormInfoCtx) throws Exception {
		Logger.info("开始获得医务部审批角色集合");
		...
		builder.append(" select a.id_user")
			   ...
			   .append(" where d.code = ? and b.id_org = ? and b.id_group = ?");
		SqlParam param = new SqlParam();
		param.addParam(ParamsetQryUtil.getParaString(orgId, IHpCpFlowSysParamConst.SYS_PARAM_FLOW_EXPORT_APPROVE));
		...
		try {
			...
			e.printStackTrace();
			throw new RuntimeException("获取临床路径医务部审批人失败！");
		}
