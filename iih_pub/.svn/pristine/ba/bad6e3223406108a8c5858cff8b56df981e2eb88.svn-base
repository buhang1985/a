...
* 医保计划数据维护服务
*/
public interface IHpDosageDOCudService {
	/**
	*  医保计划数据真删除
	*/
	...
	*/
	 ...
	 */
	public HpDosageDO[] enableWithoutFilter(HpDosageDO[] aggdos) throws BizException ;

	/**
	 * 批量启用：可部分启用成功；
	 * 启用成功的记录可通过返回值的getDos方法得到，启用不成功的记录附加日志可通过返回值的getErrLogList方法得到
	 */
	public DOWithErrLog enableDO(HpDosageDO[] aggdos) throws BizException ;

	/**
	 * 批量停用：要不全部都停用；或者出现异常，停用全部失败
	 */
	public HpDosageDO[] disableVOWithoutFilter(HpDosageDO[] aggdos) throws BizException;
	 ...
	 */
	public DOWithErrLog disableDO(HpDosageDO[] aggdos) throws BizException ;
