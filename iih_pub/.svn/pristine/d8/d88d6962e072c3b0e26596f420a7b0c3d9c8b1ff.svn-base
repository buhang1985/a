...
public class WareHouseRefModel extends RefTreeModel {
	@Override
	...
	private String BuildWherePart() {
		...
		stringBuilder.append(" and fg_active = 'Y' and fg_last = 'Y' ");
		
		//期初结存不过滤仓库启账状态
		String fg_PostFilter = (String) this.getExtendAttributeValue("fg_PostFilter");
		if (!"N".equals(fg_PostFilter)) {
			stringBuilder.append(" and id_wh in (select id_wh from mm_ini where id_su_st = '@@@@AA1000000004NW06' and sd_su_st = '1') "); // 已启账
		}
