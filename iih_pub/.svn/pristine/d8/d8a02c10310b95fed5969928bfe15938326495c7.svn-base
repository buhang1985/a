...
public class ContagionAggDO extends BaseAggDO {
	...
	public NCPDO[] getNCPDO() {
		IBaseDO[] dos = getChildren(NCPDO.class);
		if(dos==null) return null;
		NCPDO[] result=new NCPDO[dos.length];
		for(int i=0;i<dos.length;i++){
			result[i]=(NCPDO)dos[i];
		}
		return result;
	}
	
	public void setNCPDO(NCPDO[] dos) {
		setChildren(NCPDO.class, dos);
	}
	...
	public IAggDesc getAggDesc() {
     ...
     public  BaseDO createChildDO(String clzName) {
	    ...
	    else if (clzName.equals("iih.ci.rcm.contagion.d.NCPDO")) {
             return new NCPDO();
         }
         return null;
     }
