...
public class NommeetListener implements IWfBillExtHandler {
	...
	public WfFormInfoCtx[] write(WfFlowInfoCtx flwInfoCtx, WfFormInfoCtx... frmInfoCtx) {
		...
		INommeetMDOCudService psnshiftCudService = ServiceFinder.find(INommeetMDOCudService.class);
		try {
			psnshiftCudService.update(new NomMeetDO[] { billDo.getTransDo() });
