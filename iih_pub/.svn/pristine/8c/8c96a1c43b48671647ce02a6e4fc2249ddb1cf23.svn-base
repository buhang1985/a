...
public class OrdApOutDODesc extends DODesc {
	...
	private void init(){		
		...
		this.add(getName_ext1ADesc(tblDesc));
	 ...
	 * @return
	 */
	private IAttrDesc getName_ext1ADesc(ITableDesc tblDesc){
		Attr attrDesc=new Attr("Name_ext1",  getName_ext1CDesc(tblDesc), this);
		attrDesc.setJavaType(FType.String);
		attrDesc.setLabel("名称");
		attrDesc.setNullable(true);
		attrDesc.setPersistence(true);
		attrDesc.setSerializable(true);
		attrDesc.setStatic(false);
		attrDesc.setRefTblInfos(new String[]{"bd_udidoc a0b7","def1=id_udidoc","name"});
		attrDesc.setRefType(false);
		return attrDesc;
	}
	/**
	 ...
	 * @return
	 */
	private IColumnDesc getName_ext1CDesc(ITableDesc tblDesc){
		ColumnDesc column=new ColumnDesc(tblDesc,"Name_ext1");
        column.setSqlType(Types.VARCHAR);
		column.setNullable(true);
		column.setLabel("名称"); 
		return column;
