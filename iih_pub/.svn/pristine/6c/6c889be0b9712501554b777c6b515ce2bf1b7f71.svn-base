...
public class PurchseOrderItmDODesc extends DODesc {
	...
	private void init(){		
		...
		this.add(getQuan_purchasedADesc(tblDesc));
	...
	private ITableDesc getTableDesc(){
		...
		tblDesc.add(getQuan_purchasedCDesc(tblDesc));
	 ...
	 * @return
	 */
	private IAttrDesc getQuan_purchasedADesc(ITableDesc tblDesc){
		Attr attrDesc=new Attr("Quan_purchased",  getQuan_purchasedCDesc(tblDesc), this);
		attrDesc.setJavaType(FType.FDouble);
		attrDesc.setLabel("已采购数量");
		attrDesc.setNullable(true);
		attrDesc.setPersistence(true);
		attrDesc.setSerializable(true);
		attrDesc.setStatic(false);
	 ...
	 * @return
	 */
	private IColumnDesc getQuan_purchasedCDesc(ITableDesc tblDesc){
		ColumnDesc column=new ColumnDesc(tblDesc,"Quan_purchased");
		column.setLength(38);
        column.setPrecision(2);
        column.setSqlType(Types.DECIMAL);
		column.setNullable(true);
		column.setLabel("已采购数量"); 
		return column;
	}
	/**
	 * 获取采购数量_当前表列元数据
	...
	private void setDefaultValueMap(){
		...
		defaultValueMap.put("Quan_purchased",new xap.mw.coreitf.d.FDouble(0));
	}
