...
public class ConSiteDODesc extends DODesc {
	...
	private void init(){		
		...
		this.add(getOther_inva_operADesc(tblDesc));
	...
	private ITableDesc getTableDesc(){
		...
		tblDesc.add(getOther_inva_operCDesc(tblDesc));
	 ...
	 * @return
	 */
	private IAttrDesc getOther_inva_operADesc(ITableDesc tblDesc){
		Attr attrDesc=new Attr("Other_inva_oper",  getOther_inva_operCDesc(tblDesc), this);
		attrDesc.setJavaType(FType.String);
		attrDesc.setLabel("侵袭性操作其它选项");
		attrDesc.setNullable(true);
		attrDesc.setPersistence(true);
		attrDesc.setSerializable(true);
		attrDesc.setStatic(false);
	...
	private IAttrDesc getDi_infection_codeADesc(ITableDesc tblDesc){
		...
		attrDesc.setSerializable(true);
		attrDesc.setStatic(false);
		attrDesc.setRefTblInfos(new String[]{"bd_udidoc a0b3","id_di_infection=id_udidoc","code"});
		attrDesc.setRefType(false);
		return attrDesc;
	}
	/**
	...
	private IAttrDesc getDi_infection_nameADesc(ITableDesc tblDesc){
		...
		attrDesc.setSerializable(true);
		attrDesc.setStatic(false);
		attrDesc.setRefTblInfos(new String[]{"bd_udidoc a0b3","id_di_infection=id_udidoc","name"});
		attrDesc.setRefType(false);
		return attrDesc;
	}
	/**
	 ...
	 * @return
	 */
	private IColumnDesc getOther_inva_operCDesc(ITableDesc tblDesc){
		ColumnDesc column=new ColumnDesc(tblDesc,"Other_inva_oper");
		column.setLength(150);
        column.setSqlType(Types.VARCHAR);
		column.setNullable(true);
		column.setLabel("侵袭性操作其它选项"); 
		return column;
	}
	/**
	 * 获取报告科室表列元数据
