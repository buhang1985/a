...
public class NqmImprovKpiListener implements IWfBillExtHandler {
	...
	public WfFormInfoCtx[] write(WfFlowInfoCtx flwInfoCtx, WfFormInfoCtx... frmInfoCtx) {
		...
		INqmimprovkpiMDOCudService psnshiftCudService = ServiceFinder.find(INqmimprovkpiMDOCudService.class);
		try {
			psnshiftCudService.update(new NqmImprovKpiDO[] { billDo.getTransDo() });
