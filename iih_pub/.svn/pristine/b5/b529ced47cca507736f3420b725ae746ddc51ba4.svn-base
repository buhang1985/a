 ...
 *
 */
public class DrugStoreBlRefModel extends RefGridModel {
	/**
	 * 
	 */
	...
	public String[] getShowFieldName() {
		return new String[] { "仓库名称", "通用名称", "药品名称", "生产厂商", "规格", "批次", "有效期", "单位", "售价", "进价", "结存数量", "药品编码" };
	}
	...
	private String getBaseSql() {
		...
		sqlBuilder.append("bd_srv.name AS name_srv,");
		sqlBuilder.append("bd_srv.PYCODE AS pycode_srv,");
		sqlBuilder.append("bd_srv.WBCODE AS wbcode_srv,");
		...
		sqlBuilder.append("ON mm.id_srv = bd_srv.id_srv ");
	...
	public RefResultSet filterRefBlurValue(String blurValue, PaginationInfo pg) {
		...
		buf.append("( 1<>1");
		buf.append(" or lower(storebl.name_mm) like lower('%");
		buf.append(blurValue);
		buf.append("%')");
		buf.append(" or ");
		buf.append(" lower(storebl.pycode_mm) like lower('%");
		buf.append(blurValue);
		buf.append("%')");
		buf.append(" or lower(storebl.wbcode_mm) like lower('%");
		buf.append(blurValue);
		buf.append("%')");
		buf.append(" or lower(storebl.name_srv) like lower('%");
		buf.append(blurValue);
		buf.append("%')");
		buf.append(" or lower(storebl.pycode_srv) like lower('%");
		buf.append(blurValue);
		buf.append("%')");
		buf.append(" or lower(storebl.wbcode_srv) like lower('%");
		buf.append(blurValue);
		buf.append("%')");
		buf.append(")");
	...
	private String BuildWherePart() {
		...
		if(redblue == null || !redblue) {
			stringBuilder.append(" and quan_stock > 0 ");
		}
	...
	private String BuildOrderPart() {
		...
		stringBuilder.append("pycode_mm,spec,batch");
		return stringBuilder.toString();
	}
}
