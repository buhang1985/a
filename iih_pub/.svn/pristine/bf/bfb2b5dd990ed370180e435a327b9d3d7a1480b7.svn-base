...
public class AbstractCiMrDOInsertForIPListener extends AbstractCiMrDOInsertAfterListener {
	...
	protected void doActionCiMrDOInsert(CiMrDO[] ciMrDOsNew) throws BizException {
		...
		MrDocRefValueDO[] mrDocRefValueDOs = 
				mrdocrefvalueservice.find("id_mr = '" + ciMrIPBaseDTO.getId_en() + "' and id_ent = '" + ciMrIPBaseDTO.getId_en() + "'", "", FBoolean.FALSE);
				...
				for (CiDiagDO ciDiagDO : cidiagDOs) {
					...
					for (CiDiagItemDO ciDiagItemDO : ciDiagItemDOs) {
						if (id_di.equals(ciDiagItemDO.getId_di())) {
							...
							ciDiagItemDO.setSupplement(ciDiagDO.getDt_di().toString());
	...
	private void fireCYXJ(CiMrIPBaseDTO ciMrIPBaseDTO, CiMrDO ciMrDO) throws BizException {
		...
		DomainBusinessUserObj buo = new DomainBusinessUserObj(ciMrIPBaseDTO, bussiType, deptId, "0");
		buo.setMsgObj(msgobj);
		BusinessEvent event = new BusinessEvent(IMrEventConst.EVENT_SOURCE_MR_MR_CYXJ, IEventType.TYPE_UPDATE_AFTER,
				buo);
		BusinessEventListener beListener = ServiceFinder.find(BusinessEventListener.class);
		beListener.doAction(event);
		// try {
