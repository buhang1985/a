...
import xap.wf.af.server.WfFormInfoCtx;
	...
	public class NhrSchedrpCompListener implements IExecutionListener {
	...
	public void notify(WfEventExecution arg0) throws Exception {
		...
		if (billDo.getTransDo().getSd_apy_status().equals("04")) {
			setupStatus(billDo, "02");
		}
		else if(billDo.getTransDo().getSd_apy_status().equals("07")){
			setupStatus(billDo, "03");
		}
		else if(billDo.getTransDo().getSd_apy_status().equals("09")){
			setupStatus(billDo, "06");
		}
		else if(billDo.getTransDo().getSd_apy_status().equals("02")){
			setupStatus(billDo, "03");
		}
		else if(billDo.getTransDo().getSd_apy_status().equals("03")){
			setupStatus(billDo, "06");
		}
		else if(billDo.getTransDo().getSd_apy_status().equals("06")){
