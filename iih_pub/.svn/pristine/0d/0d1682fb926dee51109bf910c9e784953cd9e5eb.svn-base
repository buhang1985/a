...
import iih.nm.nhr.nmnurlv.i.INmnurlvCudService;
...
public class NmnurSpcevListener  implements IWfBillExtHandler{
	@Override
	public WfFormInfoCtx[] write(WfFlowInfoCtx flwInfoCtx, WfFormInfoCtx... frmInfoCtx) {
		...
		INmnurspcevCudService psnshiftCudService = ServiceFinder.find(INmnurspcevCudService.class);
		try {
			NmSpcEvDO DO= billDo.getTransDo();
			psnshiftCudService.update(new NmSpcEvDO[] { billDo.getTransDo() });
