...
public class IpWuweiMedSrvRefGetSql extends OrMedSrvOrderNameLength{
	...
	protected String getQueryColumn() {
		...
		sb.append(
				" distinct Name_srv_desc, Name_unit_sale,des_pri, des   || CASE WHEN hp_des_new IS NOT NULL THEN ' (' ||hp_des_new   ||') ' ELSE ''END || CASE WHEN hp_note IS NOT NULL THEN ' ('  ||hp_note  ||') ' ELSE '' END || case when drugrange is not null then ' ('||drugrange||') ' else '' end || case when abrd_name is not null then ' ('||abrd_name||') ' else '' end ||sup_name as des,id_Srv, Id_mm, sd_srvtp,id_srvtp,fg_ctm,id_unit_med,quan_med,pycode,IdSrvIdMm,name,id_unit_sale");
		sb.append(" from ( ");
		;
		return sb.toString();
	}
	...
	public String getDrugModel(String code_entp, String id_hp, String inputstr,
			String id_pripat,List<String> whereList,String mmGroupCodeParam){
		...
		formatsql.append(getQueryColumn())
			    ...
			    .append(" bd_srv_comp.fg_ctm,hp_srvorca.Des AS hp_des_new,hp_srvorca.note as hp_note,")
			    .append(" (bd_srv_comp.id_srv||'-' || A1.id_mm) as IDSRVIDMM,")
				...
				.append(" LEFT OUTER JOIN bd_hp_srvorca hp_srvorca ON bd_srv_comp.ID_SRV  =hp_srvorca.Id_Srv left outer join bd_hp_srvorca C ON bd_srv_comp.ID_SRV=C.Id_Srv")
