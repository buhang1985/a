...
package iih.bl.vv.cpnin.d.desc;
...
public class BlCpnInDODesc extends DODesc {
	public static final String CLASS_FULLNAME = "iih.bl.vv.cpnin.d.BlCpnInDO";
	public static final String CLASS_DISPALYNAME = "优惠券入库单";
	...
	private void init(){		
		...
		this.add(getCode_cpn_allADesc(tblDesc));
	...
	private ITableDesc getTableDesc(){
		...
		tblDesc.add(getCode_cpninCDesc(tblDesc));
		...
		tblDesc.add(getCode_cpn_allCDesc(tblDesc));
	 ...
	 * @return
	 */
	private IAttrDesc getCode_cpn_allADesc(ITableDesc tblDesc){
		Attr attrDesc=new Attr("Code_cpn_all",  getCode_cpn_allCDesc(tblDesc), this);
		attrDesc.setJavaType(FType.String);
		attrDesc.setLabel("优惠券编号");
		attrDesc.setNullable(true);
		attrDesc.setPersistence(true);
		attrDesc.setSerializable(true);
		attrDesc.setStatic(false);
	 ...
	 * @return
	 */
	private IColumnDesc getCode_cpn_allCDesc(ITableDesc tblDesc){
		ColumnDesc column=new ColumnDesc(tblDesc,"Code_cpn_all");
		column.setLength(50);
        column.setSqlType(Types.VARCHAR);
		column.setNullable(true);
		column.setLabel("优惠券编号"); 
		return column;
	}
	/**
	 * 获取优惠券分类的编码表列元数据
