...
public class CiorappsurgeryAggDO extends BaseAggDO {
	...
	public OrdApOpAgainDO[] getOrdApOpAgainDO() {
		IBaseDO[] dos = getChildren(OrdApOpAgainDO.class);
		if(dos==null) return null;
		OrdApOpAgainDO[] result=new OrdApOpAgainDO[dos.length];
		for(int i=0;i<dos.length;i++){
			result[i]=(OrdApOpAgainDO)dos[i];
		}
		return result;
	}
	
	public void setOrdApOpAgainDO(OrdApOpAgainDO[] dos) {
		setChildren(OrdApOpAgainDO.class, dos);
	}
	...
	public IAggDesc getAggDesc() {
     ...
     public  BaseDO createChildDO(String clzName) {
	    ...
	    else if (clzName.equals("iih.ci.ord.cior.d.OrdApOpAgainDO")) {
             return new OrdApOpAgainDO();
         }
         return null;
     }
}