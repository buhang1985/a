...
public class MrTaskDODesc extends DODesc {
	...
	private void init(){		
		...
		this.add(getIsremindedADesc(tblDesc));
	...
	private ITableDesc getTableDesc(){
		...
		tblDesc.add(getIsremindedCDesc(tblDesc));
	 ...
	 * @return
	 */
	private IAttrDesc getIsremindedADesc(ITableDesc tblDesc){
		Attr attrDesc=new Attr("Isreminded",  getIsremindedCDesc(tblDesc), this);
		attrDesc.setJavaType(FType.FBoolean);
		attrDesc.setLabel("是否已超时提醒");
		attrDesc.setNullable(true);
		attrDesc.setPersistence(true);
		attrDesc.setSerializable(true);
		attrDesc.setStatic(false);
	...
	private IAttrDesc getMrtp_change_codeADesc(ITableDesc tblDesc){
		...
		attrDesc.setSerializable(true);
		attrDesc.setStatic(false);
		attrDesc.setRefTblInfos(new String[]{"bd_mrtp a0b14","id_mrtp_change=id_mrtp","code"});
		attrDesc.setRefType(false);
		return attrDesc;
	}
	/**
	...
	private IAttrDesc getMrtp_change_nameADesc(ITableDesc tblDesc){
		...
		attrDesc.setSerializable(true);
		attrDesc.setStatic(false);
		attrDesc.setRefTblInfos(new String[]{"bd_mrtp a0b14","id_mrtp_change=id_mrtp","name"});
		attrDesc.setRefType(false);
		return attrDesc;
	}
	/**
	 ...
	 * @return
	 */
	private IColumnDesc getIsremindedCDesc(ITableDesc tblDesc){
		ColumnDesc column=new ColumnDesc(tblDesc,"Isreminded");
		column.setLength(1);
        column.setSqlType(Types.CHAR);
		column.setNullable(true);
		column.setLabel("是否已超时提醒"); 
		return column;
	}
	/**
	 * 获取名称表列元数据
