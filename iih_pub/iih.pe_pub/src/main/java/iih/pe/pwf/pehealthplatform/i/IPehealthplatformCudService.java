...
* 体检健康平台接口山东数据维护服务
*/
public interface IPehealthplatformCudService {
	/**
	*  体检健康平台接口山东数据真删除
	*/
	...
	*/
	 ...
	 */
	public PeEmrPhysicalExamItemDO[] enableWithoutFilter(PeEmrPhysicalExamItemDO[] dos) throws BizException ;

	/**
	 * 批量启用：可部分启用成功；
	 * 启用成功的记录可通过返回值的getDos方法得到，启用不成功的记录附加日志可通过返回值的getErrLogList方法得到
	 */
	public DOWithErrLog enableDO(PeEmrPhysicalExamItemDO[] dos) throws BizException ;

	/**
	 * 批量停用：要不全部都停用；或者出现异常，停用全部失败
	 */
	public PeEmrPhysicalExamItemDO[] disableVOWithoutFilter(PeEmrPhysicalExamItemDO[] dos) throws BizException;
	 ...
	 */
	public DOWithErrLog disableDO(PeEmrPhysicalExamItemDO[] dos) throws BizException ;
}
